function minotaurAddicted():Boolean {
	if(player.hasPerk("Minotaur Cum Addict") >= 0 || flags[20] >= 1) return true;
	return false;
}
function minotaurNeed():Boolean {
	if(flags[20] > 1) return true;
	return false;
}

function minoVictoryRapeChoices():void {
	spriteSelect(44);
	...
	if(flags[20] >= 1 || player.hasPerk("Minotaur Cum Addict") >= 0) {
	...
	choices("Use Cock",dickRape,"Use Vagina",cuntRape,"Use Both",hermRape,"TentacleDick",tentaRape,"UrethraFuck",urethralPen,"Get Filled",filled,tempText,temp,"MakeHimSuck",bj,feedposit,temp2,"Leave",5007);
...
function rapeMinotaurTentacle():void {
	...
	if(player.cocks.length > 2 && temp != 1) {
		temp = 1;
		outputText("It's not enough!  Your other dicks demand attention too! Fortunately, he has a large hole for you to abuse... with as many of your " + multiCockDescript() + " can stuff inside, the rest hang underneath, slapping against his sack.  ", false);
	}
...
function AddictNagaOnMinotaur():void {
	...
	outputText("", true);
	//[if(monster.lust >= 99)
	if(monster.lust > 99) outputText("You slither towards the fallen monster, wishing to taste his addictive semen.  He's currently engaged in stroking up and down his enormous shaft with both of his hands, but offers little resistance when you push him over to a sitting position.  You pull his hands away and replace them with yours, running your fingers up and down along his thickness, working yourself up with anticipation.  He leans back and exhales loudly, clearly appreciative.\n\n", false);
...
function bumRapeaMinotaur():void {
	...
	if(player.cumQ() >= 25 && player.cumQ() < 250) outputText("leaking thin ropes of pre-cum over muscular ass.  ", false);
	...
	if(player.cumQ() > 500) {
		...
		stats(0,0,0,0,0,0,-100,0);
		stats(0,0,0,0,0,0,15+player.lib/7,0);
	}
	eventParser(5007);
}
...
function minoPheromones():void {
	...
	if(player.hasPerk("Minotaur Cum Addict") >= 0 || flags[20] == 2) {
...
function getRapedByMinotaur():void {
	spriteSelect(44);
	slimeFeed();
	//BY BUTTS MCGEE
	//====================================================================
...
function takeMinoCumDirectly():void {
	...
	stats(0,0,0,0,0,1,-80,1);
	//A bit of post happy arousal
	stats(0,0,0,0,0,0,25,0);
	//bonus sensitivity if unsensitive.
	if(player.sens < 80) stats(0,0,0,0,0,1,0,0);
	if(player.sens < 60) stats(0,0,0,0,0,1,0,0);
	if(player.sens < 40) stats(0,0,0,0,0,1,0,0);
	if(player.sens < 20) stats(0,0,0,0,0,1,0,0);
	//+15 addiction
	minoCumAddiction(15);
	eventParser(5007);
}
...
function minoCumAddiction(raw:Number = 10):void {
	...
	if(flags[18] < 0) flags[18] = 0;
	if(flags[20] < 0) flags[20] = 0;
	if(flags[18] > 120) flags[18] = 120;
	
	//Turn off withdrawal
	//if(flags[20] > 1) flags[20] = 1;
	//Reset counter
	flags[19] = 0;
	//If highly addicted, rises slower
	if(flags[18] >= 60) raw /= 2;
	if(flags[18] >= 80) raw /= 2;
	if(flags[18] >= 90) raw /= 2;
	//If in withdrawl, readdiction is potent!
	if(flags[20] == 3) raw += 10;
	if(flags[20] == 2) raw += 5;
	...
	trace("MINO CUM ADDICTION %: " + flags[18]);
}

function minoCumUpdate():Boolean {
	if(flags[18] == 0) return false;
	var output:Boolean = false;
	trace("MINO CUM ADDICTION %: " + flags[18]);
	//Your addicted enough for it to be official!
	if(flags[20] == 0 && flags[18] >= 50) {
	...
	//If over 50, addicted, become addicted, withdrawl, etc
		...
		if(flags[20] == 3 && flags[19] >= 48) {
			stats(0,0,0,0,0,0,2,0);
		}
		if(flags[20] == 2 && flags[19] >= 48) {
			stats(0,0,0,0,0,0,1,0);
		}
	}

	//Decrement addiction value as needed
	if(flags[19] >= 48 && player.hasPerk("Minotaur Cum Addict") < 0) {
		if(flags[18] >= 0.15) flags[18] -= 0.15;
	}
	...
	if(flags[21] > 0) {
		...
		if(flags[21] < 0) flags[21] = 0;
	}
	return output;
}

function minoAddictionFuck():void {
	spriteSelect(44);
	outputText("", true);
...
function minoAddictionBadEndEncounter():void {
	spriteSelect(44);
	outputText("", true);
...
function minoAddictionBadEnd2():void {
	...
	outputText("One of the unoccupied monsters glances your way and gives you a predatory smile.  He puts down the axe he was sharpening and strides over, his loincloth nearly tearing itself from his groin as his member inflates to full size.  Amazingly, this minotaur bothers to speak, \"<i><i>New fuck-toy.  Suck.</i></i>\"\n\n", false);
