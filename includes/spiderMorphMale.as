...
function defeatSpiderBoy():void {
	...
	var frot:Number = 0;
	if(player.hasVagina()) mount = 2995;
	if(player.hasCock()) {
		if(player.cockThatFits(monster.analCapacity()) != -1) buttfuck = 2993;
		if(player.biggestCockArea() > monster.analCapacity()) frot = 2994;
	}
...
function victoryFrotTheSpoidah():void {
	...
	outputText(".  You sway your " + hipDescript() + " to drag your " + cockDescript(x) + " all over his body, and the defeated boy's pale white skin slowly colors pink with a full-body blush.\n\n", false);
	...
	outputText("You hump forward and smash your " + cockHead(x) + " against the spider-boy's face, popping a bubble of pre-cum on his lips before you pull back far enough to reveal the soaked foreskin that hides his cock-tip.  He's probably filled the entirety of that little flesh-balloon with his leavings, and as you slide back up you feel more of his pre-cum bursting out, confirming your suspicions.  A pair of hard, chitinous arms encircle your " + cockDescript(x) + " in a tight hug, squeezing into a slippery pleasure-vice for your penis.\n\n", false);
