...
function kitsuneStruggle():void {
	...
	if(rand(20) + player.str/20 + monster.statusAffectv1("PCTailTangle") >= 12) {
		...
		monster.removeStatusAffect("PCTailTangle");
		enemyAI();
	}
	//Failure - +5-10 LUST
	else {
...
function illusionKitsuneAttack():void {
	...
	if(rand(100) < resist) {
		...
		monster.createStatusAffect("Illusion",0,0,0,0);
		monster.spe += 20;
	}
	combatRoundOver();
}
...
function getRapedByRedHeadFutaKitsune():void {
	...
	if(player.hasVagina()) cuntChange(24,true,true,false);
	else buttChange(24,true,true,false);
	...
	if( player.gender >= 2)
	{
		outputText(((player.vaginas[0].vaginalLooseness < 3) ? "\"<i>Ah!  Nice and tight, just how I like it!</i>\"  she groans, gripping your waist tightly and beginning to strongly pump her hips." :  "\"<i>Hmm...  a little loose for my tastes, but I suppose it'll have to do.  You haven't been slutting it up on minotaurs and tentacle beasts, have you cutie?</i>\"  she says teasingly, giving you a patronizing pinch on the cheek."));
	}
	else
	{
...
function defeatTheKitsunes(display:Boolean = true):void {
	if(display) {
		clearOutput();
		if(monster.HP < 1) {
			outputText( "The kitsune hits the ground with an 'Oomph', landing roughly on her well-cushioned backside." + ((monster.hairColor == "red" && flags[redheadIsFuta] == 0) ? "  The moment her rounded rump impacts the dirt, a swirling flame crackles to life between her legs, engulfing her exposed cock.  When it dies away, all that remains of her throbbing member is a pert cherry-colored bud between her dripping lips." : "") +"  She rubs her sore posterior, wincing in pain and pouting childishly.\n\n");
		...
		//Lust victory
			...
			outputText( "The kitsune falls to the ground, one hand buried in her robes as she plays with herself shamelessly, too turned on to continue fighting." + ((monster.hairColor == "red" && flags[redheadIsFuta] == 0) ? "  The moment her rounded rump impacts the dirt, a swirling flame crackles to life between her legs, engulfing her exposed cock.  When it dies away, all that remains of her throbbing member is a pert cherry-colored bud between her dripping lips." : "") + "\n\n" + ((player.lust >= 33) ? "<b>As you watch her lewd display, you realize your own lusts have not been sated yet. What will you do to her?</b>" : "") );
		}
	}
	menu();
	var button:int = 0;
	...
	if(monster.hairColor == "red") {
		...
		if(player.hasVagina() && flags[redheadIsFuta] > 0)
			button = kitsuneButton(button,"RideHerCock",rideDatRedheadKitsuneCockIntoTheSkyDiamonds);
		if(flags[redheadIsFuta] > 0 && player.hasVagina() && player.biggestTitSize() >= 4 && player.armorName == "lusty maiden's armor")
			button = kitsuneButton(button,"B.Titfuck",lustyMaidenPaizuri);
	}
	//[Feeder]
	if(player.hasPerk("Feeder") >= 0)
...
function fuckAKitsuneVaginally():void {
	...
	outputText("You only smirk, dragging your fingers across her side and leaning in to nibble the edge of her neck.  She practically melts in your arms, rolling her eyes back and moaning, her cheeks growing flushed with desire." + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? "  You feel a light thump against your crotch and look down with a snicker, seeing a large tent in the lower half of her robes.  Teasingly, you run your hand down her front, wrapping your fingers around it through the fabric and pumping it slowly." : "" ) + "\n\n");
	...
	outputText("You toss away your " + player.armorName + ", dropping to the ground without a second thought, and you carefully lift the hem of the kitsune's robes to inspect her groin." + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? "  Her throbbing cock rises to attention immediately, precum leaking slowly but constantly from the tip, down the thick shaft and over her glistening nether lips." : "" ) + "  A small tattoo rests on her pubic mound, accentuating her" + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? " foot long member and" : "" ) + " shaven pussy." + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? "  You take her pulsing shaft into your hand and begin to pump it strongly, watching her face contort into all manner of pleasured grimaces as your fist slides up and down the pre-soaked pole." : "") + "\n\n");
	
	outputText("Your [cock biggest] rises up in between her legs, sliding along the opening of her vagina as if to remind you what you're ultimately after.  Feeling that you've teased the girl enough, you hike up her legs into your arms, pressing her against the tree to hold her up in the air.  Your shaft flops out over her stomach, and you start to grind back and forth against her " + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? "own twitching member" : "pelvis" ) + ", dripping pre all over her.\n\n");
	...
	outputText( "With a groan, you pull back, " + ((player.cor < 45) ? "holding your arms up underneath her shoulders to keep her from falling to the ground all at once, and lower her down slowly." : "letting her drop to the ground with an unceremonious 'THUD'.  She yelps weakly in protest, rubbing her bruised posterior, but says no more." ) + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? "  Her cock hangs limply over her cum-" + ((player.cumQ() > 1000) ? "bloated" : "soaked") + " stomach, still leaking copious amounts of futa-jizz over her thigh.  It twitches gently, giving one final spurt before laying to rest." : "") + "  You pant lightly, leaning on the tree to catch your breath and looking down as she stares up at you, utterly exhausted.\n\n");
...
function putItInAKitsunesAssWin():void {
	...
	if(monster.hairColor == "red" && flags[redheadIsFuta] > 0) {
	...
	outputText("\"<i>Heh...  not bad for...  a mortal,</i>\" she says, resuming her previous teasing, though you can tell her heart isn't in it by the satisfied look on her face" + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? " and the lazy droop of her spent, flaccid member" : "") + ".\n\n");
...
function tribbingWithAKitsune():void {
	...
	outputText("\"<i>Haha, what do you think you're doing, you silly mortal...</i>\" she says, chuckling as you position yourself over her, settling your [butt] down on her splayed thighs, your " + vaginaDescript() + " meshing seamlessly with hers." + ((player.cocks.length > 0) ? sMultiCockDesc() + " drape" + ((player.cocks.length > 1) ? "s" : "" ) + " over the plump round cheeks of her prodigious posterior, leaking strands of precum onto her bundle of luxurious tails" + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? ", while her own member dangles a few inches above her nose." : ".") : "" ) + "  When your slow, measured grinding resumes, you moan with approval, finding this new position to be much more satisfying.\n\n");
...
function kitsunesGetBonedBy3PlusTentacles():void {
	...
	outputText("A trembling moan escapes her throat as a pair of your writhing members begin to slide against her groin, one running across her drenched slit while the other threads itself between her plump cheeks." + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? "  Your tentacle slithers around her cock swiftly, wrapping tightly around the base and squeezing it from end to end." : "" ) + "  Moaning lewdly, she tries to buck her hips toward you, having some difficulty due to her odd positioning and entanglement in the twisting jungle of your cocks.\n\n");
	...
	outputText("Her hands tremble and shake, reaching out for you pleadingly.  She grasps your shoulders with a vicelike grip, lowering her legs to wrap around your [hips] and pull her tighter to your form.  With her newfound stability, she begins to thrust herself against your tentacles in earnest.  Tears of joy stream from her eyes as you pummel her orifices mercilessly, her flesh distorting visibly from the intrusion of your distended tendrils." + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? "  Her cock trembles and throbs within the coils of one of your tentacles, spurts of precum flinging out as you roughly stroke the pulsing shaft." : "" ) + "\n\n");
	...
	outputText( "\n\nPanting with exertion, you hold her warm body close to yours for some time, watching sense gradually return to her brutalized body.  One by one, your tentacles withdraw, lowering her" + ((player.cumQ() > 350) ? " bloated" : "") +" body to the ground carefully.  Her chest heaves deeply, " + ((monster.hairColor == "red" && flags[redheadIsFuta] > 0) ? "her cock flopping limply over her thigh as your tentacle releases its grasp and her" : "") + " tongue hanging out of one side of her open mouth.");

	outputText("\n\nShe lazes in the grass in a relaxed stupor, a huge grin plastered across her face.  Her hands slide down across her skin " + ( ((player.cocks.length > 3) || (monster.hairColor == "red" && flags[redheadIsFuta] > 0)) ? "and draw trails in the semen smearing her front" : "") + ((player.cumQ() > 350) ? ", caressing her cum-filled belly" : "") + " while she moans in satisfaction, uttering a heaving sigh of approval.\n\n");
...
function kitsuneShrine():void {
	...
	flags[KITSUNE_SHRINE_VISIT]++;
	//[Read Books] [Meditate] [Steal Statue] - [Leave]
	menu();
	addButton(0,"Read Books",readKitsuneBooks);
	if(flags[TOOK_KITSUNE_STATUE] == 0) addButton(1,"Meditate",meditateLikeAKitsuneEhQuestionMark);
	if(hasItem("GldStat",1) || flags[TOOK_KITSUNE_STATUE] == 0) addButton(2,"Statue",stealAStatue);
...
function meditateLikeAKitsuneEhQuestionMark():void {
	clearOutput();
	if(hasItem("FoxJewl",1) && player.tailType == 13 && player.tailVenom < 9 && player.tailVenom + 1 <= player.level && player.tailVenom + 1 <= player.inte/10 && player.earType == 9 && player.hasPerk("Corrupted Nine-tails") < 0 && player.hasPerk("Enlightened Nine-tails") < 0) {
