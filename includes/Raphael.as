...
public function RaphaelFirstMeetingSLAP():void {
	...
	flags[RAPHEAL_COUNTDOWN_TIMER] = 0;
	flags[REJECTED_RAPHAEL] = 1;
	doNext(1);
}
...
public function RaphaelEncounterIIDressFollowup():void {
	...
	if(RaphaelLikes()) {
		...
		outputText("What do you do?", false);
		flags[RAPHAEL_DISGUSTED_BY_PC_APPEARANCE] == 0;
		//[Reject] [Frisk] [Date]
	...
	//({If player does not meet the first encounter requirements:}
		...
		doNext(1);
		flags[RAPHAEL_DISGUSTED_BY_PC_APPEARANCE] = 1;
...
public function RaphaelChooseReject():void {
	...
	//{Game Removal.}
	flags[RAPHEAL_COUNTDOWN_TIMER] = 14;
	flags[REJECTED_RAPHAEL] = 1;
	doNext(1);
}
...
public function RaphaelPicnicSkill():void {
	...
	if(flags[RAPHAEL_RAPIER_TRANING] == 0) {
	...
	if(player.spe < 30 || flags[RAPHAEL_RAPIER_TRANING] == 0) {
		...
		flags[RAPHAEL_RAPIER_TRANING] = 1;
	}
	//{Fencing practice variables: Speed 30-39 Must play out at least once}
	else if(player.spe < 39 || flags[RAPHAEL_RAPIER_TRANING] == 1) {
		...
		flags[RAPHAEL_RAPIER_TRANING] = 2;
	}
	//{Fencing practice variables: Speed 40-49, Must play out at least once}
	else if(player.spe < 49 || flags[RAPHAEL_RAPIER_TRANING] == 2) {
		...
		flags[RAPHAEL_RAPIER_TRANING] = 3;
	}
	//{Warning when you have 50 speed, played through all 3 variables and choose {Skill} at the opening picnic}
		...
		outputText("What do you do?", false);
		flags[RAPHAEL_RAPIER_TRANING] = 4;
		//[Fence] [Discuss]
...
public function fenceRaphaelSexilyPtII():void {
	...
	if(flags[RAPHAEL_INTELLIGENCE_TRAINING] == 4) {
...
public function postRaphaelCoitus():void {
	...
	if(flags[RAPHAEL_RAPIER_TRANING] == 4) {
	...
	if(flags[RAPHAEL_RAPIER_TRANING] != 4) doNext(1);
}
...
public function RaphaelPicnicChooseThieving(newl:Boolean = true):void {
	...
	if(flags[RAPHAEL_INTELLIGENCE_TRAINING] == 0) {
	...
	if(player.inte < 30 || flags[RAPHAEL_INTELLIGENCE_TRAINING] == 0) {
		...
		flags[RAPHAEL_INTELLIGENCE_TRAINING] = 1;
	}
	//{Picnic Thieving 30-39 Int. Must play out at least once.}
	else if(player.inte < 39 || flags[RAPHAEL_INTELLIGENCE_TRAINING] == 1) {
		...
		stats(-1,-1,0,3,0,1,40,0);
		flags[RAPHAEL_INTELLIGENCE_TRAINING] = 2;
	}
	//{Picnic Thieving at 40-49 Int. Must play out at least once.}
	else if(player.inte < 39 || flags[RAPHAEL_INTELLIGENCE_TRAINING] == 2) {
		...
		flags[RAPHAEL_INTELLIGENCE_TRAINING] = 3;
	}
	//{Int Thieving Apex Warning: When the PC's intelligence is at 50 or higher.}
		...
		outputText("What do you do?", false);
		flags[RAPHAEL_INTELLIGENCE_TRAINING] = 4;
		//[Fencing] [Flirt]
...
public function thieveryEnding():void {
	...
	if(flags[RAPHAEL_RAPIER_TRANING] != 4) outputText("You try to reach for the magnificent ornament hanging off his hips, but as the fox continues to move, you lack the nimbleness to remove it.\n\n", false);
...
public function quiksilverFawkesEndGame():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00149] == 1) {
...
public function QuiksilverFawkesPtII():void {
	...
	if(flags[REJECTED_RAPHAEL] == 1) outputText("<i>\"I can smell traces of him,\"</i> Hati states, nearing the waterbarrel, <i>\"but it's hard to tell.  There're too many other scents here.\"</i>\n\n", false);
	...
	if(player.armorName == "red, high-society bodysuit") outputText("  And suddenly, his eyes and ears perk.  <i>\"Isn't that the stolen clothing of the widow Camembert that she's wearing? You know, before she mysteriously dropped the charges after a nightly visit from the fox? Should we confiscate it anyway?\"</i>", false);
	outputText("\n\n", false);

	outputText("<i>\"Come now, Hati. No need to concern ourselves with that,\"</i> Skoll reproves.  <i>\"And I'm sure we can rely on our upstanding citizens not to actively harbor a criminal.\"</i>  He looms in over you.\n\n", false);

	outputText("The knowledge that you're not technically a citizen does nothing to alleviate the most intimidating sensation of the large wolf standing in so close to you.  He smells of the night.  However, when you look him in his eyes, intimidation does not seem to be Skoll's intent.  Instead, he has a compassionate look in his eyes, enriched with years of wisdom.  He's trying to appeal to your moral sense.  <i>\"We know of the fox's ways.  We'd understand if he might have appeared dashing and handsome at first.\"</i>  And he leers deeply into your soul.  <i>\"Now, I must ask again. Have you seen him?\"</i>\n\n", false);
...
public function coverForRaphael():void {
	...
	outputText("Your hair stands upright when he leans back in over barrel and appears to look down into the water, but you struggle not to slake a sigh of relief when he merely starts drinking from it instead.  A few long laps with his tongue and the wolf is sent cringing and shaking his head at the taste.  <i>\"Even the water tastes of him.\"</i>\n\n", false);
...
public function coverForRaphaelII():void {
	...
	//Concludify things
	flags[RAPHEAL_COUNTDOWN_TIMER] = -2;
	doNext(1);	
}
...
public function betrayRaphael():void {
	...
	flags[RAPHEAL_COUNTDOWN_TIMER] = -1;
	//1k gems?
	doNext(1);
}
...
public function evenMoreRaphaelIntro():void {
	...
	outputText("\n\n\"<i>It's a little pastime I entertain, this garden of mine.  I heard tale that gentlemen are supposed to pursue the nobler arts - gardening being one of them.  Naturally, I couldn't help but tack on and see how I'd do instead.</i>\"");
...
public function cunnilingusWithRaphael():void {
	...
	outputText("\n\nWith his paws, Raphael raises up your butt and crotch just a few inches into the air, like elevating it for a special occasion.  It encourages you to bend your knees and spread your legs, opening yourself up and presenting your womanhood fully.  Lacking all shame and still looking you into the eyes, the fox then does the lewdest thing.  Maintaining eye contact and without blinking, he rolls out his long red tongue and brushes it against your " + vaginaDescript(0) + ".  The sensation of the bawdy rough thing lapping up your sensitive gash's gathered dew takes you completely by surprise and you let your head fall back and moan.  It feels like he's worshipping you by lapping through the trembling fruit.  At the very zenith of his lashes, he takes extra time to twirl around your " + clitDescript() + " in a vortex of swirling tastebuds.");
	outputText("\n\nHe's good at it.  With every new lap, he sinks a little deeper into your crevase to partake of the moist, parting the trembling lips as he runs through the furrow and ravages your trembling buzzer, sometimes doing all these three things at the same time with the full length of his limber tongue.  It roves, lays along the entire length and falls neatly into place.");
	
	outputText("\n\nAfter five minutes of this, sending you gasping and writhing, you dare to raise your head and look down with a fierce blush.  Raphael is still at it.  He looks up and patiently resumes eye contact, like you're an invitee to the discource between him and the glistening red, tongue-lashed and dilated femslit before him.  The look he throws you is enough to send you over the edge and you cum hard, ");
	//({Low wetness multiplier})
	if(player.wetness() < 3) outputText("with fluttering cunt");
...
public function talkWithRedFoxLooksOutHesASpy():void {
	...
	outputText("\n\n\"<i>What about them?</i>\"  He shrugs, \"<i>They don't bother me.  They're not much for material wealth or fancy clothes, are they?</i>\"  He chuckles as he manages to retrieve a wooden goblet and a crystal glass.  \"<i>Unless it concerns golden phalluses or jeweled nipple piercing.  Then I'd be all over it, although even I would need to consider stealing such 'used' goods.</i>\"");
