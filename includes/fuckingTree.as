...
function treeMenu(output:Boolean = true):void {
	if(output) clearOutput();
	var ride:int = 0;
	var fuck:int = 0;
	var burnIt:int = 0;
	if(flags[FOUGHT_HOLLI] > 0) {
		...
		if(output) {
			...
			if(flags[HOLLI_SUBMISSIVE] == 1) {
				outputText("\n\n\"<i>Welcome back, [name].  ");
				if(flags[TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0) outputText("Did you want to fertilize me again?  My flower is ready and waiting...");
				else if(flags[TIMES_RIDDEN_FLOWER] > 0 && rand(3) == 0) outputText("Did you want to use my tentacles again?  They've been restless without you...");
				...
				outputText("\n\n\"<i>Welcome back, [name].  ");
				if(flags[TIMES_FUCKED_FLOWER] > 0 && rand(3) == 0) outputText("Did you want to fertilize me again?  Gardening is such exhausting work, but so rewarding...");
				else if(flags[TIMES_RIDDEN_FLOWER] > 0 && rand(2) == 0) outputText("Did you come back to play again?  My tentacles are so restless.");
			...
			if(flags[HOLLI_FRUIT] > 0) {
				if(flags[HOLLI_FRUIT] == 1) outputText("\n\nA large, purple fruit hangs from Holli's branches, twisting softly in the breeze.  It looks succulent and ripe.");
				else outputText("\n\n" + Num2Text(flags[HOLLI_FRUIT]) + " large, purple fruits hang from Holli's branches, twisting softly in the breeze.  They look succulent and ripe.");
				//Optional Addition: 
				if(flags[HOLLI_FRUIT_EXPLAINED] > 0) outputText("  A very small, strange part of you actually feels proud to have made something like that with the tree-woman.");
			}
		}
		if(flags[HOLLI_SUBMISSIVE] == 0) {
			var eat:int = 0;
			if(flags[HOLLI_FRUIT] > 0) eat = 3866;
			if(player.hasCock() && player.lust >= 33) fuck = 3863;
			if(player.hasVagina() && player.lust >= 33) ride = 3865;
			
			var guardT:String = "";
			if(flags[HOLLI_DEFENSE_ON] == 1) guardT = "Stop Guarding";
			...
			if(flags[HOLLI_FRUIT] > 0) addButton(4,"Eat A Fruit",eatHolliFruit);
...
function fertilizeHolli(cock:Boolean = true):void {
	...
	if(odds >= rand(101)) {
		flags[HOLLI_FRUIT] += 1 + rand(Math.floor(odds/10));
		trace("Holli pregged!");
	}
}
...
function amilyHatesTreeFucking():void {
	clearOutput();
...
function slapAmilysWhoreFace():void {
	...
	if(flags[AMILY_NOT_FURRY] == 0) outputText("nose");
	...
	if(flags[AMILY_NOT_FURRY] == 0) outputText("mouse-");
...
function eatHolliFruit():void {
	...
	if(flags[HOLLI_FRUIT] == 1) outputText("the fruit");
	else outputText("one of the fruits");
	outputText(".  ");
	if(flags[HOLLI_FRUIT_EXPLAINED] == 0) {
	...
	if(rand(2) == 0 && player.cockTotal() > player.tentacleCocks()) {
		...
		while(temp < player.cockTotal()) {
			if(player.cocks[temp].cockType != 4) choices[choices.length] = temp;
			temp++;
		}
...
function askHolliToWatch():void {
	...
	if(flags[HOLLI_DEFENSE_ON] == 0) {
		...
		if(flags[THREATENED_HOLLI] > 0) outputText("<b>  You could always just slap the cunt.</b>");
...
//attack noun: lash
//Failing to Escape -Z
function escapeFailWithHolli():void {
	clearOutput();
	outputText("You ");
...
function holliBonusHealing():void {
	//(monster hp < 100%)
...
function lightHolliOnFireMagically():void {
	if(monster.hasStatusAffect("Jojo Is Assisting") >= 0) {
		if(monster.hasStatusAffect("Holli Burning") < 0) {
...
function fuckinJamanjiFlowerDarts():void {
...
//constrict -Z
function holliConstrictAttack():void {
...
function struggleOutOfHolli():void {
	clearOutput();
...
function waitForHolliConstrict(newScreen:Boolean = true) {
	...
	monster.lust -= 20;
	if(monster.lust < 20) monster.lust = 20;
	stats(0,0,0,0,0,0,15 + player.sens/5,0);
	combatRoundOver();
}
//heal -Z
//used if monster HP < some level
function healHolli():void {
	clearOutput();
...
//>Lose
function enjoyYourBadEndBIYAAAATCH():void {
	clearOutput();
...
function holliPlaysWithPenisesBadEnd():void {
	...
	outputText("\n\nBy the time the girl reaches her third orgasm, her partners are tense and near the breaking point; with an shudder and a stilling of the shaking branches, the silent one opens " + player.mf("his","her") + " mouth in mute ecstasy and ejaculates, [(multicock)spraying the girl's stomach and tits with waste seed from " + player.mf("his","her") + " unused manhood and ]filling the Champion's pussy with so much semen it cascades from her stretched hole as Holli continues to pump.");
...
function holliAndGenderlessSittingInATree():void {
	...
	if(player.skinType != 1 && player.skinType != 2) outputText(player.skinTone + "-colored");
	...
	if(player.skinType ==2 ) outputText("a quiver of arousal shakes its scales");
	else if(player.skinType == 1) outputText("its fur begins to prick up with arousal");
