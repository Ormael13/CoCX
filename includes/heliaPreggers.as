...
function applyHelspawnName():void {
	...
	flags[HELSPAWN_NAME] = mainView.nameBox.text;
...
function helspawnIsASlut():void {
	...
	outputText(" as the intruder passes by, ready to defend yourself and your loved ones.  The moment they're out of reach, you roll onto your [feet], looking around for signs of a massed demon attack.  Instead, you see the outline of " + flags[HELSPAWN_NAME] + " in the dying embers of the fire light, leading what looks like a spider girl by the hand toward her bed.");
	
	outputText("\n\nYou creep closer, trying to get a closer look at what your ");
	if(flags[HELSPAWN_DADDY] == 0) outputText("daughter ");
	else outputText("ward ");
	outputText("is doing.  Well, it doesn't take a genius to figure that out, really, but still, you have to be sure.  You slide right up to them, just out of their lines of sight, as " + flags[HELSPAWN_NAME] + " turns around, letting the spider-kin unclasp her bra and grab her tits, squeezing the small mounds as " + flags[HELSPAWN_NAME] + " suppresses a moan, reaching back to slip a hand into the spider's crotch, grabbing her - his dick.");
	
	outputText("\n\nWell then. You suppose you ought to go stop " + flags[HELSPAWN_NAME] + " from making a whore of herself... but then again, she's old enough to make her own mistakes by now.");
	if(player.cor >= 50) outputText("  And by mistake you clearly mean old enough to get her ass fucked in by a cute little spider trap... you contemplate jumping in on it, but with a sigh figure it's " + flags[HELSPAWN_NAME] + "'s catch.  Let her enjoy it.");
	menu();
...
function helspawnDoNothing():void {
	clearOutput();
	spriteSelect(68);
	outputText("You turn around and head back to your bed.  As soon as you're under your blanket, your ears are assaulted with the quiet moans and grunts of pleasure coming from " + flags[HELSPAWN_NAME] + "'s bed as she and her first little boyfriend get it on.  You can practically hear the tail-pegging from here.  How cute!");
	//{HelspawnSlutty +10}
	flags[HELSPAWN_PERSONALITY] += 10;
	doNext(1);
}
...
function helSpawnStopFucking():void {
	clearOutput();
	spriteSelect(68);
	outputText("You sigh and step out of the shadows, grabbing the feminine little spider boy by the shoulder and yanking him away from " + flags[HELSPAWN_NAME] + ".  \"<i>And just what the hell do you two think you're doing?</i>\" you yell, scowling ");
	if(player.tallness >= 90) outputText("up ");
	else if(player.tallness <= 72) outputText("down ");
	outputText("at your ");
	if(flags[HELSPAWN_DADDY] == 0) outputText("daughter ");
	else outputText("ward ");
	outputText(". " + flags[HELSPAWN_NAME] + " gives a shrill cry of surprise and tumbles away from you, trying to cover herself as best she can; the spider trap squirms in your unrelenting grasp, one set of arms covering his rapidly-wilting erection as the other flails around, trying to get you off of him.  Not happening.");
	
	outputText("\n\n\"<i>You!</i>\" you say, pointing an accusing finger at " + flags[HELSPAWN_NAME] + ".  \"<i>What the hell, kid?  Who the fuck is this?</i>\"");
	
	outputText("\n\n\"<i>That's, uh.... Alex, meet ");
	if(flags[HELSPAWN_DADDY] == 0) outputText("my dad");
	...
	outputText("\n\n\"<i>" + championRef() + "...</i>\" " + flags[HELSPAWN_NAME] + " groans as you grab the spider boy and toss him out of camp, watching with hawk-like eyes as he slinks back to his jungle.  When he's gone, you sigh and walk over to " + flags[HELSPAWN_NAME] + ", wiping a tear from her cheek.  Putting an arm around her shoulder, you give her a long, long talk about not being a whore.");
	
	outputText("\n\n\"<i>B-but mom...</i>\"");
	
	outputText("\n\n\"<i>That's why you </i>shouldn't<i> be bringing men home.  Or women.  Or anything else, kiddo.");
	//If No Hel Love:
	if(flags[HEL_LOVE] != 0) outputText("  You don't want to be a whore like your mother, do you?  The girl that everyone's had a ride with.</i>\"");
	//if HelLove:
	if(flags[HEL_LOVE] == 1) outputText("  I'm not going to a raise a whore, " + flags[HELSPAWN_NAME] + ".  Your mother's made mistakes, sure, but...</i>\" you sigh.");
	outputText("  You don't have to be that person, " + flags[HELSPAWN_NAME] + ".  Don't just give yourself out to the first person that asks, you know?  Save it for someone you really care about.  Trust me, it'll be worth it.</i>\"");
...
function helspawnAllGrownUp():void {
	...
	outputText("\n\n<b>" + flags[HELSPAWN_NAME] + " has been added to the Followers menu!</b>");
	flags[HELSPAWN_AGE] = 3;
	flags[HELSPAWN_GROWUP_COUNTER] = 0;
	doNext(13);
}


//Helspawn’s Main Menu @ Camp [Followers Tab]
function helspawnsMainMenu():void {
	clearOutput();
	outputText("You call " + flags[HELSPAWN_NAME] + " over to you and she comes running, ");
...
function hugHelspawn():void {
	...
	if(player.tallness <= 72) {
		outputText("You reach up and pull " + flags[HELSPAWN_NAME] + " down to your level, wrapping your arms tight around her slender shoulders.  Grinning from ear to ear, the towering girl squeezes you right back, her powerful muscles just about crushing your smaller body.  Her tail slips around your [hips], the warm leather binding you to your beloved ");
		if(flags[HELSPAWN_DADDY] == 0) outputText("daughter ");
		else outputText("ward ");
	...
	//{If PC is 6' or taller:}
		...
		outputText("You grab " + flags[HELSPAWN_NAME] + " and pull her into a tight hug.  She lets out a girlish giggle as she falls into your arms and squeezes you right back.  Her powerful arms threaten to crush your bigger body, smothering you in affection.  Her tail slips around your [leg], the warm leather binding you to your beloved ");
		if(flags[HELSPAWN_DADDY] == 0) outputText("daughter ");
		else outputText("ward ");
...
function talkToHelspawn():void {
	...
	if(flags[HAD_FIRST_HELSPAWN_TALK] == 0) {
		...
		//{if Sluttymander:
		if(flags[HELSPAWN_PERSONALITY] >= 50) outputText("  Even if she wants to be just like mom, she shouldn't be bringing monsters home.  Some of them may be dangerous, and she's not a warrior quite like you and Helia yet.");
		...
		if(flags[HELSPAWN_DADDY] == 0) outputText("Of course, as her father, you could always say it's your duty to keep her happy...");
	...
	//{Kiha must be at camp}
		...
		outputText("\n\n\"<i>C'mon, " + championRef() + ", try it!</i>\" " + flags[HELSPAWN_NAME] + " adds happily.  \"<i>It's aunt Kiha's favorite recipe.</i>\"");
	...
	//{Needs Rath at camp]
		...
		outputText("\"<i>" + flags[HELSPAWN_NAME] + "!</i>\" you hear a ragged old voice call, \"<i>Get back here this instant!  I'm not done with you yet!</i>\"");
		
		outputText("\n\n\"<i>Coming!</i>\" " + flags[HELSPAWN_NAME] + " calls back as Rathazul shuffles into view, waving around what looks like a tiny hammer.  Whispering, " + flags[HELSPAWN_NAME] + " says, \"<i>Could you come with me, " + championRef() + "? Rathazul weirds me out.</i>\"");
		
		outputText("\n\nChuckling, you nod and follow " + flags[HELSPAWN_NAME] + " as she returns to Rath's makeshift laboratory and seats herself on one of his tables.  Grumbling about kids, the old rat proceeds to rap his little hammer on her knee - nearly getting kicked in the face for his trouble - before asking her to hold her breath as he listens to her heartbeat.  She's nearly blue in the face before he nods approvingly and tells her she can go. " + flags[HELSPAWN_NAME] + " goes running, happy to get away as Rath turns to you, shaking his head.");
		...
		if((amilyFollower() && !amilyCorrupt())) outputText("Little Amily's solution was makeshift at best, and the children are beyond my reach to study.  Still.  ");
	...
	else if(temp <= 2 && milkSlave() && flags[MILK_SIZE] > 0) {
		outputText("\"<i>Hey, have you seen " + flags[MILK_NAME] + " around anywhere, " + championRef() + "?</i>\" " + flags[HELSPAWN_NAME] + " asks, looking around the camp.  \"<i>Mom asked me to milk her, but I haven't seen her around.  She wouldn't have run off, would she?</i>\"");
		if(flags[MILK_SIZE] == 1) outputText("\n\nYou tell her it'd be more of a wobble, but");
		else outputText("\n\nYou doubt she would have; " + flags[MILK_NAME] + " seems to have a fancy for you, but");
		outputText(" it wouldn't hurt to check around.  She does tend to wander into the traps around camp from time to time.  Offering " + flags[HELSPAWN_NAME] + " your hand, you start searching the camp for your missing milk maid.  The two of you make a circuit around camp, checking the traps and behind various rocks and hollows, to no avail.  Finally, as you're starting to get worried about the dusky maid, you happen to look into the steel pool in her part of camp.  Sure enough, you spy " + flags[MILK_NAME] + " curled up at the bottom of the pool, cuddled up with a ragged blanket to catch a quick nap.");
		outputText("\n\n\"<i>Aww,</i>\" " + flags[HELSPAWN_NAME] + " grins, looking down at the napping " + flags[MILK_NAME] + ".  \"<i>Guess I can milk her later, then.</i>\"");
		menu();
	...
	//{Isabella must be at camp}
		...
		outputText(".  You smile as Isabella seats herself beside " + flags[HELSPAWN_NAME] + ", joining their voices together for the final chorus that leaves them both shaking as " + flags[HELSPAWN_NAME] + " strums the last, desperate notes, nearly clawing through the strings in her passion.");
		
		outputText("\"<i>");
		if(!isabellaAccent()) outputText("Sehr gut");
		else outputText("Very good");
		outputText(" little " + flags[HELSPAWN_NAME] + ",</i>\" Isabella says, pulling the young 'mander into a great big hug that threatens to smother her betwixt the cowgirl's massive bosoms.  Grinning, you congratulate " + flags[HELSPAWN_NAME] + " on her performance, showering your girl in praise before you depart, leaving her to Isabella's instruction.  As you walk away, you can't help but notice Helia standing a short ways off, rubbing her eyes.");
		
		outputText("\n\nWhen you approach, your lover smiles at you, saying, \"<i>I always wanted to be a bard when I was a little girl.  I'm... I'm glad " + flags[HELSPAWN_NAME] + "'s getting the chance, at least.</i>\"");
		doNext(13);
	}
	else {
...
function dontFuckAlex():void {
	clearOutput();
	outputText("Stroking " + flags[HELSPAWN_NAME] + "'s hair, you try to tell her that there's a better way, that she doesn't have to just fuck everything she comes across.  Her mother's that way, sure, but she could do so much better, so much more with herself than giving in to constant lust.");
	//{If Sluttymander:}
	if(flags[HELSPAWN_PERSONALITY] >= 50) {
		outputText("\n\n" + flags[HELSPAWN_NAME] + " scoffs, shaking her head.  \"<i>But I </i>like<i> mom's way.  I love jilling off, the feeling of my cunt getting stretched and my ass being torn apart by the big toys I buy in town.  That's who I am, " + championRef() + ".  I'm like mom, and I don't think... no, I know I don't want to change.  I love sex, even if you stopped me and Alex from it.  I want to feel a gang of minotaurs raping me, I want to jump on a drider's cock and ride him down.  I want to do the things I heard mom doing, and that's just what I'm going to do. I'm sorry.</i>\"");
		
		outputText("\n\nYou try to say something, but " + flags[HELSPAWN_NAME] + " steps away, calling back, \"<i>I love you, " + championRef() + ", but I can't be what you want me to be.  I'm going to be a slut like mom, and there's nothing you can do to stop me!  It's who I was born and raised to be.</i>\"");
		
		outputText("\n\nYou may have made a horrible mistake.");
	}
	//{If Chastemander:}
	else {
		outputText("\n\n" + flags[HELSPAWN_NAME] + " nods slowly, taking in your words.  \"<i>I... I know, " + championRef() + ".  I'm sorry I brought Alex home last night.  It won't happen again.  I don't... I love mother, but I don't want to be like her.  A whore.  She's a great woman, but the things she does... they disgust me.</i>\"");
...
function ohGodEwKihaAndHelspawnSuckAtCooking():void {
	clearOutput();
	outputText("You spew the stew onto the ground and grab a nearby waterskin, trying to flush the taste from your mouth.  Kiha and " + flags[HELSPAWN_NAME] + " look on with horror as you wipe your mouth and begin to try and coherently explain just how god awful whatever that... STUFF... you just put in your mouth was.  Shock turns to anger before you’re halfway through admonishing the pair of scaly redheads, Kiha scowls at you and snatches the ladle from you.");
	outputText("\n\n\"<i>Oh, what the fuck do you know anyway you big ass!  It's perfectly fine, isn't it " + flags[HELSPAWN_NAME] + "?</i>\" she growls, spooning up a mouthful of the stuff before you can stop her.  A heartbeat after she swallows, Kiha goes completely stiff; her eyes growing as wide as saucers as they water.  She collapses backwards, falling onto her back with a muted <i>THUD</i>.");
	
	outputText("\n\n\"<i>K-Kiha?</i>\" " + flags[HELSPAWN_NAME] + " yelps, watching the dragoness collapse into a pile of limp scales and leather.  \"<i>Are you okay?  Is she okay, " + championRef() + "?</i>\"");
	...
	outputText("\n\nYou and " + flags[HELSPAWN_NAME] + " are soon working away, stirring up your own special blend of your rations and the spices she and Kiha gathered for the first attempt.  Once you've got the basics in, you step back and hand the ladle over, telling the young 'mander to go wild.  Grinning like a kid, she goes to work, mixing in a wild combination of ingredients.  The cauldron's full to bursting by the time she's done, and no sooner does she ladle out the first bowel of it than Kiha sits up, groaning.");
	
	outputText("\n\n\"<i>The fuck happened?</i>\" the dragoness hisses, rubbing her temple.  \"<i>Feel like I got hit with a hammer.</i>\"");
	
	outputText("\n\n\"<i>You, uh, fell off the edge, aunt Kiha.  And spilled all the stew.  Here, we remade it for you!</i>\" " + flags[HELSPAWN_NAME] + " lies, presenting \"aunt Kiha\" with a bowl full of an aromatic brown goop, full of carrots and spices floating atop it like boats on a placid lake.  At least it smells better.");
	...
	outputText("\n\nYou just shake your head and grab a bowl, sitting down with the scaly ladies as you enjoy your lunch, trying to ignore the little shit-eating grin " + flags[HELSPAWN_NAME] + "'s sporting all the while.");
	doNext(13);
}
function umYum():void {
	clearOutput();
	...
	outputText("\n\nCringing, you set the offered bowl back down, squeaking out that it's delicious, thank you very much, but you have something to take care of right now; maybe you can have more later.  Kiha rolls her eyes and shoos you off before she and " + flags[HELSPAWN_NAME] + " spoon out their lunch.");
...
function helSpawnMilkHerNow():void {
	...
	outputText("\n\n" + flags[HELSPAWN_NAME] + " gives a rebellious huff as she clambers down, slipping down to the bottom and giving " + flags[MILK_NAME] + " a gentle nudge.  \"<i>C'mon, cutey, wake up.  Time to relieve some pressure.</i>\"");
	
	outputText("\n\nYawning powerfully, " + flags[MILK_NAME] + " rises to her knees and rubs the sleep from her eyes");
	if(flags[MILK_SIZE] == 2) outputText(", already working to unfasten her shift");
	outputText(".  \"<i>Suckle?</i>\" she asks, turning her full teats toward " + flags[HELSPAWN_NAME] + ".");
	
	//If Sluttymander:
	if(flags[HELSPAWN_PERSONALITY] >= 50) outputText("\n\n\"<i>Wouldn't have it any other way!</i>\" the salamander answers with an eager grin, easing herself into " + flags[MILK_NAME] + "'s arms and wrapping her lips around one of her prominent teats. " + flags[MILK_NAME] + " gives a pleasured little shudder as " + flags[HELSPAWN_NAME] + " starts to suckle like a babe, drinking down the dusky maid's seemingly endless supply of milk.");
	
	//If Chastemander:
	else outputText("\n\n\"<i>Sorry, " + flags[MILK_NAME] + ", you know I'm not really into that.  Just turn around and sit back, alright?</i>\"  With a sigh, " + flags[MILK_NAME] + " does as she's asked, leaning back into " + flags[HELSPAWN_NAME] + "'s arms as the young salamander goes to work, clawed fingers gently caressing her prominent nubs until a white flow springs fort, splashing her knees as the dusky maid gives a little gasp of pleasure.");
	
	outputText("\n\n<i>They seem to get on alright</i>, you think as you watch the pair of them.  It's nice to have someone else around to help keep " + flags[MILK_NAME] + "'s production under control.  Poor thing just never stops lactating.");
	doNext(13);
}
...
function sparHelspawn():void {
	clearOutput();
	outputText("You ask " + flags[HELSPAWN_NAME] + " if she's up for some battle practice, and she answers with an eager nod as she grabs her weapon.");
	//If Sluttymander: 
...
//Combat Menu
//You're fighting " + flags[HELSPAWN_NAME] + "! Level: 18
function helSpawnDesc():void {
	monster.long = flags[HELSPAWN_NAME] + " is a young salamander, appearing in her later teens.  Clad in ";
	if(flags[HELSPAWN_PERSONALITY] >= 50) monster.long += "a slutty scale bikini like her mother's, barely concealing anything";
	...
	else if(flags[HELSPAWN_WEAPON] == "scimitar and shield") {
...
function helSpawnAI():void {
	var choices:Array = new Array();
	choices[choices.length] = helspawnTwinStrikes;
	//Bowmander only
	if(flags[HELSPAWN_WEAPON] == "bow") choices[choices.length] = calledShot;
	//Zerker ability
	if(monster.weaponAttack < 50 || flags[HELSPAWN_WEAPON] == "scimitar") choices[choices.length] = helSpawnBerserk;	//Shield Bash (Shieldmander Only)
	if(flags[HELSPAWN_WEAPON] == "scimitar and shield") choices[choices.length] = helSpawnShieldBash;
	//Tease (Sluttymander Only)
	if(flags[HELSPAWN_PERSONALITY] >= 50) choices[choices.length] = sluttyMander;
	//Focus (Chastemander Only)
	//Self-healing & lust restoration
	if(flags[HELSPAWN_PERSONALITY] < 50) choices[choices.length] = helSpawnFocus;
...
// Two light attacks
function helspawnTwinStrikes():void {
	//if Bowmander
...
function calledShot():void {
	outputText(flags[HELSPAWN_NAME] + " draws back her bowstring, spending an extra second aiming before letting fly!");
	var damage:Number = int((monster.str + monster.weaponAttack) - rand(player.tou) - player.armorDef);
...
function helSpawnBerserk():void {
	outputText(flags[HELSPAWN_NAME] + " lets out a savage warcry, throwing her head back in primal exaltation before charging back into the fray with utter bloodlust in her wild eyes!");
	monster.weaponAttack += 30;
	monster.armorDef = 0;
}

//Shield Bash (Shieldmander Only)
function helSpawnShieldBash():void {
	clearOutput();
	var damage:Number = int((monster.str) - rand(player.tou) - player.armorDef);
	// Stuns a bitch
	outputText(flags[HELSPAWN_NAME] + " lashes out with her shield, trying to knock you back!");
	//standard dodge/miss text
...
//Tail Whip
function tailWhipShitYo():void {
	// Light physical, armor piercing (fire, bitch). Random chance to get this on top of any other attack
	var damage:Number = int((monster.str) - rand(player.tou));
	outputText("\n" + flags[HELSPAWN_NAME] + " whips at you with her tail, trying to sear you with her brilliant flames!");
	//standard dodge/miss text
	if(damage <= 0 || combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) outputText("\nYou evade the strike.");
	else {
		outputText("\n" + flags[HELSPAWN_NAME] + "'s tail catches you as you try to dodge.  Your [armor] sizzles, and you leap back with a yelp as she gives you a light burning.");
		damage = takeDamage(damage);
		outputText(" (" + damage + ")");
	}
}
...
//Tease (Sluttymander Only)
function sluttyMander():void {
	// Medium Lust damage
	outputText(flags[HELSPAWN_NAME] + " jumps just out of reach before spinning around, planting her weapon in the ground as she turns her backside to you and gives her sizable ass a rhythmic shake, swaying her full hips hypnotically.");
	//if no effect:
...
//Focus (Chastemander Only)
//Self-healing & lust restoration
function helSpawnFocus():void {
	outputText("Seeing a momentary lull in the melee, " + flags[HELSPAWN_NAME] + " slips out of reach, stumbling back and clutching at the bruises forming all over her body.  \"<i>Come on, " + flags[HELSPAWN_NAME] + ", you can do this. Focus, focus,</i>\" she mutters, trying to catch her breath.  A moment later and she seems to have taken a second wind as she readies her weapon with a renewed vigor.");
	monster.lust -= 30;
...
function loseSparringToDaughter():void {
	//if Sluttymander:
	if(flags[HELSPAWN_PERSONALITY] >= 50) {
		...
		outputText(", " + flags[HELSPAWN_NAME] + " quickly sweeps your [legs] out from under you, dropping you right on your ass.  You collapse with a grunt, ");
		...
		outputText(", and a moment later " + flags[HELSPAWN_NAME] + "'s on you, straddling you with her powerful legs, pushing you down into the dirt.");
	...
	//Else If Chastemander:
		...
		outputText(", " + flags[HELSPAWN_NAME] + " grabs your arm, catching you before you can fall.  \"<i>Easy there, " + championRef() + ", I got you,</i>\" she says, pulling you into a quick hug.");
...
function beatUpYourDaughter():void {
	clearOutput();
	//{If Sluttymander loses to lust (you monster)}:
	if(flags[HELSPAWN_PERSONALITY] >= 50 && monster.lust > 99) {
		...
		//If No Incest: 
		if(flags[HELSPAWN_INCEST] == 0) outputText("  You wash your hands of the defeated slut and head back to camp, leaving her to work through her tension herself.");
		else {
		...
		outputText(flags[HELSPAWN_NAME] + " stumbles back, dropping her weapon and waving her arms, \"<i>I give, I give.</i>\"");
...
function maiVisitsHerKids():void {
	clearOutput();
	spriteSelect(68);
	outputText("As you head back into camp, you notice Hel and " + flags[HELSPAWN_NAME] + " are sitting around the cook fire in the center of camp, with your foster daughter balanced precariously on the lap of her own father, Mai the fox-girl.");
	...
	outputText("\n\n\"<i>Hi, [name],</i>\" Mai says with a demure smile, arms wrapped around " + flags[HELSPAWN_NAME] + "'s belly, trying to hold the daughter that's near a foot taller than she is.  Finally, laughing, she lets the young salamander go.  " + flags[HELSPAWN_NAME] + " drops down to sit beside her mother and father, and quickly has a both a smooth and scaly hand stroking her hair.");
	//If Chastemander:
	if(flags[HELSPAWN_PERSONALITY] < 50) outputText("\n\n" + flags[HELSPAWN_NAME] + " happily leans her head against Mai's leg, accepting the affection with her tail swishing gaily behind her, foxy ears twitching as Mai brushes them.");
	else outputText("\n\n" + flags[HELSPAWN_NAME] + " leans back, pulling out a flask and taking a deep swill before passing it to her father, who takes it with a knowing smile and knocks it back like a champ.");
	...
	outputText("\n\n\"<i>Heeeeeyyyyyy,</i>\" " + flags[HELSPAWN_NAME] + " whines, crossing her arms with a huff.");
	...
	outputText("\n\n" + flags[HELSPAWN_NAME] + " lights up as Mai slips the gold chain around her daughter's neck.  \"<i>Something to remember me by when I'm not around,</i>\" she says with a wink, letting her kid skip off to find a mirror.");
	
	outputText("\n\n\"<i>And you, [name],</i>\" she says, turning to you, \"<i>Thanks for taking care of " + flags[HELSPAWN_NAME] + ".  I-I mean, I know I was really making her for you and Hel anyway, but still... she's a good kid, and I love her anyway.  Keep her safe, alright?</i>\"");
	
	outputText("\n\nYou promise that you will, and with a quick nod, Mai runs after " + flags[HELSPAWN_NAME] + " to say goodbye for the day.  As she leaves, Helia grins, holding you tight against her.  \"<i>She's a good girl, [name].  Couldn't have picked a better father for our girl.</i>\"");
	doNext(13);
}

//Spider Bro's Gift
//{Requires Helspawn be fathered by a spiderbro. Play at morning.}
function spiderBrosGift():void {
	clearOutput();
	spriteSelect(68);
	flags[SPIDER_BRO_GIFT] = 1;
	outputText("As you're getting ready to head out for the day, you notice " + flags[HELSPAWN_NAME] + " prancing around camp with a long, brightly-colored scarf wrapped around her neck, standing painfully in contrast with her midnight-black scales and pale flesh.");
...
function hakonAndKiriComeVisit():void {
	clearOutput();
	spriteSelect(68);
	flags[HAKON_AND_KIRI_VISIT] = 1;
	...
	outputText("\n\nIt hardly matters, as a few minutes later, you hear a cry of unabashed glee from the edge of camp, and turn to see " + flags[HELSPAWN_NAME] + " running up to meet a pair of oncoming figures.  You wave as Hel's father swoops his granddaughter up into a great bear hug, hefting the half-breed 'mander off her feet in his powerful arms.  " + flags[HELSPAWN_NAME] + " squeals with delight as she's practically carried into camp, until Hakon sets her down and hugs her mother, pulling Hel into a tight hug before she can finish saying hello.");
	...
	outputText("\n\nTurning to " + flags[HELSPAWN_NAME] + ", Hakon smiles with pride.  \"<i>Gods and demons, girl, you've grown!  What have you two been feeding her, huh?</i>\"");
	
	outputText("\n\n\"<i>And you should see how strong she is!</i>\" Hel grins.  \"<i>Go on, " + flags[HELSPAWN_NAME] + ", show the old scales what you've got.</i>\"");
	
	outputText("\n\n\"<i>Old scales!?  I'll show you old, you little brat.  Come on, hit me!</i>\" Hakon says, just before " + flags[HELSPAWN_NAME] + " punches him right in the chest, knocking him right on his ass.  He tumbles back with a grunt, shaking his head with a wry laugh.  \"<i>Ha!  Maybe these scales are getting old after all.  This one's gonna be a hell of a fighter, though.  Should take her hunting with us one of these days.  What do you say, kid?</i>\"");
	
	outputText("\n\n\"<i>Sure!  ");
	if(flags[HELSPAWN_WEAPON] == "scimitar and shield") outputText(championRef() + "'s been teaching me how to fight.");
	...
	if(flags[HELSPAWN_DADDY] == 0) outputText("your old man");
	else outputText("[name]");
	outputText(", too.</i>\"");
