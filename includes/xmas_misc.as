...
public function candyCaneTrapDiscovery():void {
	...
	classes.EngineCore.outputText("Shortly after embarking into the plains, your thoughts drift back to your life and home in your village.  The slight chill in the air today reminds you of a time of cheer celebrated by your former friends and family.  As days grew shorter and colder, people exchanged humble trinkets, and great feasts were prepared.  ");
	if(player.cor < 50) classes.EngineCore.outputText("The memory brings a nostalgic smile to your [face].");
	else classes.EngineCore.outputText("The memory reminds you of how bland your world truly was in comparison to Mareth.");
	...
	classes.EngineCore.addButton(0,"Investigate",investigateCandyCaneBun);
	classes.EngineCore.addButton(4,"Leave",leaveBeforeMeetingCandyCaneBun);
...
public function leaveBeforeMeetingCandyCaneBun():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You're not willing to run the risk of getting a foreign cock stuffed up your [butt].");
	//(If corruption or libido>50) 
	if(player.cor > 50 || player.lib > 50) classes.EngineCore.outputText("  At least not right now.  You turn back, navigating your way back to your camp.");
...
public function investigateCandyCaneBun():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You make the decision to find out just what's going on.  You cautiously follow the small pools of goo, slightly on edge.  You approach a wall of low hanging vine which you quickly pull aside.  Sitting in the largest pudddle of semen you've seen yet, a trembling bunny-like figure kneels on its knees.");
	//(If corruption and libido<50) 
	if(player.cor < 50 && player.lib < 50) classes.EngineCore.outputText("  Your eyes are quickly drawn to the rabbit morph's stark nudity.");
	//(If corruption or libido>50) 
	else classes.EngineCore.outputText("  Your eyes are quickly drawn to the rabbit morph's finely curved ass.");
	classes.EngineCore.outputText("  A pair of heavily hanging balls confirm the figure to be at least partially male.");
	classes.EngineCore.outputText("\n\nYou make the move to ");
	//(If corruption and libido<50)
	if(player.cor < 50 && player.lib < 50) classes.EngineCore.outputText("leave, ");
	//(If corruption or libido<50)
	else classes.EngineCore.outputText("approach, ");
	classes.EngineCore.outputText("but one of your [feet] snaps a twig and causes the bunny to turn at the sound. As its large eyes lock with your, you get your first good look at the figure.  It is predominantly human looking, but the floppy ears sprouting from its head and fluff adorning him from the thighs down indicates at least a little mutation.  Two small nipples lie on a flat chest, confirming him to be a male.  His eight inch dick and kiwi-sized balls leave even less to your imagination.  He is lightly tanned, his skin colored a non abrasive brown.  The hair on his body stands in a sharp contrast to his relatively regular skin.  A mop of shoulder length hair on his head is as white as freshly fallen snow, with sprinklings of red stripes highlighting it.  You can't help but notice how nicely it frames his surprisingly soft, feminine face.  The fur covering his legs is smiliary covered.  His hips jut out notably, adding even further to his overall girly appearance.");
	classes.EngineCore.outputText("\n\n\"<i>You're not going to rape me?</i>\"  The bunny boy's remarkably high voice breaks your train of thought.  ");
	//(If corruption<50)
	if(player.cor < 50) classes.EngineCore.outputText("You calmly inform him that you didn't have the intention of violating him. ");
	//(If corruption>50)
	else classes.EngineCore.outputText("Your train of thought is derailed, but you agree to let the bunny believe whatever he wants to.");
	//(If silly mode on) 
	if(classes.EngineCore.silly()) classes.EngineCore.outputText("  You're willing to say anything to calm him down.  This kid's voice is even more shrill than that asshole from Glee!");
	classes.EngineCore.outputText("  A smile lights up his face, his shoulders visibly drop and lose their tension.   He sits like that for but a moment, before he realizes the full scope of the current situation.  His eyes grow wide while he throws down his arms with a rather unmanly yelp, covering his dripping cock.  Despite his embarrassment, it's evident that he's not going to get off unassisted.");
	flags[kFLAGS.CANDY_CANE_YEAR_MET] = date.fullYear;
	classes.EngineCore.outputText("\n\nDo you help him out?");
	//(Present \"<i>Yes</i>\" and \"<i>No</i>\" options)
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Yes",helpWithTheCandyCane);
	classes.EngineCore.addButton(1,"No",declineCandyCaneCawks);
...
public function declineCandyCaneCawks():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("Turning around a tad awkwardly, you stumble out of the glade.  Tempting as he is, you don't have time to help the random bunny orgasm.");
	classes.EngineCore.doNext(classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
}
...
public function helpWithTheCandyCane():void {
	...
	//(If corruption<50) 
	if(player.cor < 50) classes.EngineCore.outputText("  Not wanting to disturb him, you make your way back to camp and attempt to ignore the burning fire in your loins.");
	//(If corruption>50) 
	else classes.EngineCore.outputText("  Despite your rising urge to take advantage of the unknowing bunny, you decide to retire and return to camp.");
...
public function getAChristmasChicken():void {
	...
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Talk",talkToXmasChicken);
	classes.EngineCore.addButton(4,"Leave",leaveXmasChicken);
}

//[Leave]
public function leaveXmasChicken():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("Too exasperated by the plain absurdity of the situation to deal with it, you bury your face in your hands and decide to leave her to it.  Maybe some imps will shut her up for you.  You head back to camp with her piercing cries following you all the way, and prepare for the day ahead.  Her cries fade a while later; the snow thaws and the air warms soon after.");
	classes.EngineCore.doNext(classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
}
...
public function talkToXmasChicken():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("Figuring you should at least try asking her to pipe down, you continue your approach.  She finally takes notice of your footsteps crunching the snow beside her and goes still and silent for a moment as she looks up at you.  Now that you're closer, you can make out some more of her features against the snowy ground.  She wears a light dress, almost as pale as her skin and the snow around her, thin enough that you can just see her perky nipples through its white fabric.  A shiver of your own makes you wonder why she's wearing something like that on a day like this.  The thick, fluffy-looking feathers of her wings even manage to stand out as somehow even whiter than the frosty backdrop of the snow.  A faint, silvery glow seems to radiate from her, but it's probably just a trick of the light against the snowfall.");
	...
	//(if male)
	if(player.gender == 1) classes.EngineCore.outputText("How's a blowjob sound to you?");
	//(if female)
	else if(player.gender == 2) classes.EngineCore.outputText("How about I eat you out?");
	//(if herm)
	else classes.EngineCore.outputText("How about I give you a few minutes to do whatever you want with my mouth?");
	classes.EngineCore.outputText("</i>\"  You raise an eyebrow, not entirely sure what to make of her suddenly lascivious nature.  It's certainly quite a change.  She giggles.  \"<i>Oh, don't be surprised.  Where do you think we are");
	//(if sillymode)
	if(classes.EngineCore.silly()) classes.EngineCore.outputText(", scrub");
	classes.EngineCore.outputText("?</i>\"");
	classes.EngineCore.menu();
	classes.EngineCore.addButton(4,"Decline",declineChickenBlowjobs);
	classes.EngineCore.addButton(0,"Accept",acceptChristmasBJs);
}

//[Decline]
public function declineChickenBlowjobs():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("The harpy squints at you suspiciously.  \"<i>What do you mean, 'no thanks?'  Nobody says that around here!</i>\"  She rolls her eyes and sighs in an obviously exaggerated manner, then turns tail and flaps off into the distance.  You head back to camp, quite looking forward to the silence.  The air warms and the snow thaws soon after you return, leaving no trace of its presence but a few spots of slushy mud.");
...
public function acceptChristmasBJs():void {
	...
	if(player.gender == 1) {
		...
		classes.EngineCore.clearOutput();
		classes.EngineCore.outputText("Would you like her to service your male parts or lady bits?");
		classes.EngineCore.menu();
		classes.EngineCore.addButton(0,"Man Parts", acceptChristmasManBJ);
		classes.EngineCore.addButton(1,"Lady Bits", acceptChristmasWomanBJ);
	}
}
...
public function acceptChristmasManBJ():void {
	...
	if(player.cockArea(x) >= 80) {
		classes.EngineCore.outputText("You free your [cock " + (x+1) + "] from your [armor], signaling your acceptance.  Your monster flops free, and the harpy grabs at her throat reflexively when she looks at it.  She glances up to you with wide eyes and back down at it.  \"<i>A-a promise is a promise, but...</i>\"");
	...
	//(else if cock would please Goldilocks) (area<= 30 or some shit i dunno whatever the standard is)
		...
		classes.EngineCore.outputText("You free your [cock " + (x+1) + "] from your [armor], signaling your acceptance.  The immediate rush of cold air that greets it makes you hope she won't be wasting any time.  The pale harpy woman squeals at the sight, raising her hands to her chin and practically bouncing in place.  In a flash she's on her knees, stroking and licking at your meat until it's hard in front of her face.  Her hot breath against your shaft has you more than ready for the sanctuary from the biting cold.  She eyes your erect prick");
		if(player.cockTotal() > 1) classes.EngineCore.outputText("s");
		classes.EngineCore.outputText(" almost hungrily, licking her lips before swiftly engulfing almost its entire length in her mouth.  Her expert technique astounds you; you certainly didn't expect her to have any idea what she was doing but she's able to take you nearly to the base of your shaft in one motion.");
		...
		//(if only one cock)
		if(player.cockTotal() == 1) classes.EngineCore.outputText("She grabs onto your hips, enabling herself to move all the quicker.");
		//(if two cocks)
		else if(player.cockTotal() == 2) classes.EngineCore.outputText("With her other hands she starts to work the shaft of the second cock, pumping in time with the motions of her head.");
		//(if 3+ cocks)
		else classes.EngineCore.outputText("She grabs onto two of your other erect cocks with her hands and begins pumping away at them in time with the motions of her head.");
		...
		if(player.cumQ() < 250) classes.EngineCore.outputText("  She closes her eyes and looks quite contented while she swallows your spunk.");
		//(else if cum output is high)
		else if(player.cumQ() < 1000) classes.EngineCore.outputText("  Semen gushes out of you for what seems like forever, but the harpy doesn't seem to mind.  She closes her eyes and looks quite contented while she swallows your still-torrenting spunk.");
		//(else if cum output is ridonkulous)
		else classes.EngineCore.outputText("  Though at first she looks like she'll be able to handle it, the river of semen exploding from the rip of your cock doesn't stop.  Her eyes widen as the almost unending torrent of spunk fills her, rushing down her throat and into her belly.  You can almost see her bloating with the stuff before it starts to spill back out, dripping white from her lips around your shaft.");
		//(if multicock)
		if(player.cockTotal() > 1) classes.EngineCore.outputText("  Your cum sprays over her, coating her skin and plumage in a white to rival their own.  Strands of your semen dripping and running over her hardly even stand out on her bright body.");
		classes.EngineCore.outputText("  When you finish, you let the harpy go.  She looks just as satisfied as you are.  A trail of cum and saliva still connects your [cockHead " + (x+1) + "] to her; she breaks it with a lick of her lips and gives your softening member a longing glance before standing up, panting from exertion.");
...
public function approachKamiTheChristmasRoo():void {
	...
	if(flags[kFLAGS.KAMI_ENCOUNTER] == 1) {
		...
		classes.EngineCore.menu();
		if(player.hasCock() && player.cockThatFits(70) >= 0) classes.EngineCore.addButton(0,"Give Anal",giveKamiTheChristmasRooAnal);
		classes.EngineCore.addButton(1,"Receive Anal",takeItRooButtStyle);
		classes.EngineCore.addButton(2,"Give BJ",KamiBlowJob);
		if(player.cockTotal() > 1 && player.cockThatFits(70) >= 0 && player.cockThatFits2(70) >= 0) classes.EngineCore.addButton(3,"Doube-Fuck",KamiDoubleDickFuck);
	}
	else {
		classes.EngineCore.outputText("You summon the buxom waitress over, getting a much better view of her gentle face now that she's found the time to sit and talk with you.  With hazelnut-brown eyes, a gorgeous set of lips and tousled blonde hair, her faux-rebellious look is the icing on the festive cake for this busty waitress, her cute little santa hat being the strawberry on top.  Smooth, olive skin runs halfway down her meaty thighs before fading into soft fur, while her frail forearms follow the same pattern. Her huge kangaroo tail wags idly throughout your conversation, and you've gained a pretty good rhythm whilst engaging her in it. Determined to make things go well, you pause to pull out a charming one-liner when her index finger meets your ");
		if(player.femininity >= 75) classes.EngineCore.outputText("feminine ");
		else if(player.femininity <= 25) classes.EngineCore.outputText("masculine ");
		classes.EngineCore.outputText("lips abruptly.");
		...
		if(classes.EngineCore.silly()) classes.EngineCore.outputText(" and found space to scrawl a little penis in one corner. Clever");
		classes.EngineCore.outputText(".");
		classes.EngineCore.outputText("\n\nWow, well, that's pretty blunt.  What should you do now?");
		//[Present options, 'Let's go!' and 'Leave']
		classes.EngineCore.menu();
		classes.EngineCore.addButton(0,"Let's Go",chooseLetsGoKami);
		classes.EngineCore.addButton(4,"Leave",skipOutAfterOpeningKamisBox);
	}
}
...
public function skipOutAfterOpeningKamisBox():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You politely decline Kami's sexual advances, apologizing before making a turn for the door. You begin to wonder why you didn't buy anything at the bakery.");
...
public function chooseLetsGoKami():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You decide to give Kami exactly what she wants - 'tis the season for giving, after all!  You grin approvingly before heading out into the back alley, taking your time to stare at her wobbling ass.");
	...
	classes.EngineCore.addButton(1,"Receive Anal",takeItRooButtStyle);
	classes.EngineCore.addButton(2,"Give BJ",KamiBlowJob);
	if(player.cockTotal() > 1 && player.cockThatFits(70) >= 0 && player.cockThatFits2(70) >= 0) classes.EngineCore.addButton(3,"DoubleFuck",KamiDoubleDickFuck);
}
...
public function giveKamiTheChristmasRooAnal():void {
	...
	classes.EngineCore.outputText("You head briskly towards the voluptuous waitress, rearing her around by the wrists and pressing her against the wall.  \"<i>H-Hey, wait!</i>\" Kami yelps, but you sense excitement in her delicate plea.");
...
public function takeItRooButtStyle():void {
	...
	if(player.balls > 0) classes.EngineCore.outputText("your own");
	//[Else if have vagina]
	else if(player.hasVagina()) classes.EngineCore.outputText("your pussy");
	//[Else if neither]
	else classes.EngineCore.outputText("your taint");
	classes.EngineCore.outputText(".  Each time, the waitress fondles your form with her eager hands.  ");
	
	classes.EngineCore.outputText("\n\n");
	//[If player has pussy] 
	if(player.hasVagina()) classes.EngineCore.outputText("Your cunt oozes in return, envious of the tenderized hole above it.  ");
	classes.EngineCore.outputText("You dedicate a few more minutes to soaking up every moment of her animalistic romping, savoring the feeling of a hung rod in your punished posterior.  Your climax quickly comes to loom over you, however, the potent, ");
	if(player.hasCock()) classes.EngineCore.outputText("prostate");
	else classes.EngineCore.outputText("ass");
	classes.EngineCore.outputText("-numbing sensations at your rear becoming too much to bear.  Slamming back into Kami's pole one last time, you breathe a shuddered sigh of relief as your orgasm takes over, the sensation spreading throughout your entire body like electricity.");
	//[If male]
	if(player.hasCock()) classes.EngineCore.outputText("  Your pent-up cock ejaculates there and then, shamelessly gushing sloppy semen against the wall in front of you.");
	//[If female]
	if(player.gender == 3) classes.EngineCore.outputText("  Meanwhile, y");
	else if(player.hasVagina()) classes.EngineCore.outputText("Y");
	if(player.hasVagina()) classes.EngineCore.outputText("our eager pussy buckles in arousal, gushing fem-juices on the floor below you.");
	classes.EngineCore.outputText("  You're truly spent.");
...
public function KamiBlowJob():void {
	...
	if(!player.isNaga()) classes.EngineCore.outputText("You get down on your knees");
	//[Else if is Naga]
	else classes.EngineCore.outputText("You steer your serpentine half into position, leaning your upper body within arms reach of Kami's wanting member.  You reach out");
	classes.EngineCore.outputText(" and grab the thick rod impulsively with your hand, all 14 inches of the slick tool in your firm grasp.  Kami giggles in approval as you take the first few licks of the phallus, jerking your hand up and down its length slowly.  \"<i>Mmm... someone knows what they're doing.</i>\"  You spit on the head of her thick cock, smearing the improvised lubricant all over her girth generously, her digitigrade legs crossing and twitching agitatedly in response.  \"<i>Oh god hurry up and put it in your mouth already!</i>\"  You grin at her impatience, teasing her tip a bit longer before finally knuckling down.  Her first few inches slide in with much resistance, fresh saliva coating the shaft and easing her in against your tonsils smoothly and without fuss.");
	...
	if(player.hasCock()) classes.EngineCore.outputText("  You think you might still need to find an outlet for that itch of yours, with [eachCock] still erect and needing release.");
...
public function getWinterPudding():void {
	classes.EngineCore.clearOutput();
	if(player.gems < 35) {
		classes.EngineCore.outputText("Sadly, the pudding isn't free, and it looks like you don't have the 35 gems you'd need to try a bite!");
		classes.EngineCore.doNext(classes.Scenes.SceneLib.telAdre.bakeryScene.bakeryuuuuuu);
		return;
	}
	player.gems -= 35;
	classes.EngineCore.statScreenRefresh();
	classes.EngineCore.outputText("You opt for a slice of the tasty treat - you've only got until the festivities wade away, so you might as well take one now!  Do you eat the treat or save it for later?\n\n");
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0, "Eat", mutations.winterPudding, player, true, null, "Eat the delicious Winter Pudding.");
	classes.EngineCore.addButton(1, "Take", classes.Scenes.SceneLib.inventory.takeItem, consumables.W_PDDNG, classes.Scenes.SceneLib.telAdre.bakeryScene.bakeryuuuuuu, null, "Bring the pudding with you.");
...
public function polarPete():void {
	...
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Unwrap",unwrapPolarPete);
	classes.EngineCore.addButton(4,"Leave",leaveDisFukkinPolarBear);
}
//Unwrap
public function unwrapPolarPete():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("The temptation too strong, you walk closer to Pete.  Sinking down to your knees before him as he adjusts his hips to your level.  Your hands on his crimson pants, pulling them down with bated breath.  His dark ebony length flops free, long and swollen with heat before your nose. Your hand rubs along it's smooth black surface coaxing a pleased groan from Pete.");
	...
	if(player.hasCock()) classes.EngineCore.outputText("  [EachCock] grows stiff and soaks your [armor] with precum.");
	//(if vagina) 
	if(player.hasVagina()) classes.EngineCore.outputText("  Your [vagina] grows moist, dripping eager juices and soaking your [armor].");
	...
	player.orgasm();
	classes.Scenes.SceneLib.inventory.takeItem(consumables.PEPPWHT, classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
}

//Leave
public function leaveDisFukkinPolarBear():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You've decided not to trust the stranger, with a nod of your head you walk away from the man.  \"<i>Wait!  Before you go...</i>\" he speaks as he moves his massive sack in front of himself with a huff.  He leans forward, his upper body rummaging through the sack before reappearing.  He hands you a brightly wrapped gift.");
...
public function nieveBuilding():void {
	...
	if(flags[kFLAGS.NIEVE_STAGE] == 1) {
		...
		classes.EngineCore.outputText("You head back over to the snowy patch by your camp.  The crunch of snow again makes you feel just a little nostalgic, and you think of drinking hot cocoa topped with marshmallows.  With a smile on your face, you set about rolling up the soft powder-like crystals into a huge ball.  It takes some effort, but after a while you're left with a large ball of snow, which will serve as the perfect base.");
		...
		classes.EngineCore.menu();
		classes.EngineCore.addButton(0,"Snowwoman",nieveSnowWoman);
		classes.EngineCore.addButton(1,"Snowman",nieveSnowMan);
		//[Snowwoman] [Snowman] 
	}
	//Second Step: Eyes & Mouth
	//Alters Nieve's eyes in scenes
	else if(flags[kFLAGS.NIEVE_STAGE] == 2) {
		classes.EngineCore.outputText("You approach your snow" + nieveMF("man","woman") + " again, looking it over.");
		...
		classes.EngineCore.menu();
		if(player.hasItem(consumables.COAL___)) classes.EngineCore.addButton(0,"Coal",nieveCoalEyes);
		classes.EngineCore.addButton(1,"Gems",nieveGemEyes);
		classes.EngineCore.addButton(4,"Back",classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
	}
	//Fourth Step: The Nose
	else if(flags[kFLAGS.NIEVE_STAGE] == 3) {
		classes.EngineCore.outputText("You approach your nearly finished snowman. To others, it might be a perfectly acceptable creation, but not to you. It needs a carrot to finish it off.");
		...
		//(Else) 
			...
			classes.EngineCore.outputText("Unfortunately, you've yet to find one in your adventures.  You suppose you'll have to look more carefully.  Who knows, there might be a farm right under your nose.");
		}
		classes.EngineCore.doNext(classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
	}
	else {
		classes.EngineCore.outputText("Your snowman is done!  There's nothing more to add to it.  It looks mighty fine however, and just looking at it brings a nostalgia-fueled smile to your lips.");
		classes.EngineCore.doNext(classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
	}
}
//Coal
...
public function nieveCoalEyes():void {
	...
	classes.EngineCore.outputText("Luckily, you happen to have two lumps of coal.");
...
public function nieveGemEyes():void {
	classes.EngineCore.clearOutput();
	if(player.gems >= 9) {
		classes.EngineCore.outputText("Taking a handful of gems out of your pouch, you spread them evenly around the Snowman's face, giving it a nice, vacant smile.  It still needs a nose, however, and for that, you'll need a carrot.  Perhaps there's a farm nearby?");
		player.gems -= 9;
		classes.EngineCore.statScreenRefresh();
	...
	//Too broke to use the gems option? What the fuck is wrong with you?
		...
		classes.EngineCore.outputText("You open up your pouch, and frown.  Unfortunately, you don't have enough gems to create the eyes and mouth. With a sigh you march your broke ass back to camp.");
	}
	classes.EngineCore.doNext(classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
}
...
public function nieveSnowWoman():void {
	...
	classes.EngineCore.outputText("You grin mischievously to yourself and set about making two more balls of powdery snow.  It takes less time than any of the others, and before you know it you've attached two icy-breasts to the snowman.  They aren't terribly big, any heavier and you're sure they'd fall off, but they get the point across.");
...
public function nieveSnowMan():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You decide to leave it as is. Not everything has to have breasts, of course, even in Mareth.");
...
public function findACarrot():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("As you explore the farm, you come across several rows of green plants.  Getting closer, you recognize them... carrots!  You pull one of them from the ground.  It's expectedly dirty, but bright orange, and straight enough to be the perfect nose for your snowman.");
...
public function nieveComesToLife():void {
	...
	if(flags[kFLAGS.NIEVE_GENDER] == 1) classes.EngineCore.outputText("  It appears to be male, judging by the modestly sized cock sprouting from its loins.");
	else classes.EngineCore.outputText("  It appears to be female, judging by the modestly sized tits on its chest, and the lack of a penis, which is quite a feat in Mareth.");
	classes.EngineCore.outputText("  Pure white hair spills down from its head, almost blending in with the surrounding snow.");
	...
	if(flags[kFLAGS.NIEVE_MOUTH] == "gems") classes.EngineCore.outputText("glittering purple");
	else classes.EngineCore.outputText("coal black");
	classes.EngineCore.outputText(" orbs, fall on you, as if registering you for the first time.  Then a hint of recognition hits " + nieveMF("him","her") + ".  \"<i>Oh!  [Master]!  You're my [master]!</i>\"  In a flash " + nieveMF("he","she") + "'s standing, looking pleased as punch, with " + nieveMF("his","her") + " hands clasped excitedly in front of " + nieveMF("himself","herself") + ".");
	...
	//[Y/N]
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Yes",yesKeepNieve);
	classes.EngineCore.addButton(1,"No",noNoKeepNieve);
}

//Yes of course, what do you look like, an idiot?
public function yesKeepNieve():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You actually laugh at the question.  Of course you want " + nieveMF("him","her") + " to stay!");
	//[Silly mode: 
	if(classes.EngineCore.silly()) classes.EngineCore.outputText("  A free follower, no worries about buying worthless dye or raising their affection?  Fuck yeah, you'll take " + nieveMF("him","her") + "!");
	classes.EngineCore.outputText("  " + nieveMF("He","She") + " doesn't seem to be a threat, and indeed seems sincere in the fact that " + nieveMF("he","she") + " was sent here to be your lover.");
...
public function noNoKeepNieve():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You shake your head.  Of course not!  While " + nieveMF("he","she") + " certainly seems like a nice person on the surface, you can't help but think that's just the tip of the iceberg.  For all you know, " + nieveMF("he","she") + "'s a frigid bitch underneath, a trap set by the demons to lure you into a false sense of security.  Clever bastards, you conclude.  They certainly know the best way to serve revenge, but you won't have any of it.");
...
public function nieveCampDescs():void {
	//6:00
	if(model.time.hours == 6) classes.EngineCore.outputText("Nieve is sitting cross-legged in the snowdrift, munching on what looks to be icicles.  As you watch, " + nieveMF("he","she") + " reaches down into the glittering powder surrounding " + nieveMF("him","her") + " and produces another one.");
	//7:00
	else if(model.time.hours == 7) classes.EngineCore.outputText("Nieve is giggling and laughing as a fresh flurry flutters down on " + nieveMF("him","her") + ".  You wonder idly if " + nieveMF("he","she") + "'s causing it to happen.");
	//8:00
	else if(model.time.hours == 8) classes.EngineCore.outputText("Nieve is sitting in the white wintery wonderland, carefully constructing a snowman.  It strikes you as strange, and almost masturbatory in a way.  " + nieveMF("He","She") + " spots you and gives you a wave and a smile.  For a moment, you think the snowman has too, but you eventually conclude it's your imagination.");
	//9:00
	else if(model.time.hours == 9) classes.EngineCore.outputText("Nieve is sprawled out in the icy field, creating a series of snow angels without a care in the world.  When " + nieveMF("he","she") + " catches sight of you, she gives a friendly wave, then dives back into the snow.");
	//10:00
	else if(model.time.hours == 10) classes.EngineCore.outputText("Nieve, the ice spirit, sits quietly at the edge of the snowdrift, looking out at the landscape beyond.  When " + nieveMF("he","she") + " sees you looking, " + nieveMF("he","she") + " gives a somber wave.");
	//11:00
	else if(model.time.hours == 11) classes.EngineCore.outputText("Nieve is in your camp, poking around your supplies and water barrels. You notice " + nieveMF("he","she") + " seems a little uncomfortable to be away from " + nieveMF("his","her") + " snowdrift.");
	//12:00
	else if(model.time.hours == 12) classes.EngineCore.outputText("Nieve is carefully compressing snowballs in " + nieveMF("his","her") + " little camp.  To your surprise, " + nieveMF("he","she") + " picks one up and begins eating it like you would an apple.  When " + nieveMF("he","she") + " catches you looking mid-bite, " + nieveMF("he","she") + " gives you a grin with her mouth full, " + nieveMF("his","her") + " cheeks puffed out.");
	//13:00
	else if(model.time.hours == 13) classes.EngineCore.outputText("Nieve is sitting cross-legged, in the middle of what appears to be an argument with a snowman " + nieveMF("he","she") + "'s just made.");
	//14:00
	else if(model.time.hours == 14) classes.EngineCore.outputText("Nieve is in " + nieveMF("his","her") + " usual spot, seemingly deep in concentration.  Around " + nieveMF("him","her") + " snow begins to fall slowly, then quicker and quicker, whipping " + nieveMF("his","her") + " hair around wildly.  Although strangely, the air is still where you are.");
	//15:00
	else if(model.time.hours == 15) classes.EngineCore.outputText("\n\nNieve the ice spirit is whirling around " + nieveMF("his","her") + " area, practicing with what appears to be a translucent blue spear.  " + nieveMF("He","She") + " jabs and thrusts, spins and swipes.  " + nieveMF("He","She") + " may be new to Mareth, but you've got to hand it to " + nieveMF("him","her") + ", it looks like " + nieveMF("he","she") + " could take care of " + nieveMF("him","her") + "self.");
	//16:00
	else if(model.time.hours == 16) classes.EngineCore.outputText("\n\nNieve is at first nowhere to be seen.  Then you see " + nieveMF("his","her") + " head pop out of a snowdrift.  " + nieveMF("He","She") + " looks around quickly, then leaps into the air, diving head first into another pile of the powdery stuff.");
	//17:00
	else if(model.time.hours == 17) classes.EngineCore.outputText("\n\nNieve appears to have constructed a firepit constructed from translucent blue logs and rocks.  A vivid blue flame roars in the pit, over which Nieve appears to be roasting... marshmallows?  As you watch " + nieveMF("he","she") + " peels the burnt black skin off a marshmallow, gobbles it down, and begins roasting the rest.");
	//18:00
	else if(model.time.hours == 18) classes.EngineCore.outputText("\n\nNieve seems to be working on a spear, sharpening it with a jagged piece of ice.  When " + nieveMF("he","she") + " catches sight of you " + nieveMF("he","she") + " waves and gives a friendly smile.");
	//19:00
	else if(model.time.hours == 19) classes.EngineCore.outputText("\n\nNieve is sitting at the edge of " + nieveMF("his","her") + " icy expanse, staring off at the distant mountains.  You wonder if " + nieveMF("he","she") + "'s looking for home.");
	//20:00
	else if(model.time.hours == 20) classes.EngineCore.outputText("\n\nNieve is, surprisingly, outside of " + nieveMF("his","her") + " cold camp.  " + nieveMF("He","She") + "'s dancing along the battered, parched ground, calling down snow.  Wherever a flake hits, it's immediately devoured by the thirsty earth.  Is Nieve perhaps trying to... water the ground?");
	//21:00
...
public function approachNieve():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You wave at Nieve, getting " + nieveMF("his","her") + " attention and calling for " + nieveMF("him","her") + ".  The icy spirit-" + nieveMF("man","woman") + " happily walks over, smiling.  " + nieveMF("He","She") + " takes " + nieveMF("his","her") + " time, allowing you to take in " + nieveMF("his","her") + " naked body.");
	...
	//[Silly Mode:]
	if(classes.EngineCore.silly()) classes.EngineCore.outputText("  And you presume a butthole nestled between " + nieveMF("his","her") + " cheeks, right where it belongs.");
	classes.EngineCore.outputText("  " + nieveMF("His","Her") + " stomach is flat and toned, " + nieveMF("as is his chest","and she possesses a pair of perky B-cup breasts") + ". " + nieveMF("His","Her") + " face is the same pale blue as the rest of " + nieveMF("his","her") + " body, though it is off-set by " + nieveMF("his","her") + " glittering, ");
	if(flags[kFLAGS.NIEVE_MOUTH] == "coal") classes.EngineCore.outputText("dusky black");
	else classes.EngineCore.outputText("vibrant purple");
	classes.EngineCore.outputText(" eyes and pure white hair, which " + nieveMF("barely goes past his ears","tumbles down past her shoulders") + ".  Much of " + nieveMF("his","her") + " body glimmers with a fine layer of powdered snow or ice.");
	classes.EngineCore.outputText("\n\nNieve stops, gives you a friendly hug, and asks, \"<i>What can I do for you, [Master]?</i>\"");
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Sex",nieveSexMenu);
	classes.EngineCore.addButton(4,"Back",classes.Scenes.SceneLib.camp.campLoversMenu);
}

//Sex Menu
public function nieveSexMenu():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("What will you do with your oh-so-cool lover?");
	if(player.lust < 33) classes.EngineCore.outputText("  You aren't quite turned on enough for normal fucking.");
	classes.EngineCore.menu();
	if(flags[kFLAGS.NIEVE_GENDER] == 2) classes.EngineCore.addButton(0,"Lick Her",lickNieve);
	if(flags[kFLAGS.NIEVE_GENDER] == 1) classes.EngineCore.addButton(0,"Suck Him",suckNieveOff);
	//Fuck Her
	//Female Nieve
	...
	if((player.hasCock() || (player.hasVagina() && player.clitLength >= 3.5)) && player.lust >= 33) {
		if(flags[kFLAGS.NIEVE_GENDER] == 2) classes.EngineCore.addButton(1,"Fuck Her",fuckNieve);
	}
	//Get Fucked by Gurumash
	//Male Nieve
	//Any Gender
	//Nieve's cock is 9x1.5
	if(flags[kFLAGS.NIEVE_GENDER] == 1 && player.lust >= 33) classes.EngineCore.addButton(1,"Get Fucked",nieveFucksYou);
	classes.EngineCore.addButton(14,"Back",classes.Scenes.SceneLib.camp.campLoversMenu);
}
...
public function lickNieve():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You explain your intentions and how you'd like to be the one to give her pleasure.  She looks rather surprised at first, but then she smiles.  \"<i>You want to go down... on me?</i>\"  You nod and she giggles, \"<i>That would be wonderful!</i>\"");
	...
	if(!classes.EngineCore.silly()) {
		...
		//(Normal/Snake Tongue: 
		if(!player.hasLongTongue()) classes.EngineCore.outputText("It doesn't get far, but elicits squeals of surprise and pleasure from your elemental slave who's already grasping at the snow surrounding her for support.");
		//Demon Tongue: 
		else classes.EngineCore.outputText("Her eyes widen as you snake your impressive tongue into her. She grunts and squeals with pleasure, hands grasping piles of snow, wishing they were bedsheets.");
		classes.EngineCore.outputText("  After a minute of this, you realize you've lost the feeling in your tongue!  Pulling it back into your mouth, you find it warms back up quickly, and then you're at it again.");
		...
		if(player.isTaur()) classes.EngineCore.outputText(", or as close to it with your body,");
		classes.EngineCore.outputText(" with Nieve straddling your face.  \"<i>Mm, this is much better.  Now you wanted to lick me, [Master]? So... lick,</i>\" she whispers, lowering her chilly cunt down onto your face.");
	...
	//Silly Mode:
		...
		//(no Kid A: 
		if(flags[kFLAGS.ANEMONE_KID] != 1) classes.EngineCore.outputText(", Nieve takes a ladle full of water and pours it onto her snatch");
		//Kid A: 
		else classes.EngineCore.outputText(", your little anemone spawn giggles and laughs at your predicament before finally ladling some water onto your lover's snatch");
		classes.EngineCore.outputText(", effectively allowing you to pull your tongue away.");
...
public function fuckNieve():void {
	...
	if(x >= 0) {
		if(player.cocks[x].cockLength < 6) classes.EngineCore.outputText("  \"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
		//If overly large (>130): 
		else if(player.cocks[x].cockLength >= 24) classes.EngineCore.outputText("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
		else classes.EngineCore.outputText("  \"<i>Mmm,</i>\" she smacks her lips and takes a quick breath, \"<i>Such a nice example of a candycane right here.  Let's go in for another taste.</i>\"");
	}
	else {
		if(player.clitLength < 6) classes.EngineCore.outputText("\"<i>It's so cute, [Master],</i>\" Nieve says while catching a breath, \"<i>Like a little toy cock.</i>\"");
		else if(player.clitLength >= 24) classes.EngineCore.outputText("  She has to take a breath almost immediately, saying, \"<i>By the fat man's beard this thing is huge.  You must make the rest of this world jealous.</i>\"");
		else classes.EngineCore.outputText("  \"<i>Mmm,</i>\" she smacks her lips and takes a quick breath, \"<i>Such a nice example of a candycane right here.  Let's go in for another taste.</i>\"");
	}
	classes.EngineCore.outputText("  And with that she dives back down onto your " + cockClit(x) + ".  Her head bobs and bows, giving the sensation of an icecube running up and down your ");
	if(x >= 0) classes.EngineCore.outputText("dick");
	else classes.EngineCore.outputText("clit");
	classes.EngineCore.outputText(" at great speed.");
	
	//(Variable depending on dick-type)
	if(x >= 0) {
		//(Human) 
		if(player.cocks[x].cockType == CockTypesEnum.HUMAN) classes.EngineCore.outputText("  Your normal, unaltered cock seems to take on a slight blue tint, although you can confirm it's not going numb.  Quite the contrary, it seems to be getting more sensitive as Nieve's lips roll over it.");
		//(Equine) 
		else if(player.cocks[x].cockType == CockTypesEnum.HORSE) classes.EngineCore.outputText("  Nieve fawns over your horsecock like a cultist praying to a god.  She sucks and kisses, running the tip of her tongue all along the sides until she reaches the sheathe, and then right back up the other side.");
		//(Canine) 
		else if(player.hasKnot(x)) {
			classes.EngineCore.outputText("  Nieve eyes the knot on your cock as though it were a challenge.  She inches down your shaft, getting ever closer.");
			//(Cocksize >40)
			if(player.cockArea(x) >= 40) classes.EngineCore.outputText("  Eventually, though, she admits defeat and goes back to working your shaft, though she eyes that knot from time to time with no small amount of anger.");
			//(Cocksize <40) 
			else classes.EngineCore.outputText("  It takes her a few good minutes, but eventually she works her way down and takes the entire knot into her mouth.  She raises her hands in triumph and lets out a \"<i>Mmmmrrrrrpphhh!</i>\" before pulling back for a breath, inadvertently exposing your wet hard cock to the cold, wintery air.");
		}
		/*Cock types
		0 - human
		1 - horse
		...
		*/	
		//(Demonic)
		else if(player.cocks[x].cockType == CockTypesEnum.DEMON) classes.EngineCore.outputText("  Your purple, demonic shaft pulses and throbs, seemingly immune to the unnatural cold of Nieve's mouth. Seemingly oblivious to its nature, Nieve slavers over each nodule, giving every ridge and bump a kiss and more than just a lick.");
		//(Tentacle) 
		else if(player.cocks[x].cockType == CockTypesEnum.TENTACLE) classes.EngineCore.outputText("  Nieve does not seem put off by your tentacle cock in the least, in fact she confesses, \"<i>It reminds me of a comic I read before coming here.</i>\"  As she fawns over your cock, you're certain you hear whispers of \"<i>Oh, you sexy tentacle beast...</i>\" coming from her mouth when she gets her breath.");
		//(Feline) 
		else if(player.cocks[x].cockType == CockTypesEnum.CAT) classes.EngineCore.outputText("  Though the barbs on your cock are soft and pliable, they reflexively harden up after Nieve's tongue washes over them.  She doesn't pay particular attention to the barbs, but she does seem rather careful not to get one hooked on her lip.");
		//(Lizard)
		else if(player.cocks[x].cockType == CockTypesEnum.LIZARD) classes.EngineCore.outputText("  Nieve fawns over your scaled and ridged reptilian cock, slurping and smooching up and down its length. It seems a bit more resistant to the biting cold of Nieve's mouth, which is lucky when she backs off, exposing your entire wet, frigid length to the wintery air surrounding you.");
		//(Anemone) 
		else if(player.cocks[x].cockType == CockTypesEnum.ANEMONE) classes.EngineCore.outputText("  Nieve giggles whenever one of your wriggling tentacles stings her, seemingly immune to its effects, and kisses it back.  Soon after she devours your whole length, delighted to feel the sting of your cock in the back of her throat.");
		//(Kangaroo) 
		else if(player.cocks[x].cockType == CockTypesEnum.KANGAROO) classes.EngineCore.outputText("  It takes Nieve a little while to catch on that your cock isn't quite human, and by the time she figures it out, she mutters, \"<i>Huh, that's strange... I thought kangaroos had balls above the cock,</i>\" before shrugging and getting back to work.");
		//(If you add dicks later, placeholder text:) 
		else classes.EngineCore.outputText("  Nieve works on your mutant cock, spending time working every unique little facet of it.");
	}
	//(Clitoris) 
	else {
		classes.EngineCore.outputText("  Nieve nuzzles and lavishes your elongated love button, showering it with attention. \"<i>I didn't know clits could get this big,</i>\" she says with wonder in her voice. \"<i>");
		if(player.clitLength < 5) classes.EngineCore.outputText("It's just like a little cock");
		else if(player.clitLength < 9) classes.EngineCore.outputText("It's just like a cock");
		else classes.EngineCore.outputText("I bet even men get jealous of this bad girl");
		classes.EngineCore.outputText(".</i>\"");
	}
	...
	if(x >= 0) classes.EngineCore.outputText("  Your " + cockClit(x) + " quivers and spurts cum directly into Nieve's eager mouth, who gulps it down as if she were drinking direct from the tap.");
	classes.EngineCore.outputText("\n\nAs you come down from your little orgasmic high, you glance down at the ice woman, ");
	if(x < 0 || player.cumQ() <= 250) classes.EngineCore.outputText("who has a bit of cum dribbling down her chin");
	else if(player.cumQ() <= 400) classes.EngineCore.outputText("who has a steady stream of cum pouring from her mouth");
	else if(player.cumQ() < 600) classes.EngineCore.outputText("who is patting her stomach as though she'd just had a large meal");
	else if(player.cumQ() < 1000) classes.EngineCore.outputText("whose stomach appears slightly distended from the amount of cum you've poured into her");
	else classes.EngineCore.outputText("whose stomach is rather large, like a pregnant woman's, from the sheer amount of cum you've pumped into her gullet");
	...
	//(if too big: 
	if(x >= 0) {
		if(player.cockArea(x) >= 130) classes.EngineCore.outputText("at least as far as it will go, ");
	}
	classes.EngineCore.outputText("with a wet \"<i>schluck.</i>\"  The words coming out of her mouth are lost in a loud \"<i>ooohhhh</i>\" as her eyes roll backwards, closely followed by her head.  Her wet pussy grips you tightly, giving you a strange tingling sensation.  Your hot rod combined with her arctic nethers actually causes some steam to rise up, and you're a little captivated to see more tufts of steam rise with every slow, gentle thrust you make.");
	...
	if(x >= 0) {
		//((cocksize 3/Clit 3)
		if(player.cockArea(x) <= 7) classes.EngineCore.outputText("Your meager penis isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
		//(Cocksize 30/Clit 12) 
		else if(player.cockArea(x) <= 30) classes.EngineCore.outputText("At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can't even make out.");
		//(Cocksize 70/Clit 24)
		else if(player.cockArea(x) <= 70) classes.EngineCore.outputText("Nieve's cunt feels like an icy vice on your oversized cock, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.");
		//(Cocksize 130+/Clit 48+)
		else classes.EngineCore.outputText("Though you can't fit your entire " + cockClit(x) + " inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.");
	}
	else {
		if(player.clitLength <= 4) classes.EngineCore.outputText("Your meager clit isn't much, but at this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes staring up at you, urging you on faster and faster.");
		else if(player.clitLength <= 12) classes.EngineCore.outputText("At this angle you manage to hit all the right places.  Soon Nieve is wriggling around, eyes tightly shut as she whispers dirty words you can't even make out.");
		else if(player.cockArea(x) <= 24) classes.EngineCore.outputText("Nieve's cunt feels like an icy vice on your oversized clitty, but even so, you manage to hit all the right places.  Soon she is wriggling around, eyes tightly shut as she pants for breath.");
		//(Cocksize 130+/Clit 48+)
		else classes.EngineCore.outputText("Though you can't fit your entire " + cockClit(x) + " inside, you are amazed at how much she takes.  Her pale blue stomach bulges with every thrust.  Soon Nieve is wriggling, her eyes slightly open as she bites her lip in ecstasy.");
	}
	...
	//((Clitfucking?)
	if(x < 0) classes.EngineCore.outputText("  Your [clit] suddenly feels suddenly warm, like every nerve ending inside were on fire. It aches and pulses with the rest of your body, as though it were a little cock longing to cum.");
	else {
		classes.EngineCore.outputText("  Your " + cockClit(x) + " aches and pulses with need before ");
		if(player.cumQ() < 100) classes.EngineCore.outputText("spilling its meager offering into the ice spirit's womb");
		else if(player.cumQ() < 250) classes.EngineCore.outputText("ejecting streams of white hot jizz into the ice spirit's hungry snatch");
		else if(player.cumQ() < 400) classes.EngineCore.outputText("painting the ice spirit's insides with your cream until some begins to pour out");
		else if(player.cumQ() < 700) classes.EngineCore.outputText("arcs of pearly white cum shoot forth, filling the ice spirit's womb so much her belly visibly inflates");
		else classes.EngineCore.outputText("erupting inside, filling the ice spirit up with so much of your hot cum so much that she looks visibly pregnant");
		classes.EngineCore.outputText(".");
	}
...
public function nieveFucksYou():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("Your arrival to Nieve's part of the camp elicits a friendly smile from the ice man.  He notices the longing in your eyes with a grin, and speaks in a gentle, yet low tone, \"<i>[Master], you seem to need help from me...</i>\"");
	classes.EngineCore.outputText("\n\nTo which you nod, ");
	if(player.cor < 50) classes.EngineCore.outputText("telling him that you find his form... appeasing to say the least, and would like to know how his 'parts' are working properly.");
	else classes.EngineCore.outputText("leting your creation know that you are interested in 'how' it can please you, giving him no room to imagine what you're insinuating you then tell him bluntly that you want him to fuck you like the plaything he is.");
	...
	//{Silly Mode:}
	if(classes.EngineCore.silly()) classes.EngineCore.outputText("   He did say that his kind were effectively love slaves, and boy does this prove it!");
	//[if (hasVagina = true) 
	if(player.hasVagina()) classes.EngineCore.outputText("  Your [vag] is a sodden box so soon after he started, and you start to want more, much more!");
	//[if (hasCock = true) 
	if(player.hasCock()) classes.EngineCore.outputText("  Your [cock biggest] is also rock hard in a short while and you quickly desire some more stimulation from your icy love slave.");
...
public function takeNieveAnal():void {
	...
	//[if (hasCock = true) 
	if(player.hasCock()) classes.EngineCore.outputText("  Nieve then bends forward while keeping his pace to give you a reach-around.  Grabbing your own [cock biggest] in one hand, he starts to pump it to the speed of his thrusts, torturing you with the extra-stimulation in such a good way.");
	//[if (hasVagina = true)
	else if(player.hasVagina()) classes.EngineCore.outputText("  As Nieve continues to pound away, he places one hand on your sopping wet cunt and begins to finger you in time with his assault, making you lust-drunk with the stimulation as he finger bangs you.");
	
	classes.EngineCore.outputText("\n\nNieve continues to grunt as he prods and plunges into your ");
	//if (silly mode) 
	if(classes.EngineCore.silly()) classes.EngineCore.outputText("pooper");
	else classes.EngineCore.outputText("[asshole]");
	...
	if(player.gender > 0) {
		...
		if(player.hasCock()) {
			classes.EngineCore.outputText(", ");
			classes.EngineCore.outputText("while your body undulates and seizes as cum starts to spew forth from [eachCock]");
			//(normal volume: 
			if(player.cumQ() < 200) classes.EngineCore.outputText(", as you cum, you leave a small puddle on the floor below you");
			else if(player.cumQ() < 500) classes.EngineCore.outputText(", as you cum you start to dress your arms and hands with your own cream as you shoot your stuff out onto the floor");
		}
		//[if (hasVagina = true)
		if(player.hasVagina()) classes.EngineCore.outputText(".  Your soaking vagina makes your body quake and shudder as you orgasm, splashing your femcum all over Nieve and the ground");
	}
	classes.EngineCore.outputText(".");
...
public function takeNieveVaginal():void {
	...
	//[if (hasBalls = true) 
	if(player.balls > 0) classes.EngineCore.outputText(", gently moving your [balls] out of the way in the process");
	classes.EngineCore.outputText(".  He slams his nine inch blue rod balls deep into your sodden box.  A loud squelch fills the air at the penetration, giving Nieve the go ahead to start in top gear, pistoning in and out at a hurried pace.");
	//[if (hasCock = true)
	if(player.hasCock()) classes.EngineCore.outputText("  During the pounding Nieve grabs your dangling [cock biggest] and starts to pump it in time with his own thrusts, quite eager to please his [master].  Every time your feminine half cums, so does your male half, splashing its happy seed into the valley that your combined bodies have made.");
	//[if (isLactating = true) 
	if(player.lactationQ() >= 100) classes.EngineCore.outputText("  Your ice man reaches out with a free hand to caress your [chest], twiddling his fingers around your delicate [nipples]. After some rough pulling and flicking, pounding away all the while, he feels milk dribble onto his hand.  He catches a quick glance of the liquid before diving head-first to suckle on your milk faucets, drinking heavily of your cream as he's about to give you his.");
...
public function nieveIsOver():void {
	...
	if(flags[kFLAGS.NIEVE_STAGE] < 5) {
		classes.EngineCore.outputText("You wake up with a yawn and a stretch. It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter's grasp on Mareth has slipped, and it's entering Spring.  Curiously, you glance over towards the snow which had been recently deposited in your camp, and note it's melting away before your eyes.");
		classes.EngineCore.outputText("\n\nOh well, not a huge loss, you figure.  It was fun while it lasted.");
		flags[kFLAGS.NIEVE_STAGE] = 0;
	}
	//Nieve Completed
	else {
		classes.EngineCore.outputText("You wake up with a yawn and a stretch.  It feels just like any other day, though a bit warmer.  Quite a bit warmer in fact.  It feels like winter's grasp on Mareth has slipped, and it's entering Spring.  You consider the fact absently at first, but then you stop dead in your tracks.  What about Nieve?!");
		...
		if(player.cor < 50) classes.EngineCore.outputText("  You even tear up yourself, sad to see such a good friend leave, and so abruptly.");
		//Corruption >50: 
		else classes.EngineCore.outputText("  You even feel a little sad yourself, it's such a pity to see a good, obedient fucktoy leave.");
		classes.EngineCore.outputText("  The snowman collapses on itself, melting away before your very eyes.  In a matter of hours, this entire area will be nothing but a dried up, dehydrated desert once more.  But as the jingling fades out into the distance, you vow to hold on to the tear.");
...
private function fixNieve():void {
	if(flags[kFLAGS.NIEVE_GENDER] == 0) {
		classes.EngineCore.clearOutput();
		classes.EngineCore.outputText("(There was an error with stat tracking that cleared Nieve's stats out at the end of last year's event. <b>What gender do you want Nieve to be?</b>)");
		classes.EngineCore.menu();
		classes.EngineCore.addButton(0,"Male",fixNieveGender,1);
		classes.EngineCore.addButton(1,"Female",fixNieveGender,2);
	}
	else if(flags[kFLAGS.NIEVE_MOUTH] == "" || flags[kFLAGS.NIEVE_MOUTH] == 0) {
		classes.EngineCore.clearOutput();
		classes.EngineCore.outputText("(There was an error with stat tracking that cleared Nieve's stats out at the end of last year's event. <b>What were Nieve's eyes and mouth made out of?</b>)");
		classes.EngineCore.menu();
		classes.EngineCore.addButton(0,"Gems",fixNieveMouth,0);
		classes.EngineCore.addButton(1,"Coal",fixNieveMouth,1);
	}
...
public function returnOfNieve():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("As you awake in the morning you find yourself shivering slightly.  A cool breeze sweeps over your camp, while in the distance jingling bells can be heard.  How odd.  You haven't heard bells like that since...");
	...
	//[Next]
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Next",fixNieve);
}

private function nieveReturnsPartII():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You sigh, resigning yourself to your companion's fate.  However, as you rise from the snow to return to camp, you hear a soft, muffled voice.  Perplexed, you crawl forward in the snow, frantically seeking out the source.");
	...
	if(flags[kFLAGS.NIEVE_MOUTH] == "coal") classes.EngineCore.outputText("coal black");
	else classes.EngineCore.outputText("glittering purple");
	classes.EngineCore.outputText(" eyes shine with crystallizing tears, not entirely unlike the ones " + nieveMF("he","she") + " left with you on your last meeting.  \"<i>I'm so glad to be back!  I was worried I would be sent somewhere else, but I had so much fun here, and I told the big man that, and then Winter came and I wasn't sure you were still here, and then it turned out I might have gone to someone else, but oh my goodness I'm so happy I came back!</i>\" Nieve belts out without pause for breath before wrapping " + nieveMF("his","her") + " arms around you once again.");
