...
function cultistLustTransfer():void {
	if(monster.lust <= 30 || rand(2) == 0) {
		...
		stats(0,0,0,0,0,0,(monster.lust/3 * (1 + player.cor/300)),0);
		monster.lust -= 50;
...
function cultistRapesYou():void {
	...
	if(rand(2) == 0 && !sexed) {
		...
		if(player.biggestTitSize() > 1 && !changed) {
			...
			if(player.breastRows.length == 2) outputText("Once about a quarter of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your " + allBreastsDescript() + " have been injected.  ", false);
			//[if the player has three rows of breasts]
			if(player.breastRows.length > 2)  outputText("Once about a sixth of the fluid has been injected into the first breast, she moves on to the second and you feel a similar sharp pain followed by the glowing pleasure.  She continues this until all of your " + allBreastsDescript() + " have been injected.  ", false);
			outputText("Soon the glowing pleasure fills your breasts completely and you feel an orgasm shoot through your body as your "+ allBreastsDescript(), false);
			//[if breasts are already lactating]
...
function playerRapesCultist():void {
	...
	if(monster.armorName == "pervy nun's clothing") {
		...
		if(player.cocks.length > 0 && (player.gender != 3 || rand(2))) {
			...
			if(player.lowerBody == 4) outputText("You command her to get on all fours, then cover her and push ", false);
		...
		else if(player.vaginas.length > 0 && player.lowerBody != 4) {
		...
		//Horsey
		if(player.vaginas.length > 0 && player.lowerBody == 4) {
...
function swimsuitVibrators():void {
	...
	if(player.vaginalCapacity() <= 30) {//PC does not have a very loose or looser vagina)
		...
		if(player.vaginas[0].virgin) {
			...
			cuntChange(50,false,false,false);
		}
		else {
...
function cultistBadEnd2():void {
	...
	outputText("Your mind has fallen prey to the followers of the fetish, and you are now one yourself.  You have completely forgotten who or what you where, including why you came to this world in the first place.  Perhaps the next champion will not lose their sense of self so easily.", false);
	eventParser(5035);
}
