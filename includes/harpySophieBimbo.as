...
function bimboSophie():Boolean {
	return (flags[282] > 0 && flags[283] == 0 && flags[SOPHIE_DEBIMBOED] == 0);
}

function sophieIsInSeason():Boolean {
	return (flags[SOPHIE_HEAT_COUNTER] > 551 && flags[SOPHIE_INCUBATION] == 0);
}

function sophiePregChance():void {
	//25% + gradually increasing cumQ bonus
	if(rand(4) == 0 || player.cumQ() > rand(1000)) {
		flags[SOPHIE_INCUBATION] = 168;
	}
}
function sophieCapacity():Number {
	return 179;
}


//*Summary:  Sophie is defeated, give draft.
function bimbotizeMeCaptainSophie():void {
	sophieSprite();
	outputText("", true);
...
function acceptBimboSophie():void {
	...
	else if(flags[238] == 1) outputText("  Afterwards, she offers to suck Izma's cock.  Izma looks at you for a moment before shaking her head, no.  Her skirt rises visibly in spite of her negative response.  Maybe she'll like having someone lower on the food chain to boss around?", false);
	else if(flags[43] > 0 && flags[78] == 0) {
		outputText("  Afterwards, she offers to lick Amily's twat, if Amily will do the same to her.  Amily ", false);
		if(flags[43] == 1) outputText("flushes hotly and denies the bimbo's request with a terse 'no'.", false);
	...
	else if(monk >= 5 && player.hasStatusAffect("noJojo") < 0 && flags[80] == 0) 
...
//Approach Texts
function approachBimboSophieInCamp(output:Boolean = true):void {
	sophieSprite();
	if(output) {
		outputText("", true);
		...
		else if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) {
			if(flags[SOPHIE_INCUBATION] > 120) {
	...
	if(flags[SOPHIE_BROACHED_SLEEP_WITH] == 1) {
...
function bimboSophieSexMenu():void {
	...
	if(player.hasCock()) {
		...
		//TIT FUCKS!
		if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] < 150) addButton(5,"Titty-Fuck",sophiePreggoTitJobs);
	...
	if(flags[284] > 0 && (player.gender > 0) && flags[SOPHIE_INCUBATION] == 0) {
		if(hasItem("OviElix",1) || hasItemInStorage("OviElix")) {
	...
	if(flags[TOLD_SOPHIE_TO_IZMA] == 0 && flags[TIMES_SOPHIE_AND_IZMA_FUCKED] > 0 && flags[IZMA_NO_COCK] == 0) {
		outputText("\n\nYou could tell Sophie to visit Izma again.  She'd probably wind up tied AGAIN.");
		addButton(7,"Visit Izma",tellSophieToVisitIzma);
	}
	if(flags[SOPHIE_ADULT_KID_COUNT] > 0 && flags[SOPHIE_FAMILY_INCEST] > 0 && player.cockThatFits(sophieCapacity()) >= 0 && player.hasCock())
...
function tellSophieToVisitIzma():void {
	...
	flags[TOLD_SOPHIE_TO_IZMA] = 1;
	doNext(13);
}


//Lipstick Resistance Building
function bimboSophieLustStickSurprise():void {
	outputText("\n", false);
...
function daughterCheckup():void {
	...
	else if(flags[SOPHIE_DAUGHTER_MATURITY_COUNTER] > 100) {
	...
	else if(flags[SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0) {
	...
	else addButton(4,"Back",followerSophieMainScreen);
}

//Unlock resistance:
function unlockResistance():void {
	if(player.hasCock()) {
...
function bimboSophieLicksRugs():void {
	...
	outputText("The ditzy blond giggles and tweaks her hefty nipples, cooing, \"<i>That was absolutely yummy!  Your puss is like, totally tasty.  Lets go again!</i>\"  You feel a little fucked out after the fantastic oral, and you decline.\n\n", false);
...
function bimboSophieSixtyNineAfterCunnilingus():void {
	...
	if(!player.hasCock() || player.cockThatFits(sophieCapacity()) == -1) {
		...
		if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) outputText("Her pregnant belly rests against your middle, untouched by the rough fall.  ");
		...
		if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) outputText(", egg-swollen belly smushing between you");
	...
	//SixtyNine Continuation - DUDES
		...
		if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) outputText("Her gravid belly bumps up against your own, somehow untouched by the rough-and-tumble fall.  ");
		outputText("How did she do that?  Your questions fade away as you look up and spot your target - the cum-slicked delta of Sophie's cunt, so moist it seems to fog the air, beckoning you to plunge your tongue inside and taste it.\n\n", false);
		...
		outputText("As her imperatives roll through you, you shudder and shake.  " + OMultiCockDesc() + " immediately releases a dollop of pre-cum, and Sophie dives down on it like an addict seeking her fix.  The return of her drugged suction is nearly too much for you, and you feel your " + cockDescript(x) + " swelling in her mouth, the " + cockHead(x) + " thickening against her hungry tongue.  She shakes on top of you and releases another wave of syrupy fluids.  You swallow a mouthful, compulsively drinking her prodigious pussy's copious lady-spunk.  Your back arches, your " + player.legs() + " clench, and you erupt inside your bimbo's puckered, sucking mouth-hole, giving her tonsils a bath of your creamy semen.  Rivulets of semen run over her juicy lips as you cum too fast for her to keep up with.", false);
		if(player.cumQ() >= 500) {
		...
		if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) outputText(" and the way her egg-stuffed womb jiggles");
...
function buttFuckBimboSophie():void {
	...
	outputText(" delic... de... yummy!</i>\"  You slap her ass, grab her tail feathers, and pull the harpy's winking asshole down onto your " + cockDescript(x) + ". Her voices rises, not in anger, but in delight, and your " + cockHead(x) + " pulses pleasantly in the first few inches of the harpy's rectum.  The bimbo relaxes and sinks down, giggling and asking, \"<i>Is-ooh-is my butt, like, good for you?  It's soooo tight, and it's making my cunny all wet and stuff!</i>\"\n\n", false);
	...
	if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) outputText("  Just like her breasts, her gravid tummy sways a little as well, plush with the weight of the egg you've given her.");
	...
	if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) {
...
function fuckBimboSophiesTits():void {
	...
	outputText("Suddenly, Sophie's puckered lips are on your " + cockHead(x) + ", sucking it softly while her tongue tickles the sensitive slit of your urethra.  She pops off for a breath with a lusty look in her eyes and spit drooling from the corner of her puffy lips to lubricate her tits.  A golden lip print sits around the circumference of your " + cockDescript(x) + ", marking just how much she's managed to smother in oral pleasure.  Pulling her hands up, she presses her palms into the heavy weight of breasts, nearly vanishing from view as she drags them up your " + num2Text(Math.round(player.cocks[x].cockLength)) + "-inch length.  Her drugged lipstick makes your " + cockHead(x) + " tingle deliciously, and you sigh, happy to have such glorious udders growing from your bleach-blonde bitch.\n\n", false);
	...
	if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) outputText("her swollen midriff is glazed, ");
...
function sophieEggApocalypse():void {
	sophieSprite();
	outputText("", true);
	//First time
	if(flags[284] == 0) {
	...
	var mommy:Number = 0;
	flags[284]++;
	if(player.hasCock()) daddy = 3034;
	if(player.hasVagina()) mommy = 3035;
...
function beBimboSophiesSugarDaddy():void {
	...
	else if(hasItemInStorage("OviElix")) consumeItemInStorage("OviElix");
...
function randomLargeEgg():String {
...
function rideDatSophieVag(nice:Boolean = false):void {
	...
	if(nice) {
		...
		outputText("\n\n\"<i>Well, like, you totally want to fuck now, right?</i>\" Sophie asks you, voice hitching into a soft, squawky moan.  You mutter under your breath as the harpy turns and saunters away, her fat rump jiggling with each step of her talon-capped legs, and she turns once more to sit down on it, the bulging cheeks apparently making a pretty comfortable cushion.");
		...
		outputText("\n\nYou lean down afterwards, holding one big, floppy tit up, and lock your drug-suffused lips onto it; Sophie lets out a squawk, her lips flying open, but as you look up at her, she shuts her mouth again.  It seems you <i>can</i> teach a bimbo new tricks.  You start to suckle on her teat, tongue roving over her sensitive areola and nipple, and her body shudders as your free hand mauls her other breast, all gropes and pinches and squeezes as you attack the slutty bird's oversized cleavage.");
		...
		outputText("\n\nYour hips turn into a staccato of smacks and slaps as you help Sophie thrust against your constant pounding, her walls trying their very damnedst to milk your [cock " + (x + 1) + "] of its' seed, and her feathery arms run across your back as she lets out another squawky moan - and then her body seizes up.");
	...
	if(player.cockTotal() == 1) {
		...
		//{if (cocks = 1) and (cumMedium = true)} 
			...
			if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) outputText(" (or at least all over her channel, since her womb is bound up with your egg)");
	...
	if(flags[SOPHIE_INCUBATION] > 0 && flags[SOPHIE_INCUBATION] <= 150) outputText("resting your head on her gravid tummy and smiling to yourself.");
	else outputText("hugging her tight as she tries her best to show her appreciation.");
	if(sophieIsInSeason()) sophiePregChance();
	stats(0,0,0,0,.5,-4,-100,0);
	if(!nice && player.cor < 50) stats(0,0,0,0,0,0,0,1);
	luststickApplication(5);
	doNext(13);
}
...
function sophieGoesIntoSeason():void {
	sophieSprite();
...
function sophieSeasonExpiration():void {
	sophieSprite();
...
function sophieGotKnockedUp():void {
	sophieSprite();
...
function sophieImpregnationSex():void {
	...
	if(player.cockTotal() > 1) outputText(" over your extra, thighslapping cock");
	...
	outputText(" with confident, sure strokes.  Your answer, never spoken aloud becomes evident by your actions: she got you this horny, and you're going to impregnate every hole, no matter how long it takes.  Her asshole is tight, almost <ii>too tight</i> after having experienced her pussy, and you can feel the next batch of cum already beginning to churn in your [balls], a heavy, warm weight growing larger and larger.");
...
function sophieMoveInAttempt():void {
	clearOutput();
	sophieSprite();
	flags[SOPHIE_BROACHED_SLEEP_WITH] = 1;
	...
	outputText("\n\nThe bimbo harpy seems genuine in her desire for companionship, though judging from the moisture that trickles from her cleft and down her feathery thighs, she'll probably be fucking at some point, even if it isn't in the middle of the night.  Do you want to take the posh-bodied girl to bed with you at night?");
	menu();
...
function yesSophieSleepWith():void {
	...
	if(flags[SOPHIE_INCUBATION] > 150) outputText("burgeoning");
	else if(flags[SOPHIE_INCUBATION] > 120) outputText("swollen");
	else if(flags[SOPHIE_INCUBATION] > 100) outputText("huge, gravid");
	else outputText("taut");
...
function noSophieSleepWith():void {
	...
	if(flags[SLEEP_WITH] == "Sophie") flags[SLEEP_WITH] = "";
	doNext(13);
}
...
function moveSophieInRepeat():void {
	...
	if(flags[SOPHIE_INCUBATION] == 0) outputText(" even though she's not pregnant any more");
...
function sophieFenCraftedSex(morning:Boolean = false):void {
	var x:int = player.cockThatFits(sophieCapacity());
	if(x < 0) x = player.smallestCockIndex();
	sophieSprite();
	//{Morning Sex}
...
//Alternate Intro/Outro for consensual fucking when it isn't morning?
function fuckYoPregnantHarpyWaifu(morning:Boolean = false):void {
	var x:int = player.cockThatFits(sophieCapacity());
	//{Morning Fuck}
	if(morning) {
		sophieSprite();
	...
	outputText(" prove the veracity of her statements many times over.  You can't SEE the wetness, but your " + cockDescript(x) + " certainly feels it, twitching inside Sophie's silky slit when the waves of wetness roll over it.");
...
function sophiePreggoTitJobs():void {
	...
	outputText("\n\nSuddenly, the amber nubs are withdrawn, and you stagger dizzily for a moment, realizing you hadn't been breathing this whole time.  Sophie drops to her knees in front of you, supporting you by your [butt].  Her hands squeeze you affectionately as she kisses against your " + cockHead(x) + ".");
	if(player.cockTotal() > 1) {
	...
	else outputText("your " + cockHead(x) + "");
...
function goHogWildOnSophieBewbs():void {
	...
	if(player.cocks[x].cockLength <= 30) outputText(cockHead(x) + " pushes into through her startled cock-pillows, muffling her more effectively than any gag.  She immediately sucks on it, a happy look in her eyes.  The bimbo seems to want nothing more than to lie there and polish your rod.");
	...
	outputText(".  Your [butt] is squished up against her belly, both of you sweating and slipping all over each other.  Bulging in her breasty embrace, your " + cockDescript(x) + " flexes, and your " + cockHead(x) + " thickens.");
	...
	outputText("  You push your " + cockHead(x) + " and get a kiss from her as you finish cumming, oozing one last dollop onto her fat lips.  She whimpers in the most exciting way as you withdraw, wiping your cock off on her swollen belly, marking her as yours once more.");
...
function sophieBirthsEgg():void {
	...
	if(flags[SOPHIE_ADULT_KID_COUNT] == 0) outputText("  Can you handle having another harpy living in your camp?\n");
...
function sophiesEggHatches():void {
	...
	if(flags[SOPHIE_ADULT_KID_COUNT] < 4) {	
		//First Harpy:
		...
		if(flags[SOPHIE_ADULT_KID_COUNT] == 0) {
			...
			if(flags[282] > 0) outputText(", though her lips are a little larger than you'd think was normal - maybe she inherited it from her mom?");
			else outputText(".");
			outputText("  The harpy chick is actually about as big as a four or five year old, and she looks up at Sophie curiously.");
			if(flags[282] > 0)  {
				flags[DAUGHTER_ONE_BIMBO] = 1;
			}
		}
		//Second daughter
		//Second Harpy:
		//-Badonkadonk to 9000.  Blond feathers that grow very long. (violet eyes)
		else if(flags[SOPHIE_ADULT_KID_COUNT] == 1) {
		...
		//-Not that curvy, but has human feet and purple feathers. (violet eyes)
		else if(flags[SOPHIE_ADULT_KID_COUNT] == 2) {
			...
			if(flags[282] > 0) flags[DAUGHTER_THREE_BIMBO] = 1;
		}
		//Fourth Harpy:
		//-Curvy like Sophie, but with well-developed wings and extra-soft feathers (WINGJOBS).  Pink plumage.  (blue eyes)
			...
			if(flags[282] > 0) outputText(" like her mother used to have before her bimbofication");
			...
			if(flags[282] > 0) flags[DAUGHTER_FOUR_BIMBO] = 1;
		}
	...
	else
	{
		...
		if(rand(2) == 0) outputText("pink");
			...
			if(flags[282] > 0) outputText("blonde");
	...
	flags[SOPHIE_DAUGHTER_MATURITY_COUNTER] = 336;
}
	
//Mature alert
function sophieKidMaturation():void {
	clearOutput();
	sophieSprite();
	...
	//Fifth+ daughter
		...
		if(flags[282] > 0) outputText("bimbo ");
	...
	if(flags[SOPHIE_ADULT_KID_COUNT] > 0) {
		//If Yes Incest
		if(flags[SOPHIE_FAMILY_INCEST] == 1) {
...
function sophieAndIzmaPlay():void {
	outputText("\n");
	flags[TIMES_SOPHIE_AND_IZMA_FUCKED]++;
	outputText("<b>Something odd happens that ");
	if(hours < 6 || hours > 20) outputText("night");
	else if(hours < 12) outputText("morning");
	else if(hours == 12) outputText("noon");
	...
	if(flags[TIMES_SOPHIE_AND_IZMA_FUCKED] == 1) {
