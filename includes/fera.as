...
public function pumpkinFuckEncounter():void {
	classes.EngineCore.awardAchievement("Pump-kin-kin-kin", kACHIEVEMENTS.HOLIDAY_HALLOWEEN_I);
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("Just ahead, in the middle of the path, lies a seeming innocuous gourd - a pumpkin to be precise.  The growth is oddly out of place.  Nothing else grows on the soft animal-forged path you now tread, and the pumpkin and its vines are moist, as if wet with morning dew.  They are the only plants in the vicinity to bear such moisture, gleaming dully in the muted arboreal light.");
	if(flags[kFLAGS.PUMPKIN_FUCK_YEAR_DONE] > 0) classes.EngineCore.outputText("  An old, haunting memory surfaces, and you realizes this seems... familiar.  Did you find this last year?  Yes... it could have been this very same pumpkin...");
	...
	if (player.gender == 3) classes.EngineCore.outputText("\n\nSo many choices, so little time.");
	//If PC has neither
	if (player.gender == 0) {
	...
	classes.EngineCore.simpleChoices("Fuck It", fuck, "Mount It", mount, "", null, "", null, "No Way", classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
...
public function pumpkinSmash():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You have no idea but to be safe, you pick up the pumpkin and throw the pumpkin at the tree, shattering the pumpkins into several pieces.");
	classes.EngineCore.outputText("\n\nAs you walk away, you are interrupted by someone.");
	classes.EngineCore.doNext(pumpkinFuckPartII);
}
...
public function pumpkinFuck():void {
	...
	classes.EngineCore.outputText("Well, fuck it.  You're horny, and this little pumpkin doesn't look like it could stop you from fucking that gushing gourd-pussy.  As you shed your [armor], a few of the rough, slime-sweating vines feebly rise up to embrace you, but you brush them away, having no time for corrupt foolishness.  You're going to bust a nut in this fruit's sweet, supple folds, not let some malformed tentacle-horror molest you.");
	...
	if(player.cumQ() < 200) classes.EngineCore.outputText("Your weak load disappears into the plant's sucking gullet, wicked away as fast as you fire.");
	else if(player.cumQ() < 1000) classes.EngineCore.outputText("Your thick load disappears into the plant's sucking gullet almost as fast as you're firing it, though you can see a bit of white creampie forming around the edges of your cock.");
	else {
		classes.EngineCore.outputText("Your massive load floods the pumping, a deluge of semen squirting around your [cock biggest] even as the pumpkin starts to enlarge.  Every titanic spooge-squirt pumps the gourd up another level.  Soon, the rind is rubbing your belly.  A few blasts later, it's lifting you from the ground, your still-spurting cock lodged firmly in its cunt.  The suction feels so much stronger now - you're not sure you could pull out now if you wanted to, and you don't!");
	}
	if(player.cumQ() < 1000) {
		...
		if(player.weaponName == "fists") classes.EngineCore.outputText("brandishing your fists");
		else classes.EngineCore.outputText("picking up your [weapon]");
		classes.EngineCore.outputText(" as if it would somehow protect your nude body.");
		classes.EngineCore.doNext(pumpkinFuckPartII);
		player.orgasm();
		player.dynStats("cor", 2);
	}
...
public function pumpkinFuckPartII():void {
	...
	if(player.balls > 0) classes.EngineCore.outputText("[balls]");
	else classes.EngineCore.outputText("base");
	...
	if(player.cockTotal() > 1) {
		classes.EngineCore.outputText(", her free hand working ");
		if(player.cockTotal() == 2) classes.EngineCore.outputText("your other");
		else classes.EngineCore.outputText("another");
		classes.EngineCore.outputText(" dick with child-like glee");
	}
	classes.EngineCore.outputText(".  \"<i>");
	if(player.race() != "human") classes.EngineCore.outputText("I know what you were, virile suitor.  ");
	classes.EngineCore.outputText("There's never too much of a good thing.  Of course, my champion must have the proper train of thought...</i>\"");
	...
	classes.EngineCore.hideUpDown();
	player.orgasm();
	classes.EngineCore.doNext(pumpkinFuckPartIII);
}

public function pumpkinFuckPartIII():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozily in your plant-woven prison.  \"<i>Did you enjoy yourself?</i>\"  You slowly nod, feeling... liberated, free.  The situation is less worrying than before.  Fera just wanted to thank you, right?");
	classes.EngineCore.outputText("\n\nThe sultry orange-haired maiden ");
	if(player.balls > 0) classes.EngineCore.outputText("pats your sloshing sack");
	else classes.EngineCore.outputText("caresses a sensitive spot beneath your dick");
	classes.EngineCore.outputText(" as the vines unwind, allowing you to stand on your own [feet].  You go to wipe the cum from your nose and upper lip as soon as you're free.  It's dry.  Did any of that even happen?");
...
public function mountPumpkin():void {
	...
	if(player.pregnancyIncubation > 0 || (player.wetness() < 2 && player.totalFertility() < 30)) {
		classes.EngineCore.outputText("Well, fuck it.  You're horny, and this little pumpkin doesn't look like it could stop you from fucking that gushing stem-rod.  As you shed your [armor], a few of the rough, slime-sweating vines feebly rise up to embrace you, but you brush them away, having no time for corrupt foolishness.  You're going to mount this fruit's hard, pulsing phallus, not let some malformed tentacle-horror molest you.");
	...
	//Female characters don't have an easy analogue to cum production unless we count milk production, and female gender doesn't require either breasts or lactation to play.  So I just went for high vaginal wetness, not pregnant, and likely to become pregnant if she had sex with a male creature.  I hope that works out OK.
		...
		classes.EngineCore.outputText("\n\nA faint rumbling is the only warning you get (or need).  You jump back and ready yourself for anything, ");
		if(player.weaponName != "fists") classes.EngineCore.outputText("picking up your [weapon]");
		else classes.EngineCore.outputText("raising your guard");
		classes.EngineCore.outputText(" as if it would somehow protect your nude body.");
		
		classes.EngineCore.doNext(pumpkinMountingLevelII);
	}
	player.orgasm();
...
public function pumpkinMountingLevelII():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("The pumpkin's titanic stem shivers, the mighty shaft trembling immensely before the pumpkin splits from the top causing it to topple.  The crack goes around the rind in multiple directions, causing it to split open like a flower as strings of orangey-purple goo hang wetly between the pieces for a moment.  A moment later, the sectioned fruit falls apart, at last revealing the corrupt core.  No trace of the mammoth copulation remains.");
	...
	if(player.biggestTitSize() >= 2) classes.EngineCore.outputText(", her free hand working your [fullChest] with glee");
	classes.EngineCore.outputText(".  <i>\"There's never too much of a good thing.  Of course, my champion must have the proper train of thought...\"</i>");
	...
	//>Next
	classes.EngineCore.doNext(mountPumpkinsIII);
}

public function mountPumpkinsIII():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("SLAP!  The crack of a palm on your face rouses you, a red handprint left on your cheek.  Fera smirks at you as you stir woozy in your plant-woven prison.  <i>\"Did you enjoy yourself?\"</i>  You slowly nod, feeling... liberated... free.  The situation is less worrying than before.  Fera just wanted to thank you, right?");
...
public function feraSceneTwoIntroduction():void {
	...
	classes.EngineCore.addButton(0,"Explore",exploreFerasWonderland);
	classes.EngineCore.addButton(4,"Leave",leaveFerasWonderland);
}
//Leave
public function leaveFerasWonderland():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("Nah, that whole place is probably some kind of giant venus fly trap. Fuck that.");
...
public function exploreFerasWonderland():void {
	...
	classes.EngineCore.menu();
	if(player.inte >= 40) {
		classes.EngineCore.outputText(", and with that accomplished, you could head back to camp.  Now, though, you'd rather stay; it smells and looks too good to leave this place.  If you focus on your better sense, you could still leave.");
		if(player.hasCock()) classes.EngineCore.addButton(0,"Continue",fuckDemCorruptFlowerLips);
		else classes.EngineCore.addButton(0,"Continue",stickAStamenInIt);
		classes.EngineCore.addButton(4,"Leave",leaveAfterFlowerHuffing);
	}
	else {
		classes.EngineCore.outputText(", but that thought slips out of your head as it fills with more of that sinfully exciting aroma, making you feel so hot, horny, and eager.  You have to get more of it, submerse yourself deeper in its cloying clutches as you watch the pretty colors drain away your worries and concerns.");
		if(player.hasCock()) classes.EngineCore.addButton(0,"Next",fuckDemCorruptFlowerLips);
		else classes.EngineCore.addButton(0,"Next",stickAStamenInIt);
	}	
	if(player.hasCock() && player.hasVagina()) {
		classes.EngineCore.outputText("\n\nThe only ");
		if(player.inte >= 40) classes.EngineCore.outputText("other ");
		classes.EngineCore.outputText("question left to you is, are you more concerned with slipping [oneCock] into those subliminally-whispering lips or guiding a writhing stamen into your [vagina] and [asshole] so that you can take in as much nectar as possible?");
		classes.EngineCore.addButton(0,"Lips",fuckDemCorruptFlowerLips);
		classes.EngineCore.addButton(1,"Stamen",stickAStamenInIt);
	}
...
public function leaveAfterFlowerHuffing():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("The farther you get from that glade, the more your head clears and the more you realize how close you came to disaster.  You resolve to avoid it in the future, lest you fall prey to that entrancing pollen and the hypnotic petals once more.");
	classes.EngineCore.doNext(classes.Scenes.SceneLib.camp.returnToCampUseOneHour);
}
//Fuck the Lips
public function fuckDemCorruptFlowerLips():void {
	...
	if(player.longestCockLength() >= 20) {
		classes.EngineCore.outputText("easily ramming your huge endowment");
		if(player.cockTotal() > 1) classes.EngineCore.outputText("s");
		classes.EngineCore.outputText(" straight into the erotic-looking mouth");
	}
	else if(player.longestCockLength() >= 8) {
		classes.EngineCore.outputText("slipping your large endowments");
		if(player.cockTotal() > 1) classes.EngineCore.outputText("s");
		classes.EngineCore.outputText(" right into the erotic-looking mouth");
	}
	else if(player.longestCockLength() >= 4) {
		classes.EngineCore.outputText("easing the top hal");
		if(player.cockTotal() > 1) classes.EngineCore.outputText("ves");
		else classes.EngineCore.outputText("f");
		classes.EngineCore.outputText(" of your phallus");
		if(player.cockTotal() > 1) classes.EngineCore.outputText("es");
		classes.EngineCore.outputText(" into the erotic-looking mouth");
	}
	else {
		classes.EngineCore.outputText("barely managing to slip your tip");
		if(player.cockTotal() > 1) classes.EngineCore.outputText("s");
		classes.EngineCore.outputText(" into the erotic-looking mouth");
	}
	classes.EngineCore.outputText(". The contact is everything you thought it would be: surprisingly warm, soaking-wet, and squeezing just tight enough to make your pillar");
	if(player.cockTotal() > 1) classes.EngineCore.outputText("s");
	classes.EngineCore.outputText(" of lust twitch in excitement.");
	...
	if(player.balls > 0) classes.EngineCore.outputText("[balls] against the cushy lower lip, wreathing them in syrupy sap or nectar.  You're not really sure what the appropriate name for such a substance is, and you're well past caring.");
	else classes.EngineCore.outputText("loins against the cushy, sap-slicked lips, compressing their hot, smooth surfaces against your [skin].  You're not really sure if sap is the appropriate name for such a substance, but you're far beyond caring at this point.");
	classes.EngineCore.outputText("  The fast-diminishing light doesn't even register, as your eyes have closed as you subsume yourself in pleasure.");
	...
	if(player.cockTotal() > 1) classes.EngineCore.outputText("are");
	else classes.EngineCore.outputText("is");
	classes.EngineCore.outputText(" buried in, so hot, wet, and soft. With each passing moment, you become more relaxed, more open and exposed, and more aroused than before. You would probably be afraid if you were capable of any kind of reason.");
...
public function dickBoostingFuntimes():void {
	...
	if(player.cockTotal() > 1) classes.EngineCore.outputText("s");
	classes.EngineCore.outputText(" just in time to take the first thrust from your hips, burying yourself into them and immediately pulling back, setting an easy, average rhythm.  At the same time, the suction starts, causing your ");
	if(player.cockTotal() > 1) classes.EngineCore.outputText("heads");
	else classes.EngineCore.outputText("[cockHead]");
	classes.EngineCore.outputText(" to swell up a little, and when the rest of your length is plowed through the tight, slippery ring, the tugging vacuum increases, plumping your dick");
	if(player.cockTotal() > 1) classes.EngineCore.outputText("s");
	classes.EngineCore.outputText(" bigger, harder, and fatter than ");
	if(player.cockTotal() == 1) classes.EngineCore.outputText("it");
	else classes.EngineCore.outputText("they");
	classes.EngineCore.outputText(" should be.");
	...
	if(player.balls > 0) classes.EngineCore.outputText("empty");
	else classes.EngineCore.outputText("empties");
	classes.EngineCore.outputText(" so that you can start the cycle anew.  You love that, and this is the perfect place for that kind of behavior, isn't it? Everybody in this world wants to fuck, they just like to fight to see who gets to be on top, and you're good at winning, aren't you, pet champion?</i>\" Fera ");
	if(player.balls > 0) classes.EngineCore.outputText("cups your [sack] tenderly");
	else classes.EngineCore.outputText("tickles your taint");
	classes.EngineCore.outputText(" to spur you to thrust harder while she fills your head with lurid thoughts.");
	...
	if(player.balls > 0) classes.EngineCore.outputText("[balls]");
	else classes.EngineCore.outputText("taint");
	classes.EngineCore.outputText(" as you do, whispering, \"<i>Good " + player.mf("boy","girl") + ",</i>\" into your ears as dark energies flow into you, refilling your fast-emptying reservoirs faster than they can empty.  \"<i>Don't stop yet, [name].  You're going to be doing this a lot, so I'm giving you lots of practice.</i>\"");
...
public function stickAStamenInIt():void {
	...
	if(player.isNaga()) classes.EngineCore.outputText("tail");
	else if(player.isGoo()) classes.EngineCore.outputText("goo");
	else classes.EngineCore.outputText("knees");
	classes.EngineCore.outputText(", you drag yourself up until the 'floor' flattens out and you nearly ram into the pursed lips, stopping yourself abruptly once you realize you've reached your goal.");
	...
	if(player.hasCock()) classes.EngineCore.outputText("or ");
	classes.EngineCore.outputText("a tight, dick-milking rape-mother");
	if(!player.hasCock()) classes.EngineCore.outputText(", or a libidinous dick-girl, sowing your seed everywhere you can and saving your cunt for only the strongest of mates");
	classes.EngineCore.outputText("?</i>\"");
...
public function gapeMePleaseFera():void {
	...
	if(player.vaginas[0].vaginalLooseness >= classes.AppearanceDefs.VAGINA_LOOSENESS_LEVEL_CLOWN_CAR) classes.EngineCore.outputText(", spreading its well-widened lips as wide as they've ever been");
	else if(player.vaginas[0].vaginalLooseness >= classes.AppearanceDefs.VAGINA_LOOSENESS_GAPING) classes.EngineCore.outputText(", spreading your lips well beyond the widest they've ever been and still farther.");
	else if(player.vaginas[0].vaginalLooseness >= classes.AppearanceDefs.VAGINA_LOOSENESS_LOOSE) classes.EngineCore.outputText(", meeting a bit of token resistance at your unprepared entryway and slithering stamen after stamen inside, the forerunners widening you until the entirety of the column can pass inside, gaping you almost inhumanly wide.");
	else classes.EngineCore.outputText(", meeting more than a little resistant at your unstretched entry and finding a way in anyhow. First one tentacle slithers in, then a second, then a fourth, and so on. One after another, they spread you wider and wider, opening you until the entire wiggling column is buried deep, pulling your lips ridiculously wide.");
	...
	if(player.lowerBody == classes.AppearanceDefs.LOWER_BODY_TYPE_HUMAN) classes.EngineCore.outputText("toe");
	else if(player.tailType > classes.AppearanceDefs.TAIL_TYPE_NONE) classes.EngineCore.outputText("tail");
	else classes.EngineCore.outputText("[foot]");
	...
	if(player.wetness() < 4) classes.EngineCore.outputText(" even though you're not a squirter");
	classes.EngineCore.outputText(".  They pump on with intense, unrelenting energy, squishing wetly with every thrust, splattering juice everywhere as they ruin your poor, convulsing cunny.");
	player.cuntChange(90000,true,true,false);
	...
	//[Next]
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Next",gapeByFeraEpilogue);
}

public function gapeByFeraEpilogue():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You awaken fully dressed with a familiar ache between your legs. Damn, that was a good nap.  You rise, stretching as you rub your back, sore from lying against a tree for so long.  You suppose you could hunt down a tentacle monster and see if it'd really give you a filling");
	if(player.pregnancyIncubation == 0) classes.EngineCore.outputText(", but you'd rather fuck something that'll get you good and pregnant with a well-hung fuck-monster, like a minotaur!");
	else classes.EngineCore.outputText(".");
	classes.EngineCore.outputText("  You idly reach into your [armor] and stuff four fingers into yourself before sighing and removing them.  It just isn't the same as being full of cock.");
...
public function tightLikeAToigar():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("Fera smiles.  \"<i>I had hoped we might do that, little champion.  Now, you simply relax and enjoy the pleasure that only my normal weak-willed prey are allowed to experience.  You will be the first to taste it and be allowed free.  How wonderful!</i>\"  She pets your forehead.");
	...
	if(player.vaginas[0].vaginalLooseness >= classes.AppearanceDefs.VAGINA_LOOSENESS_LEVEL_CLOWN_CAR) classes.EngineCore.outputText(", slipping past your capacitive entrance with ease");
	else if(player.vaginas[0].vaginalLooseness >= classes.AppearanceDefs.VAGINA_LOOSENESS_GAPING) classes.EngineCore.outputText(", sliding into your welcoming entrance fairly easily");
	else if(player.vaginas[0].vaginalLooseness >= classes.AppearanceDefs.VAGINA_LOOSENESS_LOOSE) classes.EngineCore.outputText(", slowly working its way into your decently unstretched entrance");
	else classes.EngineCore.outputText(", slowly burrowing past your tight, restrictive entrance");
	classes.EngineCore.outputText(" and leaving a trail of slick, black corruption in its wake.  The tendril pushes the whole way up to your cervix, gently butting up against your womb's portal before dragging back out, sputtering more inky cum all the way.  When it pops out, the head catches the tip of your [clit], causing your [vagina] to spasm and contract, squeezing down on the foreign fluid with such strength that a geyser of sable spunk erupts from your quivering quim.");
	...
	//[Next]
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Next",tightnessEpilogue);
}
...
public function growADickForFera():void {
	...
	if(player.tailType > classes.AppearanceDefs.TAIL_TYPE_NONE) classes.EngineCore.outputText(" and press your tail to the ground");
	classes.EngineCore.outputText(".  You feel the wetness of the hollow tube as it presses against you along with a lance of pure desire and heat in the very center, undoubtedly caused by the tainted needle as it slips through your unprotected flesh.  The thing is compressed so tightly against you that it looks like the entire injector was able to bury itself in your body.");
	...
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Next",afterFeraGivesYouACawk);
}
