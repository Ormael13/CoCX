...
function hyenaJavelinAttack():void {
	...
	//Determine if evaded
		...
		outputText("Using your skills at evading attacks, you anticipate and sidestep " + monster.a + monster.short + "'s thrown spear.\n", false);
	}
	//("Misdirection"
	else if(player.hasPerk("Misdirection") >= 0 && rand(100) < 10 && player.armorName == "red, high-society bodysuit") {
		outputText("Using Raphael's teachings, you anticipate and sidestep " + monster.a + monster.short + "' thrown spear.\n", false);
	}
	//Determine if cat'ed
	else if(player.hasPerk("Flexibility") >= 0 && rand(100) < 6) {
		outputText("With your incredible flexibility, you squeeze out of the way of " + monster.a + monster.short + "'s thrown spear.", false);
	}
...
function hyenaSnapKicku():void {
	...
	//Determine if evaded
		...
		outputText("Using your skills at evading attacks, you anticipate and sidestep " + monster.a + monster.short + "'s snap-kick.\n", false);
	}
	//("Misdirection"
	else if(player.hasPerk("Misdirection") >= 0 && rand(100) < 10 && player.armorName == "red, high-society bodysuit") {
		outputText("Using Raphael's teachings, you anticipate and sidestep " + monster.a + monster.short + "' snap-kick.\n", false);
	}
	//Determine if cat'ed
	else if(player.hasPerk("Flexibility") >= 0 && rand(100) < 6) {
		outputText("With your incredible flexibility, you squeeze out of the way of " + monster.a + monster.short + "'s snap-kick.", false);
	}
	//Determine damage - str modified by enemy toughness!
	else {
...
function hyenaSpearLossAnal():void {
	...
	if(player.tailType > 0) outputText("One furred paw wraps around your tail before pulling up sharply, exposing your " + assholeDescript() + " to the hot, savanna air.  Sharp fangs clamp onto the tail, holding it up to rob you of any remaining modesty.", false);
	...
	//<For Slimes> 
	if(player.lowerBody == 8) outputText("  Even as your eyes slide closed, you see the hyena kicking open the circle of dust.", false);
...
function hyenaVictory():void {
	...
	var vagoo:Number = 0;
	if(player.hasVagina()) vagoo = 5149;
	var penor:Number = 0;
