...
function harpyUberCharge():void {
	...
	//(Harpy special attack 1, part two if PC does anything but "Wait")
		...
		if(flags[IN_COMBAT_USE_PLAYER_WAITED_FLAG] == 0) {
...
function harpyVictoryuuuuu():void {
	...
	var eggs:Number = 0;
	if(player.canOvipositSpider() && (player.faceType == 5 || player.faceType == 10)) eggs = 3835;
	var anal:Number = 0;
	var oral:Number = 2514;
	var pussy:Number = 0;
	var scissor:Number = 0;
	var clitFuck:int = 0;
	
	if(player.hasVagina() && player.clitLength >= 3.5) clitFuck = 3963;
	if(player.hasVagina()) {
		if(player.isNaga()) outputText("  If you weren't a naga, you could scissor her.");
		else scissor = 3530;
	}
	if(player.cockThatFits(monster.vaginalCapacity()) >= 0) pussy = 2516;
	if(player.cockThatFits(monster.analCapacity()) >= 0) anal = 2515;
	
	//Rape options
	if(player.lust >= 33) {
		outputText("  What do you do to her?", false);
		
		choices("Anal",anal,"Oral",oral,"Pussy",pussy,"Scissor",scissor,"Lay Eggs",eggs,"Clit Fuck",clitFuck,"",0,"",0,"",0,"Nothing",5007);
	}
	//Not horny?  Iz over
...
function harpyLossLust():void {
	...
	if(player.hasVagina() && x < 0) {
		...
		if(player.skinType == 0) outputText(" and quickly alleviating it by brushing their wings across your hide", false);
		...
		if(player.skinType == 0) outputText(", always followed by a soothing brush of their feathers", false);
		...
		if(player.skinType == 0) outputText(" across your reddening skin", false);
...
function harpyDamageLoss():void {
	...
	if(player.hasCock() && x >= 0) {
		...
		if(!player.hasVagina()) {
			...
			//(If breasts)
			if(player.biggestTitSize() >= 1) outputText("her petite bosom grinding across your " + allBreastsDescript() + ", her nipples tickling at your " + nippleDescript(0) + ", ", false);
		...
		//[(Herm)
			...
			//(If breasts)
			if(player.biggestTitSize() >= 2) outputText(", causing your " + allBreastsDescript() + " to bounce and sway violently", false);
	...
	//[(Female) 
		...
		//(If breasts)
		if(player.biggestTitSize() >= 3) outputText(" and causing your " + allBreastsDescript() + " to jiggle, slapping into one another", false);
	...
	if(x >= 0 && player.cockTotal() == 1) {
		...
		else if(player.biggestTitSize() >= 2) outputText(" while your " + allBreastsDescript() + " are shaking and jiggling around helplessly.", false);
		else outputText(".", false);
	...
	//[(Female)
		...
		//{(If breasts)
		if(player.biggestTitSize() >= 2) outputText("her belly squishing your " + allBreastsDescript() + " beneath her weight, ", false);
...
function victoryHarpyGetsHerPussyRaped():void {
	...
	if(player.gender == 1) {
		...
		//[(Male w/ breasts)
			...
			outputText(" while you knead over your " + allBreastsDescript() + " hungrily.  You start to pound her tight, harsh hole roughly, groaning and licking your lips, trying to master the power of the sensations her hot, clenching hole are giving to you. You lean down low over her back, your " + allBreastsDescript() + " pressing against the tickly, prickling feathers, as you bite down onto her shoulder.  Your " + hipDescript() + " raise and drop mercilessly into that jiggling, jelly-like ass, giving a noticeable bouncing effect from it as the squishing flesh catapults your rear back into the air with every lewd, slapping thrust.\n\n", false);
		}
	}
	//[(Herm)
...
function winAndRapeHarpyAnally():void {
	...
	if(player.biggestTitSize() >= 2) outputText(" Her petite bust squishes against your " + allBreastsDescript() + ". Her downy tits tickle, and grind against your chest maddeningly, tickling and scratching them gently as the lust-fueled bird-girl seems to ache for you.", false);
	outputText("\n\n", false);
	...
	//[(Herms/Shemales) 
		...
		if(player.biggestTitSize() >= 2) outputText(", the motions causing your " + allBreastsDescript() + " to bounce and sway wildly, slapping into one another lewdly, adding to your growing lust", false);
...
function WinOnHarpyAndOralRape():void {
	...
	//(if boobs)
	if(player.biggestTitSize() >= 2) outputText(", but soon she presses her toned, petite chest against your " + allBreastsDescript() + ", that stubby tongue tangling around yours lustily.", false);
	...
	//[(Male, Shemale, Herm, multicock)
		...
		if(player.biggestTitSize() >= 2) {
			outputText(", and your " + allBreastsDescript() + " jiggle slightly from the sheer intensity of the pins-and-needles feeling coating your body.  Your " + nippleDescript(0) + "s perk up and harden.", false);
	...
	if(player.gender == 2) {
		...
		//(If breasts)
		if(player.biggestTitSize() >= 2) outputText("You grasp onto your " + allBreastsDescript() + " violently, kneading and tweaking your " + nippleDescript(0) + " in a horny haze as the harpy's fingers tease around your pussy.  ", false);
