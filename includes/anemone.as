...
function approachAnemoneBarrel():void {
	...
	if(flags[KID_SITTER] <= 1) {
		...
		if(flags[HAD_KID_A_DREAM] > 0 && flags[ANEMONE_KID] >= 2) {
			...
			else outputText("\n\nKid A isn't self-confident enough to have sex with right any more...  Perhaps if you could tutor her with a weapon she seems to agree with?");
		}
...
function kidASex(cont:Boolean = true):Boolean {
	...
	//if KidXP >= 40 and lust > 99 after tutor and PC has vag and no cox
		...
		//[(if pc is loose)
		if(player.vaginas[0].vaginalLooseness >= 3) outputText("  You can barely feel her little shaft in your stretched cunt, but the chemical stimulation from the tentacles stroking your insides goes a long way toward making up for that.");
