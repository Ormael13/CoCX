...
public function dontExploreGatsChristmasTale():void {
	classes.EngineCore.clearOutput();
	//i. No
	classes.EngineCore.outputText("In a place like the high mountains, there's no warning to be found if you happen to be confronted by danger.  Deciding not to take your chances, you continue to walk down the path you came, leaving the peaks to head straight back to camp.");
...
public function encounterQuoteUnquoteAngel():void {
	...
	if(!isHolidays()) {
		...
		if(classes.EngineCore.silly()) classes.EngineCore.outputText(" (you didn't.)");
		else classes.EngineCore.outputText(".");
		classes.EngineCore.outputText("  She beams a toothless grin, popping one into her mouth and blowing out a stream of candy-scented smoke.  Suddenly, you're glad you didn't take any.");
	...
	//ii. Yes
		...
		classes.EngineCore.outputText("You haven't found anything as of yet, so you figure you might as well keep going.  Following the quiet cries from above, you trace the spiralling side of the snow-capped behemoth, eventually reaching a small cliff hanging beneath the mountain's peak.  The sight you're met with manages to take your breath away - and considering the inhabitants of the land around you, that in itself is quite a difficult feat.");
		...
		if(classes.EngineCore.silly()) classes.EngineCore.outputText(", without a penis");
		classes.EngineCore.outputText(" - a loving aura surrounds the feminine creature and you can't help but feel as if she's here without ill intent.  Stating your name, you question her purpose at the high mountains.");
...
public function findTheWindUpKey():void {
	...
	//[if (tallness > 60) 
	if(player.tallness > 60) classes.EngineCore.outputText("  You can't even stand straight up below the low-hanging ceiling, forced to carefully tread along on your knees over the slick, hard rock.");
	classes.EngineCore.outputText("  Squinting around the small, deep green grotto, a pedestal in the center calls out to you.  As you approach it, it becomes evident to you why things are so chilly in here; the outcrop is made up of clear ice - and within, the bodies of the many who had found this place before you.  At least that mystery is cleared up.  Atop the frozen prop lies a small, crystal... crank? Upon closer inspection, it actually resembles a - kitty titties, it's a windup key!  Despite the fact that you should've seen this one coming, you surmise that this is the item you've come searching for.");
...
public function giveThatBitchAKeyYo():void {
	...
	if(flags[kFLAGS.GATS_ANGEL_TIME_TO_FIND_KEY] < 150) {
		classes.EngineCore.outputText("All throughout your perilous climb, you cannot help but ogle at the brilliant little crystal key.  You have a suspicion as to how this innocent little key will help the crying colossus on the peak, but you are not completely sure this is the artifact she required.  But you have a hunch that it is.");
		...
		classes.EngineCore.addButton(0,"Give The Key",goodEndForChristmasMiracleICant);
		classes.EngineCore.addButton(1,"FuckSoldier",theWorstEndingEverDotJPG);
		classes.EngineCore.addButton(2,"Fuck Spirit",fuckTheAngelBadEnd);
	}
	else {
		...
		classes.EngineCore.outputText("All throughout your perilous climb, you cannot help but ogle at the brilliant little crystal key.  You have a suspicion as to how this innocent little key will help the crying colossus on the peak, though you aren't entirely sure this is the artifact she required.  But you have a hunch that it is.");
...
public function theWorstEndingEverDotJPG():void {
	...
	//[if (corruption > 49)
	if(player.cor > 49) classes.EngineCore.outputText("  It seems like the best response to you for now.  Based on the giantess's sobbing soliloquy, you figure that standing in for the love of the soldier's life will get you straight to the action.");
	else classes.EngineCore.outputText("  Wait, what?  It appears that in your desperation to contain the situation, your lingering doubt was enough to get you blurt out a bold faced lie.  You figure that you may as well roll with it for now.");
	...
	if(player.hasCock()) classes.EngineCore.outputText("[eachCock]");
	if(player.gender == 3) classes.EngineCore.outputText(" and ");
	if(player.hasVagina()) classes.EngineCore.outputText("your [vagina]");
	classes.EngineCore.outputText(" to try and convince the confused soldier.  You look North over, figuring the best way to work him under your spell.  It isn't a long search, as you decide that his ");
	//[if (hasCock = true) 
	if(player.hasCock()) classes.EngineCore.outputText("keyhole");
	//[if (hasCock = false) 
	else classes.EngineCore.outputText("windup key");
	classes.EngineCore.outputText(" would make a worthy candidate for your first alluring act of debauchery.");
	...
	//[if (hasCock = true) 
	if(player.hasCock()) classes.EngineCore.outputText("You slide the key playfully down your wrist, rubbing your [cock] with your free hand.  You aren't entirely sure if he'll even feel a sensation from your penetration, but perhaps it'll at least get him in the mood.  With your manhood at full mast, you gently skirt along the edges of the keyhole.");
	else classes.EngineCore.outputText("A mischievous tongue traces along the smooth, perfect surface of the windup key.  You promptly lower the makeshift toy precariously in front of your [cunt]; if there was ever a time to make a wind up pun or remark as some sort of goofy foreplay, now would be the time.");
	...
	//[if (corruption > 49) 
	if(player.cor > 49) classes.EngineCore.outputText("  ...Well, that was a waste of time.");
	...
	//[if (corruption > 49) 
	if(player.cor > 49) classes.EngineCore.outputText("  More importantly, you have some unsatisfied, pent up lust that you'd like to expend.");
...
public function fuckTheAngelBadEnd():void {
	...
	if(player.hasCock()) classes.EngineCore.outputText("\n\nTaking out your junk");
	else classes.EngineCore.outputText("\n\nCaressing your cunt");
	classes.EngineCore.outputText(", you wander your way towards the giant's mouth-muscle, climbing on top of her warm, pink bed. There's gotta be some kinda magic involved - her mint saliva sends tingles through your body, while dragging your ");
	if(player.hasCock()) classes.EngineCore.outputText(player.multiCockDescriptLight());
	else classes.EngineCore.outputText(Appearance.vaginaDescript(player,0));
	classes.EngineCore.outputText(" against each bump and curve is beginning to get you hard.  She winces at the taste of your crotch against her tongue, though she's obviously never given oral to something with a heartbeat before.  Well, there's a first time for everything.");
	...
	if(player.hasCock()) {
		classes.EngineCore.outputText("member");
		if(player.cockTotal() > 1) classes.EngineCore.outputText("s");
	}
	else classes.EngineCore.outputText(Appearance.vaginaDescript(player,0));
	classes.EngineCore.outputText(" along the softness of her maw causing her to twitch and drool uncomfortably.  You're beginning to fuck along her slowly, chuckling at every wince and moan she makes from the taste of your genitals.");
	...
	if(player.hasCock()) classes.EngineCore.outputText("dick");
	else classes.EngineCore.outputText("vagina");
	classes.EngineCore.outputText(" on a person's tongue to fuck it thoroughly.  You wonder if she's already long forgotten the pile of bolts she was originally trying to save.  A man frozen in time, never to wake up because - whoops!  You're taking care of her needs now.  Maybe you'll invest in a giant goblin-built sex machine, who knows?  Judging by the bitch's aroused whines, she was probably waiting for the chance to strip for the next person to find her up the mountain.  It's about time she let herself go, after all.");
...
public function goodEndForChristmasMiracleICantII():void {
	...
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Stay",stayForPretendNTRGoodWinjobSex);
	classes.EngineCore.addButton(4,"Leave",goodEndLeaveBeforeSexings);
}

//If Leave
public function goodEndLeaveBeforeSexings():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You get the message, feeling that letting them do their own thing might be for the best.  With a hasty pardon, you promptly turn around and walk a good twenty yards down the mountainside, idly playing in the snow while the two reunited lovebirds do their thing.");
	...
	//[Go to Conclusion]
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0,"Next",conclusionOfGiantessGoodEnd);
}
//If Stay
public function stayForPretendNTRGoodWinjobSex():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You prefer to stay here, but the frosty giant has already gone back to her mechanical man, so you hold your explanation and continue to observe.");
	...
	if(player.lust >= player.maxLust()) {
		...
		if(player.wetness() >= 4 || player.cumQ() >= 1000) classes.EngineCore.outputText(" - figuratively, and literally, as well, as your fluids drip off the edge to add to the rain of sexual fluids the giantess is laying in the midst of");
		classes.EngineCore.outputText(".");
	}
...
public function conclusionOfGiantessGoodEnd():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("The air is crisp, and the light powdering of snow floats through the air as if playing an invisible melody.  The warm couple are together.  Lifting her head, the giantess beams at you with a refreshing sense of spirit.");
	...
	if (flags[kFLAGS.PC_ENCOUNTERED_CHRISTMAS_ELF_BEFORE] == 0) classes.EngineCore.outputText("\n\nYou have no idea what that is.");
	classes.EngineCore.awardAchievement("A Christmas Carol", kACHIEVEMENTS.HOLIDAY_CHRISTMAS_II);
