...
function yesDebimboSophie():void {
	...
	flags[SOPHIE_DEBIMBOED] = 1;
	if(flags[SOPHIE_ADULT_KID_COUNT] > 0) flags[SOPHIES_DAUGHTERS_DEBIMBOED] = 1;
...
function bimboSophieAgain():void {
	...
	flags[SOPHIE_DEBIMBOED] = 0;
	flags[SOPHIES_DAUGHTERS_DEBIMBOED] = 0;
...
function beatUpDebimboSophie():void {
	clearOutput();
...
function debimboSophieBeatsYouUp():void {
	clearOutput();
...
function sophieDaughterDebimboUpdate():void {
...
function followerSophieMainScreen():void {
	...
	if(sophieIsInSeason() && player.hasCock()) {
		...
		if(bimboSophie()) outputText("restoring her intellect made her quite skilled at it again");
...
function sexWithFollowerSophie():void {
	...
	if(player.hasCock()) {
		if(player.cockThatFits(sophieCapacity()) >= 0) addButton(0,"Vaginal",fuckFollowerSophie);
		addButton(1,"Blowjob",sophieFollowerGivesBlowjobs);
	}
	if(flags[SOPHIE_ADULT_KID_COUNT] > 0 && flags[SOPHIE_FAMILY_INCEST] > 0 && player.cockThatFits(sophieCapacity()) >= 0)
...
function fuckFollowerSophie():void {
	...
	//Medium
	else if(player.cockArea(x) <= sophieCapacity()/2) {
	...
	if(sophieIsInSeason()) outputText("gushing, sperm-hungry");
	...
	if(sophieIsInSeason()) outputText("  Her pussy seems to drink the spunky deposit down, her cervix opening to allow in as much in as possible.  You can feel the suction, and if you weren't already riding out a mind-bending orgasm, you're sure you'd be cumming harder than normal right now.");
	...
	if(player.cumQ() >= 5000 || (player.cumQ() >= 500 && sophieIsInSeason())) {
...
function sophieVagFollowerFollowup():void {
	...
	if(sophieIsInSeason()) outputText("If that didn't take, I don't know what will!");
...
function sophieFollowerGivesBlowjobs():void {
	...
	else if(player.cockArea(x) <= sophieCapacity()/2) outputText("needy boy-cock");
...
function sophieBlowsSixtyNine():void {
	...
	if(player.tongueType > 0 && player.tongueType <= 3) outputText("Longer and longer, your tongue continually unspools from your mouth, plunging deeper into the slippery channel with each passing moment.  Her taste saturates your many taste buds, and the velvety folds of her interior feel wonderful as you repeatedly slither your inhuman muscle through that syrupy passage.");
	...
	flags[SOPHIE_FOLLOWER_IRRITATION] = 0;
	//Slimefeed
	slimeFeed();
	//8 hours lust stick
...
function forceSophieBlowjob():void {
	...
	flags[SOPHIE_FOLLOWER_IRRITATION]++;
	...
	outputText(" \"<i>but don't make a habit of this, or you'll wake up alone.</i>\"  Sophie's tongue flicks across your " + cockHead(x) + " experimentally.  \"<i>At least you taste good.</i>\"  The slippery muscle slides across your sensitive dick-skin again, longer this time, lingering on the underside as the golden lips edge closer.");
	
	outputText("\n\nImpatient, you slide your fingers through her feathery locks and edge her closer.  Her shining, amber mouth bumps into your " + cockHead(x) + ", followed by a surprised chirp from the unresisting harpy.  At that first touch, the electric tingle of her glossy gold lipstick hits you, causing your heart to beat faster and your " + cockDescript(x) + " to thicken slightly with arousal in her hand.  You involuntarily push harder without meaning to, spreading Sophie's plump cock-kissers around the thickening meat, her spit slicking the slow, steady journey into her mouth.  Like magic, she begins to push forward on her own, almost pulling out of your grip in her eagerness.  Her mouth smoothly slides down ");
	...
	else if(flags[SOPHIE_FOLLOWER_IRRITATION] < 4) outputText("Ahhh... I... I don't know how many times I'll put up with this kind of treatment!");
	else if(flags[SOPHIE_FOLLOWER_IRRITATION] < 8) outputText("Ahhh... are... are you ever going to reciprocate?");
	else if(flags[SOPHIE_FOLLOWER_IRRITATION] == 8) outputText("Ahhh... I think... I've had enough...");
...
function sophieSmallDongTeases():void {
	...
	if(flags[TIMES_MORNING_SOPHIE_FEMDOMMED] > 0) outputText("In the back of your mind, you know that if her words weren't true you wouldn't have kept sleeping with her and letting her do this to you.");
	...
	flags[TIMES_MORNING_SOPHIE_FEMDOMMED]++;
...
//Similar to bimbo sophie
function sophieFollowerGoesIntoSeas():void {
...
function sophieFertilityExpired():void {
...
function sophieFertilityKnockedUpExpired():void {
...
function getOutSophieRecruitment():void {
	...
	flags[NO_PURE_SOPHIE_RECRUITMENT] = 1;
	doNext(1);
}
...
function sophieIncestInHerCooterOrSomethingIDunno():void {
	clearOutput();
	outputText("Approaching the harpy nest you call out for one of your daughters.  ");
	if(flags[SOPHIE_ADULT_KID_COUNT] > 1) outputText("The prospect of their " + player.mf("father","mother") + "'s attention causes a small scuffle to break out between your feathery daughters. Thankfully, the small fight is soon ended and one of your daughters flies down and stands before you.  ");
	...
	var daughterCap:int = flags[SOPHIE_ADULT_KID_COUNT];
	...
	//second daughter: 
		...
		if(flags[DAUGHTER_TWO_BIMBO] > 0) outputText("bimbo ");
		...
		if(flags[DAUGHTER_TWO_BIMBO] > 0 && flags[SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\"<i>Mmmm... doncha want to just like, pound my sexy ass, " + player.mf("Daddy","Mommy") + "?  It's so big, it's just begging to be spanked and squeezed till it's totally red and sensitive</i>\"");
	...
	//third daughter:
		...
		if(flags[DAUGHTER_FOUR_BIMBO] > 0 && flags[SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("\"<i>Come on " + player.mf("Daddy","Mommy") + ", can't you see how totally hot I am?  Don't you just want to let loose and like, just fuck me till I scream.</i>\"");
		...
		//bimbo speech
		if(flags[DAUGHTER_FOUR_BIMBO] > 0 && flags[SOPHIES_DAUGHTERS_DEBIMBOED] == 0) outputText("  \"<i>Ahh... ohhh... oh, " + player.mf("Daddy","Mommy") + "!  You like my butt, don't you; isn't it nice?  It's so good to grab and squeeze.  I bet it would be fun to, like, spank it too....</i>\"");
		//[normal speech:]
...
function phaseTwoOfIncest(daughter:int):void {
	clearOutput();
	var x:int = player.cockThatFits(sophieCapacity());
	if(x < 0) x = player.smallestCockIndex();
	daughter++;
	...
	if(flags[DAUGHTER_FOUR_BIMBO] > 0 && flags[SOPHIES_DAUGHTERS_DEBIMBOED] == 0)
	
	var bimbo:Boolean = (((daughter == 4 && flags[DAUGHTER_FOUR_BIMBO] > 0) || (daughter == 3 && flags[DAUGHTER_THREE_BIMBO] > 0) || (daughter == 2 && flags[DAUGHTER_TWO_BIMBO] > 0) || (daughter == 1 && flags[DAUGHTER_ONE_BIMBO] > 0)) && flags[SOPHIES_DAUGHTERS_DEBIMBOED] == 0);
	...
	outputText(" she croons, her words dripping with raw, passionate need for you and your incestuous love.  The sight of her so needy, so wanting, her body aflame with an unquenchable hunger for you is just too tempting.  Your own lust boils over as a thick dollop of pre-cum bursts from your " + cockHead(x) + " and splatters across your daughter's sodden snatch, adding to the lube already waiting to welcome you into her body.");
