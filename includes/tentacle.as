...
function tentaclePhysicalAttack():void {
	...
	if(temp < 0) temp = 0;
	//Miss
	if(temp == 0 || (player.spe - monster.spe > 0 && int(Math.random()*(((player.spe-monster.spe)/4)+80)) > 80)) {
...
function tentacleVictoryRape():void {
	...
	//Female:
		...
		if(player.cor >= 66) {
			...
			outputText("Laughing like a true psychotic, you stuff the tentacle into your womb. Your vaginal muscles quickly go to work stroking, squeezing and kneading the appendage. The creature, more intent with escape than hammering your box, begins struggling. You summarily slug the beast as well as any professional pugilist to stop its throes.\n\n", false);
		...
		//Rape Win Female-Low Corruption:: 
			...
			outputText("Sensing your needs, the tamed beast extends a tendril from the main tentacle that easily pushes past your cervical opening and breeches the deepest parts of your womb. The feeler penetrates past your uterus and lodges itself as deeply as possible. The beast begins rapidly vibrating and undulating its member, stimulating the deepest parts of your sex.\n\n", false);
...
function tentacleLossRape():void {
	...
	if(player.biggestLactation() >= 3.5 && player.gender > 0) {
		...
		if(player.gender == 2) {
			...
			if(cuntChange(20,true)) outputText("\n", false);
			outputText("\n", false);
		}//HERMS
		if(player.gender == 3) {
	...
	if(player.gender == 3) 
	{
		...
		growTits(1,player.breastRows.length, false, 2);
...
function centaurGenderlessRetardation():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00247] == 0 || player.balls == 0) {
		flags[UNKNOWN_FLAG_NUMBER_00247] = 1;
