...
public function meetJackFrostInTheMountains():void {
	...
	classes.EngineCore.addButton(0,"Yes",jizzSnowOnMyCampPlease);
	classes.EngineCore.addButton(1,"No",noJizzingInMyCampPlease);
}

//[=No=]
public function noJizzingInMyCampPlease():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You thank the giant for his offer, but you'll have to refuse.  You really can't take even one day off your quest.");
...
public function processJackFrostEvent():void {
	...
	//Rathazul
		...
		classes.EngineCore.outputText("Rathazul approaches you.  \"<i>[name]?  What is going on?  It hasn't snowed in Mareth for years now.  And why only around the camp?  I wandered out to investigate, but outside there is no snow at all...</i>\" he asks, concerned.");
	...
	//Izma
		...
		classes.EngineCore.outputText("You wonder where Izma is; as a shark, maybe she doesn't like the cold?  Even as the thought crosses your mind, you note a surprisingly large pile of snow sitting not too far - with a very suspicious striped fin jutting out of it.  You rush over, calling for Izma and asking if she's all right.");
	...
	//Jojo
		...
		//Corrupt
			...
			classes.EngineCore.outputText("Out in the outskirts of the camp you notice your pet mouse-slut Jojo, casually fucking a small mound of snow like an animal.  You laugh to yourself, now, now... doesn't he know that there are better things to fuck than a pitiful mound of snow on the ground...");
			...
			classes.EngineCore.menu();
			if(player.hasCock()) classes.EngineCore.addButton(0,"Pen Ass",penetradeCorruptJojoAss);
			if(player.hasVagina()) classes.EngineCore.addButton(1,"Ride Cock",rideCorruptJojoCock);
			classes.EngineCore.addButton(2,"Spank n Finger",spankNFingerCorruptJojoAss);
		}
		return;
	}
	//Amily
	//less than 5!
		...
		if(!amilyScene.amilyCorrupt()) {
			...
			classes.EngineCore.addButton(1,"Ride Cock",makeOutWithPureMouseWaifu);
			return;
		}
		//Corrupt
		else {
			classes.EngineCore.outputText("Out in the outskirts where your favorite cum-slut lives, you spot her slowly rubbing the snow over her erect nipples, the cold making her shiver in pleasure.");
			...
			if(player.gender > 0) {
				...
				if(player.hasCock()) {
					...
					if(player.balls > 0) classes.EngineCore.outputText(", even tickling your [balls]");
					else if(player.hasVagina()) classes.EngineCore.outputText(", even tickling your [cunt] for added tantalization");
					classes.EngineCore.outputText(".");
	...
	//Marble (Written by TDM himself)
		...
		if(flags[kFLAGS.MARBLE_KIDS] > 0 && flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] >= 100) {
			classes.EngineCore.outputText("You find Marble simply staring into the sky, a look of wonder covering her face.  Dancing around her in excitement ");
			if(flags[kFLAGS.MARBLE_KIDS] > 1) classes.EngineCore.outputText("are ");
			else classes.EngineCore.outputText("is ");
			classes.EngineCore.outputText("your kid");
			if(flags[kFLAGS.MARBLE_KIDS] > 1) classes.EngineCore.outputText("s");
			classes.EngineCore.outputText(", playing in the snow, blowing the snow, and experimenting with the snow.  Exactly what ");
			if(flags[kFLAGS.MARBLE_KIDS] == 1) classes.EngineCore.outputText("she");
			else classes.EngineCore.outputText("they");
			classes.EngineCore.outputText(" are up to at any given moment is really hard to say, and you're not even sure that the little girl");
			if(flags[kFLAGS.MARBLE_KIDS] > 1) classes.EngineCore.outputText("s");
			classes.EngineCore.outputText(" know");
			if(flags[kFLAGS.MARBLE_KIDS] == 1) classes.EngineCore.outputText("s herself.");
			else classes.EngineCore.outputText(" themselves.");
			...
			if(flags[kFLAGS.MARBLE_KIDS] == 1) classes.EngineCore.outputText("child");
			else classes.EngineCore.outputText("children");
			classes.EngineCore.outputText(" playing together once more.  A moment later, Marble puts her hand to her expansive backend again, before suddenly exclaiming again in realization.  \"<i>Sweetie, watch our kid");
			if(flags[kFLAGS.MARBLE_KIDS] > 1) classes.EngineCore.outputText("s");
			classes.EngineCore.outputText(" for a moment, I'll be right back!</i>\" and she leaves the camp as quickly as she can, without slipping and falling on her ass again.");

			classes.EngineCore.outputText("\n\nYou certainly don't mind staying behind for now, it is certainly a lot of fun spending time with your kid");
			if(flags[kFLAGS.MARBLE_KIDS] > 1) classes.EngineCore.outputText("s");
			classes.EngineCore.outputText(" and just having fun.  Your cow-girl daughters are certainly not lacking in energy in the cold snow, their fur covered legs and built in shoes probably help a lot with this.  ");
			if(flags[kFLAGS.MARBLE_KIDS] > 1) classes.EngineCore.outputText("They");
			else classes.EngineCore.outputText("She");
			classes.EngineCore.outputText(" listen");
			if(flags[kFLAGS.MARBLE_KIDS] == 1) classes.EngineCore.outputText("s");
			classes.EngineCore.outputText(" with rapt attention when you explain what snow is, and about all the things you use to do in it back home.  Hands-on demonstrations are also in high demand.");
			...
			if(flags[kFLAGS.MARBLE_KIDS] > 1) classes.EngineCore.outputText("s");
			classes.EngineCore.outputText(" to ride on!  Happy cries of joy fill the air for some time, as you and Marble take turns in giving rides on the sled.");
			
			//PC strength check:
			//(>=85)
			if(player.str >= 85) {
				classes.EngineCore.outputText("\n\nIt would seem that your strength has won you more favor with ");
				if(flags[kFLAGS.MARBLE_KIDS] > 1) classes.EngineCore.outputText("the girls");
				else classes.EngineCore.outputText("your daughter");
				classes.EngineCore.outputText(" when it comes to giving sled rides, Marble doesn't seem to mind too much though.  Your mate actually decides to join in the riding part after a while!");
			} 
			//(60-84)
				...
				classes.EngineCore.outputText(" in deciding who gives the best rides.  In the end, a clear winner is never decided, but everyone agrees that it was a lot of fun.");
			}
			//(<=59)
			else {
	...
	//if she ain't a follower and is a lover, just have her visit.
		...
		if(player.gender > 0) {
			classes.EngineCore.outputText(", as evidenced by ");
			if(player.hasCock()) classes.EngineCore.outputText("your erect " + player.multiCockDescriptLight());
			if(player.gender == 3) classes.EngineCore.outputText(" and ");
			else if(player.hasVagina()) classes.EngineCore.outputText("your ");
			if(player.hasVagina()) classes.EngineCore.outputText("dripping [vagina]");
		}
		classes.EngineCore.outputText(".");
		...
		//(If PC has a cock)
		if(player.hasCock()) classes.EngineCore.outputText("  A torrent of seed splashes against Helia's taut belly, as [eachCock] throbs and spews its seed while sandwiched between the two of you.");
		//(if PC has a pussy)
		if(player.hasVagina()) classes.EngineCore.outputText("  Your [vagina] quivers and spill your juices with the force of a wave.  Moisture falls on the ground to join in with Helia's own discharge.");
	...
	//Kiha
		...
		classes.EngineCore.menu();
		if(player.hasCock() && player.cockThatFits(67) >= 0) classes.EngineCore.addButton(0,"Fuck Her",kihaXmasFuck);
		classes.EngineCore.addButton(1,"Finger Her",kihaXmasFingering);
	}
...
public function kihaXmasFuck():void {
	...
	classes.EngineCore.outputText("Stripping off your [armor], you tell the dragon-girl that a good workout is all she needs to get nice and warm.  She snorts in dismissal, but the way she's looking at you is a pretty good indication she's willing to try it.  \"<i>So, what do you have in mind?</i>\" she blusters.  You stroke your quickly erecting [cocks] and, smirking, ask her if your intentions aren't obvious?");
...
public function kihaXmasFingering():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You examine Kiha.  A small snowflake lands on one of her nipples and she yelps with a start at the sudden contact.   \"<i>Hey, I asked you to help warm me up, not to stare at me!</i>\" she complains. ");
...
public function penetradePureMouseWaifu():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("Stripping down your [armor] and indicating Amily should do the same... you tell her that you were hoping for some good old-fashioned sex between a horny " + player.mf("man","herm") + " and " + player.mf("his","her") + " lovely mousette.  You playfully ask Amily if she has any objections?");
	...
	if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) classes.EngineCore.outputText(", not to mention her dick's pretty hard, too");
	classes.EngineCore.outputText(".");
	...
	if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) classes.EngineCore.outputText(" and painting your belly white with futa-spooge");
	classes.EngineCore.outputText(".");
...
public function makeOutWithPureMouseWaifu():void {
	...
	if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) classes.EngineCore.outputText("Smiling sheepishly you  tell her to mind the erection... though you're not actually looking for sex, you can't help with how cute she looks...  \"<i>Flirt.</i>\"  Amily giggles, but otherwise doesn't comment on it.  ");
	classes.EngineCore.outputText("The mouse promptly walks over and sits down just in front of you, her back facing you and giving you a perfect opportunity to hug her.  Not keen on missing such an opportunity, you do so.");
...
public function penetradeCorruptJojoAss():void {
	...
	if(player.cumQ() >= 700) classes.EngineCore.outputText(" all over his own cum-engorged gut and ");
	classes.EngineCore.outputText("onto the snowy earth underneath him.  He cums and cums until he's exhausted even his prodigious balls, creating a salty quagmire of semi-melted snow and spooge under him that he promptly collapses face-first into.");
...
public function rideCorruptJojoCock():void {
	...
	if(player.hasCock()) classes.EngineCore.outputText(", your own futa-cock spewing its load, which you deliberately spray in his face for amusement");
	classes.EngineCore.outputText(".  Your orgasm triggers Jojo's own; though he has cum without permission, at least he waited for you to cum first like a good slut should.  Rope after rope of mousy sperm bats your cervix, trying to get past the barrier and into the waiting womb inside, even as Jojo tries his best to fuck you over and over as he spills his load, until he finally slumps into the ground, panting, with his mouth open and his tongue lolling out, eyes glazed over in a pleasured daze.");
