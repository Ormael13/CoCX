...
function chameleonTongueAttack():void {
	monster.weaponName = "tongue";
	monster.weaponVerb = "tongue-slap";
	monster.weaponAttack = 10;
	monster.createStatusAffect("attacks",1,0,0,0);
	eAttack();
...
//Ignores armor
function chameleonClaws():void {
	//Blind dodge change
	if(monster.hasStatusAffect("Blind") >= 0 && rand(3) < 1) {
		outputText(monster.capitalA + monster.short + " completely misses you with a blind claw-attack!\n", false);
	}
	//Evade: 
...
//Attack 3: 
function rollKickClawWhatTheFuckComboIsThisShit():void {
	//Blind dodge change
	if(monster.hasStatusAffect("Blind") >= 0 && rand(3) < 1) {
		outputText(monster.capitalA + monster.short + " completely misses you with a blind roll-kick!\n", false);
	}
	//Evade: 
	else if(combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
		var damage2:Number = 1 + rand(10);
		damage2 = doDamage(damage2);
	...
	//Get hit
		...
		var damage:Number = int((monster.str + monster.weaponAttack) - rand(player.tou) - player.armorDef) + 25;
		if(damage > 0) {
			damage = takeDamage(damage);
...
function defeatChameleonGirl():void {
	...
	var herm:int = 0;
	var item:int = 0;
	if(player.hasCock()) dick = 3779;
	if(player.hasVagina()) pussy = 3780;
	if(player.gender == 3) herm = 3778;
	//let PC use item
...
function fuckDatChameleonAsACoolGuyGirlHerm():void {
	...
	outputText("\n\nTaking the hint, she starts playing with your dripping gash again, sometimes teasing your " + clitDescript() + " with her fingers.  Your throbbing cock aches for attention, though, and an interesting idea pops into your head; you pick her tail up off the ground and, though she tries to twitch it away instinctually, you hold tight.  It relaxes, and you guide it towards your needy member, then gently wrap its end around a few times.  With a mischievous smile, the girl tightens her grip more than you expected, slowly pumping your shaft up and down with her strong tail.  pre-cum begins to leak from the tip of your " + cockDescript(x) + " and your pussy drools with sexual fluids as extra the stimulation affects your body.");
	outputText("\n\nNot ");
...
function useAnItemOnTheChamcham():void {
	...
	var milk:int = 0;
	var drafts:int = 0;
	if((hasItem("SucMilk",1) || hasItem("P.S.Mlk",1)) && player.hasCock()) milk = 3782;
	if(hasItem("SensDrf",1) && (hasItem("L.Draft",1) || hasItem("F.Draft",1))) drafts = 3783;

	simpleChoices("SuccMilk",milk,"LustnSensD.",drafts,"",0,"",0,"Back",3784);
}
