...
function gooGalAttack():void {
	...
	doNext(1);
	if(monster.hasPerk("Acid") >= 0) outputText("Her body quivering from your flames, the goo-girl ", false);
	...
	if(player.spe - monster.spe > 0 && int(Math.random()*(((player.spe-monster.spe)/4)+80)) > 80) {
		if(monster.hasPerk("Acid") >= 0) outputText("tries to slap you, but you dodge her attack.", false);
		else outputText(", missing as you dodge aside.", false);
		return;
	}
	//Determine if evaded
	if(monster.short != "Kiha" && player.hasPerk("Evade") >= 0 && rand(100) < 10) {
		if(monster.hasPerk("Acid") >= 0) outputText("tries to slap you, but you evade her attack.", false);
	...
	if(player.hasPerk("Misdirection") >= 0 && rand(100) < 10 && player.armorName == "red, high-society bodysuit") {
		if(monster.hasPerk("Acid") >= 0) outputText("tries to slap you.  You misdirect her, avoiding the hit.", false);
		else outputText(", missing as you misdirect her attentions.", false);
		return;
	}
	//Determine if cat'ed
	if(player.hasPerk("Flexibility") >= 0 && rand(100) < 6) {
		if(monster.hasPerk("Acid") >= 0) outputText("tries to slap you, but misses due to your cat-like evasion.", false);
		else outputText(", missing due to your cat-like evasion.", false);
		return;
	}
	//Determine damage - str modified by enemy toughness!
	if(monster.hasPerk("Acid") >= 0) damage = int((monster.str + 10 + monster.weaponAttack) - rand(player.tou) - player.armorDef);
	else damage = int((monster.str + monster.weaponAttack) - rand(player.tou) - player.armorDef);
	if(damage > 0) damage = takeDamage(damage);
	if(damage <= 0) {
		damage = 0;
		if(monster.hasPerk("Acid") >= 0) {
	...
	//everyone else
		...
		else outputText(", painfully smacking her gooey limbs against your head.  You shake your " + hairDescript() + ", clearing your head of the dazing slap.", false);
...
function gooThrow():void {
	...
	stats(0,0,0,0,0,0,5+rand(3)+player.sens/10,0);
	combatRoundOver();
}
//Engulf – 
...
function hermLoseToGooGal():void {
	...
	if(player.biggestLactation() >= 2) outputText("  The mouths sucking at your " + allBreastsDescript() + " ripple against your flesh until milk sprays from your mammaries, her body suckling the entirety of your lactate-heavy chest, cleavage to teat.", false);
...
function femaleLoseToGooGal():void {
	outputText("", true);
	outputText("You sway, finding it difficult to maintain your balance. When you fall, your " + buttDescript() + " splashes wetly in the enveloping folds of the goo-girl's eager slime. You weakly hold your hand up to keep her back, but the wide-eyed victor disregards the gesture with playful disdain. She slithers against you, moist muck slurping at your flesh with a hungry heat. Her semi-solid hands take yours, pulling your arms apart to bare your chest. She cranes her dripping head around your shoulders, skillfully removing your " + player.armorName + " with only her mouth. Your " + chestDesc() + " heaves as the irrepressible heat of the slime sears your " + nippleDescript(0) + " until beads of sweat well up on your " + player.skin() + " and trickle down your curves, leaving a wet sheen over your body.\n\n", false);
...
function victoryHermSex():void {
	...
	cuntChange(player.cockArea(0),true,false,true);
