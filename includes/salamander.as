...
function encounterAJerkInThePlains():void {
	spriteSelect(68);
...
function helAttack():void {
	...
	damage = int((monster.str + monster.weaponAttack) - rand(player.tou/2) - player.armorDef/2);
	if(damage > 0 && monster.short != "anemone") damage = takeDamage(damage);
	//No damage
	if(damage <= 0) {
		damage = 0;
		//Due to toughness or amor...
		if(rand(player.armorDef + player.tou) < player.armorDef) outputText("You absorb and deflect every " + monster.weaponVerb + " with your " + player.armorName + ".", false);
...
function helAttack2():void {
	...
	//Determine if dodged!
	if(player.spe - monster.spe > 0 && int(Math.random()*(((player.spe-monster.spe)/4)+80)) > 83) {
	...
	if(player.hasPerk("Evade") >= 0 && rand(100) < 5) {
		outputText("Using your skills at evading attacks, you anticipate and sidestep " + monster.a + monster.short + "'s tail-swipe.\n", false);
		return;
	}
	//("Misdirection"
	if(player.hasPerk("Misdirection") >= 0 && rand(100) < 5 && player.armorName == "red, high-society bodysuit") {
		outputText("Using Raphael's teachings, you anticipate and sidestep " + monster.a + monster.short + "' tail-swipe.\n", false);
		return;
	}
	//Determine if cat'ed
	...
	//Determine damage - str modified by enemy toughness!
	damage = int((monster.str) - rand(player.tou) - player.armorDef);
	if(damage > 0 && monster.short != "anemone") damage = takeDamage(damage);
	//No damage
	if(damage <= 0) {
		damage = 0;
...
function helCleavage():void {
	//FAIL
	if((player.hasPerk("Flexibility") >= 0 && rand(100) < 6) || (player.hasPerk("Evade") >= 0 && rand(100) < 10) || (player.spe - monster.spe > 0 && int(Math.random()*(((player.spe-monster.spe)/4)+80)) > 80)) {
	...
	//Attack 3 – Lust – Cleavage (Failure)
		...
		stats(0,0,0,0,0,0,lust,0);
		//Apply resistance
		lust *= lustPercent()/100;
		//Clean up
...
function loseToSalamander():void {
	spriteSelect(68);
	outputText("", true);
	//(If HP loss) 
...
function helDefeatedCorrupt():void {
	...
	var ass:Number = 0;
	if(player.hasCock() && player.cockThatFits(85) >= 0 && player.lust >= 33) ass = 3289;
	if(player.hasCock()) {
...
function helFuckMenu():void {
	spriteSelect(68);
	//Leave appropriate way
	var leave:Number = 3276;
	...
	var tailFuck:int = 3283;
	//ALTERNATE BODY BUTTONS
	var bodyText:String = "";
	var bodyButt:Number = 0;
	var bodyText2:String = "";
	var bodyButt2:Number = 0;
...
function beatUpHelAndStealHerWalletFromHerVagina():void {
	spriteSelect(68);
	outputText("", true);
...
function fuckHelsAss():void {
	spriteSelect(68);
	outputText("", true);
	var x:Number = player.cockThatFits(heliaAnalCapacity());
...
function helTailWanksYourDickBecauseSheLovesYouDesuDesuHoraHora():void {
	spriteSelect(68);
	outputText("", true);
	var x:Number = player.biggestCockIndex();
...
function salamanderXIsabellaPlainsIntro():void {
	spriteSelect(68);
	outputText("", true);
...
function followrIzzyxSallyThreesomePretext():void {
	spriteSelect(68);
...
function isabellaXHelThreeSomePlainsStart():void {
	spriteSelect(68);
	outputText("", true);
...
function isabellaXHelThreeSomeCampStart():void {
	spriteSelect(68);
	outputText("\n<b>Something odd happens during the night...</b>\n", false);
...
function leaveIsabellaSallyBehind():void {
	...
	if(this.model.time.hours < 6) doNext(1);
	else doNext(13);
}
...
function nomOnIzzyTitWithSallyMancer():void {
	...
	else if(player.hasVagina()) vag = 3314;
...
function heliaPlusFoxyFluffs():void {
	spriteSelect(68);
	outputText("", true);
...
function foxyFluffsFoursomeAsMale():void {
	spriteSelect(68);
	outputText("", true);
	var x:Number = player.cockThatFits(85);
	if(x < 0) x = player.smallestCockIndex();
