...
function xmasBitchEncounter():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00034] > 0) outputText("nother");
	outputText(" present, straight from the big man himself!</i>\"\n\n", false);
	
	if(flags[UNKNOWN_FLAG_NUMBER_00034] > 0) {
	...
	if(player.cor >= 90 || monk >= 5 || player.hasStatusAffect("Exgartuan") >= 0 || amilyCorrupt() || flags[UNKNOWN_FLAG_NUMBER_00283] > 0 || flags[UNKNOWN_FLAG_NUMBER_00282] > 0 || flags[NIAMH_STATUS] > 0) {
		...
		if(flags[UNKNOWN_FLAG_NUMBER_00034] > 0) outputText("Here we go again...\n\n");
...
function declineXmasPresent():void {
	...
	flags[UNKNOWN_FLAG_NUMBER_00034] = date.fullYear;
	doNext(1);
}
//[Open Present]
...
function dickXmasElfGo():void {
	...
	if(player.cockArea(0) < 80) {
		...
		outputText("Your effort at holding out totally fails – you start counting in time with each thrust, tying the numbers into the feel of the velvet cock-sleeve as it massages your " + cockDescript(0) + ".   The elf smiles condescendingly and taunts, \"<i>I can see you've already lost the ability to resist the pleasure.  Just give in, and enjoy the night.  I'll be riding you till morning.</i>\"\n\n", false);
...
function vagFuckXmasElf():void {
	...
	if(player.vaginas[0].vaginalWetness == 5) outputText("gushing fluids", false);
	else if(player.vaginas[0].vaginalWetness >= 3) outputText("dripping fluids", false);
