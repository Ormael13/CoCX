...
function trapLevel(adjustment:Number = 0):Number {
	...
	if(adjustment != 0) {
		monster.addStatusValue("level",1,adjustment);
		//Keep in bounds ya lummox
		if(monster.statusAffectv1("level") < 1) monster.changeStatusValue("level",1,1);
		if(monster.statusAffectv1("level") > 4) monster.changeStatusValue("level",1,4);
	}
	return monster.statusAffectv1("level");
}
...
function encounterASandTarp():void {
	...
	if(flags[TIMES_ENCOUNTERED_SAND_TRAPS] == 1) {
		...
		simpleChoices("Save",3822,"Don't Save",3823,"",0,"",0,"",0);
	}
	else {
		//Standard encounter: 
...
function nestleQuikSandAttack():void {
	...
	//Quicksand attack success: (Speed and Strength loss, ability to fly free lost) 
		...
		trapLevel(-1);
		if(monster.hasStatusAffect("Climbed") < 0) monster.createStatusAffect("Climbed",0,0,0,0);
	}
}

//Wait: 
function sandTrapWait():void {
	clearOutput();
	if(monster.hasStatusAffect("Climbed") < 0) monster.createStatusAffect("Climbed",0,0,0,0);
	...
	if(trapLevel() == 4) {
		outputText("\n\nYou eye the ground above you.  The edge of the pit is too sheer, the ground too unstable... although it looks like you can fight against the current's carrying you further down, it seems impossible to win through to freedom with the sand under the monster's spell.");
	}
	else {
...
function pcBeatsATrap():void {
	...
	var nagaThreeSome:int = 0;
	var putYourDickInIt:int = 0;
	var rideDatSantTrap:int = 0;
	var useSandTarpsHand:int = 0;
	...
	//\"<i>Ride</i>\" (Z)
	if(player.hasVagina()) rideDatSantTrap = 3827;
	//\"<i>Hands</i>\" (Z)
	if(player.gender > 0) useSandTarpsHand = 3828;
...
function genderlessLoss():void {
	...
	else outputText("Trap doesn't have fertilised eggs:  \"<i>Not a worker OR a drone! Wwwell, that isz ok.  Hwwe can still have fun, can't we?</i>\"");
...
function useSandTarpsHands():void {
	...
	//Herm: 
		...
		if(player.cockTotal() == 1) {
			...
			outputText("\n\nThe warm, oily double sensation is unbearably good and although you try to hold on to savour it as long as possible, it isn't long before you reach a toe-curling peak and, with a harsh moan, spurt out line after line of jizz whilst gushing femcum coat its hands and your thighs.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing and frigging you as you ride your glorious orgasms out.");
		}
		//Two cocks: 
		else if(player.cockTotal() == 2) {
...
function stickWangInSandgina():void {
	...
	outputText("\n\n\"<i>Thank you for your seed, lion,</i>\" its sleepy words reach you as you pull yourself clear of the hollow and step out towards camp.  \"<i>Perhaps next time we meet I will return the favour, hmm?</i>\"  In your wearily tranquil fug you can't tell if it intends this as an invitation or a threat.");
	stats(0,0,0,0,0,-1,-100,0);
	eventParser(5007);
}
