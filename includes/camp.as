...
function camp():void {
	...
	if(nieveHoliday() && this.model.time.hours == 6) {
	...
	if(flags[HEL_PREGNANCY_INCUBATION] >= 1 && followerHel()) {
		...
		if(flags[HEL_PREGNANCY_INCUBATION] == 1 && (this.model.time.hours == 6 || this.model.time.hours == 7)) {
			hideMenus();
			heliaBirthtime();
			return;
		}
	...
	if(this.model.time.hours >= 10 && this.model.time.hours <= 18 && (this.model.time.days % 20 == 0 || this.model.time.hours == 12) && flags[HELSPAWN_DADDY] == 2 && helspawnFollower()) {
		hideMenus();
		maiVisitsHerKids();
		return;
	}
	if(this.model.time.hours == 6 && flags[HELSPAWN_DADDY] == 1 && this.model.time.days % 30 == 0 && flags[SPIDER_BRO_GIFT] == 0 && helspawnFollower())
	{
		hideMenus();
		spiderBrosGift();
		return;
	}
	if(this.model.time.hours >= 10 && this.model.time.hours <= 18 && (this.model.time.days % 15 == 0 || this.model.time.hours == 12) && helspawnFollower() && flags[HAKON_AND_KIRI_VISIT] == 0) {
		hideMenus();
	...
	if(flags[HELSPAWN_AGE] == 2 && flags[HELSPAWN_WEAPON] == 0 && flags[HELSPAWN_GROWUP_COUNTER] == 3 && this.model.time.hours >= 10 && this.model.time.hours <= 18) {
		hideMenus();
		helSpawnChoosesAFightingStyle();
		return;
	}
	if(flags[HELSPAWN_AGE] == 2 && (this.model.time.hours == 6 || this.model.time.hours == 7) && flags[HELSPAWN_GROWUP_COUNTER] == 7 && flags[HELSPAWN_FUCK_INTERRUPTUS] == 1) {
		helspawnAllGrownUp();
		return;
	}
	...
	if(!nieveHoliday() && this.model.time.hours == 6 && flags[NIEVE_STAGE] > 0) {
		nieveIsOver();
		return;
	}
	//Amily followup!
	...
	if(player.pregnancyIncubation <= 280 && player.pregnancyIncubation >= 0 && player.pregnancyType == 20 &&
	   	flags[COTTON_KNOCKED_UP_PC_AND_TALK_HAPPENED] == 0 && (this.model.time.hours == 6 || this.model.time.hours == 7)) {
	...
	*/
	...
	if(isabellaFollower()) {
		...
		if(this.model.time.days >= 20) outputText("  You've even managed to carve some artwork into the rocks around the camp's perimeter.", false);
	}
	//Live in-ness
	else {
		if(this.model.time.days < 10) outputText("Your campsite is fairly simple at the moment.  Your tent and bedroll are set in front of the rocks that lead to the portal.  You have a small fire pit as well.", false);
		else if(this.model.time.days < 20) outputText("Your campsite is starting to get a very 'lived-in' look.  The fire-pit is well defined with some rocks you've arranged around it, and your bedroll and tent have been set up in the area most sheltered by rocks.", false);
	...
	if(player.hasStatusAffect("Camp Marble") >= 0) {
		...
		if(flags[MARBLE_KIDS] >= 1 && (this.model.time.hours == 19 || this.model.time.hours == 20)) {
		...
		else if(this.model.time.hours == 6 || this.model.time.hours == 7) outputText("Marble is off in an open area to the side of your camp right now.  She is practicing with her large hammer, going through her daily training.");
		//after nightfall, scene always displays at this time unless PC is wormed
		else if(this.model.time.hours == 21 && player.hasStatusAffect("infested") < 0) {
...
function stash(exists:Boolean = true):Boolean {
	...
	if(flags[ANEMONE_KID] > 0) {
		//(morning)
		if(this.model.time.hours < 6) outputText("Kid A is sleeping in her barrel right now.");
		else if(this.model.time.hours <= 10) outputText("Kid A stands next to her barrel, refilling it from one of your waterskins.  A second full skin is slung over her shoulder.  She gives you a grin.\n\n");
		else if(flags[KID_SITTER] > 1) outputText("Kid A is absent from her barrel right now, dragooned into babysitting again.\n\n");
		//(midday)
		else if(this.model.time.hours < 16) outputText("Kid A is deep in her barrel with the lid on top, hiding from the midday sun.\n\n");
		//(night hours)
		else if(this.model.time.hours < 22) outputText("Kid A is peeking out of her barrel.  Whenever you make eye contact she breaks into a smile; otherwise she just stares off into the distance, relaxing.\n\n");
		...
		barrel = 3546;
		if(this.model.time.hours < 6) barrel = 0;
	}
...
function campLoversMenu():void {
	...
	if(followerHel()) {
		if(flags[HEL_FOLLOWER_LEVEL] == 2) {
			...
			//(6-7) 
			if(this.model.time.hours <= 7) outputText("Hel is currently sitting at the edge of camp, surrounded by her scraps of armor, sword, and a few half-empty bottles of vodka.  By the way she's grunting and growling, it looks like she's getting ready to flip her shit and go running off into the plains in her berserker state.\n\n");
			//(8a-5p) 
			else if(this.model.time.hours <= 17) outputText("Hel's out of camp at the moment, adventuring on the plains.  You're sure she'd be on hand in moments if you needed her, though.\n\n");
			//5-7) 
			else if(this.model.time.hours <= 19) outputText("Hel's out visiting her family in Tel'Adre right now, though you're sure she's only moments away if you need her.\n\n");
			//(7+)
	...
	if(followerKiha()) {
		//(6-7) 
		if(this.model.time.hours < 7) outputText("Kiha is sitting near the fire, her axe laying across her knees as she polishes it.[pg]");
		else if(this.model.time.hours < 19) outputText("Kiha's out right now, likely patrolling for demons to exterminate.  You're sure a loud call could get her attention.\n\n");
	...
	if(isabellaFollower()) {
		isabellaButt = 3243;
		if(this.model.time.hours >= 21 || this.model.time.hours <= 5) outputText("Isabella is sound asleep in her bunk and quietly snoring.", false);
		else if(this.model.time.hours == 6) outputText("Isabella is busy eating some kind of grain-based snack for breakfast.  The curly-haired cow-girl gives you a smile when she sees you look her way.", false);
		else if(this.model.time.hours == 7) outputText("Isabella, the red-headed cow-girl, is busy with a needle and thread, fixing up some of her clothes.", false);
		else if(this.model.time.hours == 8) outputText("Isabella is busy cleaning up the camp, but when she notices you looking her way, she stretches up and arches her back, pressing eight bullet-hard nipples into the sheer silk top she prefers to wear.", false);
		else if(this.model.time.hours == 9) outputText("Isabella is out near the fringes of your campsite.  She has her massive shield in one hand and appears to be keeping a sharp eye out for intruders or demons.  When she sees you looking her way, she gives you a wave.", false);
		else if(this.model.time.hours == 10) outputText("The cow-girl warrioress, Isabella, is sitting down on a chair and counting out gems from a strange pouch.  She must have defeated someone or something recently.", false);
		else if(this.model.time.hours == 11) outputText("Isabella is sipping from a bottle labelled 'Lactaid' in a shaded corner.  When she sees you looking she blushes, though dark spots appear on her top and in her skirt's middle.", false);
		else if(this.model.time.hours == 12) outputText("Isabella is cooking a slab of meat over the fire.  From the smell that's wafting this way, you think it's beef.  Idly, you wonder if she realizes just how much like her chosen food animal she has become.", false);
		...
		else if(this.model.time.hours == 14) outputText("Isabella is working a grindstone and sharpening her tools.  She even hones the bottom edge of her shield into a razor-sharp cutting edge.  The cow-girl is sweating heavily, but it only makes the diaphanous silk of her top cling more alluringly to her weighty chest.", false);
		else if(this.model.time.hours == 15) outputText("The warrior-woman, Isabella is busy constructing dummies of wood and straw, then destroying them with vicious blows from her shield.  Most of the time she finishes by decapitating them with the sharp, bottom edge of her weapon.  She flashes a smile your way when she sees you.", false);
		else if(this.model.time.hours == 16) outputText("Isabella is sitting down with a knife, the blade flashing in the sun as wood shavings fall to the ground.  Her hands move with mechanical, practiced rhythm as she carves a few hunks of shapeless old wood into tools or art.", false);
		else if(this.model.time.hours == 17) outputText("Isabella is sitting against one of the large rocks near the outskirts of your camp, staring across the wasteland while idly munching on what you assume to be a leg of lamb.  She seems lost in thought, though that doesn't stop her from throwing a wink and a goofy food-filled grin toward you.", false);
		else if(this.model.time.hours == 18) outputText("The dark-skinned cow-girl, Isabella, is sprawled out on a carpet and stretching.  She seems surprisingly flexible for someone with hooves and oddly-jointed lower legs.", false);
		...
		else if(this.model.time.hours == 20) outputText("Your favorite chocolate-colored cowgirl, Isabella, is moving about, gathering all of her scattered belongings and replacing them in her personal chest.  She yawns more than once, indicating her readiness to hit the hay, but her occasional glance your way lets you know she wouldn't mind some company before bed.", false);
...
function doSleep(clrScreen:Boolean = true):void {
	...
	campQ = true;
	if(timeQ == 0) {
		if(this.model.time.hours == 21) timeQ = 9;
		if(this.model.time.hours == 22) timeQ = 8;
		if(this.model.time.hours >= 23) timeQ = 7;
		if(this.model.time.hours == 0) timeQ = 6;
		if(this.model.time.hours == 1) timeQ = 5;
		if(this.model.time.hours == 2) timeQ = 4;
		if(this.model.time.hours == 3) timeQ = 3;
...
function sleepWrapper():void {
	if(this.model.time.hours == 16) timeQ = 14;
	if(this.model.time.hours == 17) timeQ = 13;
	if(this.model.time.hours == 18) timeQ = 12;
	if(this.model.time.hours == 19) timeQ = 11;
	if(this.model.time.hours == 20) timeQ = 10;
	if(this.model.time.hours == 21) timeQ = 9;
	if(this.model.time.hours == 22) timeQ = 8;
	if(this.model.time.hours >= 23) timeQ = 7;
	if(this.model.time.hours == 0) timeQ = 6;
