...
function hellhoundFire():void {
	//Blind dodge change
	if(monster.hasStatusAffect("Blind") >= 0) {
		outputText(monster.capitalA + monster.short + " completely misses you with a wave of dark fire! Thank the gods it's blind!", false);
		combatRoundOver();
		return;
	}
	...
	if(player.hasPerk("Evade") >= 0 && player.spe >= 35 && rand(3) != 0) {
		...
		outputText("Using Raphael's teachings and the movement afforded by your bodysuit, you anticipate and sidestep " + monster.a + monster.short + "'s fire.\n", false);
	}
...
function HellHoundMasterEncounter():void {
	outputText("", true);
	//Initial encounter (A)
	...
	else if(flags[UNKNOWN_FLAG_NUMBER_00141] == 1) {
		//Second Encounter (D)
...
function acceptCrazyPresencesII():void {
	...
	flags[UNKNOWN_FLAG_NUMBER_00141] = 1;
	doNext(13);
}
...
function giveLethicitePiece():void {
	...
	flags[UNKNOWN_FLAG_NUMBER_00141]++;
	doNext(13);
}
...
function giveALLTHELETHICITES():void {
	...
	if(player.dogCocks() >= 2)
	{
		...
		outputText("After what feels like hours of painful pleasure without release, you're smacked on your rear one last time, and it feels like Cimer bursts through your body completely.  This finally frees the floodgates, and you feel like you're releasing everything in your body in the most powerful orgasm you've ever felt.  You pant for a moment and look down in front of you.  You can see your " + allBreastsDescript() + " and your pregnant belly in front of you, you also see a package of demonic dicks sticking out in front of you, with a quad of hot balls resting underneath.  In front of you is a large pool of white steaming fluid, with a pink crystal resting in the middle.\n\n", false);
...
function hellHoundPropahRape():void {
	...
	//--- ELSE (CORRUPTION >= 20) ---
		...
		buttChange(monster.cockArea(1),true,false,true);
