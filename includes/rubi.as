...
public function rubiSprite():void {
	if(flags[RUBI_HORNTYPE] > 0) spriteSelect(102);
	spriteSelect(101);
}
...
//Bakery Intro scenes
public function rubiIntros():Number {
	...
	else if(rubiBimbo() && flags[RUBI_DEBIMBO] == 0) outputText("Rubi is here, moving from table to table, not quite as efficiently as [rubi ey] used to, but with a lot more exuberance.  [rubi Eir] long red tail swishes around, seemingly drawn towards the crotch of patrons and co-workers alike.");
...
public function approachRubiScenes():void {
	...
	//(Approach repeat, relationship 20+.  At this stage, randomly select an outfit from what the PC has given him, plus his waitress outfit.  Variants included for Bimbo /Incubus Rubi.)
		...
		if(select == 1) {
			...
			if(flags[RUBI_COCK_SIZE] >= 9) outputText("  You notice Rubi holds [rubi emself] a little awkwardly, and it's only when [rubi ey] sits down that you spy the slight bulge in [rubi eir] skirt.");
		}
		//(Suit Clothes (Normal))
		else if(select == 2) {
			//(Suitclothes)
			...
			if(flags[RUBI_BREAST_SIZE] >= 4) outputText(", though with [rubi eir] large bosom, [rubi ey] can't quite button up all the way, giving the appearance of a sexy secretary");
			outputText(".");
		}
		else if(select == 3) {
			...
			if(flags[RUBI_BREAST_SIZE] >= 4) outputText(" impressive bosom, and sort of makes [rubi em] look like the kind of rubber doll one might buy for those lonely nights.");
			...
			//(D breasts or higher)
			if(flags[RUBI_BREAST_SIZE] >= 4) outputText("  [rubi Eir] breasts strain the fabric of the tube top, any wrong move threatening to have them burst forth.");
			//(dick 9" or higher)
			if(flags[RUBI_COCK_SIZE] >= 4) outputText("  [rubi Eir] rather large bulge can't be concealed, however, and you sometimes catch Rubi holding [rubi eir] notepad down across [rubi eir] crotch when [rubi ey] moves from table to table.");
			...
			if(flags[RUBI_COCK_SIZE] >= 9) outputText("  However, with an outfit this tight, it's almost impossible for [rubi em] to hide [rubi eir] impressive package.  Rubi's cock pokes straight upwards, creating a bulge from [rubi eir] crotch up towards [rubi eir] belly button.");
			else if(flags[RUBI_COCK_SIZE] > 0) outputText("  However, with an outfit this tight you can see a slight outline where [rubi eir] small package is tucked away.  You wonder how many other patrons have noticed the same thing.");
		}
		else if(select == 7) {
			//(Long Dress (Bimbo))
			...
			if(flags[RUBI_BREAST_SIZE] <= 1) outputText("  The dress must have come with a padded bra, because you're surprised to see Rubi with  breasts.  Though small, they help to accentuate [rubi eir] feminine figure.");
		}
		else if(select == 8) {
			...
			if(flags[RUBI_BREAST_SIZE] >= 4) outputText("  With [rubi eir] large rack, [rubi ey] looks much like a busty piratess.  From the way [rubi eir] breasts strain at the top, you're certain that one wrong breath could send a button flying across the room.");
			...
			//Cock variants: 
			//(9" or higher)
			if(flags[RUBI_COCK_SIZE] >= 9) outputText("  [rubi Eir] generous cock is on display, much to [rubi eir] chagrin, as a bulge running down the length of one pant leg.");
			...
			//(D breasts or higher)
			if(flags[RUBI_BREAST_SIZE] >= 4) outputText("  Rubi's large breasts strain the fabric, drawing the dress further up [rubi eir] body and showing off more of [rubi eir] legs and ass.");
		}
		//(Slutty Swimwear (Normal))
		else if(select == 10) {
			//(Slutty Swimwear)
			...
			else if(flags[RUBI_BREAST_SIZE] <= 1) outputText("  The top only emphasizes Rubi's relatively flat chest, though with the rest of [rubi eir] figure, [rubi ey] still looks extremely feminine.");
		}
		//(Minidress Outfit (Bimbo))
			...
			if(flags[RUBI_COCK_SIZE] > 0) outputText("  [rubi Eir] skirt rides so high you catch glimpses of [rubi eir] unsurprisingly unpantied crotch, and the enticing package held within.  You wonder how many people have gotten a surprise with their drinks today!");
		}
		//(Bondage Straps)
		else if(select == 12) {
		...
		if(select == 15) {
			...
			if(flags[RUBI_BREAST_SIZE] >= 4) outputText("  In an odd sort of way, [rubi ey] looks like an adventurer who met the wrong end of a witch's curse.");
		}
		...
		//(Long Dress)
			...
			if(rubiBimbo() && flags[RUBI_DEBIMBO] == 0) outputText("And then [rubi ey] begins to shake [rubi eir] ass, making spanking motions and effectively ruining the air of sophistication.");
		}
		//(Nurse Uniform)
		else if(select == 9) {
			outputText("\n\nAs [rubi ey] bends over, you get a good look up [rubi eir] skirt, ");
...
public function fuckRubi():void {
	...
	if(flags[RUBI_ORGASM_DENIAL] > 0 && !rubiBimbo()) outputText("  You give Rubi's ass a hard smack and remind [rubi em] that no matter what happens, [rubi ey] still can't cum yet.  [rubi Eir] eyes flutter open and [rubi ey] actually whimpers a little, but can't form the words to object.");
	//(If Orgasm Denial is NOT activated; male/herm Rubi) 
	else if(flags[RUBI_COCK_SIZE] > 0) outputText("  Rubi’s body shivers and trembles, and almost collapses.  [rubi Ey] grasps at the blankets with wild abandon as ropes of pearly white cum erupt from [rubi eir] cock, splattering against [rubi eir] chest and the blankets.");
	//(Female Rubi)
...
public function releaseRubi():void {
	...
	if(flags[RUBI_COCK_SIZE] > 0) outputText("[rubi eir] [rubi cock] straining at the seams, trying to break free.");
	...
	//(if Blueballs 4-9) 
	else if(flags[RUBI_BLU_BALLS] <= 9) outputText("  Not to mention [rubi eir] sizeable balls, which seem to have swelled up to twice their normal size, thanks to your relentless teasing.  They even have a slight bluish tinge to them, although it's not immediately visible.");
	...
	//(Blueballs 1-3) 
	if(flags[RUBI_BLU_BALLS] <= 3) outputText("  It's quickly followed by another, less forceful jet of pearly white cum, which lands on [rubi eir] face, and several even smaller jets which splatter the area around [rubi em].  [rubi Eir] cock twitches and spasms, then slowly softens.");
	//(Blueballs 4-5) 
	else if(flags[RUBI_BLU_BALLS] <= 5) outputText("  It's quickly followed by another, less forceful jet of pearly white cum, then another, and another, each landing closer to [rubi eir] face.  Finally, it gives a few more token jerks, spurting out tiny globules of cum onto the surrounding area.  [rubi Eir] cock twitches and spasms, squirting nothing but air, as if [rubi eir] body is still craving release.");
	//(Blueballs 6-7) 
...
public function rubiAppearance():void {
	...
	if(flags[RUBI_SKIN] != 2) outputText(" and completely hairless, except for [rubi eir]");
	...
	if(flags[RUBI_WHISKERS] > 0) outputText(", with a cute set of whiskers sprouting from [rubi eir] cheeks");
	...
	if(flags[RUBI_HORNTYPE] > 0) outputText("Two tiny horns extend upwards from [rubi eir] hairline, solid black in color.  ");
	outputText("A pair of ");
	if(flags[RUBI_EAR_TYPE] == 0) outputText("cute, normal ears protrude from the sides of [rubi eir] head, each one pierced with a small dangling earring");
	else if(flags[RUBI_EAR_TYPE] == 1) outputText("cute, cat-like ears pop up from the top of [rubi eir] head, each one seemingly able to move independently");
	else if(flags[RUBI_EAR_TYPE] == 2) outputText("feathered ears protrude from the sides of [rubi eir] head, each one fluttering cutely from time to time, and each pierced with a small dangling earring");
	outputText(".");
	...
	if(flags[RUBI_COCK_SIZE] > 0) {
		...
		if(flags[RUBI_BALLS_TYPE] == 1) outputText(", which cling tight to [rubi eir] body in a cute spherical pouch");
	}
	...
	if(flags[RUBI_FEET] == 1) outputText("Finally, Rubi stands on a long black spike that makes [rubi em] walk as though [rubi ey] were wearing high heels.");
...
public function playDressUp2():void {
	clearOutput();
	rubiSprite();
	outputText("You look over Rubi's vast collection of clothes.  What outfit would you like Rubi to put on?\n\n");
	menu();
...
public function dressUpRouter(arg:String):void {
	clearOutput();
	rubiSprite();
...
public function putRubiInAWaitressUniform():void {
	...
	if(flags[RUBI_BREAST_SIZE] < 1) outputText(", though it's obvious [rubi ey] doesn't need it with [rubi eir] flat-chest");
	...
	if(flags[RUBI_BREAST_SIZE] < 1) outputText(", showing off some of [rubi eir] flat chest");
	...
	if(flags[RUBI_FEET] == 1) outputText("what appears to be flats, with a hole in the heel for [rubi eir] demonic heels");
...
public function putOnADashingOutfitYouWhore():void {
	...
	if(flags[RUBI_COCK_SIZE] > 0) outputText("tucking [rubi eir] cock firmly into the triangular strip of cloth covering [rubi eir] crotch, while ");
	...
	if(flags[RUBI_COCK_SIZE] > 0) outputText(", not to mention the sizeable package in [rubi eir] crotch");
	outputText(".");
...
public function goOnAnAnalAdventureRubiNotReallyJustAnAdventurersOutfit():void {
	...
	if(flags[RUBI_EAR_TYPE] == 1) outputText(", covering [rubi eir] feline ears");
...
public function giveRubiALongDressToHideHerHideousBody():void {
	...
	if(flags[RUBI_BREAST_SIZE] < 1) outputText(", despite the fact that [rubi ey] clearly doesn't need one");
...
public function rubiPutsOnNursesClothesSoSheCanCheckYourTemperature():void {
	...
	if(flags[RUBI_FEET] > 0) outputText(" with the heels removed and a hole for [rubi eir] demonic heels");
	outputText(".");
...
public function rubiHasARubberFetish():void {
	...
	//(If Not Bimbo)
	if(!rubiBimbo() || flags[RUBI_DEBIMBO] > 0) outputText("[rubi Ey] first takes a pair of transparent rubber panties and slides them up [rubi eir] legs until they lovingly cling around [rubi eir] ass.  ");
	...
	if(flags[RUBI_FEET] == 1) outputText(", the actual heel cut off to make room for [rubi eir] demonic heel,");
...
public function putOnASuitYouSlut():void {
	...
	if(flags[RUBI_FEET] == 1) outputText(", with the actual heel cut off to make room for [rubi eir] demonic heel,");
...
public function putOnSluttySwimwearYouSkank():void {
	...
	if(flags[RUBI_FEET] > 0) outputText(", with a hole in the sole to make room for [rubi eir] demonic heels");
	outputText(".");
...
public function putOnADressYouBimbo():void {
	...
	if(flags[RUBI_FEET] > 0) outputText(", with the heels removed to make room for [rubi eir] own demonic heels");
	outputText(".");
...
public function putOnMyBodysuitYouWhore():void {
	...
	if(flags[RUBI_COCK_SIZE] > 0) outputText("[rubi eir] package is neatly tucked away");
...
public function whyTheFuckIsRubiWearingAnInquisitorsCorset():void {
	...
	if(flags[RUBI_FEET] == 1) outputText(", with the heels removed to make room for [rubi eir] own demonic heels");
	outputText(".");
	outputText("\n\nNow that Rubi is dressed up, what do you want to do?");
	//[Date (go to bar date)] [Back]
	menu();
...
public function putOnAWaitressUniformYouWhore():void {
	...
	if(!rubiBimbo() || flags[RUBI_DEBIMBO] > 0) {
		...
		if(flags[RUBI_COCK_SIZE] > 0) outputText("Oddly enough, these panties only seem to cover Rubi's balls, leaving [rubi eir] cock right out in the open.  ");
	}
	...
	if(flags[RUBI_COCK_SIZE] > 0) outputText("  To top it all off, Rubi takes a pink lacy cocksock and slips it down [rubi eir] [rubi cock], fastening it into place with a pink ribbon that [rubi ey] ties into a bow around the base of [rubi eir] cock.");
...
public function exhibitionistDate():void {
	rubiSprite();
...
public function anallyTrainYourBitchBoySlutHowToBeAnalForDCL():void {
	...
	if(flags[RUBI_TIMES_ANALLY_TRAINED] == 1) {
		...
		if(flags[RUBI_COCK_SIZE] > 0) outputText(", [rubi eir] dick wobbling enticingly");
		outputText(".");
		...
		if(flags[RUBI_COCK_SIZE] > 0) outputText(rubiCock() + " is standing at full attention, bobbing up and down as Rubi fucks [rubi emself] on the dildo");
		if(flags[RUBI_COCK_SIZE] > 0 && flags[RUBI_NO_CUNT] == 0) outputText(", and [rubi eir] ");
	...
	//Rubi's capacity won't go larger than 100.
		...
		if(flags[RUBI_COCK_SIZE] > 0) outputText("  Even, no, <i>especially</i> [rubi eir] cock twitches and spasms jerking upwards and splattering globules of sticky white goo against the inside of Rubi's babydoll before dripping down onto the floor between [rubi eir] legs, creating a rapidly growing puddle.");
...
public function getFuckedByRubi():void {
	...
	if(flags[RUBI_COCK_TYPE] == CockTypesEnum.HUMAN) outputText("pink, mushroom-like tip");
	else if(flags[RUBI_COCK_TYPE] == CockTypesEnum.DEMON) outputText("purple, nodule-laden tip");
	else if(flags[RUBI_COCK_TYPE] == CockTypesEnum.HORSE) outputText("blunted, musky tip");
	else if(flags[RUBI_COCK_TYPE] == CockTypesEnum.TENTACLE) outputText("tentacle-ringed crown");
...
public function rubiHotdogging():void {
	...
	if(flags[RUBI_COCK_SIZE] > 0) outputText("taking hold of [rubi eir] cock");
...
public function popButtjobs():void {
	...
	outputText("\n\nRubi's ");
	if(flags[RUBI_NO_CUNT] == 0) outputText("cunt clamps down on your thumb and gushes, coating your hand in [rubi eir] slick juices");
	...
	//(If Rubi has Penis)
	if(flags[RUBI_COCK_SIZE] > 0) outputText("  Beneath [rubi em], [rubi eir] cock jerks and twitches, loosing a spray of pearly white cum which splatters against the couch below");
	if(flags[RUBI_BLU_BALLS] < 4) {}
	//((blueballs 4-5)
	else if(flags[RUBI_BLU_BALLS] <= 5) outputText(" which is quickly followed by another, more powerful stream of cum, which splashes against [rubi eir] stomach, coating [rubi eir] [rubi breasts] in semen");
	else if(flags[RUBI_BLU_BALLS] <= 7) outputText(" which is quickly followed by another, more powerful stream of cum, and a third, and a fourth.  Before you know it, Rubi's entire torso, including [rubi eir] [rubi breasts] are practically glazed with cum");
	else if(flags[RUBI_BLU_BALLS] <= 9) outputText(" which is followed up by several more, each seemingly more poweful than the one previous.  Rubi's body twitches and shudders with every jet, until [rubi eir] entire torso, and the couch, is covered in the musky jizz");
...
public function giveRubiATFItem(short:String):void {
	clearOutput();
	rubiSprite();
	...
	if(short == "SucMilk" || short == "P.S.Mlk") {
		...
		if(rand(3) == 0 && flags[RUBI_SKIN] != 1 && short == "SucMilk") {
			...
			if(flags[RUBI_SKIN] == 2) outputText("Meanwhile the silky soft fur that covered [rubi eir] body seems to be falling out as this red menace takes over.  ");
		...
		if(short == "IncubiD") outputText("You hand over the bottle of milky white fluids to Rubi.  [rubi Ey] takes a moment to read over the label before pulling out the cork and downing the whole bottle in one gulp.");
		...
		if(flags[RUBI_COCK_SIZE] > 0 && ((flags[RUBI_COCK_SIZE] < 12 || (flags[RUBI_COCK_TYPE] == CockTypesEnum.HORSE && flags[RUBI_COCK_SIZE] < 20)) ||
	...
	//Bimbo Liqueur (Modified for variant genders)
		...
		if(flags[RUBI_COCK_SIZE] > 0) {
			...
			if(flags[RUBI_NO_CUNT] == 0) outputText("[rubi eir] vagina");
	...
	//Grows cock. (As Incubus Draft above)
		...
		if(flags[RUBI_COCK_TYPE] != CockTypesEnum.HORSE) {
			...
			flags[RUBI_COCK_TYPE] = CockTypesEnum.HORSE;
	...
	//Trap Oil
		...
		if(flags[RUBI_BREAST_SIZE] > 0) outputText(", while [rubi eir] [rubi breasts] gently rock up and down with the motions of [rubi eir] breathing");
		outputText(".");
	...
	if(short == "PurPeac") {
		...
		//(Any changes? Yes)
		if (flags[RUBI_EAR_TYPE] != 2 || flags[RUBI_SKIN] != 3 || (flags[RUBI_COCK_SIZE] > 0 && flags[RUBI_COCK_TYPE] != CockTypesEnum.TENTACLE))
		{
			...
			//(Shark Skin)
				...
				else if(flags[RUBI_SKIN] ==1 ) outputText("which is barely noticeable on [rubi eir] already cherry red flesh");
			...
			flags[RUBI_COCK_TYPE] = CockTypesEnum.ANEMONE;
		}
		//(No)
		else {
...
//(Give Clothes)
public function giveRubiClothes(short:String = ""):void {
	clearOutput();
	rubiSprite();
