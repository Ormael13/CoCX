...
public function appearance(e:MouseEvent = null):void 
{
	...
	if(player.faceType == FACE_HUMAN || player.faceType == FACE_SHARK_TEETH || player.faceType == FACE_BUNNY || player.faceType == FACE_SPIDER_FANGS || player.faceType == FACE_FERRET_MASK || player.faceType == FACE_MANTICORE) 
	{
		...
		if(player.skinType == SKIN_TYPE_SCALES || player.skinType == SKIN_TYPE_CHITIN) 
			...
			if(player.skinType == SKIN_TYPE_CHITIN) outputText("" + player.chitinColor + " chitin.", false);
		if (player.skinType == SKIN_TYPE_STONE)
		{
			outputText("  Your face is human in shape and structure, with " + player.skinTone + " skin.", false);
		}
	...
	else if(player.faceType == FACE_RACCOON) 
	{
		...
		else if(player.skinType == SKIN_TYPE_SCALES || player.skinType == SKIN_TYPE_PARTIAL_SCALES) 
			outputText("  The presence of said scales gives your visage an eerie look, more reptile than mammal.");
		else if(player.skinType == SKIN_TYPE_CHITIN || player.skinType == SKIN_TYPE_PARTIAL_CHITIN)
	...
	else if(player.faceType == FACE_FOX) 
	{
		...
		if(player.skinType == SKIN_TYPE_PLAIN) 
			...
			outputText("  Oddly enough, there's no fur on your animalistic muzzle, just " + player.skinFurScales() + " covered with magical tattoo."); 
		else if(player.skinType == SKIN_TYPE_FUR) 
			outputText("  A coat of " + player.skinFurScales() + " decorates your muzzle.");
		else if(player.skinType == SKIN_TYPE_PARTIAL_SCALES || player.skinType == SKIN_TYPE_PARTIAL_FUR) 
			outputText("  Strangely, " + player.skinFurScales() + " adorn your animalistic visage.");
		else if(player.skinType == SKIN_TYPE_SCALES || player.skinType == SKIN_TYPE_CHITIN || player.skinType == SKIN_TYPE_PARTIAL_CHITIN) 
	...
	else if(player.faceType == FACE_BUCKTEETH) 
	{
		...
		if (player.skinType == SKIN_TYPE_PARTIAL_SCALES || player.skinType == SKIN_TYPE_PARTIAL_FUR) outputText(" covered with " + player.skinFurScales());
		outputText(".");
	}
	...
	if(player.faceType == FACE_WOLF) 
	{
		if(player.skinType == SKIN_TYPE_PLAIN || player.skinType == SKIN_TYPE_TATTOED || player.skinType == SKIN_TYPE_GOO) {
			...
			outputText("The odd visage is hairless and covered with " + player.skinFurScales() + "");
			if(player.skinType == SKIN_TYPE_TATTOED) outputText(" covered with magical tattoo");
			outputText(".", false);
		}
		if(player.skinType == SKIN_TYPE_FUR) {
	...
	if(player.faceType == FACE_COW_MINOTAUR) 
	{
		...
		if(player.skinType == SKIN_TYPE_PARTIAL_SCALES || player.skinType == SKIN_TYPE_PARTIAL_FUR || player.skinType == SKIN_TYPE_PARTIAL_FUR) 
	...
	if(player.faceType == FACE_LIZARD) 
	{
		...
		if (player.skinType == SKIN_TYPE_PARTIAL_SCALES || player.skinType == SKIN_TYPE_PARTIAL_FUR) {
			outputText("  You have a face resembling that of a lizard, and with your toothy maw, you have quite a fearsome visage.  The reptilian visage does look a little odd with just " + player.skinFurScales() + ".", false);
		}
		if(player.skinType == SKIN_TYPE_FUR) 
	...
	if (player.faceType == FACE_ECHIDNA)
	{
		...
		else if (player.skinType == SKIN_TYPE_SCALES || player.skinType == SKIN_TYPE_CHITIN || player.skinType == SKIN_TYPE_PARTIAL_SCALES || player.skinType == SKIN_TYPE_PARTIAL_FUR)
	...
	//BODY PG HERE
	outputText("\n\nYou have a humanoid shape with the usual body", false);
	if (player.skinType == SKIN_TYPE_PARTIAL_FUR || player.skinType == SKIN_TYPE_PARTIAL_SCALES) outputText(" partialy covered with " + player.skinFurScales() + "", false);
	if (player.skinType == SKIN_TYPE_FUR || player.skinType == SKIN_TYPE_SCALES || player.skinType == SKIN_TYPE_CHITIN) outputText(" covered with " + player.skinFurScales() + "", false);
	...
	else if(player.armType == ARM_TYPE_LIZARD)
	...
	else if(player.armType == ARM_TYPE_DRAGON)
		outputText("  Shining thick, leathery red scales covers your arms from the biceps down and your fingernails are now a short curved claws.", false);
	...
	if((player.buttPregnancyType == PregnancyStore.PREGNANCY_FROG_GIRL) || (player.buttPregnancyType == PregnancyStore.PREGNANCY_SATYR) || player.isPregnant()) 
	{
		...
		else if (player.buttPregnancyType == PregnancyStore.PREGNANCY_SATYR && player.buttPregnancyIncubation > player.pregnancyIncubation) 
		{
			...
			{ //Surely Benoit and Cotton deserve their place in this list
				if (player.pregnancyType == PregnancyStore.PREGNANCY_IZMA || player.pregnancyType == PregnancyStore.PREGNANCY_MOUSE || player.pregnancyType == PregnancyStore.PREGNANCY_AMILY || (player.pregnancyType == PregnancyStore.PREGNANCY_JOJO && (monk <= 0 || flags[kFLAGS.JOJO_BIMBO_STATE] >= 3)) || player.pregnancyType == PregnancyStore.PREGNANCY_EMBER || player.pregnancyType == PregnancyStore.PREGNANCY_BENOIT || player.pregnancyType == PregnancyStore.PREGNANCY_COTTON || player.pregnancyType == PregnancyStore.PREGNANCY_URTA || player.pregnancyType == PregnancyStore.PREGNANCY_BEHEMOTH) 
		...
		else 
		{
			...
			if (player.pregnancyIncubation <= 48) 
			{ //Surely Benoit and Cotton deserve their place in this list
				if(player.pregnancyType == PregnancyStore.PREGNANCY_IZMA || player.pregnancyType == PregnancyStore.PREGNANCY_MOUSE || player.pregnancyType == PregnancyStore.PREGNANCY_AMILY || (player.pregnancyType == PregnancyStore.PREGNANCY_JOJO && monk <= 0) || player.pregnancyType == PregnancyStore.PREGNANCY_EMBER || player.pregnancyType == PregnancyStore.PREGNANCY_BENOIT || player.pregnancyType == PregnancyStore.PREGNANCY_COTTON || player.pregnancyType == PregnancyStore.PREGNANCY_URTA || player.pregnancyType == PregnancyStore.PREGNANCY_MINERVA || player.pregnancyType == PregnancyStore.PREGNANCY_BEHEMOTH) 
					outputText("\n<b>Your belly protrudes unnaturally far forward, bulging with the spawn of one of this land's natives.</b>", false);
				else if (player.pregnancyType != PregnancyStore.PREGNANCY_MARBLE) 
					outputText("\n<b>Your belly protrudes unnaturally far forward, bulging with the unclean spawn of some monster or beast.</b>", false);
				else outputText("\n<b>Your belly protrudes unnaturally far forward, bulging outwards with Marble's precious child.</b>", false);
...
public function RacialScores():void {
	...
	if (player.dragonScore() >= 6) outputText("\n<font color=\"#0000a0\">Dragon: " + player.dragonScore() + " (+" + (10 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Str, +" + (20 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Tou, +" + (10 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Spe, +" + (20 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Int, +" + (100 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max HP)</font>", false);
	else if (player.dragonScore() >= 1 && player.dragonScore() < 6) outputText("\n<font color=\"#008000\">Dragon: " + player.dragonScore() + "</font>", false);
	else if (player.dragonScore() < 1) outputText("\n<font color=\"#ff0000\">Dragon: 0</font>", false);
	...
	if (player.gorgonScore() >= 5) outputText("\n<font color=\"#0000a0\">Gorgon: " + player.gorgonScore() + " (+" + (10 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Tou, +" + (20 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Spe, +" + (50 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max HP)</font>", false);
	else if (player.gorgonScore() >= 1 && player.gorgonScore() < 5) outputText("\n<font color=\"#008000\">Gorgon: " + player.gorgonScore() + "</font>", false);
	...
	if (player.humanScore() == 14) outputText("\nHUMANITY: 14 (+" + (40 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Str, +" + (40 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Tou, +" + (40 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Spe, +" + (40 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Int, +" + (40 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Lib)</font>", false);
	else if (player.humanScore() == 13) outputText("\nHUMANITY: 13 (+" + (30 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Str, +" + (30 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Tou, +" + (30 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Spe, +" + (30 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Int, +" + (30 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Lib)</font>", false);
	else if (player.humanScore() == 12) outputText("\nHUMANITY: 12 (+" + (20 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Str, +" + (20 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Tou, +" + (20 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Spe, +" + (20 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Int, +" + (20 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Lib)</font>", false);
	else if (player.humanScore() == 11) outputText("\nHUMANITY: 11 (+" + (10 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Str, +" + (10 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Tou, +" + (10 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Spe, +" + (10 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Int, +" + (10 * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Lib)</font>", false);
	else if (player.humanScore() < 11) outputText("\n<font color=\"#008000\">HUMANITY: " + player.humanScore() + "</font>", false);
	if (player.internalChimeraScore() >= 1) {
		outputText("\n<font color=\"#0000a0\">INTERNAL CHIMERICAL DISPOSITION: " + player.internalChimeraScore() + " (+" + (5 * player.internalChimeraScore() * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Str, +" + (5 * player.internalChimeraScore() * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Tou,", false);
		outputText(" +" + (5 * player.internalChimeraScore() * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Spe, +" + (5 * player.internalChimeraScore() * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Int, +" + (5 * player.internalChimeraScore() * (1 + flags[kFLAGS.NEW_GAME_PLUS_LEVEL])) + " max Lib)</font>", false);
	}
