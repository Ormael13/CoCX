...
public function approachPonies():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You slowly begin your approach, though you are spotted almost immediately by the pink one.\n\n");

	classes.EngineCore.outputText("You aren't two steps out of the forest before it seems she, and it HAS to be a she, seemingly teleports in front of you with an expression of rapture and joy pasted on her face.  You almost instinctively pull out your [weapon], startled by the appearance of the pink one in your face.  Immediately she starts asking a myriad of questions, speaking at a speed which makes it impossible to understand a word.  Almost as stunning is the fact that she is speaking in a human tongue. Before you have the time to gather yourself up and figure out what the pink one is saying, or even what is going on, the rest of her group have managed to catch up to her, and begin calming her down.\n\n");

	classes.EngineCore.outputText("The purple one turns to you looking apologetic and a bit exasperated, \"<i>Sorry for our friend startling you. She gets excited <b>really</b> easily.</i>\"\n\n");
	
	classes.EngineCore.outputText("You respond that it's no problem, but she should be more careful approaching strangers");
	if(player.weaponName != "fists") classes.EngineCore.outputText(", especially armed strangers");
	classes.EngineCore.outputText(".\n\n");
	
	classes.EngineCore.outputText("The blue one looks at you in a puzzled manner while hovering above the ground, \"<i>Actually, you are the first person we have seen who isn't a pony.</i>\"\n\n");

	classes.EngineCore.outputText("To be fair, you tell them, you haven't seen any one who IS a pony.  The pink one, now slightly calmer, interjects herself into the conversation \"<i>Ponies, not ponies, whatever!  We have a new friend, and I say PARTY!!!</i>\"\n\n");

	classes.EngineCore.outputText("These little horses are definitely a rambunctious bunch, especially the white one, who, from the moment she spotted you, hasn't stop complaining about your awful sense of fashion. The only exception seems to be that yellow one hiding behind a screen of squirrels, birds and what you think are three generations of rabbits.\n\n");

	classes.EngineCore.outputText("What should you do? You could party with the horses; after all, you haven't had a reason to party since getting here. You can politely decline for now or leave these oddly colored and slightly disturbing creatures for the more familiar sight of demons.\n\n");

	classes.EngineCore.outputText("Whichever you choose, something tells you that you won't see these ponies again.");
	//Option one: Leave Politely
	//Option Two: Too creepy...
	//Option three: Yay, party?
	classes.EngineCore.simpleChoices("Too creepy",derpCreepy,"Yay Party!",derpyParty,"",null,"",null,"Leave",derpPolitely);
}
...
public function DLCPrompt(dlcName:String, dlcPitch:String, dlcPrice:String, nextFunc:Function = null):void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText(dlcPitch);
	classes.EngineCore.outputText("\n\nYou can purchase " + dlcName + " for " + dlcPrice + ". Would you like to purchase it now?");
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0, "Yes", buyDLCPrompt, dlcName, dlcPrice, nextFunc);
	if (nextFunc != null) classes.EngineCore.addButton(1, "No", nextFunc);
	else classes.EngineCore.addButton(1, "No", classes.EventParser.playerMenu);
}
private function buyDLCPrompt(dlcName:String, dlcPrice:String, nextFunc:Function = null):void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("<b>Item:</b> " + dlcName + "\n");
	classes.EngineCore.outputText("<b>Price:</b> " + dlcPrice + "\n");
	classes.EngineCore.outputText("Please select a purchase method.");
	classes.EngineCore.menu();
	classes.EngineCore.addButton(0, "Credit/Debit", proceedToCheckout, "Credit/Debit Card", nextFunc);
	classes.EngineCore.addButton(1, "PayPal", proceedToCheckout, "Paypal", nextFunc);
	classes.EngineCore.addButton(2, "Interac", proceedToCheckout, "Interac", nextFunc);
	classes.EngineCore.addButton(3, "COMING SOON!", classes.EngineCore.doNothing);
	classes.EngineCore.addButton(4, "COMING SOON!", classes.EngineCore.doNothing);
	classes.EngineCore.addButton(14, "Cancel", nextFunc);
}
private function proceedToCheckout(method:String, nextFunc:Function = null):void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText("You will be taken to an external website to complete your checkout. Proceed?");
	classes.EngineCore.doYesNo(reallyCheckout, nextFunc);
}
//You just got TROLLED!
private function reallyCheckout():void {
	classes.EngineCore.clearOutput();
	classes.EngineCore.outputText(images.showImage("monster-troll"));
	classes.EngineCore.outputText("APRIL FOOLS! The game will ALWAYS be entirely free to play. :)");
