...
function isUrta():Boolean {
	if(flags[URTA_QUEST_STATUS] > 0 && flags[URTA_QUEST_STATUS] < 1) {
...
//Revelations Talk(C)*
//Requires Lover Urta
function talkWithUrtaAboutFamFam():void {
	clearOutput();
...
function infertilityQuestions():void {
	clearOutput();
...
function startUrtaQuest():void {
	...
	applyArmorStats("leather armor segments",false);
	//DISPLAY SOME SHIT YO
	clearOutput();
	flags[URTA_QUEST_STATUS] = .75;
...
function getHelBangedAsUrta():void {
	...
	if(flags[HEL_HARPY_QUEEN_DEFEATED] > 0) outputText(", and even how they worked together to bring down an evil sorceress and save her father. Your heart swells with pride as she recounts " + player2.short + " doing so a noble thing, and for a moment you know you made the right pick in a lover..");
...
function visitPCPreUrtaQuest(truth:Boolean):void {
	...
	if(flags[ANEMONE_WATCH] > 0 && flags[ANEMONE_KID] > 0) 
		outputText("A confused-looking anemone with a " + itemLongName(flags[ANEMONE_WEAPON]) + " nearly sees you, but you duck around a rock and escape her notice.  ");
	if(flags[FUCK_FLOWER_LEVEL] >= 4 && flags[FUCK_FLOWER_KILLED] == 0 && flags[HOLLI_DEFENSE_ON] > 0) 
	...
	if(companionsCount() > 2) outputText("There are a number of people in the camp, but you avoid them as you head towards " + player2.short + "'s bunk.  ");
	...
	if(truth)
	{
		...
		if(flags[SLEEP_WITH] != "" && flags[SLEEP_WITH] != 0) outputText("  " + player2.short + " suggests the two of you move off, so as not to wake " + flags[SLEEP_WITH] + " in the night.");
	}
	//{LIE}
	else {
	...
	if(player2.cockThatFits(urtaCapacity()) >= 0) 
...
//Put Urta dick in the PC's asshole [Cocked PCs](C)*
function urtaPutsHerDickInPCsWithDicks():void {
	clearOutput();
...
//PC blows Urta(C)*
function pcBlowsUrtasDong():void {
	clearOutput();
...
//Put a PC dick in Urta's vag(C)*
function putAPCDickInUrtaCoochies():void {
	clearOutput();
...
//Put Urta dick in girl PC's vag(C)*
function putUrtasWangInPCsCunt():void {
	clearOutput();
...
//Goblin Encounter(C)*
//Intro(C)*
function runIntoAGoblin(camped:Boolean = false):void {
	clearOutput();
	if(camped) {
...
//Urta Wins(C)*
function winFuckAGoblinBroodmotherAsUrta():void {
	clearOutput();
...
//Urta Loses(C)*
function urtaLosesToGoblin():void {
	clearOutput();
...
function urtaMetabolize():void {
	clearOutput();
	var damage:int = takeDamage(Math.round(maxHP()/10));
...
function nagaPleaseNagaStoleMyDick():void {
	clearOutput();
	//Encounter Intro
...
function manNagaTease():void {
	...
	if(monster.hasStatusAffect("Blind") >= 0) {
		...
		stats(0,0,0,0,0,0,(5 + player.lib/10 - player.inte/20),0);
	}
	//Hit:
	else {
		...
		stats(0,0,0,0,0,0,(10 + player.lib/7 - player.inte/20),0);
	}
	combatRoundOver();
}
	
//Poison Bite*
function poisonBite():void {
...
function nagaSiriusRadioAI():void {
...
//Urta Wins*
function urtaBeatsUpSiriusRadio():void {
	//Beaten via Lust:
...
//Urta Loses*
function urtaLosesToSirriusSnakeRadio():void {
	clearOutput();
	flags[URTA_BECOMES_SNAKE_SLAVE] = 1;
	//Loss by HP:
...
function winRapeHyenaPrincess():void {
	clearOutput();
...
function useSuccubiMilkOnGnollPrincesses():void {
	clearOutput();
	flags[SUCCUBI_MILKED_GNOLL_PRINCESS] = 1;
	...
	if(flags[URTA_FUCKED_HEL] == 1) outputText("that salamander slut's cunt");
...
function loseToGnollPrincessAndGetGangBanged():void {
	clearOutput();
	flags[URTA_TAKEN_BY_GNOLLS] = 1;
	if(player.lust > 99) {
...
function urtaFinishesWithBigOrange():void {
	...
	flags[URTA_TENTACLE_GAPED] = 1;
	flags[URTA_TENTACLE_RAPED] = 1;
...
//Attacks*
//Entangle*
function minotaurLordEntangle():void {
	outputText("The minotaur lord lashes out with his chain, swinging in a wide arc!\n");
	monster.createStatusAffect("Timer",2 + rand(4),0,0,0);
	//{dodge/whatever}
	if(combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
		...
		monster.createStatusAffect("Minotaur Entangled",0,0,0,0);
	}
	combatRoundOver();
}
...
function minotaurCumPress():void {
	...
	stats(0,0,0,0,0,0,22+player.lib/8 + player.sens/8,0);
...
function minotaurDrankMalk():void {
	...
	if(monster.statusAffectv1("Mino Milk") < 3) {
		...
		if(monster.hasStatusAffect("Mino Milk") < 0)
			...
			monster.addStatusValue("Mino Milk",1,1);
	}
	//Failure:*
	else {
...
//Mino Pre-Tease*"
function minotaurPrecumTease():void {
	...
	if(rand(2) == 0) {
		...
		stats(0,0,0,0,0,0,15+player.lib/8 + player.sens/8,0);
	}
	else {
		...
		stats(0,0,0,0,0,0,11+player.lib/10,0);
	}
	//(1)
...
function fidoAI():void {
	if(monster.HP < 300 && monster.statusAffectv1("Mino Milk") < 4) minotaurDrankMalk();
	else if(rand(4) == 0 && player.weaponName != "fists") minotaurDisarm();
	else if(monster.hasStatusAffect("Timer") < 0) minotaurLordEntangle();
	else if(monster.hasStatusAffect("Minotaur Entangled") >= 0) minotaurCumPress();
	else {
...
function urtaSubmitsToMinotaurBadEnd():void {
	clearOutput();
...
function submissiveMinoUrtaBadEndPtIII():void {
	...
	flags[URTA_MINO_AND_SUCCUBI_SLAVE] = 1;
	menu();
	addButton(0,"Next",urtaGameOver);
}

//Urta Win Rapes Minotaur*
function winRapeAMinoLordAsUrta():void {
	clearOutput();
...
//Urta Loses Minotaur
function urtaLosesToMinotaurRoughVersion():void {
	clearOutput();
	...
	stats(0,0,0,0,50,50,-100,0);
	flags[URTA_IS_VULQUINE_MENACE] = 1;
	menu();
...
//Post-Encounter*
function beatMinoLordOnToSuccubi():void {
	clearOutput();
	...
	//{If Urta raped Fido:}
	if(flags[URTA_RAPED_MINO_LORD] > 0) {
...
//Attacks
function milkCubiGainsHPFromTeasing():void {
	outputText(monster.capitalA + monster.short + " smiles, rubbing her hands across herself as she watches your display.  She does not seem greatly affected by your show - at least in the sense of increasing arousal.  She does seem oddly more... vital, as if she drew strength from the very display you put on.");
	monster.str += 5;
	monster.HP += 50;
}
			
function milkSuccubusAI():void {
	if(monster.hasStatusAffect("Milky Urta") < 0 && rand(3) == 0) cowCubiMilkSprayAttack();
	else if(monster.HP < 400) drinkMinoCum();
...
function cowCubiMilkSprayAttack():void {
	...
	//Hit:
		...
		monster.createStatusAffect("Milky Urta",3,0,0,0);
	}
	combatRoundOver();
}

//Tick Message:
function milkyUrtaTic():void {
...
function drinkMinoCum():void {
	...
	if(monster.hasStatusAffect("drank mino cum") < 0 || monster.hasStatusAffect("drank mino cum2") < 0) {
		...
		monster.lust += 25;
		monster.HP += 400;
		if(monster.hasStatusAffect("drank mino cum") < 0) monster.createStatusAffect("drank mino cum",0,0,0,0);
		else monster.createStatusAffect("drank mino cum2",0,0,0,0);
	}
	//Failure:
	else {
...
//Urta Wins*
function urtaBeatsUpCowcubi():void {
	clearOutput();
	//HP:
	if(monster.HP < 1) {
...
//Urta Loses*
function urtaLosesToCowCubi():void {
	clearOutput();
	...
	if(player.HP < maxHP()) outputText(" and healing your wounds");
...
function postFuckUrtaUp():void {
	...
	if(followersCount() >= 2) outputText("leave instructions to your fellow camp-dwellers to look after the place, ");
...
function getKnockedUpWithGodChild():void {
	...
	if(flags[VALARIA_AT_CAMP] == 1) ballWorship[ballWorship.length] = 5;
	var choice:int;
...
function urtaAndEdrynGodChildEpilogue():void {
...
function urtaAndEdrynGodChildEpilogueII():void {
	...
	if(flags[EDRYN_PREGNANCY_INCUBATION] > 0) outputText("  Just how did she get pregnant again when she was already knocked up?  ...Magic, maybe.");
	...
	if(flags[EDRYN_PREGNANCY_INCUBATION] > 0) outputText("massively ");
	...
	if(flags[EDRYN_PREGNANCY_INCUBATION] > 0) outputText("  The centaur guardswoman is still just as pregnant with your offspring as before.  Your child is unharmed by whatever just transpired.");
