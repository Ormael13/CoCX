...
public function totalXP():Number {
...
public function startCombat(monsterNum:Number):void {
	mainView.hideMenuButton( MainView.MENU_DATA );
	mainView.hideMenuButton( MainView.MENU_APPEARANCE );
	mainView.hideMenuButton( MainView.MENU_LEVEL );
	mainView.hideMenuButton( MainView.MENU_PERKS );
	...
	monster.long = "<b>You've incountered a BUG (game bug, not monster bug)! Description: <u>startCombat(" + monsterNum + ")</u></b>";

	//LOAD IMP
	if(monsterNum == 1) {
		monster = new Imp(this);
	}
	//LOAD Sand witch
	if(monsterNum == 2) {
		monster = new SandWitch(this);
	}
	//LOAD JOJO
	if(monsterNum == 3) {
	...
	//Fertile-Caste Bee
	if(monsterNum == 5) {
		monster = new BeeGirl(this);
	}
	//Green Slime
	if(monsterNum == 6) {
		monster = new GreenSlime(this);		
	}
	//LOAD DEMON PACK
	if(monsterNum == 7) {
		monster = new DemonPack(this);		
	}
	//LOAD WORM COLONY
	if(monsterNum == 8) {
		monster = new WormMass(this);
	}
	//LOAD FETISH CULTIST
	if(monsterNum == 9) {
		monster = new FetishCultist(this);	
	}
	//LOAD HELLHOUND
	if(monsterNum == 10) {
		monster = new HellHound(this);
	}
	//LOAD SUCCUBUS
	if(monsterNum == 11) {
		plotFight = true;
		player.createStatusAffect("FactorySuccubusDefeated",0,0,0,0);
		monster = new SecretarialSuccubus(this);
	}
	//LOAD INCUBUS
	if(monsterNum == 12) {
		plotFight = true;
		player.createStatusAffect("FactoryIncubusDefeated",0,0,0,0);
		monster = new IncubusMechanic(this);
	}
	//LOAD OMNIBUS
	if(monsterNum == 13) {
		plotFight = true;
		player.createStatusAffect("FactoryOmnibusDefeated",0,0,0,0);
		monster = new OmnibusOverseer(this);
	}
	//LOAD Tentacle beast
	if(monsterNum == 14) 
	{
		monster = new TentacleBeast(this);
	}
	//LOAD GOBLIN
	if(monsterNum == 15) 
	{
	...
	{
		plotFight = true;
		monster = new Marble(this);	
	}
	//LOAD TAMANI
	if(monsterNum == 17) {
		monster = new Tamani(this);
	}
	//LOAD SHARK GIRL
	if(monsterNum == 18) {
		monster = new SharkGirl(this);
	}
	//LOAD FETISH ZEALOT
	if(monsterNum == 19) {
		monster = new FetishZealot(this);	
	}
	// Infested Hellhound
	if(monsterNum == 20) {
		monster = new InfestedHellhound(this);	
	}
	//LOAD NAGGA PLEAS!
	if(monsterNum == 21) {
		monster = new Naga(this);	
	}
	//AKUBAL
	if(monsterNum == 22) {
		monster = new Akbal(this);
	}
	//LOAD CERAPH!
	if(monsterNum == 23) {
		monster = new Ceraph(this);
	}
	//LOAD TAMANI'S DAUGHTERS
	if(monsterNum == 24) {
		monster = new TamanisDaughters(this);
	}
	//LOAD GENERIC HARPY
	if(monsterNum == 25) {
		monster = new Harpy(this);
	}	
	//LOAD SOPHIE
	if(monsterNum == 26) {
		monster = new Sophie(this);
	}	
	//IMP WHORED HORDE
	if(monsterNum == 27) {
		plotFight = true;
		monster = new ImpHorde(this);
	}
	//Encapsulation pod
	if(monsterNum == 28) 
	{
		plotFight = true;
		monster = new EncapsulationPod(this);	
	}
	//LOAD ANEMONE!
	if(monsterNum == 29) {
		monster = new Anemone(this);
	}
	//LOAD VALA!
	if(monsterNum == 30) {
		plotFight = true;
		monster = new Vala(this);
	}
	//LOAD Zetaz
	if(monsterNum == 31) {
		plotFight = true;
		monster = new Zetaz(this);
	}
	//LOAD GNOLL!
	if(monsterNum == 32) {
		plotFight = true;
	...
	//LOAD IZMA!
	if (monsterNum == 35) {
		monster = new Izma(this);
	}
	//LOAD ISABELLA!
	if (monsterNum == 36) {
		monster = new Izabella(this);
	}
	//LOAD SPIDERGAL
	if (monsterNum == 37) {
		monster = new FemaleSpiderMorph(this);
	}
	//LOAD SPIDERGUY
	if(monsterNum == 38) {
		monster = new MaleSpiderMorph(this);
	}
	//LOAD Basilisk
	if (monsterNum == 39) {
		monster = new Basilisk(this);
	}
	//LOAD Drider
	if (monsterNum == 40) {
		monster = new CorruptedDrider(this);
	}
	//Minotaur gang!
	if(monsterNum == 41) {
		monster = new MinotaurMob(this);
	}
	//Kiha the dragoness
	if (monsterNum == 42) {
		monster = new Kiha(this);
	}
	//Plain Girl (Shouldra)
	if(monsterNum == 43) {
		monster = new Shouldra(this);
	}
	//Goo-Girl
	if (monsterNum == 44) {
		monster = new GooGirl(this);
	}
	//LOAD Hel/Salamander
	if(monsterNum == 45) {
		monster = new Hel(this);
	}
	//LOAD MOB OF SPIDERS!
	if (monsterNum == 46) {
		monster = new SpiderMorphMob(this);
	}
	//LOAD GOO ARMOR!
	if(monsterNum == 47) {
		monster = new GooArmor(this);
	}
	//LOAD HARPY WING
	if (monsterNum == 48) {
		monster = new HarpyMob(this);
	}	
	//LOAD BRIGID JAILER
	if(monsterNum == 49) {
		monster = new Brigid(this);
	}
	//LOAD PHOENIX PLATOON
	if(monsterNum == 50) {
		monster = new PhoenixPlatoon(this);
	}
	//LOAD HARPY QUEEN
	if(monsterNum == 51) {
		monster = new HarpyQueen(this);
	}
	//LOAD FARMERS
	if (monsterNum == 52) {
		monster = new Farmers(this);
	}
	//LOAD EMBER
	if(monsterNum == 53) {
		monster = new Ember(this);
	}
	//LOAD CHAMELEON
	if (monsterNum == 54) {
		monster = new ChameleonGirl(this);
	}
	//LOAD SANDTRAP
	if(monsterNum == 55) {
		monster = new SandTrap(this);
	}
	//LOAD SATYR
	if(monsterNum == 56) {
		monster = new Satyr(this);
	}
	//LOAD Kitsune
	if(monsterNum == 57) {
		monster = new Kitsune(this, monster.hairColor);// TODO refactor
	}
	//LOAD NAGGA PLEAS!
	if(monsterNum == 58) {
		monster = new Sirius(this);
	}
	if(monsterNum == 59) {
		monster = new MinotaurLord(this);
	}
	//LOAD COW SUCCUBUS
	if (monsterNum == 60) {
		plotFight = true;
		monster = new MilkySuccubus(this);
	}
	if(monsterNum == 61) {
		monster = new Kelt(this);
	}
	//Sheila
	if(monsterNum == 62) {
		monster = new Sheila(this);
	}
	//LOAD MINERVA
	if(monsterNum == 63) {
		plotFight = true;
		monster = new Minerva(this);
	}
	//LOAD IMP
	if (monsterNum == 64) {
		monster = new ImpLord(this);
	}
	//SAND WITCHES
	if(monsterNum == 65) {
		plotFight = true;
		monster = new SandWitchMob(this);
	}
	//LOAD CUM WITCH
	if(monsterNum == 66) {
		monster = new CumWitch(this);
	}
	//SAND MOTHER
	if(monsterNum == 67) {
		plotFight = true;
		monster = new SandMother(this);
	}
	//LOAD HOLLI
	if(monsterNum == 68) {
...
public function runAway():void {
	...
	if(player.tailType == TAIL_TYPE_RACCOON && player.earType == EARS_RACCOON && player.hasPerk("Runner") >= 0) escapeMod -= 25;
	...
	if(player.spe > rand(monster.spe + escapeMod)) {
		...
		else if(player.tailType == TAIL_TYPE_RACCOON && player.earType == EARS_RACCOON && player.hasPerk("Runner") >= 0) {
	...
	//FAIL FLEE
		...
		//fail
		else if(player.tailType == TAIL_TYPE_RACCOON && player.earType == EARS_RACCOON && player.hasPerk("Runner") >= 0) outputText("Using your running skill, you build up a head of steam and jump, but before you can clear the ground more than a foot, your opponent latches onto you and drags you back down with a thud!");
		//Nonflyer messages
		else {
			//Huge balls messages
...
public function tailWhipAttack():void {
	...
	if((player.hasStatusAffect("Blind") >= 0 && rand(2) == 0) || (monster.spe - player.spe > 0 && int(Math.random()*(((monster.spe-player.spe)/4)+80)) > 80)) {
		...
		if(player.tailType == TAIL_TYPE_RACCOON) monster.addStatusValue("Coon Whip",2,2);
	}
	outputText("\n\n");
	enemyAI();
}
