//TODO: Break up this file into few smaller files and move from includes to Classes.
...
private function wait():void {
	...
	if (monster.findStatusEffect(StatusEffects.Level) < 0) fatigue(-5);
...
public function dropItem(monster:Monster, nextFunc:Function = null):void {
	...
	if (!plotFight && rand(6) == 0 && plains.bunnyGirl.isItEaster()) {
		temp = rand(13);
...
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
	...
	if (player.findStatusEffect(StatusEffects.Poison) >= 0) {
		...
		if (player.findPerk(PerkLib.Medicine) >= 0 && rand(100) <= 14) {
			...
			takeDamage(8+rand(maxHP()/20) * player.statusEffectv2(StatusEffects.Poison));
		}
	}
...
//Pass false to combatMenu instead:	menuLoc = 3;
	if (inCombat && player.findStatusEffect(StatusEffects.Sealed) >= 0 && player.statusEffectv2(StatusEffects.Sealed) == 2) {
		clearOutput();
...
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
	...
	if (monster is LivingStatue)
	{
		...
		monster.createStatusEffect(StatusEffects.Blind,5*spellMod(),0,0,0);
		if (monster.short == "Isabella")
...
public function magicalSpecials():void {
	if (inCombat && player.findStatusEffect(StatusEffects.Sealed) >= 0 && player.statusEffectv2(StatusEffects.Sealed) == 6) {
		clearOutput();
...
public function physicalSpecials():void {
	...
	if (inCombat && player.findStatusEffect(StatusEffects.Sealed) >= 0 && player.statusEffectv2(StatusEffects.Sealed) == 5) {
		clearOutput();
