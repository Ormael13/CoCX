...
function spiderMorphWebAttack():void {
	...
	if(monster.hasStatusAffect("Blind") >= 0 && rand(3) < 2) {
		outputText(monster.capitalA + monster.short + " misses completely due to the blindness.", false);
	}
	//Determine if dodged!
	...
	//Determine if cat'ed
		...
		outputText("You throw yourself out of the way with cat-like agility at the last moment, avoiding " + monster.mf("his","her") + " attack.\n", false);
	}
	//Got hit
	else {
...
function getBitten():void {
	...
	if(player.spe < 2 && player.hasStatusAffect("Web") >= 0) {
		...
		if(player.hasCock()) {
			outputText(SMultiCockDesc() + " turns rock hard and squirts weakly, suddenly so aroused that it starts soaking your " + player.armorName, false);
			if(player.hasVagina()) outputText(" along with your " + vaginaDescript(), false);
			outputText(".  ", false);
		}
		else if(player.hasVagina()) outputText("Your " + vaginaDescript() + " grows wet as hell and so sensitive that every step and movement reminds you of the powerful need for something between your sopping nether-lips.  ", false);
	...
	outputText("The spider-" + monster.mf("boy","girl") + " lunges forward with " + monster.mf("his","her") + " mouth open, " + monster.mf("his","her") + " two needle-like fangs closing rapidly.  ", false);
	//Blind dodge change
	if(monster.hasStatusAffect("Blind") >= 0 && rand(3) < 2) {
		outputText(monster.capitalA + monster.short + " misses completely due to the blindness.", false);
	}
	//Determine if dodged!
		...
		outputText("You dodge away, avoiding " + monster.mf("his","her") + " bite!", false);
	}
	//Determine if evaded
	else if(player.hasPerk("Evade") >= 0 && rand(100) < 10) {
...
function defeatASpiderBitch():void {
	...
	if(player.lust >= 33 && player.gender > 0) {
		...
		var pussyFuck:Number = 0;
		var analFuck:Number = 0;
		if(player.hasVagina()) scissor = 2989;
		if(player.hasCock()) {
			if(player.cockThatFits(monster.vaginalCapacity()) != -1) pussyFuck = 2990;
