...
function isabellaGreeting():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00256] == 0) {
		...
		//Approach - sets flags[UNKNOWN_FLAG_NUMBER_00256] to 1 and calls this function
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00257] == 0) {
		flags[UNKNOWN_FLAG_NUMBER_00257]++;
		...
		//(Shorter PC's) 
			...
			if(flags[UNKNOWN_FLAG_NUMBER_00259] < 0) flags[UNKNOWN_FLAG_NUMBER_00259]++;
	...
	else if(player.tallness > 78 && flags[UNKNOWN_FLAG_NUMBER_00258] == 0) {
	...
	else if(flags[UNKNOWN_FLAG_NUMBER_00258] > 0 && flags[UNKNOWN_FLAG_NUMBER_00259] == 0 && player.tallness <= 78) {
		flags[UNKNOWN_FLAG_NUMBER_00259]++;
	...
	//Camp Meeting – Welcomed Short but Not Tall
	else if(flags[UNKNOWN_FLAG_NUMBER_00259] > 0 && flags[UNKNOWN_FLAG_NUMBER_00258] == 0 && player.tallness > 78) {
		...
		flags[UNKNOWN_FLAG_NUMBER_00258]++;
		if(player.hasCock()) {
...
function unwelcomeFightCowGal():void {
	...
	startCombat(36);
	flags[UNKNOWN_FLAG_NUMBER_00260] += 72;
	spriteSelect(31);
	doNext(1);
}
//Fuck-fight
...
function nomOnMommaIzzysTits():void {
	...
	if(player.tallness > 78 || player.isTaur()) {
		...
		if(player.hasCock() && x >= 0) {
			...
			if(flags[UNKNOWN_FLAG_NUMBER_00261] == 0) outputText("  S-she fucked you in your sleep?  That explains how good your dreams were.  On one hand you feel a little violated, but on the other you have to wonder how long this woman has held her camp against the demons with nothing to please her.", false);
			...
			flags[UNKNOWN_FLAG_NUMBER_00261]++;
		}
		//(Generic End)  
		else {
	...
	//Voluntary Nursing (Small Characters)
		...
		if(player.hasCock() && x >= 0) {
			...
			if(flags[UNKNOWN_FLAG_NUMBER_00261] == 0) {
...
function defeatIsabella():void {
	...
	if(monster.HP < 1) outputText("Vhy must you be so like them?  You act like a demon!  Stay away from mein milk!", false);
	else outputText("I didn't vant this!  ...but, please, I have so much milk... drink my moOOOO-ilk!", false);
	outputText("</i>\"  ", false);
	...
	var lactation:Number = 0;
	if(player.biggestLactation() >= 1) lactation = 2962;
	var buttsex:Number = 0;
	var sixtyNine:Number = 0;
	if(player.gender > 0 && player.lust >= 33) sixtyNine = 2964;
	var bigTitFuck:Number = 0;
	var smallTitFuck:Number = 0;
	var vaginalSex:Number = 0;
	if(player.hasCock()) {
		if(player.cockThatFits(monster.analCapacity()) != -1 && player.lust >= 33) buttsex = 2963;
		if(player.cockThatFits(monster.vaginalCapacity()) != -1 && player.lust >= 33) vaginalSex = 2966;
...
//[LACTATION 69]
function victoryLactation69():void {
	outputText("", true);
	...
	//Reset anger
	flags[UNKNOWN_FLAG_NUMBER_00260] = 0;
	stats(0,0,0,0,0,0,-100,0);
	eventParser(5007);
}
//[VICTORY BUTTSEX]
function PCVictoryOnIsabellaButtsex():void {
...
//[Victory 69]
function victoryAgainstIzzzzzySixtyNine():void {
	var x:Number = player.cockThatFits(38);
	if(x < 0) x = 0;
	outputText("", true);
...
function tooBigVictoryTittyFuckingFuntimesWithMilk():void {
	outputText("", true);
	...
	outputText("Isabella is getting off on being disrobed!  You don't stop at her corset either; every bit of clothing above her waist is pulled of and discarded, saving the silken shirt for last.  ", false);
	...
	else if(player.cor < 66) outputText("You remove the garment without much care for it's condition, not damaging it too much as you wrench it free.", false);
	else outputText("You tear off the offending garment without a single care for the cow-girl's property.", false);
	outputText("  She looks up at you with teary brown eyes and asks, \"<i>Please be gentle with me, ja?</i>\"\n\n", false);
...
//[VAGINAL PROD N' POKE]
function vaginalProdNPokeIsabella():void {
	...
	outputText(" cow-girl pales and asks, \"<i>But vhy not?  Mein milk is so gooood.</i>\"  The last word emphasizes the 'o's so much that it comes out almost like a moo.  You push her legs apart and flip up her skirt to reveal the cleft of her womanhood.  She didn't even bother with panties, so there's nothing to protect her ", false);
	...
	outputText("\"<i>Are you sure?</i>\" you ask.  Isabella bites her lip petulantly, but you brush the sensitive skin of her inner thigh and the dusky cow-girl moans, \"<i>Fine!  I-I vant it in my pussy!  Fuck me!  Fuck mein cunt...</i>\"  She trails off into half-cries, half-moans in between shudders of inadvertent pleasure.  Her hips twitch at you, begging for more stimulation, more pleasure; anything to satisfy the aching need you've stoked betwixt her thighs.\n\n", false);
...
function tinyVictoryTittyFuckingFuntimesWithMilk():void {
	var x:Number = player.smallestCockIndex();
	outputText("", true);
	outputText("You toss aside your " + player.armorName + " to reveal your " + cockDescript(x) + " to the ", false);
	if(monster.lust > 99) outputText("lusty", false);
	...
	outputText(".  You clear your throat and try to regain control of the situation by demanding that she service you with her breasts.  Isabella happily accedes, even going to so far as to clap excitedly as she sheds her top.  She seems to genuinely relish the idea, to a degree that makes her forget ", false);
	...
	outputText(".  Isabella smirks up at you as she begins to jiggle her boobs back and forth while saying, \"<i>Does your cute little cock like zis?  Ooooh, I zink it does.  It vants to squirt into my cleavage like a good little dick, ja?</i>\"  You start to nod before you catch yourself and arrest the motion.  You're the one in control, not her!\n\n", false);
	...
	outputText("Your whole body starts to blush before long, your tiny cock unable to cope with the teasing cow-girl's ministrations.  She looks you in the eye and begins to bob her whole body up and down, shaking her tits while she asks, \"<i>Are you going to cum for me soon?  I vant zat tiny cock to spurt all its meek little cum over my breasts until it's limp.  Isn't zat vat you want?</i>\"  Her sultry teases arouse you past the tipping point, and you feel your cock begin to release ", false);
	...
	if(player.cumQ() >= 1000) outputText("  It pumps and pumps, blotting out her chest and face, turning her expression to one of ecstatic shock.  \"<i>Oh my!  Such an obedient little cock!  Zat's right; cover me vis your seed, my adorable little weenie!</i>\"", false);
...
//[Discuss Isabella]
function talkWithIsabella():void {
	spriteSelect(31);
	outputText("", true);
	flags[UNKNOWN_FLAG_NUMBER_00262]++;
	if(flags[UNKNOWN_FLAG_NUMBER_00262] == 1) {
