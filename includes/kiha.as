...
//[Fight]
function meetKihaAndFight():void {
	outputText("", true);
	spriteSelect(72);
...
function kihaVictoryIntroduction():void {
	...
	choices("Masturbate ",forceMasturbate,"Use Tail",useHerTail,"FuckHerPussy",fuckHer,"FuckHerAss",buttFuck,"Talk",wordRape,"",0,"",0,"",0,"",0,"Leave",5007);
}
//*Generic PC Loss Intro 
function kihaLossIntro():void {
	outputText("", true);	
	spriteSelect(72);
	//(Lust)
...
function kihaMilkTitHumiliation():void {
	...
	outputText("\"<i>What's that?  Speak up, or I might milk you hard enough to rip one of these udders off,</i>\" threatens Kiha with a sadistic smile.  You immediately moo, louder this time.  She rewards you with quick pump on your udders, squirting more milk into the air.  You shiver, growing more and more turned with every splatter of creamy white that's forced from your body.\n\n", false);
...
function kihaRapesMen():void {
	...
	if(player.isNaga() || player.tailType == 9) outputText("\"<i>Heh, if you had half as much dexterity as you do impudence, I'd let you do this for me.  Fat chance of that,</i>\" she remarks off-handedly.  ", false);
	...
	//[Lizard/naga tail: 
	if(player.isNaga() || player.tailType == 9) outputText(" while your tail curls around to find your own asshole.  The grip of the dragon's nethers so intense, you barely feel the pressure of your own tail, pushing in easily to soothingly stroke your overtaxed prostate", false);
...
function tsundereMasturbationChristmasCarol():void {
	...
	if(!player.hasCock()) stats(0,0,0,0,0,0,120,0);
	eventParser(5007);
}
...
function victoryDickKiha():void {
	...
	if(y != -1 && player.cockTotal() > 1) {
		...
		if(player.isNaga() || player.tailType == 9 || z >= 0) {
		...
		if(player.isNaga() || player.tailType == 9 || z >= 0) outputText(" gasping mouth,", false);
...
function kihaRapesLittleGirlsISawItOnTheNews():void {
	...
	if(player.tongueType == 1) outputText("  You are grateful for your forked tongue, which allows you to cover more of her pussy at once.", false);
	else if(player.tongueType == 2) outputText("  Your inhumanly long, prehensile tongue is a blessing here; you can reach into her deepest recesses, slurp your way into every nook and cranny, and generally provide a tongue-fuck that no human ever could.", false);
	outputText("\n\n", false);
...
function rapeKihaWithWORDS():void {
	...
	//Conversation Level 3 (40- Corruption! PUREBABIES ONLY!) (Zed)
		...
		outputText("The dragonic woman looks very much like a young, nervous girl in that moment.  Her cheeks blush and she leaps up, flying away before you can ask her anymore.", false);
	}
...
function sillyModeKihaAttack():void {
	spriteSelect(72);
...
function analRapuzulaKiha():void {
	...
	else if(player.cor < 66) outputText("Since she seems to be concentrating on your hands, you frantically try to think of a solution that doesn't involve them.  An idea forms and, deciding its now or never, you lean forward and nibble on the tip of her ear.  ", false);
	else outputText("Since she seems to be concentrating on your hands at the moment, your mind races trying to find a solution that doesn't involve them.  An idea forms and, deciding its now or never, you lean forward and slowly slide your tongue across her cheek.  ", false);
