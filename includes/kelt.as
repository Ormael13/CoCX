...
function keltEncounter():void {
	...
	//Repeated encounter
		...
		//Centaur bad end
		if(player.lowerBody == 4 && player.statusAffectv2("Kelt") >= 100 && player.gender > 1) {
...
function keltEagerlyGetNaked():void {
	...
	stats(0,0,0,0,0,0,int(player.lib/10)+5,0);
	keltMainEncounter2();
}
...
function keltRequiresBlowjobs():void {
	...
	if(player.inte > 40 && player.cor > 70 && player.lowerBody != 4) {
		outputText("\n\n<b>If you fight back and take him down a peg, you might never see him again...</b>");
		simpleChoices("Shamefully",2117,"Eagerly",2118,"Fight Back",2262,"",0,"Never!",2119);
	}
	else simpleChoices("Shamefully",2117,"Eagerly",2118,"Resist",0,"",0,"Never!",2119);
}
...
function keltMainEncounterPostBlowjob():void {
	...
	//(Blowjob Requirement Off, 40% Chance)
		...
		//Otherwise)
			...
			//Submit				Resist!
			simpleChoices("Submit",2120,"Resist",2121,"",0,"",0,"",0);
			return;
		}
	}
	keltMainEncounter3();
...
function keltResistance():void {
	...
	if(player.faceType == 1 || player.faceType == 2) outputText("a low growl building in the back of your throat", false);
...
function fuckKeltsShitUp():void {
	...
	outputText("", true);
	//If naga folks
	if(player.faceType == 5 && player.tongueType == 1 && player.lowerBody == 3) {
		...
		if(player.hasCock()) {
			...
			if(player.cocks[0].cockLength <= 10) {
				...
				outputText("You feel a mighty groan around your " + cockDescript(0) + " and Kelt's cock begin to pulse against your tailurgently.  Quickly and expertly, you wrap the end around the centaur's penis tightly, denying him release and rewarding yourself with another pained, muffled squeal reverberating through your prick.\r\r", false);
	...
	//Standard anti-kelt scene
		...
		if(player.gender == 0) {
			...
			if(player.vaginas[0].vaginalWetness < 3) outputText("tasting them experimentally.  ", false);
