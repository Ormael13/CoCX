...
function loseToThisShitPartII():void {
	...
	if(player.gender == 1 || (player.gender == 3 && rand(2) == 0)) {
		...
		outputText(" and squeeze, forcing " + sMultiCockDesc() + " to inflate to readiness.  Deep inside your " + assholeDescript() + ", the tentacle starts to rub against your prostate.  It caresses the male organ on each side and pauses to squeeze the center of it, pushing a few drops of sticky cum from your trembling " + cockNoun(CockTypesEnum.HUMAN) + ".\n\n", false);
...
function kinathisValaStuff():void {
	...
	if(player.cocks[x].cockType == CockTypesEnum.HUMAN) outputText("Her tongue licks around the throbbing glands, the heady animal musk filling her nose and mind with sweet lust.  ");
