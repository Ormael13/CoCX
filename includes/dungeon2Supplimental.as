...
public function impGangVICTORY():void {
	outputText("", true);
	//Flag them defeated!
	flags[ZETAZ_IMP_HORDE_DEFEATED] = 1;
...
public function loseToThisShitPartII():void {
	...
	//Done if escaped
	if(monster.lust == 100) {
		flags[ZETAZ_FUNGUS_ROOM_DEFEATED]++;
		eventParser(5007);
		return;
	}
	//[BAD-END GO]
...
public function useValaPtII():void {
	...
	flags[TIMES_FUCKED_VALA_IN_DUNGEON]++;
	doNext(1);
}
...
public function rejectFuckingVala():void {
	spriteSelect(85);
	outputText("", true);
	if(flags[TIMES_PC_DEFEATED_VALA] == 0) {
		...
		if(flags[TIMES_PC_DEFEATED_VALA_AND_RAEPED] > 0) outputText("  You smirk in wonder at the horny little slut.  You guess you'll have to put her back into her place and give her another dose of 'love'.", false);
		outputText("\n\n", false);
...
public function valaFightVictoryFuckPtII():void {
	...
	flags[TIMES_PC_DEFEATED_VALA_AND_RAEPED]++;
	eventParser(5007);
}
...
public function leftValaAlone():void {
	...
	outputText("", true);
	//first post meeting
	if(flags[INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED] == 0 || flags[TIME_SINCE_VALA_ATTEMPTED_RAPE_PC] == 0) {
		...
		flags[INVESTIGATED_VALA_AFTER_ZETAZ_DEFEATED]++;
		flags[TIME_SINCE_VALA_ATTEMPTED_RAPE_PC] = 6;
	}
	//Anytime within 6 hours of waking/raping/meeting her
	else if(flags[TIME_SINCE_VALA_ATTEMPTED_RAPE_PC] > 0) {
...
public function wakeMeUpBeforeValaGoGosPtII():void {
	...
	if(player.gender == 1) {
		...
		outputText("She's unconscious by the time you're finished seeding the fairy, the girl's chest barely rising and falling under her disproportionately huge breasts and massively inflated womb. Even better than a goblin, you reflect, marveling at the fairy's resilience to the rapid sequence of orgasms. Though, you suppose, once you've already been fucked mindless, there's little enough left to break. You resolve to check back on her again.", false);
	}
	//f2
	else if(player.gender == 2) {
...
public function purifiedFaerieBitchBar():Boolean {
	...
	if(flags[FREED_VALA] == 0) return false;
	...
	if(model.time.hours >= 12 && model.time.hours <= 21) {
		...
		if(flags[FUCKED_VALA_AT_BAR_TODAY] > 0) {
...
public function chooseValaInBar():void {
	...
	menu();
	//(First meeting)
	if(flags[ENCOUNTERED_VALA_AT_BAR] == 0) {
		flags[ENCOUNTERED_VALA_AT_BAR]++;
...
public function cleansedFirstRemeet():void {
	...
	//[You][Leave]
	simpleChoices("You",cleansedValaFuckHerBrainsOut,"",0,"",0,"",0,"Leave",barTelAdre);
}
...
public function cleansedValaFuckHerBrainsOutPtII():void {
	...
	//dicks + growth
	if(player.totalCocks() > 0) {
		outputText("Vala leans way back and grabs the edge of the stool while she locks her legs behind your waist, the fairy's startling strength pining the two of you together. She slaps her generous butt onto your dick in long, slow motions, making sure you feel every inch of her rippling passage", false);
		...
		if(player.tallness >= 120) {
			outputText("\n\nOr was that just your imagination?  On closer examination you're not any bigger, but you're huge to begin with!", false);
			player.tallness--;
		}
		//[Player grows 1", lust drops to 0, corruption drops by 1]
		player.tallness++;
		if(player.cor > 40) stats(0,0,0,0,0,0,0,-.3);
	}
	//Part 2 - girls
		...
		if(player.cor > 40) stats(0,0,0,0,0,0,0,-.5);
	}
	//[End Encounter]
	cheatTime(1);
	doNext(barTelAdre);
...
public function faerieOrgyFuck():void {
	...
	flags[WEEKLY_FAIRY_ORGY_COUNTDOWN] = 6;
	//[Herm]
	if(player.gender == 3) {
...
public function faerieOrgyFuckMaleContinue():void {
	...
	if(flags[TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED] + flags[SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP] + flags[SCYLLA_MILK_THERAPY_TIMES] + flags[SCYLLA_CUM_THERAPY_TIMES] + flags[SCYLLA_SEX_THERAPY_TIMES] > 0) {
...
public function faerieOrgyFuckFemaleContinue():void {
	...
	//[If the Player has unlocked Scylla's Addiction Counseling event] 
	if(flags[TIMES_SCYLLA_ADDICT_GROUP_EXPLOITED] + flags[SCYLLA_TIMES_SHARED_IN_ADDICT_GROUP] + flags[SCYLLA_MILK_THERAPY_TIMES] + flags[SCYLLA_CUM_THERAPY_TIMES] + flags[SCYLLA_SEX_THERAPY_TIMES] > 0) outputText("Another fairy flies toward Vala and you blink through the building lusty haze when you recognize the little girl. Wearing a slightly slutty crystalline gown of translucent gossamer, you recognize Pastie, the fairy from Scylla's addiction counseling meeting. When you call out to her, she bashfully waves back. \"<i>Hey, uh, " + player.short + ". It's kinda weird seeing you outside of the meetings. How are you? How have things been?</i>\" She looks at your naked, reclined form on the bed and blushes. \"<i>Oh, right, right. Um. I'm just helping out Vala, you know. Sisters and all. Uh. I'm… I'm going to just give her this petal and get something to drink, okay? All this sobriety is making this really uncomfortable for me.</i>\" The other fairies laugh and Pastie hands off her blossom before zipping out of the room, probably to steal someone's drink downstairs.\n\n", false);
...
public function gigaArouse():void {
	...
	stats(0,0,0,0,0,0,rand(player.lib/10)+player.cor/10+15,0);
	...
	if(player.cocks.length > 0) {
		if(player.lust >= 60 && player.cocks.length > 0) outputText("You feel " + sMultiCockDesc() + " dribble pre-cum, bouncing with each beat of your heart and aching to be touched.  ", false);
...
public function endZetaz():void {
	...
	player.createKeyItem("Zetaz's Map",0,0,0,0);
	//(ZETAZ IS DEAD)
	flags[ZETAZ_DEFEATED_AND_KILLED]++;
	eventParser(5007);
}
...
public function loseToZetaz():void {
	...
	if(player.gender == 0) {
		outputText("He squints down at you with a bemused look and laughs, \"<i>How did you lose your gender anyhow?  Nevermind, we've got to do something about that!</i>\"\n\n", false);
...
public function femaleZetazOver():void {
	...
	if(flags[NUMBER_OF_TIMES_MET_SCYLLA] > 0) outputText("Unbidden, Scylla's face comes to mind, and you realize the drugs coursing through your veins must be doing something similar to you!  Her visage changes to your own, though the thick, cock-sucking lips remain behind, eager to be penetrated.", false);
...
public function hermZetazOver():void {
	...
	outputText("\"<i>Ey boss!  She's already starting to drip!  To think she tried to fight us.  She's showing us her true nature – that of a pervert-slut,</i>\" raves one of the horde.  You can't pick out the source of his voice in the crowd, but the words sting enough to make your whole body blush with ", false);
	if(player.cor < 33) outputText("shame", false);
...
public function hermZetazOverPtII():void {
	...
	outputText("Wait-what!?  You shake your head and clear away the fantasy, though your sexual organs' constant delightful throbbings aren't doing much to help.  Zetaz is still fucking your pussy, taking it with long slow strokes that would've made your demonic legs give out ages ago if you you weren't hanging on a wall.  The chanting is so loud, so insidious.  You can feel it snaking through your brain, twisting your thoughts and ideas.  You close your eyes, desperate to fight it, but it only enhances the sensation of dick-milking and being fucked by your-no, by that demon!\n\n", false);
...
public function malesZetazOver():void {
	...
	outputText("It has a bubblegum-like taste that makes your tongue tingle as it passes into your belly, but the more pressing sensation of " + sMultiCockDesc() + " getting rock-hard lets you know exactly what you just drank.  Even though you just finished chugging down that foul drink, the imps uncork another pair of potions and dump them into the funnel.  The sweet fluids flood your mouth, and once again you swallow and chug rather than drown.   After you finish the last swallow, you pant, completely out of breath and getting hotter by the moment.  Your " + player.skinDesc + " tingles and sweats, growing more and more sensitive with every passing second while " + sMultiCockDesc() + " begins to drip and drool.\n", false);
	
	outputText("Zetaz hands the funnel to an underling with a knowing laugh and repositions himself over your " + player.legs() + ".  Warm pressure pushes at your " + assholeDescript() + ", forcing your clenching flesh to yield around the intruder.  Normally such an instant penetration would be irritating, or perhaps painful, but the sudden pressure on your prostate only serves to release a copious squirt of pre-cum.  An unwelcome moan slips past your lips and sends a titter of laughter through the mob.  As if losing wasn't bad enough – they all know you're getting off on having your " + assDescript() + " penetrated.  The worst part is that the humiliation is just making the situation hotter and " + sMultiCockDesc() + " harder.\n\n", false);
...
public function incubusDeal():void {
	...
	//[Next – to room]
	flags[ZETAZ_LAIR_DEMON_VENDOR_PRESENT] = 1;
	doNext(1);
}
...
public function incubusShop():void {
	spriteSelect(52);
	if(flags[NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER] == 1) {
		getBimboozeFromSean();
		return;
	}
	outputText("", true);
	...
	var bimbo:Number = 0;
	if(hasItem("BimboCh",1) && flags[NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER] == 0) outputText("\n\nSean could probably do something with the Bimbo Champagne if you had enough of it...");
	if(hasItem("BimboCh",5) && flags[NIAMH_SEAN_BREW_BIMBO_LIQUEUR_COUNTER] == 0) {
		bimbo = 3542;
...
public function valaCumBath2():void {
	...
	flags[VALA_CUMBATH_TIMES]++;
	stats(0,0,0,0,-1.5,0,-100,0);
	doNext(13);
}
...
public function valaBigYou():void {
	clearOutput();
	flags[TIMES_VALA_CONSENSUAL_BIG]++;
	//{FIRST TIME}
	if(flags[TIMES_VALA_CONSENSUAL_BIG] == 1) {
...
public function bigValaDomsPC():void {
	...
	outputText("\n\n\"<i>Lick,</i>\" comes the booming command, her voice quivering with delight and embarrassment.  \"<i>If you do a good job, I'll let you taste a pure pearl,</i>\" she giggles.");
	//{Male:}
	if(player.gender == 1) {
...
public function valaVapulaThreesome():void {
	//{First Meeting} 
	flags[TIMES_VAPULA_AND_GIANT_VALA]++;
	if(flags[TIMES_VAPULA_AND_GIANT_VALA] == 1) {
