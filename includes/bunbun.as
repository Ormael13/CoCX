...
function bunbunFucksYourVag():void {
	...
	if(player.vaginas[0].vaginalWetness >= 4) outputText("  Moisture leaks in a steady drizzle, mixing with bunny-pre to totally slick the soon-to-be invading member.", false);
	outputText("\n\n", false);
	...
	if(player.vaginas[0].vaginalWetness >= 3) outputText(", not that you needed it", false);
...
function bunbunFucksPCInAss():void {
	...
	if(player.hasVagina()) {
		if(player.vaginas[0].vaginalWetness >= 3) outputText("Juices begin to run from your " + vaginaDescript(0) + ", but you ignore the empty feeling and focus on the throbbing meat inside your backdoor.  ", false);
	}
...
function bunbun69():void {
	...
	if(player.cockTotal() > 0) {
		...
		if(isEaster()) {
			...
			if(player.tailType != 10) {
				//[No Tail: 
				if(player.tailType == 0) outputText("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as <b>a fluffy bunny's tail pops out of your back</b>, twitching excitedly!", false);
				//[Existing tail: 
		...
		//REGULAR SHOW
			...
			if(player.tailType != 10) {
				//[No Tail: 
				if(player.tailType == 0) outputText("  The heat around your lower back clutches at your tail bone and you feel a fluffy explosion swell around your butt cheeks as a fluffy bunny's tail pops out of your back, twitching excitedly!", false);
				//Existing tail: 
			...
			if(player.earType != 7) outputText("new ", false);
...
function ovipositBunnyEaster():void {
	...
	outputText(" claiming an incubator.  When she feels your groin pressing on the small of her back, the fluffy hare grows concerned, looking over her shoulder and asking, \"<i>You... you can't put eggs in me like THAT!</i>\"");
...
function freeHerOhGodWhyDidYouDoThis():void {
	...
	outputText("\n\nGently bending her forward at the waist, you examine the ropes around her wrists.  A simple slip knot designed to cinch tighter the harder she struggles against it.  The girl's tuffed tail twitches eagerly atop her supple rump, as she bobs up and down on her large bunny toes.  With a few firm tugs, you manage to undo the cords and loose her arms.  You almost expect the bunny-girl to make a grab for you, but she's so overcome by the need for release, she all but forgets her emancipator.  Breathlessly, the energetic hare seizes the constraining loop of metal and rolls it off her shaft with moaning delight, throwing it into the tall grasses triumphantly.");
...
function fuckTheEggBoundBun():void {
	...
	else outputText("savory, chocolate thighs with pale crème");
