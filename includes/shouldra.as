...
function shouldrattack():void {
	...
	if(monster.short != "Kiha" && player.hasPerk("Evade") >= 0 && rand(100) < 10) {
		outputText("The girl wades in for a swing, but you deftly evade the attack. She recovers quickly, spinning back at you.", false);
		return;
	}
	//("Misdirection"
...
//(lust attack 1) 
function shouldraLustAttack():void {
...
//(magic attack) 
function shouldraMagicLazers():void {
	var damage:Number = 0;
	damage = 20 + rand(10);
...
//Victory Scenarios
//Ghost Girl Defeated
function defeatDannyPhantom():void {
	spriteSelect(67);
	outputText("", true);
	//defeat by damage)
	if(monster.HP < 1) {
...
function littlePigLittlePigLetMeIn():void {
	...
	var gender:Number = 0;
	//Genderless Scene
	if(player.gender == 0) gender = 3219;
	else if(player.gender == 1) gender = 3220;
...
function slimeGinaFlation():void {
	...
	else if(player.tailType == 0) outputText("arms and " + player.legs(), false);
...
//(two days later)
function paladinModeFollowup():void {
	spriteSelect(67);
	outputText("\n", false);
	...
	if(this.model.time.hours < 6 || this.model.time.hours >= 23) outputText("\n\nYou rage fap and consider going back to bed.", false);
	stats(0,0,0,0,0,0,-100,0);
	doNext(1);
}
...
function slimeGhostGalForHerms(clearOut:Boolean = false):void {
	...
	if(this.model.time.hours < 11) outputText("morning");
	else if(this.model.time.hours < 17) outputText("afternoon");
