...
function multiCockDescript():String {
	...
	if(currCock == 2) {
		...
		if(same) {
			...
			//Tentacles
			if(player.cocks[0].cockType.Index > 2) descript += ", " + cockNoun(player.cocks[0].cockType) + "s";
		}
		//Nonidentical
		else {
			rando = rand(3);
...
function multiCockSuperLight():String {
	...
	if(player.horseCocks() == player.totalCocks()) output += cockNoun(CockTypesEnum.HORSE);
	else if(player.dogCocks() == player.totalCocks()) output += cockNoun(CockTypesEnum.DOG);
	else if(player.demonCocks() == player.totalCocks()) output += cockNoun(CockTypesEnum.DEMON);
	else if(player.tentacleCocks() == player.totalCocks()) output += cockNoun(CockTypesEnum.TENTACLE);
	else if(player.catCocks() == player.totalCocks()) output += cockNoun(CockTypesEnum.CAT);
	else if(player.lizardCocks() == player.totalCocks()) output += cockNoun(CockTypesEnum.LIZARD);
	else if(player.anemoneCocks() == player.totalCocks()) output += cockNoun(CockTypesEnum.ANEMONE);
	else if(player.kangaCocks() == player.totalCocks()) output += cockNoun(CockTypesEnum.KANGAROO);
	else output += cockNoun(CockTypesEnum.HUMAN);
	//Add s if plural
...
// BLARGH. THERE ARE TWO INSTANCES OF cockNoun. Because STUPID. 
// There is this one, and one in creature.as.
// they're both used. blarurgushtkalskdjha;lsjdhfaljdfha;sldf
// Only one had been updated to use CockTypesEnum, so shit was asploding all over the place.
// Ideally, we really, really should get rid of one of them entirely, but I don't know how the namespace
// stuff is working at the moment.
// therefore:
// TODO: FIX DIS SHIT
...
function cockDescript(cockNum:Number):String 
{
	...
	if(cockNum != 99) 
	{
		...
		{
			// trace("Cock type failed to match. ERROR");
		}
	}
	else 
		cockNum = 0;

	trace("Fell through the if statement. Wut?", cockNum);
...
function demonDescript(cockNum:Number):String 
{
	...
	// trace("CockNoun = ", cockNoun(player.cocks[cockNum].cockType), "cockType", player.cocks[cockNum].cockType);
	return descript;
}
