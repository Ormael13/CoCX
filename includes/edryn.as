...
function edrynBarTalk():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00072] == 1) {
		...
		if(flags[UNKNOWN_FLAG_NUMBER_00069] == 1) {
			...
			outputText("I can't believe we've had " + num2Text(flags[UNKNOWN_FLAG_NUMBER_00069]) + " kids together!  You better come see me later on - ", false);
			if(flags[UNKNOWN_FLAG_NUMBER_00069] < 3) outputText("I miss the way you fill me", false);
		...
		cheatTime(1);
		flags[UNKNOWN_FLAG_NUMBER_00072] = 0;
		doNext(barTelAdre);
		return;
	}
	//Mid-pregnancy talk
	...
	//Post kids talk
		...
		//(Too small)
			...
			menuLoc = 2;
			if(flags[UNKNOWN_FLAG_NUMBER_00073] < 2) {
				...
				shortName = "GroPlus";
				flags[UNKNOWN_FLAG_NUMBER_00073]++;
			}
			else if(flags[UNKNOWN_FLAG_NUMBER_00073] < 5) {
				outputText("Here, take some of this stuff.  We confiscated it off one of the miscreants we kicked out the other day, and I KNOW it'll get you to be big enough for me.  Just don't go too nuts with it, okay?</i>\"\n\n", false);
				flags[UNKNOWN_FLAG_NUMBER_00073]++;
	...
	if(player.lowerBody == 4 && player.totalCocks() > 0) {
		...
		if(player.cocks[0].cockType == CockTypesEnum.HUMAN) outputText("pours out of its sheath", false);
...
function fuckEdrynNonTaur():void {
	...
	if(player.cocks[x].cockType == CockTypesEnum.HUMAN) outputText("feeling your ring of prepuce slipping between her lips.  ", false);
...
function approachHelAtZeBitch():void {
	...
	simpleChoices("Edryn3Some",edryn,"Fox Girls",heliaPlusFoxyFluffs,"",0,"",0,"Leave",leaveHelInZeBitch);
}
...
function beAnAssholeToPregdryn():void {
	...
	flags[UNKNOWN_FLAG_NUMBER_00070] = 1;
...
function pregdrynOffer(cs:Boolean = true):void {
	...
	//VERY Pregnant Offer
	if(flags[EDRYN_PREGNANCY_INCUBATION] < 250) {
	...
	if(player.cockArea(x) >= 24 && player.cockArea(x) < 300) {
		...
		if(flags[UNKNOWN_FLAG_NUMBER_00069] == 0) outputText("Does my scent have an even stronger effect on you now", false);
...
function edrynPregChance():void {
	...
	if(preg) {
		flags[EDRYN_PREGNANCY_INCUBATION] = 500;
		flags[UNKNOWN_FLAG_NUMBER_00071] = 0;
	}
}
...
function edrynFucktroduction():void {
	...
	if(!player.isTaur()) {
		...
		if(player.cocks[0].cockType == CockTypesEnum.HUMAN) outputText("making your " + cockDescript(x) + " twitch and ache.  It floods your body with animalistic desires, overpowering any sense of propriety you might have once had.", false);
...
function eatEdrynPussyLikeABawss():void {
	...
	if(flags[TIMES_EATEN_EDRYN_PUSSY_RUT] == 0) outputText("smile tugging at the corners of her mouth at your bravado");
	...
	outputText(".  Her ");
	if(flags[EDRYN_PREGNANCY_INCUBATION] > 0) outputText("milky tits, swollen from her pregnancy,");
	...
	if(flags[EDRYN_PREGNANCY_INCUBATION] > 0) outputText(" as they drip milk everywhere");
...
function postEdrynEatOutRut():void {
	...
	edrynPregChance();
	edrynPregChance();
	flags[TIMES_EATEN_EDRYN_PUSSY_RUT]++;
	doNext(15);
}