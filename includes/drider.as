...
function driderEncounter():void {
	...
	startCombat(40);
	//First time
	if(flags[UNKNOWN_FLAG_NUMBER_00277] == 0) {
	...
	//repeat intro:
		...
		if(player.tailType > 0) outputText("tail ", false);
...
function driderAI():void {
	...
	else if(hasSpells() && player.hasStatusAffect("Web-Silence") < 0 && rand(4) == 0) {
		spiderSilence();
	}
...
function driderKiss():void {
	...
	//Get hit 2nd time) 
		...
		if(player.statusAffectv1("Drider Kiss") == 1) {
			...
			outputText("Again, the drider ties your mouth up in her syrupy lip-lock, seeming to bind your mouth as effectively as her webs bind your body.  Her sweet venom bubbles and froths at the corners of the oral embrace, dripping over her many-breasted bosom and your " + chestDesc() + ".", false);
			...
			if(player.hasVagina()) outputText("  Fem-cum dribbles down your " + player.legs() + " while your " + clitDescript() + " gets so hard you think it'll explode.", false);
...
function defeatDriderIntro():void {
	...
	var buttFuckBUTTFUCKBUTTTFUCKBUTTFUCK:Number = 0;
	if(player.cockThatFits(monster.analCapacity()) >= 0) buttFuckBUTTFUCKBUTTTFUCKBUTTFUCK = 3273;
	var vagFuck:Number = 0;
	var fuckSpinner:Number = 0;
	var careful:Number = 0;
	if(player.hasCock() && player.lust >= 33) {
		if(player.cockThatFits(monster.vaginalCapacity()) >= 0) {
			...
			vagFuck = 3014;
			if(player.tailType == 5 || hasItem("T.SSilk",1)) {
	...
	var rideCock:Number = 0;
	var rideOvi:Number = 0;
...
function loseToDrider():void {
	...
	//SELECT SCENE HERE
	var choice:Number = 0;
	choice = select[rand(select.length)];
...
function winVSDriderTakeItsCockInCunt():void {
	...
	if(player.tongueType > 0) outputText(" as the rest of its demonic length lashes out and wraps around her pillowy bosom", false);
...
function victoryVSDriderStickDickInSpinneret():void {
	...
	if(player.biggestTitSize() >= 3) outputText("  The semi-sticky stuff soaks into the cleavage of your " + allBreastsDescript() + ", the feeling surprisingly enjoyable.  You even knead the web in a bit, biting your bottom lip from the sensation.", false);
...
function loseVSDriderAndVaginallyImpreggleOvi():void {
	...
	if(!player.isTaur()) outputText("Her fingers then move to the sides of your head, lacing webbing through your hair and sticking it firmly to the ground. More vulnerable than ever, you can't lift your head without ripping your hair from your scalp in chunks.", false);
		...
		if(player.biggestTitSize() > 4) outputText("With pain and then a pop, your " + allBreastsDescript() + " squash out to the sides as she presses your torso into your knees.  ", false);
...
function driderPregVagBirth():void {
	...
	if(player.eyeType != 1) outputText("  Suddenly she flops forward, leaning against your eyebrows and causing you to cross your eyes in an attempt to focus on her.  She seems confused by the way your eyes are different to hers.", false);
