...
function timesFellatrixSucked():Number {
	return (flags[DOMINIKA_TIMES_HYPNO_BJ] + flags[DOMINIKA_TIMES_MULTICOCK_SLOBBERED] + flags[156] + flags[151] + flags[152] + flags[153] + flags[174]);
}
function fellatrixBarApproach():void {
	...
	//Req's d2 finished
	if(fellatrixSucked() && flags[150] > 0 && player.hasKeyItem("Zetaz's Map") >= 0 && flags[DOMINIKAS_SWORD_GIVEN] == 0) {
		dominikaSpellblade();
		return;
	}
...
function fellatrixSexWarmup():void {
	...
	//First time
	if(flags[150] == 2)	outputText("\"<i>I have not been unaffected by this land. I have certain... dietary needs.</i>\" She coughs slightly, glancing aside and looking as though she's doing her very best not to blush.\n\n", false);
	//Repeat
...
function acceptFellatrixOral():void {
	...
	//After 3+ blowjobs
	if(timesFellatrixSucked() >= 3 && flags[157] == 0) {
		if(flags[54] > 0) {
...
function dominikaBlowjobs2():void {
	...
	if(flags[157] == 1 || flags[157] == 4) outputText("At the same time, you get the sense that perhaps it's for the best you did so.", false);
