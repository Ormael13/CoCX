...
function fellatrixBarApproach():void {
	...
	//[First encounter]
	if(flags[UNKNOWN_FLAG_NUMBER_00150] == 0) {
		...
		outputText("Blushing slightly, you ask her about her own home. She leans back in her chair and gazes off for a moment before responding. \"<i>I come from... far away. Farther than most the people here know. Far enough that it's unlikely I'll be returning.</i>\" You hear a familiar longing in her voice, one that you have known on some dark nights, and feel a certain solidarity with her. She looks back at you and shrugs. You suspect that beneath the cloth she is giving you a half-smile. \"<i>Perhaps, if circumstances work in my favour, one day... I make the best of my situation here.</i>\"\n\n", false);
	...
	//Second encounter]
	else if(flags[UNKNOWN_FLAG_NUMBER_00150] == 1) {
		outputText("You make your way over to Dominika's table again, saying hello once more. The two of you talk about your exploits for a time, before you ask her what she has been up to. \"<i>Research, mostly,</i>\" she explains. \"<i>I try to document what this land has, in the hopes that it may be useful to someone one day. I do consider myself somewhat of a learned woman which... seems to be something of a rarity here, so there are few qualified to do the job.</i>\"\n\n", false);
...
//[\"<i>No thanks</i>\"]
function turnDownDominikasKnowledge():void {
	spriteSelect(10);
	outputText("", true);
...
function acceptDominikasKnowledge():void {
	...
	flags[UNKNOWN_FLAG_NUMBER_00150]++;
	if(flags[UNKNOWN_FLAG_NUMBER_00150] == 2) {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00155] == 0) {
		flags[UNKNOWN_FLAG_NUMBER_00155] = 7 + rand(3);
		...
		if(spellCount() == 6) {
			if(flags[UNKNOWN_FLAG_NUMBER_00154] == 0) {
				flags[UNKNOWN_FLAG_NUMBER_00154]++;
...
function fellatrixSexWarmup():void {
	...
	//First time
	if(flags[UNKNOWN_FLAG_NUMBER_00150] == 2)	outputText("\"<i>I have not been unaffected by this land. I have certain... dietary needs.</i>\" She coughs slightly, glancing aside and looking as though she's doing her very best not to blush.\n\n", false);
	//Repeat
...
//If \"<i>No.</i>\"]
function declineFellatrixOral():void {
	spriteSelect(58);
	outputText("", true);
	//No BJs yet.
	if(!fellatrixSucked()) {
...
//[If \"<i>No.</i>\" twice]
function declineFellatrixOralHARDCORE():void {
	spriteSelect(58);
	outputText("", true);
...
function declineFellatrixOralSUPERHARDCORE():void {
	spriteSelect(58);
	outputText("", true);
	...
	flags[UNKNOWN_FLAG_NUMBER_00150] = -1;
	cheatTime(1);
	doNext(barTelAdre);
}


function acceptFellatrixOral():void {
	spriteSelect(58);
	outputText("", true);
	var x:Number = player.cockThatFits(36);
	//[Random one-shot after Player has done a Scylla scene]
	//Mind control at 6th suck
	if(timesFellatrixSucked() >= 6 && timesFellatrixSucked() - 4 >= flags[DOMINIKA_LAST_HYPNO_SUCK_COUNT]) {
		dominikaBlowjobs();
		return;
	}
	//After 3+ blowjobs
	...
	if(player.hasVagina() && (player.gender == 2 || rand(2) == 0)) {
		if(flags[UNKNOWN_FLAG_NUMBER_00153] == 0) {
		...
		flags[UNKNOWN_FLAG_NUMBER_00153]++;
	}
	//MULTICAWWWWK
	else if(player.cockTotal() >= 2 && rand(2) == 0) {
		//[If \"<i>Yes</i>\" and multi-cock]
		if(flags[DOMINIKA_TIMES_MULTICOCK_SLOBBERED] == 0) {
		...
		flags[DOMINIKA_TIMES_MULTICOCK_SLOBBERED]++;
	}
	//[If \"<i>Yes</i>\" and penis small enough to suck]
		...
		if(player.hasKnot(x)) {
			// - first time dogsuck
			if(flags[UNKNOWN_FLAG_NUMBER_00156] == 0) {
		...
		//Horsecock suck
		else if(player.cocks[x].cockType == CockTypesEnum.HUMAN) {
			...
			flags[UNKNOWN_FLAG_NUMBER_00174]++;
		}
		//Human/fallback wang
		else {
			// - first time wangsuck
			if(flags[UNKNOWN_FLAG_NUMBER_00151] == 0) {
			...
			flags[UNKNOWN_FLAG_NUMBER_00151]++;
		}
	}
	//[If \"<i>Yes</i>\" and penis too big to suck]
	else {
		// - first time wangsuck
		if(flags[UNKNOWN_FLAG_NUMBER_00152] == 0) {
		...
		flags[UNKNOWN_FLAG_NUMBER_00152]++;
	}
	stats(0,0,0,0,-1,1,-100,0);
	
	if(timesFellatrixSucked() >= 4 && flags[UNKNOWN_FLAG_NUMBER_00175] == 0) doNext(dominidrama);
	else doNext(14);
}
...
//[Agree with her]
function agreeWithDominika():void {
	spriteSelect(58);
	outputText("", true);
	...
	flags[UNKNOWN_FLAG_NUMBER_00157] = 1;
	doNext(13);
}

//[Non-committal]
function ehhhhDominika():void {
	spriteSelect(58);
	outputText("", true);
	...
	flags[UNKNOWN_FLAG_NUMBER_00157] = 2;
	doNext(13);
}
//[Disagree with her]
function disagreeDominika():void {
	spriteSelect(58);
	outputText("", true);
	//(Corruption decrease)
	stats(0,0,0,0,0,0,0,-2);
...
function dominidramaYes():void {
	...
	flags[UNKNOWN_FLAG_NUMBER_00175] = 1;
	doNext(13);
}
//[No]
function dominidramaNo():void {
	outputText("", true);
	...
	flags[UNKNOWN_FLAG_NUMBER_00175] = 2;
	doNext(13);
}

//[Binding scene]
//Occurs after 8 servicings
function dominikaBlowjobs():void {
	outputText("", true);
	var x:Number = player.cockThatFits(36);
...
function dominikaBlowjobs2():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00157] == 1 || flags[UNKNOWN_FLAG_NUMBER_00157] == 4) outputText("At the same time, you get the sense that perhaps it's for the best you did so.", false);
