...
public function shielaXPThreeSexyTimePostSexStayII():void {
	...
	//(else if no cockFit 32 and no vag)
		...
		if(player.hasCock()) dynStats("lib", -2, "lus=", 0);
...
public function sheilaMutualMasturbation():void {
	...
	if(target == "cunt" || target == "cock" || target == "nipplecunt") dynStats("lus=", 0);
...
public function forcedSheilaOral(dick:Boolean = true):void {
	...
	if(dick) {
		...
		if(player.hasStatusAffect("hairdresser meeting") >= 0 || flags[kFLAGS.LUMI_MET] > 0) {
