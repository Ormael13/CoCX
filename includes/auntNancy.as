...
function auntNancy(display:Boolean = true):Boolean {
	if(this.model.time.hours >= 6 && this.model.time.hours <= 14) {
		if(display) {
			...
			//[If MetNancy = 0]
			if(flags[UNKNOWN_FLAG_NUMBER_00263] == 0) outputText("  She", false);
			else outputText("  Aunt Nancy", false);
			outputText(" glances up at you and smiles as you enter", false);
			//[(if unmet)
			if(flags[UNKNOWN_FLAG_NUMBER_00263] == 0) outputText(", and you see that she has additional red eyes spread across her forehead", false);
			outputText(".", false);
		}
		return true;
	}
...
function interactWithAuntNancy():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00264] >= 30 && this.model.time.hours == 14 && flags[UNKNOWN_FLAG_NUMBER_00266] == 0) {
	...
	else if(flags[UNKNOWN_FLAG_NUMBER_00264] >= 30 && this.model.time.hours >= (14 - (flags[UNKNOWN_FLAG_NUMBER_00264]/30)) && flags[UNKNOWN_FLAG_NUMBER_00266] > 0) {
	...
	//If Relationship with Nancy < 30
		...
		if(flags[UNKNOWN_FLAG_NUMBER_00263] < 1) flags[UNKNOWN_FLAG_NUMBER_00263] = 1;
		//[Choice: Strong, Light]
...
function strongStuff():void {
	...
	if(player.tou >= 30) {
		...
		flags[UNKNOWN_FLAG_NUMBER_00264] += 10;
	}
	//[Else]
	else {
		...
		flags[UNKNOWN_FLAG_NUMBER_00264] += 5;
	}
	doNext(13);
}
//[Light:]
function lightStuff():void {
	...
	//[If FirstTalkNancy = 0]
	if(flags[UNKNOWN_FLAG_NUMBER_00265] == 0) {
		flags[UNKNOWN_FLAG_NUMBER_00265]++;
		...
		*/
		...
		if(rand(2) == 0) stats(0,0,0,1+rand(2),0,0,0,0);
		flags[UNKNOWN_FLAG_NUMBER_00264] += 20;
	}
	//[Else:]
	else {
...
function auntNancyPoonPartII():void {
	...
	if(player.tailType > 0) outputText(" to the top of your tail", false);
	outputText(".\n\n", false);
	...
	//[If player has vagina but no dicks: (Nancy prefers dicks and will always choose them over lesbian sex)]
		...
		if(player.tailType > 0) outputText("tail before rising to your ", false);
...
function auntNancyPoonPartIV():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00266] == 0) {
	...
	flags[UNKNOWN_FLAG_NUMBER_00266] += 1;
	doNext(14);
}