...
public function fappingItems(menus:Boolean = true):Boolean {
	...
	if(player.hasKeyItem("Dual Belt") >= 0 && player.gender == 3 && !player.isTaur()) {
		if(menus) {
			addButton(button,"Dual Belt", dualBeltMasturbation);
			button++;
		}
		hasItems = true;
	}
	if(player.hasKeyItem("Fake Mare") >= 0 && player.hasCock() && player.isTaur()) {
		if(menus) {
			addButton(button, "Fake Mare", centaurDudesGetHorseAids);
			button++;
		}
		hasItems = true;
	}
	if(player.hasKeyItem("Centaur Pole") >= 0 && player.hasVagina() && player.isTaur()) {
		if(menus) {
			addButton(button, "C. Pole", centaurGirlsGetHorseAids);
			button++;
		}
		hasItems = true;
	}
...
public function onaholeUse():void
{
	...
	if (player.findStatusAffect(StatusAffects.PlainOnaholeUsed) < 0) {
		...
		if(player.cocks.length > 1)
			outputText("With your free hand, you slap your " + player.multiCockDescriptLight() + " to 'attention' and ease the onahole over your cocks.  ", false);
		else
	...
	if (player.cocks.length > 1 && player.gender == 3) {
		switch(rand(2)) {
			case 0: doNext(camp.onaholeMulticockContinuation); break;
			case 1: doNext(camp.onaholeFutaContinuation); break;
			default: trace("Onahole to-be-continued unexpected state..."); break;
		}
	}
	else if (player.cocks.length > 1) {
		doNext(camp.onaholeMulticockContinuation);
	}
	else if (player.gender == 3) {
		doNext(camp.onaholeFutaContinuation);
	}
	else {
		player.orgasm();
...
public function allNaturalOnaholeUse():void {
	...
	if (player.findStatusAffect(StatusAffects.AllNaturalOnaholeUsed) < 0) {
		...
		outputText("\n\nYou begin gently stroking yourself with the toy. You decide for a nice, leisurely pace over your usual hectic moods. The toy is warm and is very pleasurable. While hardly worthy of the sales pitch made by Giacomo, you feel that it was worth the money. If nothing else, it is different.", false);
		...
		outputText("\n\nYou have no clue how much time has passed. However, you wake up with your sex organs sore from the inside out. There are small dribbles of cum on the ground and you see the thing has swelled up to twice its size. Horrified, you reach to grab the thing to kill it--but stop. After you got over being penetrated, they were THE best orgasms you have ever had.", false);
	}
	//If player is already flagged, show repeated use text!
	else {
...
public function allNaturalStimBeltUse():void {
	...
	if(player.hasKeyItem("All-Natural Self-Stimulation Belt") >= 0) {
		...
		if(player.findStatusAffect(StatusAffects.UsedNaturalSelfStim) < 0) {
			...
			doNext(camp.allNaturalSelfStimulationBeltContinuation);
		}
		//Multiple uses
		else {
			//Low corruption variant
			...
			//High corruption variant
				...
				outputText("The creature lets out another torrent of cum and repeats the process. Drunk with lust, you are uncertain how you are containing so much spunk without it gushing out, as before. Every time you try to think about it, another orgasm destroys any attempt at rational thought. By the time the thing is done with you, hours and hours have already passed you by. You crash from your hours-long orgasm, exhausted, and can only think of the next opportunity to have the belt about your loins.", false);
				player.orgasm();
				dynStats("lib", -.5, "sen", 1, "cor", 1.5);
				//Game over if fully corrupt!
				if(player.cor >= 100) doNext(camp.allNaturalSelfStimulationBeltBadEnd);
...
public function masturbateJojo():void {
	...
	if(player.gender == 2 && player.biggestTitSize() < 2) {
		...
		if(selection == 3) {
			...
			if(temp == 2) outputText("You moan as he takes you at your word, spearing his tongue deep into your anus and thrusting it in and out as though it were a sleek muscled shaft, making your body tremble in pleasure. It makes you wonder where he learned such a trick in his life as a pious monk.  ", false);
	...
	if(player.gender == 3) {
		...
		if(selection == 1) {
			if(player.biggestTitSize() < 2) 
				...
				outputText("You lay yourself out for a quiet moment of self pleasure, your hands moving to your breasts and fondling them gently, when the sound of a snapping twig brings your attention to the edge of camp. Jojo stands timidly, half hidden within the shadows just outside your encampment, watching you with a look of submissive desire. You smile and lift your hand, beckoning him towards you with a crook of your finger.  ", false);
				outputText("Your mouse slut obediently slips from the darkness and into your camp, approaching you and kneeling at your side. You can see the lust in his eyes as he looks at your breasts, longing and love reflecting wonderfully. You nod your approval and let him worship your bosom.  ", false);
				if(player.biggestLactation() > 1) outputText("He leans in and starts to kiss along your nipples before taking one into his mouth. He gives a firm suckle at the engorged teat, and you can see his eyes open wider in surprise at the sudden surge of milk that fills his muzzle. He shivers and starts to suckle in earnest, drinking from first one breast, then the other, partaking of your blessing until his belly is full.  ", false);
				if(player.biggestLactation() <= 1 && player.biggestTitSize() <= 5) outputText("He leans in to nuzzle and kiss at your breasts, his hands moving to caress the soft and full orbs in gentle worship. His kissing and licking slowly circles in on your nipples, bringing them to firm points that send jolts of warm pleasure through your body when he at last takes them into his mouth. You reach down between your legs, taking hold of your shaft and masturbating it lazily as he works.  ", false);
		...
		if(selection == 3) {
			...
			if(player.vaginalCapacity() >= 10 && player.vaginalCapacity() < 36) outputText("You lower your hand to take hold of his cock, lining it up with your entrance, and then with a moan of pleasure, you lower your weight atop him. His cock slides into your pussy like a hand into a glove, fitting perfectly, as though he were made for you. You begin to rise and fall over him, setting a loving pace as you roll your hips. It doesn't last near as long as you would wish, however, as soon enough you can feel him cumming within your body, filling you with his seed. Not dissuaded, you grind at him, working your clit against his sheath and belly fur.  ", false);
			if(player.vaginalCapacity() >= 36) outputText("You shift forward, and then tilt your hips and drive back, taking his length into your wide stretched body. You laugh at him, barely able to feel his dick within you, and whisper into his ear, \"<i>Just like a mouse to be tiny...</i>\" You watch his blush as you start to grind and roll atop his cock and belly, taking all the pleasure that you can from your slut.  ", false);
			outputText("You cry out in pleasure as your orgasm floods through your body, causing your juices to splash out around your mouse slut's cock, and your own " + player.multiCockDescriptLight() + " to explode with thick splashes of your hot cum across his chest and belly.  ", false);
...
public function masturbateGo():void {
	...
	if(player.cocks.length > 0) {
		if(player.cocks.length == 1) {
			...
			if(player.lib >= 45 && player.lib < 70) outputText(" languidly, reveling at its near-constant hardness.  ", false);
	...
	//MULTICOCK
		...
		if(player.cocks.length == 2) {
			...
			outputText("You grasp one of your " + player.multiCockDescriptLight() + " in each hand, ", false);
			...
			if(player.averageCockLength() >= 20) {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				outputText("Your " + player.multiCockDescriptLight() + " brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ", false);
				//Try to stick it in a titty!
				if(player.hasFuckableNipples()) 
				{
					//check for dogcocks
		...
		if(player.cocks.length >= 3) {
			...
			if(player.averageCockLength() >= 20) {
				//autofellatio only gets to be true if you have any dicks left to suck on after sticking them in your boobs
				outputText("Your " + player.multiCockDescriptLight() + " brush against your face, throbbing with need.   You open your mouth and suck one of your pricks into your mouth, taking as much of it in as possible, running your tongue along the underside.  You smile and pop it free, then take a different shaft into your maw.  ", false);
				//Try to stick it in a titty!
				if(player.hasFuckableNipples()) 
				{
					//check for dogcocks
		...
		if(Math.round(player.cockArea(0)) == player.vaginalCapacity())
		{
			...
			if (player.averageVaginalLooseness() >= 4) {
				...
				case CockTypesEnum.TENTACLE:
					...
					break;
					default:
						outputText("tips of your bloated nipples wrap around the " + player.cockHead() + " of your " + cockDescript(0) + ", swallowing it like an enormous mouth.  ");
				}
/* Old method
		...
		if(Math.round(player.cockArea(0)) > player.vaginalCapacity())
		{
			...
			if (player.averageVaginalLooseness() >= 4) {
				...
				case CockTypesEnum.TENTACLE:
					...
					break;
					default:
						outputText("The swollen lips of your bloated nipple gape wide, but the " + player.cockHead() + " of your " + cockDescript(0) + " spreads them even wider.  ");
				}
/* Old method
...
{			
	...
	if(fittableCocks > 2 && holeTotal < fittableCocks) 
	{
		...
		if(player.vaginalCapacity() >= player.cockArea(randomCock)*2)
		{
			...
			outputText("The words \"multiple insertions\" drifts through your lust-fogged brain, and you begin to position two dicks against a poor, unsuspecting nipple, preparing to double-penetrate.  ", false);
			//does this give enough space?
			if(holeTotal*2 >= fittableCocks) outputText("You giggle with glee as you realize that you will be able to jam every single one of your " + player.multiCockDescriptLight() + " into your abused nipples, thanks to double-penetration! ", false);
	...
	if(Math.round(player.cockArea(randomCock)) < player.vaginalCapacity())
	{
		if(!doubleUp)
		{
			...
			//Different based on looseness
			if(player.averageVaginalLooseness() < 2) outputText("the small, over-stretched openings on your swollen nipples. Your hard rods penetrate your engorged nipples easily, sliding halfway in on your first thrust. ", false);
	...
	if(player.cocks.length > 0) 
	{
		...
		if(player.cocks.length > 1) {
			...
			if(nippleFuck) 
			{
				...
				if(player.cumQ() >= 500) outputText("With an explosive release, you finally cum.  Your orgasm never seems to end, and your world dissolves into the feelings from your " + player.multiCockDescriptLight() + " as they erupt jet after jet of cum into your abused tits.  With a series of wet *pops*, your cocks are forced free by the pressure. They continue to squirt semen everywhere, while rivers of cum pour out of your dilated nipples, pooling around you.  ", false);
			}

			//Doing the perverted sprinkler after dicks have popped out.
			outputText("Pleasure overwhelms your fragile mind as cum erupts from your " + player.multiCockDescriptLight() + ".  Your hips jerk in the air in time with your eruptions, cum spraying from you as your body does its best impression of a perverted sprinkler.  ", false);
			if(selfSucking) {
...
public function deluxeDildo():void {
	...
	if(player.hasVagina()) {
		...
		//(Pretty wet)
			...
			if(player.cocks.length > 0) {
				outputText("Even your " + player.multiCockDescript() + " ache", false);
...
public function centaurMasturbation():Boolean {
	...
	if(selector == 0) {
		...
		if(player.hasVagina() || player.hasCock()) {
			...
			if(player.hasCock()) {
				if(player.hasVagina()) outputText(player.multiCockDescriptLight() + " and " + vaginaDescript(0), false);
			...
			if(player.totalCocks() > 1) outputText("  You feel each of your " + player.multiCockDescriptLight() + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowments.", false);
			else if(player.totalCocks() == 1) outputText("  You feel your " + player.multiCockDescriptLight() + " tapping against your belly as your muscles clench, getting as much pleasure as possible from your male endowment.", false);
		...
		if(player.hasCock()) {
			...
			outputText("released from your " + player.multiCockDescriptLight() + " by the intense climax.", false);
	...
	//Huge dick jerk
		...
		outputText("You shrug out of your " + player.armorName + ", not that it does much to impede your " + player.multiCockDescriptLight() + " as a centaur.   ", false);
		...
		outputText("your " + player.multiCockDescriptLight() + " swells out underneath you, hanging down from your equine hindquarters as ", false);
		if(!plural) outputText("it grows full and hard.", false);
		else outputText("they grow full and hard.", false);
		if(plural) outputText("  After a moment they peek out from under your forelegs, proudly displaying your " + player.multiCockDescriptLight() + ".", false);
		else {
		...
		if(player.tallness * 1.5 >= player.cocks[player.longestCock()].cockLength) {
			...
			else if(player.totalCocks() > 2) outputText("  Your other " + player.multiCockDescriptLight() + " mimic their lucky brother's pleasure, even though they've been ignored in order for you to focus on your current 'toy'.", false);
		...
		//Cumjaculation
		if(player.tallness * 1.5 >= player.cocks[player.longestCock()].cockLength) 
			outputText("Squeezing tightly around your " + player.cockHead(), false);
...
public function catAutoLick():void {
	...
	outputText(" gems falls down upon your body instead of cum, bouncing off your " + player.skinFurScales() + " as you arc your back higher and higher until the only thing touching the ground is the top of your head and the tips of your toes! Your hips continue jerking in the air from the intense orgasm for a little while after the cum stops flowing.\n\n<b>You take a few moments to collect all the glittering gems you just squirted all over the place, all " + gemsCreated + " of them</b>, before curling up and taking a short cat nap.", false);
				...
				if (flags[kFLAGS.TIMES_AUTOFELLATIO_DUE_TO_CAT_FLEXABILITY] > 10 && player.balls > 1 && rand(5) == 0) {
					...
					outputText("\n\nAbrubtly you realize that your " + player.sMultiCockDesc() + " have soaked your torso in precum--apparently you've been on the edge for some time. In one swift motion, you pick your head up and slam your mouth down over your " + cockDescript(0) + " leaving your hands to continue their ball massage.", false);
...
private function dualBeltMasturbation():void {
	clearOutput();
	outputText("You look at the thing in front of you and sigh, your " + player.multiCockDescriptLight() + " and " + vaginaDescript(0) + " dripping with fluids. With a nervous sigh you step into the underwear looking device and slip it up your legs, the cold metal feeling so good on your " + player.skinDesc + ", sending a rush of feelings up your body. You slip your " + cockDescript(0) + " down and into a hole on the front of the belt, the extra length sticking out on the other side of it. Underneath the hole and right above your pussy is another metal square with what feels like a small nub on the inside rubbing against your clit. Shivering from the feeling of it, you stay there for a moment, waiting in anticipation. Finally, you reach down to the side of the belt and flick the switch to the on position.\n\n", false);
...
public function centaurDudesGetHorseAids():void {
	...
	outputText("Seeing the toy's exposed, gaping genitals, you feel a stirring in your " + player.multiCockDescriptLight() + ".  You yearn to touch yourself, but your centaur lower body prevents you, as usual.  Grunting with annoyance, you trot up to the toy and give its wide cunt an experimental fisting.  ", false);
	//[If small cock: 
...
public function tentacleSelfFuck():void {
	...
	//No bonus cawks!
		...
		if(y >= 0) {
			if(player.cocks[y].cockType == CockTypesEnum.TENTACLE && player.cocks[y].cockLength >= 20) outputText("  Luckily, you're so big down there that a juicy cock-head is right there within reach, and you bend to slurp it up without thought, busily self-sucking with reckless abandon.");
		}
	}
	//MORE
