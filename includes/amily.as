...
function FirstTimeAmilyTakeCharge():void {
	...
	if(player.cocks[0].cockLength >= 14) {
		...
		else if(player.cocks[0].cockType == 3) {
		...
		else if(player.cocks[0].cockType == 1) {
...
function fuckAmilyPreg():void {
	...
	outputText("Amily teases you for a little while, running her pussy-lips and tail along the tip of your erection a few times, earning a few moans and groans from you. Amily smirks slightly before sliding herself down your fully erect " + cockNoun(0) + ", taking as much as she can. You're a little worried that this might be harmful for your offspring, or worse that they'll know what's going on...But Amily really seems to know more about this than you do, so you're just going to go along with her suggestions on the matter.\n\n", false);
...
function giveAmilyPureIncubusDraft():void {
	...
	if(flags[AMILY_FOLLOWER] == 1) {
		...
		if(flags[AMILY_WANG_LENGTH] > 0) {
			...
			if(flags[AMILY_WANG_LENGTH] >= 13 && flags[AMILY_WANG_GIRTH] >= 2) {
...
function giveAmilyAWhiteEgg():void {
	...
	if(flags[AMILY_FOLLOWER] == 1) {
		...
		if(flags[AMILY_NIPPLE_LENGTH] < 4) {
			...
			if(flags[AMILY_NIPPLE_LENGTH] > 4) flags[AMILY_NIPPLE_LENGTH] = 4;
		}
...
function giveAmilyABrownEgg():void {
	...
	if(flags[AMILY_FOLLOWER] == 1) {
		...
		if(flags[AMILY_ASS_SIZE] < 16) {
			...
			if(flags[AMILY_ASS_SIZE] > 19) flags[AMILY_ASS_SIZE] = 19;
		}
