...
function defeatBasilisk():void {
	...
	var evil:Number = 0;
	var eggs:int = 0;
	if(player.canOvipositSpider()) eggs = 3836;
	if(player.canOvipositBee() && player.gender > 0) eggs = 3847;
	if(player.cockThatFits(monster.analCapacity()) >= 0 && player.cor >= 66) evil = 3013;
	//Player HP victory: 
...
function layBeeEggsInABasilisk():void {
	...
	if(player.isTaur() || player.gender == 0) outputText("\n\nYou drool messily on your hand and cup it to the basilisk's hole; he hisses in displeasure - though he must have had an inkling of what was coming, the visceral sensation of having his asshole lubed for entry is probably sending his imagination careening off in reckless, tormenting anticipation.");
		...
		outputText("; rubbing enthusiastically, you start at base but end in a gentle stroke at sensitive tip.  With you doing your best to milk as much pre as you can, the viscous ooze that dribbles out of your dong");
		if(player.totalCocks()) outputText("s");
