...
public function RoxanneAppearance():Number {
	//When she there?
	if(model.time.hours > 12 && model.time.hours < 19) {
		...
		if(flags[UNKNOWN_FLAG_NUMBER_00221] == 0) return 2876;
		else return 2877;
	}
	return 0;
}
...
public function RoxanneChooseApproachOrRepeat():void {
	spriteSelect(78);
	outputText("", true);
	if(flags[UNKNOWN_FLAG_NUMBER_00221] == 0) {
		flags[UNKNOWN_FLAG_NUMBER_00221]++;
		...
		outputText("I need to blow off some steam, " + player.short + ".</i>\"  She spreads her legs and pats at the left one, revealing a dangerously large bulge that would rival that of a minotaur.  \"<i>Oh, you hadn't noticed?  I'm not exactly a girl.  I didn't always look like this, but I foolishly bought a pill off a demon named Ceraph and have regretted it since.  Sure, the pill she gave me made my cock bigger, but it NEVER stops growing.  The only way I can shrink it back is to orgasm in someone's poop-deck, but it's tough finding some who can take it. Cinnabar won't even let me buy her services, though I think she's waiting for the fruit to ripen, so to speak,</i>\" she chortles while shifting the equally large shapes of her huge, twin testes.\n\n", false);
		outputText("\"<i>Now, I feel like getting drunk nearly as much as I feel like screwing.  Why don't we have a drinking contest?  If I win, I get to bury this bad-boy in your ass and cum till I'm normal.  If you win, I'll give you the best oral service a " + player.mf("guy","girl") + " can get!</i>\"\n\n", false);
	...
	else if(flags[UNKNOWN_FLAG_NUMBER_00222] + flags[UNKNOWN_FLAG_NUMBER_00223] <= 0) {
	...
	else if(flags[UNKNOWN_FLAG_NUMBER_00224] == 2 && flags[UNKNOWN_FLAG_NUMBER_00225] >= 200) {
	...
	else if(flags[UNKNOWN_FLAG_NUMBER_00224] == 1 && flags[UNKNOWN_FLAG_NUMBER_00225] < 200) {
	...
	flags[UNKNOWN_FLAG_NUMBER_00226] = 0;
...
public function roxanneDrinkingContest():void {
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00225] >= 200) outputText("stumbles over her huge manhood, working towards", false);
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00222] + flags[UNKNOWN_FLAG_NUMBER_00223] <= 0 || flags[UNKNOWN_FLAG_NUMBER_00224] == 2) {
		...
		if(flags[UNKNOWN_FLAG_NUMBER_00224] == 2) outputText(" after your last humiliation.", false);
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00222] + flags[UNKNOWN_FLAG_NUMBER_00223] < 3) outputText("hesitantly sniff at the brew, taking in its dark color and heady, hoppy aroma before", false);
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00226] == 0) {
		...
		if(flags[UNKNOWN_FLAG_NUMBER_00225] >= 200) outputText("massive, seam-ripping bulge", false);
	...
	if(flags[UNKNOWN_FLAG_NUMBER_00226] == 0) score = (player.tallness * ((player.thickness + 100)/200) * (player.tou/100)) + flags[UNKNOWN_FLAG_NUMBER_00227];
	//If score is less than 30-50 (Strahza is inconsistant!)
	//[Lose!] 
	if(score < (45 + rand(20))) {
		//Increment loss count!
		flags[UNKNOWN_FLAG_NUMBER_00223]++;
		//Set who won contast last
		flags[UNKNOWN_FLAG_NUMBER_00224] = 2;
		//Gain big bonus
		flags[UNKNOWN_FLAG_NUMBER_00227] += 10;
		...
		if(flags[UNKNOWN_FLAG_NUMBER_00226] == 0) outputText("Nooooo! You're cut off! That means Roxanne won...", false);
		...
		if(player.buttRating > 12 && player.tone <= 50 && flags[UNKNOWN_FLAG_NUMBER_00223] > 1 && rand(2) == 0) doNext(bigBootyRoxanneContestLoss);
		//TO huge or regular anal
		else if(flags[UNKNOWN_FLAG_NUMBER_00225] >= 200) doNext(roxanneFucksYourAssOHGODITSHUGE);
		else doNext(roxanneReamsYouNormal);
		//Reset roxanne's cock
		flags[UNKNOWN_FLAG_NUMBER_00225] = 1;
	}
	//[WIN]
	else {
		//Increment win count!
		flags[UNKNOWN_FLAG_NUMBER_00222]++;
		//Set who won contest last
		flags[UNKNOWN_FLAG_NUMBER_00224] = 1;
		//Gain small bonus
		flags[UNKNOWN_FLAG_NUMBER_00227] += 4;
...
public function roxanneGivesABlowjob():void {
	...
	outputText("A few demons start to approach, but the lizan crew interposes themselves between their captain and the interlopers, shooing them off before they can start something.  You barely notice, so focused are you on the feel of saliva running down your " + cockDescript(x) + " and the soft, pliant mouthflesh trying to wring the jizm from your shaft.   The warmth of orgasm starts to build in your loins, and you begin to pump at the tongue instinctively, hips rising up off the chair in spite of the sharp fingernail's warning.\n\n", false);
...
public function roxanneCunnilingus():void {
	...
	outputText(" against her.  She tightens her grip and speeds up her tongue's sensual massaging, nearly blacking you out while you writhe in her embrace.  Heat, pure, blissful heat, spreads through you, draining the strength from your uncontrollable twitches and leaving languid warmth in it's place.  ", false);
...
public function bigBootyRoxanneContestLoss():void {
	...
	outputText("  Roxanne grunts, \"<i>That's a good girl... [if (hasVagina = false) \"Oh, I know you don't have a pussy, but tonight, you're my big-bootied, cum-dumpster bitch!\" else \"Oh, you'll make such a great anal cum-dumpster for me.  Your poor pussy will be so cum-starved, but you'll be so stuffed with jizz that you'll look pregnant anyhow!\"]</i>\"");
