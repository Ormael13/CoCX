...
//			var matches:*;
	...
	public function setOutputText(text:String):void {
		...
		this.mainText.defaultTextFormat = fmt;
		if (fmtnew.bold != fmt.bold || fmtnew.italic != fmt.italic || fmtnew.underline != fmt.underline) {this.mainText.htmlText += " /!\\ UNCLOSED TAG DETECTED (When reporting this bug, give information on your previous actions. You can check text history by pressing [H] (Mobile version does not support this.). Please report this to the #scribes_corner channel!) /!\\ "
		}
		this.scrollBar.update();
	}
