	...
	public class Player extends Character {
		...
		*/	if (hasStatusEffect(StatusEffects.ChargeWeapon) && !isUsingTome()) {
		...
		public function updateRacialAndPerkBuffs():void{
			updateRacialCache();
			if (tallness>=80 && hasPerk(PerkLib.TitanicStrength)) statStore.replaceBuffObject({'str.mult':(0.01 * Math.round(tallness/2))}, 'Titanic Strength', { text: 'Titanic Strength' });
			if (tallness<80 && statStore.hasBuff('Titanic Strength')) statStore.removeBuffs('Titanic Strength');
			if (tallness<=30 && hasPerk(PerkLib.CondensedPower)) statStore.replaceBuffObject({'str.mult':(0.01 * ((110 - Math.round(tallness))*10))}, 'Condensed Power', { text: 'Condensed Power' });
			if (tallness>30 && statStore.hasBuff('Condensed Power')) statStore.removeBuffs('Condensed Power');
