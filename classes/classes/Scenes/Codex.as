package classes.Scenes 
{
	...
	public class Codex extends BaseContent
	{
		...
		public function setFlags():void {
			if (player.hasStatusAffect(StatusAffects.KnowsCharge) || player.hasStatusAffect(StatusAffects.KnowsBlind) || player.hasStatusAffect(StatusAffects.KnowsWhitefire) || player.hasStatusAffect(StatusAffects.KnowsArouse) || player.hasStatusAffect(StatusAffects.KnowsHeal) || player.hasStatusAffect(StatusAffects.KnowsMight)
			|| player.hasStatusAffect(StatusAffects.KnowsBlink) || player.hasStatusAffect(StatusAffects.KnowsBlizzard) || player.hasStatusAffect(StatusAffects.KnowsChargeA) || player.hasStatusAffect(StatusAffects.KnowsFireStorm) || player.hasStatusAffect(StatusAffects.KnowsIceRain) || player.hasStatusAffect(StatusAffects.KnowsIceSpike))
			{
		...
		private function codexEntryGolems():void {
			...
			outputText("Dummy ones are the simples one and usually created by golem makers apprentices or beginner golem makers. Their shared with other types golems innate abilities could be described as subpar at best compared to other two types. Despite this they're still formidable enemies for anyone on similar power level like them. In such cases best way is to deal with their master first as his death cut off magical power needed for those golems existence. In cases of meeting golem alone it's best to stay alert as master usually is somewhere near and in case of starting fight with golem can join the fray. Most golem makers use this type as training before been able to create true golems, as loyal servants performing 'all' types of services for it master or as cannon fodder to overwhelm enemy with sheer numbers.");
			headerSub("True Golems");
			outputText("True ones sometimes simply called as golems are almost as much often meet as dummy ones. All of their battle characteristics are whole class over their lesser brothers making them one of most tough enemies to deal with in direct fight. Their only one real weakness are still their masters but in this case such person not need to be near golem to control of support it with magical power. Due to higher intelligence master could give them more complicated commands that allow them to perform task for a many days after receiving order and during this time their supply themselves with energy gathered in their bodies (dummy golems also could store magical energy but it amount is puny small compared to true golems) or if their are in magical rich place drawn it directly from surrounding.");
