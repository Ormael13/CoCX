...
private function helFuckMenu(isAmbush:Boolean = false):void {
	...
	addButton(10, "DoublePen", dpHel).disableIf(cockV < 0 || cockA < 0,
		helFollower.helNofitMsg + "; " + helFollower.helAnalNofitMsg);
	...
	addButton(11, "Possess", helPossessionShitPoopCock).disableIf(player.hasPerk(PerkLib.Incorporeality),
		"Req. 'Incorporeality' perk from the ghost race!");
	addButton(2, "Tail Wank", helTailWank).disableIf(!player.hasCock(), "Req. a cock!");
	addButton(7, "GetTailPegged", helTailPegging);
	addButton(3, "CoilFuck", nagaCoilsUpHel).disableIf(!player.isNaga() || cockV < 0,
		"Req. to have Naga lower body; " + helFollower.helNofitMsg);
	addButton(8, "TailsInButts", nagaCoilsUpAnalNaga).disableIf(!player.isNaga(), "Req. to have Naga lower body!");
	addButton(4, "Mount Her", mountHel).disableIf(!player.isTaur() || cockV < 0,
		"Req. to have Taur lower body; " + helFollower.helNofitMsg);
	addButton(9, "Hanging69", helVaginaTaur69).disableIf(!player.isTaur() || !player.hasVagina(),
		"Req. to have Taur lower body and a vagina!");
