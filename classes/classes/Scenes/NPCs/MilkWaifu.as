...
private function superReductoUsage():void {
	...
	if(flags[kFLAGS.MILK_SIZE] == 0) {
		...
		if(isabellaFollowerScene.isabellaFollower()) {
			...
			if(player.hasStatusAffect(StatusAffects.CampMarble)) outputText(" and Marble");
			outputText(" pause");
		}
		else if(player.hasStatusAffect(StatusAffects.CampMarble)) outputText(", enough to give Marble pause");
...
private function milkBathTime():void {
	...
	if(player.hasStatusAffect(StatusAffects.PureCampJojo)) count++;
