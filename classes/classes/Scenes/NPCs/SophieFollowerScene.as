...
private function letDebimboSophieGo():void {
	...
	if(player.lib >= 50) {
		...
		if(player.cor < 40) {
			...
			outputText("\n\nSuddenly, the minotaur pounding Sophie's face emits a loud bellow, thick spongy equine shaft widening apart her already tortuously stretched, yellow lips with thick pulses of cum that race towards a fully-widened flared head.  It blocks any hope of air reaching Sophie's aching lungs, a bestial flesh cone pushed directly into her stomach, dripping, and cum-tube pointed towards its deepest, hungry depths.  As hot cum emerges from the equine fleshcork, Sophie's blue eyes roll back in her head, completely overwhelmed by the minotaur's aphrodisiac ball juice.  She doesn't feel pain from each progressively thicker ribbed-shaft layer scraping past her teeth, forcing her jaw wider and wider, or from the minotaur's flared head pulling up against her narrow esophagus, trying to escape.  The minotaur gives in fully to his victory orgasm, hilting all two-feet of his cock down Sophie's throat, her nose pressing firmly against his animalistic crotch, saliva frothing over the minotaur's rapidly deflating black orbs.  Sophie responds immediately to the minotaur jism bulging and doming her increasingly shiny stomach, her battered cock holders flexing powerfully as a masochistic orgasm wracks her, pounding her into the ground with great force and raising her exposed, tailfeathered bubble-butt high to the musk-filled air for deeper penetration.  The thick, darkly ribbed minotaur meat moving in and out of her tight butthole and squishy pussy produce more squeezing shudders, further teasing the cocks inside her as she's orgasming on them.");
...
public function followerSophieMainScreen():void {
	...
	menu();
	addButton(0, "Appearance", sophieAppearance);
	addButton(1, "Sex", sexWithFollowerSophie)
		.disableIf(player.lust < 33, "You aren't aroused enough to sleep with Sophie at the moment.");
	if (flags[kFLAGS.SOPHIE_DAUGHTER_MATURITY_COUNTER] > 0) addButton(7, "Daughter", sophieBimbo.daughterCheckup);
	if (flags[kFLAGS.FOLLOWER_AT_FARM_SOPHIE]) {
		...
		} else addButton(3, "Harvest Eggs", harvestEggs);
		...
		if (flags[kFLAGS.FARM_CORRUPTION_STARTED]) addButton(2, "Farm Work", sendToFarm);
		if (flags[kFLAGS.SOPHIE_BROACHED_SLEEP_WITH] == 1) {
			if (flags[kFLAGS.SLEEP_WITH] != "Sophie") addButton(8, "Sleep With", sleepWithSophieToggle);
			else {
				outputText("\n\nYou're currently sharing your bed with Sophie at night, but you could kick her out, if you wanted.");
				addButton(8, "NoSleepWith", sleepWithSophieToggle);
			}
...
private function sexWithFollowerSophie():void {
	...
	if (!(flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] > 0 && flags[kFLAGS.SOPHIE_FAMILY_INCEST] == 0))
		addButton(4, "DaughterFuck", sophieIncest)
			.disableIf(!sophieBimbo.cockFits, sophieBimbo.nofitMsg)
			.disableIf(flags[kFLAGS.SOPHIE_ADULT_KID_COUNT] == 0, "This scene requires some actions when Sophie's... not so smart.\n\n", "???");
...
private function sleepWithSophieToggle():void {
	...
	if(flags[kFLAGS.SLEEP_WITH] != "Sophie") {
		...
		if(flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] == 0) outputText("Well, it is your bed.  Let me know if you want me coming back for snuggles, okay?");
...
private function sophieFollowerGivesBlowjobs():void {
	...
	if(flags[kFLAGS.SOPHIE_BIMBO_ACCEPTED] > 0) outputText("You may have given me a bimbo's body and an insatiable need for sex,");
...
internal function sophieIncest():void {
	...
	sceneHunter.print("You have "+daughterCap+" daughters. Up to 4 can be picked randomly.");
	...
	//second daughter: 
		...
		outputText("\n\nYou bite your lip at her erotic display, the sight of your own daughter trying so hard to entice you into an incestuous romp.  The growing desire to grab your girl and fuck her right then and there only grows stronger when you smell her.  The hungry harpy fans her long feathers to blow her hot and heady scent to you, letting you smell her arousal, the scent of a ripe and wanting female, ready to be bred.  ");
