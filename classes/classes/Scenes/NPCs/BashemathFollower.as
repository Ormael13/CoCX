...
public function repeatEnc():void
{
	...
	/*if (player.hasStatusEffect(StatusEffects.WildMalicore)) */startCombat(new Malicore());
	//else startCombat(new Bashemath());
	doNext(playerMenu);
}
public function repeatEncWM():void {
	player.createStatusEffect(StatusEffects.WildMalicore, 0, 0, 0, 0);
	repeatEnc();
}
