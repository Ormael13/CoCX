...
private function slimeGinaFlation():void {
	...
	if(player.hasStatusAffect("meet wanderer") >= 0) outputText("both of Markus's testicles, combined!", false);
...
private function slimeyPenorsAhoy():void {
	...
	if(player.balls > 0) outputText("balls to get a-churn", false);
...
private function penisLossThatIsntALoss():void {
	...
	else if(player.inte < 90 || player.cockThatFits(80) == -1) outputText("You begin to gain an edge, but the ghost girl is craftier than you expected. She begins to pulse mental intrusions at you, scrambling your thoughts just long enough for her to overtake you and push you to the side.  \"<i>You are one hundred years early to challenge me,</i>\" she chides. \"<i>My intellect has long surpassed most any living creature on this plane.</i>\"", false);
	else {
...
private function ghostGinaWinSexings():void {
	if(player.statusAffectv1("Exgartuan") == 2 && rand(2) == 0) {
...
internal function loseToShouldra():void {
	...
	if(player.gender == 1) {
		if(player.hasStatusAffect("infested") >= 0) loseToShouldraWithWorms();
		else if(player.statusAffectv1("Exgartuan") == 1) ghostGartuanLossSceneOrSomeShit();
		...
		if(player.statusAffectv1("Exgartuan") == 1) ghostGartuanLossSceneOrSomeShit();
		else if(player.biggestCockArea() >= 200) shouldraGiantCockLoss();
