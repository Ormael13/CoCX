...
private function littlePigLittlePigLetMeIn():void {
	...
	if(player.gender > 0 && ((player.hasStatusAffect(StatusAffects.BoatDiscovery) && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00230] > 0) || flags[kFLAGS.TIMES_MET_OOZE] > 0)) {
...
private function nowOnVickiLake():void {
	...
	if(player.hasStatusAffect(StatusAffects.BoatDiscovery) && flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00230] > 0)
...
private function slimeGinaFlation():void {
	...
	if(player.hasStatusAffect(StatusAffects.MeetWanderer)) outputText("both of Markus's testicles, combined!", false);
...
internal function loseToShouldra():void {
	...
	if(player.gender == 1) {
		if(player.hasStatusAffect(StatusAffects.Infested)) loseToShouldraWithWorms();
