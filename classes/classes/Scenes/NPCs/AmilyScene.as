...
public class AmilyScene extends NPCAwareContent implements TimeAwareInterface
	{
		...
		public function exploreVillageRuin():void {
			...
			if ((flags[kFLAGS.SHOULDRA_MAIDEN_COUNTDOWN] == 0 && rackCount() >= 2 && rand(10) <= 4) && !followerShouldra() && !player.hasStatusEffect(StatusEffects.ShouldraOff) && flags[kFLAGS.SHOULDRA_FOLLOWER_STATE] != .5 && !player.hasPerk(PerkLib.Ethereal)) {
			...
			if (flags[kFLAGS.AMILY_CORRUPT_FLIPOUT] > 0 && player.cor > 25 + player.corruptionTolerance) {
				//Cook amily a snack if player doesn't have key item for it.
		...
		private function talkToAmily():void {
			...
			//[Medium Affection]
				...
				outputText("\"<i>Of course, [name], I always enjoy our talks.  What shall we discuss this time?</i>\" she asks happily.\n\n");
			}
			//[High Affection]
			else {
		...
		private function thisIsAReallyShittyBadEnd():void {
			...
			outputText("Except for today. Amily walks out to confront you casually. \"<i>Ah, [name], there you are,</i>\" she states. \"<i>Good. I was hoping to see you one last time; I thought you at least deserved a goodbye.</i>\"\n\n");
			...
			outputText("\"<i>This is goodbye, [name]. You may not have been the most noble of [men]... but you did help me in my quest, and I am grateful – no matter how selfish your reasons may have been.</i>\"\n\n");
		...
		private function thisFunctionProbablySucksTooOhYeahAmilyFunction():void {
			...
			outputText("\"<i>[name]! Darling! You're here!</i>\"\n\n");
			...
			if(flags[kFLAGS.AMILY_MET_AS] == 1) outputText("When we first met, you were just sperm to me, a pure male to father the children I needed. ");
		...
		public function amilySexHappens():void {
			...
			//Moderate Affection Sex:
				...
				if (pregEvent >= 1 && pregEvent <= 5) outputText("  However, she does sometimes touch the swell signifying the litter growing inside her, and when she does her attitude becomes uncertain and nervous.");
				outputText("\n\n");
				...
				if (pregEvent >= 6) outputText("  However, her confidence visibly slips when she has to fully bare the bulging belly that marks her pregnant state, but she musters the confidence and starts to show it off for you as well.");
		...
		private function amilySexBusiness():void {
			clearOutput();
			amilySprite();
			outputText("Allowing Amily to take care of her clothes, you hastily remove your own [armor]. Once the two of you are naked in front of each other, Amily looks you up and down, and then sniffs - not in disdain, but honestly trying to get a good scent of you. You speculate that this is some kind of check to see that you haven't somehow managed to become corrupted since last you met.\n\n");
			amilySexPtII();
		}
		...
		private function amilyEnjoyShow():void {
			...
			if (pregnancy.event >= 6) outputText("  Even though she was clearly a little nervous about her gravid state in the beginning, as she continues, she grows in confidence to the point it seems she has almost forgotten about it.");
			outputText("\n\n");
			AmilyMidSexLevel2();
		}
		...
		private function StripForAmilyYouSlut():void {
			...
			amilySprite();
			var x:Number = player.cockThatFits(61);
			outputText("It is your turn to give her a mischievous smile back. Feeling turned on and excited, and remembering the elders in the village telling you that fair is only fair, you decide to give her a little show of her own. Standing up, you tilt your head back and thrust out your chest, trying to look enticing. As Amily watches, at first bemused and then pleased, you slowly strip off your [armor], working hard to make it as sensual and suggestive as possible. You show off your body for her, leisurely stroking your own limbs and down your midriff to finally reveal that which lies inside your pants; your " + cockDescript(x) + ". Amily is definitely appreciative of the show.\n\n");
		...
		private function getDownWithSexTiem():void {
			...
			amilySprite();
			var x:Number = player.cockThatFits(61);
			outputText("Too horny to think of anything else than what lies ahead, you hastily remove your [armor].  Amily smiles at what she can see, enjoying the sight of your body and your " + cockDescript(x) + ".\n\n");
			continueWithMoreMidLevelAmilySex();
		}
		...
		private function amilyHighAffectionSecks():void {
			...
			if (pregnancy.isPregnant) outputText("  Even in a slightly pregnant state, she goes surprisingly fast, though she's also rather cautious of her small bump.");
			outputText("\n\n");
			...
			if (pregnancy.isPregnant) outputText("  Really it seems the only thing between you two now is Amily's small stomach bulge.");
			//(If Amily is herm:
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  You can feel her erection, hot and solid, pressed between your two bodies.");
			outputText("\n\n");
			...
			if(flags[kFLAGS.AMILY_WANG_LENGTH] == 0) outputText("sucking on her sensitive clit");
			...
			if (pregnancy.isPregnant) outputText("  As you move your head across her beautiful form, you stop at her growing baby bump and give it a small kiss.");
			outputText("  Your head hovers at her breasts");
			if (pregnancy.isPregnant) outputText(", which seem to have grown from the pregnancy,");
			outputText(" and get to work licking and suckling her nipples, rubbing the sensitive mounds.");
			if (pregnancy.isPregnant) outputText("  When you get a dribble of milk in your mouth it surprises you, but you certainly don't stop.  \"<i>Hah...You're going to have to teach the children how that's done,</i>\" Amily says, in between fevered breaths.");
			outputText("\n\n");
			...
			outputText("As an apology you quickly push your " + cockDescript(x) + " past her dampened nether-lips, and set to work thrusting in and out of your mousegirl lover.");
			if (pregnancy.isPregnant) outputText("  You are of course mindful of her baby bump, not going too fast and making sure you're in a position that's comfortable for the two of you, not wanting to harm your future offspring with the lovely mouse maiden.");
			...
			if (!pregnancy.isPregnant) outputText("Fill me up with everything you have... I want to be a mother for your children, just as much as I want to be a mother of my own people,");
			...
			//(if Amily is herm:
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" and cum spurts into the air between you, splattering on you both");
			outputText(".\n\n");

			if (!pregnancy.isPregnant) outputText("\"<i>If that didn't knock me up... I don't care as much as you'd think. It was magnificent either way,");
		...
		private function fuckAmilyPreg():void {
			amilySprite();
			clearOutput();
			outputText(images.showImage("amily-forest-fuckpreg"));
			...
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  The erection tenting her pants isn't helping.");
			outputText("\n\n");
		...
		public function amilyFollowerEncounter():void {
			...
			switch (pregnancy.event) {
				case 2: if (flags[kFLAGS.AMILY_FOLLOWER] == 1) { //Pure
							...
							outputText("You notice that Amily seems to be ill. Despite that, she seems to be happy about something. You wonder what could be going on, and decide to ask; Amily grins at you. \"<i>Oh, [master]! You did it! You're going to be a father... I can't wait to birth many more mouse-sluts for you, [master].</i>\"\n\n");
						}
						break;
				case 3:
		...
		public function amilyMenu(output:Boolean = true) :void {
			...
			if(flags[kFLAGS.AMILY_FOLLOWER] == 1) {
				...
				addButton(6, "Lay Eggs", layEggsInAmily)
					...
					.disableIf(!flags[kFLAGS.AMILY_OVIPOSITION_UNLOCKED], "She's not ready yet.")
				...
				if(player.gender > 0 && flags[kFLAGS.AMILY_FOLLOWER] == 1 && flags[kFLAGS.AMILY_VISITING_URTA] == 0)
				{
				...
				/*}
				if (AbandonedTownRebuilt.InTown = true) {
					addButton(14, "Leave", AbandonedTownRebuilt.EnterTown);
				}*/
			}
			//Corrupt
			else {
				...
				addButton(2, "Sex", fuckTheMouseBitch).disableIf(player.lust < 33, "Not aroused enough.");
		...
		private function amilyCorruptSexMenu():void {
			amilySprite();
			if(player.gender > 0) {
				outputText("Amily asks, \"<i>How would [master] like to use " + player.mf("his","her") + " cum-bucket today?</i>\"");
				menu();
				addButton(0, "Anal", corruptAmilyBuckFutter)
					.hint("Fuck Amily in the ass!")
					.disableIf(!player.hasCock(), "Req. a cock!");
				addButton(1, "Get BJ", corruptAmilyOralSuckOff)
					.hint("Have Amily suck you off.", "Get Blowjob")
					.disableIf(!player.hasCock(), "Req. a cock!");
				addButton(5, "Vagina", corruptAmilysPussyGetsMotherfuckingFucked)
					.hint("Fuck Amily the classic way by penetrating her vaginally.")
					.disableIf(!player.hasCock(), "Req. a cock!");
				addButton(2, "Get Licked", corruptAmilyLickPussiesLikeAPro)
					.hint("Have Amily lick your pussy.")
					.disableIf(!player.hasVagina(), "Req. a vagina!");
				addButton(3, "GetPen'ed", corruptAmilyCampBonesPCWithHerCock)
					.hint("Have Amily put her cock to good use by penetrating you vaginally.", "Get Penetrated")
					.disableIf(!player.hasVagina(), "Req. a vagina!")
					.disableIf(flags[kFLAGS.AMILY_WANG_LENGTH] == 0, "Amily doesn't have a cock to penetrate you with.");
				...
				outputText("Eagerly, you strip off your [armor] and present yourself to your admiring slut-mouse... but when her eyes fall upon the bare expanse of flesh that is your crotch, she lets out a cry of horror. \"<i>Nothing! There's nothing there! [Master], what's happened to your genitals?</i>\" she squeals in dismay, rushing forward on her hands and knees and slamming her face roughly into your crotch.\n\n");
		...
		private function amilyAppearance():void {
			...
			if (flags[kFLAGS.AMILY_FOLLOWER] == 1) {
				//(Start [horsecock]
				outputText("Amily is a 5' 2\" tall "+what+", with a lean and wiry build. Her pink eyes normally twinkle merrily, but they can turn hard and cold if the circumstances warrant, just as the normally friendly grin "+onHerMuzzle+"can turn cruel and harsh when she is angry. "+furDesc+" a long, hairless mouse's tail that sways and twitches constantly from her behind. She is currently wearing " + clothes + ". She has " + amilyHips() + " and a " + amilyButt() + ".\n\n");
				//(End [horsecock]
				...
				outputText("Amily has a ");
				if(flags[kFLAGS.AMILY_VAGINAL_WETNESS] == 2) outputText("moist, ");
				if(flags[kFLAGS.AMILY_VAGINAL_WETNESS] == 3) outputText("wet, ");
				if(flags[kFLAGS.AMILY_VAGINAL_WETNESS] == 4) outputText("dripping, ");
				if (flags[kFLAGS.AMILY_VAGINAL_WETNESS] >= 5) outputText("soaked, ");
				// [Horsecock]
		...
		public function talkToAmilyCamp():void {
			...
			//if (AbandonedTownRebuilt.InTown = true) outputText("A few of your children watch, occasionally joining the conversation. Amily seems to enjoy the additional interaction.\n\n");
		...
		private function fuckTheMouseBitch():void {
			...
			if(flags[kFLAGS.AMILY_WAIT_FOR_PC_FIX_JOJO] > 0) {
				outputText("Amily pushes you away and says, \"<i>Not until we fix Jojo.</i>\"  You sigh and grumble.  No sex today!");
				doNext(amilyFollowerEncounter);
				return;
			}
			...
			outputText("She strides over to take you in her arms, \"<i>So, what would you like to do, [name]?</i>\"\n\n");
			menu();
			...
			if (flags[kFLAGS.AMILY_X_IZMA_POTION_3SOME] == 0) {
				var izmaHint:String = "";
				if (izmaFollower()) {
					if (flags[kFLAGS.AMILY_ALLOWS_FERTILITY] == 0) izmaHint += "Asking her to stop taking herbs ";
					else izmaHint += "Asking her to start taking herbs, and then stop AGAIN ";
					if (!izmaScene.pregnancy.isPregnant && !pregnancy.isPregnant) izmaHint += "right now";
					else izmaHint += "when neither she nor Izma is pregnant";
					izmaHint += " could lead to VERY interesting results.";
				} else izmaHint += "A new half-hidden option will be available when you get a specific follower.";
				sceneHunter.print(izmaHint);
			}
            ...
            if (flags[kFLAGS.AMILY_VISITING_URTA] == 4 && flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] >= 0)
				addButton(3, "Urta", SceneLib.followerInteractions.amilyUrtaSex)
					.hint("Take Amily for a visit to Urta in Tel'Adre for some threesome sexy times.")
					.disableIf(SceneLib.urtaQuest.urtaBusy(), "Urta is not available right now.");
            if (flags[kFLAGS.AMILY_OWNS_BIKINI] > 0) addButton(4, "Swim", amilySwimFuckIntro)
				.hint("What's a better pleasure than to take Amily for a swim and do some fuck?")
		...
		private function amilyTakesChargeSex():void {
			...
			menu();
			addButton(0, "Fuck", takeChargeAmilyFuck).hint("Penetrate Amily vaginally.")
				.disableIf(player.cockThatFits(61) < 0, "Req. a cock fitting 61 area.");
			addButton(5, "Buttfuck", fuckPureAmilysHeiny).hint("Take Amily from behind.")
				.disableIf(player.cockThatFits(50) < 0, "Req. a cock fitting 50 area.");
			addButton(10, "Get Sucked", takeChargeAmilyGetSucked).hint("Have Amily suck you off.")
				.disableIf(!player.hasCock(), "Req. a cock.");
			addButton(1, "Scissor", takeChargeAmilyScissorMeTimbers)
				.hint("Get into some girly fun-times by rubbing your girl-bits against Amily's.")
				.disableIf(!player.hasVagina(), "Req. a vagina.");
			addButton(6, "Eat Out", takeChargeAmilyEatOut).hint("Get a taste of Amily's pussy.");
			addButton(11, "Drink Milk", takeChargeAmilyMouseMilk).hint("Drink some of Amily's yummy mouse-milk.")
				.disableIf(flags[kFLAGS.AMILY_LACTATION_RATE] < 1, "She doesn't lactate enough.")
				.disableIf(flags[kFLAGS.AMILY_BIRTH_TOTAL] == 0, "She needs to birth first to start lactating.");
			addButton(2, "Mount Her", takeChargeAmilyMountHer).hint("Ride Amily's cock until she cums!")
				.hint("Get into some girly fun-times by rubbing your girl-bits against Amily's.")
		...
		private function takeChargeAmilyFuck():void{
			...
			var x:Number = player.cockThatFits(61);
			clearOutput();
			outputText("With a smile, you gently place the tip of a finger on her nose, then slowly run it down along her body, over her lip, between her breasts, across her stomach, and finally stopping between her legs, where you playfully circle her secret spot with the tip of your finger.");
			//(if Amily is herm:
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  Her pants begin to tent as her male organ reacts, and you start to idly stroke it as well for emphasis.");
			outputText("\n\n");

			outputText("Amily giggles, and then starts to wriggle out of her pants, shaking her " + amilyHips() + " and giving you a chance to grab hold of her " + amilyButt() + " as she does so. Soon, she's naked from the waist down, her little pink netherlips already winking at you");
			//(if Amily is herm:
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" and her " + amilyCock() + " standing erect");
			...
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" her " + amilyCock() + " spasming and spraying futa-jism in between your bodies");
			outputText(".\n\n");
			...
			amilyPreggoChance();
			/*if (AbandonedTownRebuilt.InTown = false) */doNext(camp.campLoversMenu);
		...
		private function takeChargeAmilyEatOut():void {
			...
			outputText("She's obviously expecting you to strip down, and is surprised when you instead flop down and wriggle backwards, looking a little disconcerted as she realizes just what you have in mind. You are staring at her vagina");
			//(if herm:
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(", and her " + amilyCock() + " is sticking up where her clitoris should be.  Y");
			else outputText(", and y");
			outputText("ou reach out with your tongue to give it a probing lick. Amily squeaks loudly at the sensation and, emboldened, you start to lick more enthusiastically, running your tongue around each of her netherlips, reaching up to tickle ");
			if(flags[kFLAGS.AMILY_WANG_LENGTH] == 0) outputText("her clitoris.");
			else outputText("the base of her cock.");
			outputText("\n\n");
			...
			//(if herm:
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" as her penis erupts and sprays its own cum into the air to sprinkle both of you");
			outputText(".\n\n");
			...
			player.sexReward("vaginalFluids");
			//if (AbandonedTownRebuilt.InTown = false)if (AbandonedTownRebuilt.InTown = false) {
			doNext(camp.campLoversMenu);
			//if (AbandonedTownRebuilt.InTown = true) doNext(AbandonedTownRebuilt.EnterTown);
		...
		private function takeChargeAmilyGetSucked():void {
			...
			outputText("You try to gasp a warning to Amily that you are about to cum, but all that emerges from your throat is a hollow groan.  Even as cum pours from your cock into her waiting mouth, ");
			if(player.cumQ() < 50) outputText("she swallows it all without skipping a beat");
			else if(player.cumQ() < 500) outputText("her cheeks bulge but she manages to swallow it all");
			else outputText("her cheeks bulge hugely and, though she swallows valiantly (her stomach bulges from the amount she takes), a lot of it spurts back from between her lips, drizzling onto your crotch and her " + amilyTits());
			outputText(".  Finally, you finish, and she releases your [cock] with a wet \"pop\" noise.\n\n");
		...
		private function takeChargeAmilyMountHer():void {
			...
			//Otherwise:
			else outputText("You grin lustfully as you feel it filling you. Hungry for more, you start to rise and fall, letting it pierce you as deeply as possible.");
			outputText("\n\n");

			outputText("Amily gasps in delight at the sensation of being surrounded by your " + vaginaDescript() + ". \"<i>Oh, yes, [name], please!</i>\" she cries, clearly happy.\n\n");

			//(If player is herm with "self-titfuck" cock:
			if(player.longestCockLength() >= 18) {
				outputText("Amily grabs hold of your " + cockDescript(player.longestCock()) + ", eagerly cramming it between her " + amilyTits() + ", mashing them together and running them up and down against your " + cockDescript(player.longestCock()) + ", surrounding it with warm, soft, velvety flesh.");
				//(If player has "autofellatio" cock:
				if(player.longestCockLength() >= 22) outputText("  She then starts to lick and kiss the tip, sucking on it, playing with its slit with her tongue, smothering it with oral affection.");
				outputText("\n\n");
			}
			//(If player is herm:
			...
			outputText("\"<i>Flatterer.</i>\" Is all that she says, but she's smiling happily, even as you both clean up and go your separate ways again.");
			//PREGGO CHECK HERE
		...
		private function letAmilyLead():void {
			amilySprite();
			clearOutput();
			outputText("Not saying anything, you simply grin at her. After a moment, Amily realizes what you want her to do (or maybe what you're offering her). She blushes a little, but then answers your grin with one of her own, before grabbing your hand and leading you to her nest. You're not completely sure, but you think you notice a certain spring in her step - and her tail seems to almost have a mind of its own, weaving back and forth and occasionally caressing your [leg].\n\n");
			if(player.hasCock() && player.cockThatFits(61) < 0) outputText("Amily glances down at the beast between your legs and says, \"<i>I was going to ride you, but since you're SOOO big I think I'll have to get creative...</i>\"\n\n");
			sceneHunter.selectLossMenu([
					[0, "Get Ridden", getRidden, "Req. a cock fitting 61 area.", player.cockThatFits(61) >= 0],
					[5, "Tailjob", tailjob, "Req. a cock", player.hasCock()],
					[10, "Handjob", handjob, "Req. a cock", player.hasCock()],
					[1, "TakeHerCock", takeHerCock, "Amily needs a cock and you need a vagina for this one.",
			...
			function tailjob():void {
				...
				outputText("Suddenly, you almost jump as you feel something entering your pants - it feels almost like a snake or something, but then you realize that it's Amily's tail that's now wrapping itself around your [cock]. Your eyes widening, you look at her in surprise as she begins to methodically squeeze and relax her tail's grip on your member.\n\n");
			...
			//B2: Hand/Footjob
			function handjob():void {
				outputText("Once you are at her nest, Amily makes you sit down and kneels in front of you. She leans over to kiss you passionately, but you can feel her hands being busy on your pants. Drawing back, the mousegirl smiles at you, as her hands pull out your [cock] and start stroking it. Shuddering a little, you sit back and enjoy Amily's ministrations. You knew that she's quite dexterous, but you'd never have thought... Shuddering a little at the pleasure you feel, you partly close your eyes.\n\n");

				outputText("Suddenly, your eyes snap open again and you look with something very much like disappointment at Amily as she withdraws her hands. \"<i>What? Did you think I'd only use my hands on you?</i>\" she asks while sitting down. \"<i>Close your eyes again!</i>\" she demands. Not really sure what she has in mind, you still comply. After a moment or two, you feel something else on your [cock]. You open one of your eyes a little and realize that Amily's now using her feet. Smiling, you close your eye again and simply enjoy the feeling. The mousegirl continues stroking your cock with her feet for some time before switching back to her hands. Alternating between them (and sometimes using hands and feet at once), Amily soon makes you cum with a pleasured groan. Opening your eyes again, you smile at her in thanks.\n\n");
			...
			//B3: Eat Out
			function eatHerOut():void {
				outputText("Once you are at the nest, Amily pushes you down onto your back. You lay there in the soft, sweet-smelling vegetation as she strips off. Then, to your surprise, she pounces onto your chest, straddling your neck and leaving you face to face with her vagina.");
				//(if Amily is herm:
				if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  Her cock bobs merrily in your field of vision.");
				outputText("\n\n");
				...
				//(if Amily is herm:
				if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  Her penis erupts and sprays its own cum into the air to sprinkle both of you.");
				outputText("\n\n");

				outputText("She flops over onto your belly on her back, gasping for air, leaving her crotch sitting on your neck. Finally, she recovers and sits up, dragging her damp pussy ");
				//(if Amily is herm:
			...
			function getRidden():void {
				...
				outputText("She howls in unison with you at this final climax");
				//(if high cum amount:
				if(player.cumQ() > 500) outputText(", your torrents of spooge making her belly distend visibly");
			...
			function suckHerOff():void {
				...
				outputText("Amily's cock pops from your mouth as she flops over onto your chest, " + amilyTits() + " heaving as she gasps for breath. \"<i>Was... Was that...</i>\" She swallows and tries again. \"<i>Is that what it's like? ");
			...
			function takeHerCock():void {
				...
				outputText("You smile up at her and spread your [legs] to let her have a full look at your " + vaginaDescript() + ", smirking at her as an exhortation to come and get it. From her own smirk, and the fully erect penis she's sporting, you know she's about to get it as much as she can.\n\n");

				outputText("Amily situates herself between your legs and starts to stroke your " + vaginaDescript() + "; needless to say, her skill and experience from her own efforts to masturbate herself soon have you bucking with pleasure, lubing up your passage with your own feminine fluids.");
				//(if player is herm:
				if(player.hasCock()) outputText("  She strokes your [cock] a few times, but in a prefunctory manner; it's obvious that she's focusing on her own penis, this time.");
				outputText("\n\n");
				...
				//Otherwise:
				else outputText("You gasp in pleasure and do your best to grip Amily's dick with your netherlips.");
				outputText("\n\n");

				outputText("\"<i>I can hardly believe I've got a cock stuck in another [man]'s pussy... but, I gotta admit, this is actually really good, even if it is really weird too.</i>\" Amily murmurs, but then she starts doing what you consider really important and she begins thrusting into you. You wrap your [legs] around Amily's back and pull her tightly against you, eagerly meeting her every thrust with one of your own. She moans and gasps at your efforts, pounding your pussy with everything she has,");
		...
		private function giveAmilyAPresent():void {
			...
			function addItemAltButton(num:int, small:ItemType, large:ItemType, fun:Function):CoCButton {
				return addItemButton(num, !player.hasItem(small) && player.hasItem(large) ? large : small, fun);
			}
			...
			menu();
			if (flags[kFLAGS.AMILY_FOLLOWER] == 2) {
				addItemAltButton(0, consumables.SUCMILK, consumables.P_S_MLK, giveAmilyPurifiedSuccubusMilk);
				addItemAltButton(5, consumables.INCUBID, consumables.P_DRAFT, giveAmilyPureIncubusDraft);
				addItemAltButton(10, consumables.SDELITE, consumables.PSDELIT, giveCorruptAmilySuccubusDelight);
			} else {
			...
			addItemAltButton(1, consumables.WHITEEG, consumables.L_WHTEG, giveAmilyAWhiteEgg);
			addItemAltButton(6, consumables.BROWNEG, consumables.L_BRNEG, giveAmilyABrownEgg);
			addItemAltButton(11, consumables.PURPLEG, consumables.L_PRPEG, giveAmilyAPurpleEgg);
			if (flags[kFLAGS.AMILY_WANG_LENGTH] > 0)
				addItemAltButton(2, consumables.PINKEGG, consumables.L_PNKEG, giveAmilyAPinkEgg);
		...
		public function giveAmilyPureIncubusDraft():void {
				...
				if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) {
					...
					if (
						...
						if(player.cor >= 50 || player.vaginalCapacity() >= 50) {
							outputText("You struggle to contain your disappointment, having entertained lurid fantasies of being stretched beyond all reason by her ever-growing member, but you can't deny her logic: an unconscious mouse-girl wouldn't be nearly as much fun, even if she was exceptionally well endowed.");
							//if corruption VERY high -75+?]
							if(player.cor >= 75) outputText("  Although the idea of raping yourself with your futa-mouse lover's helpless body is appealing, you realize this is one argument you simply won't win.");
							outputText("\n\n");
						}
						//otherwise]
						...
						outputText("You put the vial back in its pouch, as Amily excuses herself and walks off.");
						doNext(playerMenu);
						return;
					}
			...
			//CORRUPT
				...
				if(flags[kFLAGS.AMILY_WANG_LENGTH] == 0) {
					...
					outputText("You pick up a vial of incubus draft and show it to Amily. \"<i>Drink this,</i>\" you tell her, passing the bottle to her. \"<i>[Master], this will make me grow a cock. Are you sure you wish to do that?</i>\"\n\n");

					outputText("You tell her yes, this is exactly what you want. Amily smiles and replies seductively, \"<i>As you wish [master].</i>\"\n\n");
					...
					outputText("Do you make her grow a huge one?");
				...
				else if(flags[kFLAGS.AMILY_WANG_LENGTH] < maxSizeCorr && flags[kFLAGS.AMILY_WANG_GIRTH] < 3)
				{
					...
					outputText("You pick up a vial of incubus draft and show it to Amily. \"<i>I want you to have a bigger cock; drink this.</i>\" You order her, passing the bottle to her. Amily smiles and says, \"<i>As you wish, [master], your orders are my pleasure.</i>\"\n\n");

					outputText("She opens her legs to give you a good view and downs the whole bottle in one go. She moans as her cock turns purple and grows impossibly hard, her veins bulging along the shaft as she cums. With every throb her cock grows a bit longer and a bit thicker as well, finally stopping when it's a couple inches bigger. She pants with pleasure and says, \"<i>Thank you, [master], for allowing this horny cum-bucket the honor of possessing such a wonderful tool.</i>\"\n\n");

					outputText("Satisfied, you dismiss her with a wave and go about your business.");
					amilyDickGrow();
					doNext(amilyFollowerEncounter);
				}
				//Too big!
				else {
					outputText("You reconsider - her small frame probably couldn't handle anything larger.");
					doNext(amilyFollowerEncounter);
				}
			}
		}
		private function corruptAmilyYouDeclineMaxxingHerDick():void {
			clearOutput();
			amilySprite();
			outputText("You decide to leave her as she is. If you want her to have a bigger dick you can always give her more drafts. \"<i>I want you to practice using your new tool, so you'll be ready whenever I need you,</i>\" you order Amily. \"<i>Yes, [master],</i>\" she answers. You leave her on the floor and go about your business.");
			doNext(amilyFollowerEncounter);
		}
		...
		private function giveAmilyPureIncubusDraft4Realz():void {
			...
			outputText("Catching her breath, she stares at her new appendage with an unreadable expression, then pulls her clothes back on with a grimace. You decide to give her some time alone to adjust to the change.");
			amilyDickGrow();
			doNext(amilyFollowerEncounter);
		}
		...
		public function giveAmilyPurifiedSuccubusMilk():void {
			...
			//CORRUPT UNZ
				...
				outputText("You pick up a vial of Succubi's Milk and show it to Amily. \"<i>Do you know what to do with this, slut?</i>\" you ask her. \"<i>You want me to drink it, [master]?</i>\" she asks you, licking her lips. You smile and reply, \"<i>That's right.</i>\" Then you push her on her back and lift her legs over her shoulders, exposing her wet pussy to you.  \"<i>Only, you're going to drink with these lips,</i>\" you add. Amily smiles in approval and excitement.");
				//[(If Amily has a dick)
				if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  Her cock hardens with delight at your idea.");
				outputText("\n\n");

				outputText("You uncork the vial, spread Amily's pussy with a hand and pour the contents of the vial into Amily's hungry snatch. \"<i>Ah [master]! It feels so good! Just like cum,</i>\" Amily moans in pleasure. The contents quickly disappear inside Amily's womb, triggering an orgasm within the whorish mouse-slave.");
				flags[kFLAGS.AMILY_VAGINAL_WETNESS]++;
				//[(if Amily reached/is a squirter)
				if(flags[kFLAGS.AMILY_VAGINAL_WETNESS] >= 5) outputText("  A jet of femcum flies straight out of her pussy, only to arch and hit her straight in the face.");
				else outputText("  Juices flood out of her pussy like water comes out of a spring, running down her body and pooling underneath her.");
				outputText("\n\n");
				...
				if(flags[kFLAGS.AMILY_VAGINAL_WETNESS] < 5) {
					...
					outputText("It'll be more fun to let her lust build; no doubt her pussy must be extra sensitive after this healthy dose of milk. You toss the vial away and casually stride back into the camp, leaving Amily panting on her juices.");
				}
				//(else)
				else {
					...
					outputText("Looks like giving her more milk only makes her pussy sensitive. You're tempted to make use of her new, sensitive pussy, but refrain from doing so. It'll be more fun to let her lust build. You toss the vial away and casually stride back into the camp, leaving Amily panting in her juices.");
				}
			}
			doNext(amilyFollowerEncounter);
		}
		...
		private function amilyDrinksSuccubusDelight():void {
			...
			if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] == 0) {
				if (flags[kFLAGS.AMILY_FOLLOWER] == 2) {
					...
					outputText("You tell her you need her to be able to cum more, so balls would help with that. Amily smiles and says, \"<i>Of course, [master]. Forgive your stupid mouse slut for questioning you.</i>\"\n\n");
					outputText("She opens her legs so you can watch and downs the bottle in one go.");
					if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("Her cock grows hard, but nothing else seems to happen...");
					outputText("\n\n");
					...
					if(player.hasCock() && (player.gender == 1 || rand(2) == 0)) {
						outputText("You drop your pants and stand over her, your cock hardening at what you're about to do. \"<i>Pleasure me.</i>\" You order Amily. She lifts a "+footpaw+" and begins stroking your exposed [cock]; she takes utmost care not to hurt you with her claws as she grips your shaft expertly between her toes. Her first "+footpaw+" is soon joined by her other one and together she begins nimbly masturbating you, giving you an expert footjob. No doubt she's been practicing; you'll have to reward her for this later.\n\n");

						outputText("You focus your corruptive powers, and a dollop of black pre forms on your tip; it slides over your shaft and lubes your [cock], allowing Amily to better stroke you. You can feel your orgasm coming fast, and you tell Amily to stop and open wide. She spreads her legs as far as they will go and watches, panting, as you finish yourself off. You shoot jet after jet of black cum all over her crotch. Amily moans with every jet that is spilled onto her corrupt form, relishing in the warmth that spreads throughout her.\n\n");
					}
					//[=Pussy=]
					else {
					...
					outputText("Catching her breath, she stares at her new balls with an unreadable expression, then pulls her clothes back on with a grimace. You decide to give her some time alone to adjust to the change.");
					flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE]++;
				}
			}
			//Too much
			else if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] >= 6) {
				outputText("You realize that giving Amily any more Succubi's Delight would make her practically immobile.  No, she doesn't need any more.  Maybe once you've secured a more appropriate place to keep an over-endowed harem.");
			}
			//[Give Succubus' Delight - repeat]
			else {
				player.consumeItem(consumables.SDELITE);
				outputText("You pick up a vial of Succubi's Delight and show it to Amily. \"<i>Drink this; you need bigger balls,</i>\" you order her, passing the bottle to her. Amily replies, \"<i>Yes, [master]</i>.\" Then she opens her legs and downs the bottle. She moans as her balls grow bigger and denser, churning with the extra cum her sack now holds.");
				flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE]++;
			}
			doNext(amilyFollowerEncounter);
		}
		...
		public function giveCorruptAmilySuccubusDelight():void {
			...
			//No balls yet?  QUERY!
			if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] == 0) {
				outputText("You pick up a vial of Succubi's Delight and show it to Amily. \"<i>Drink this,</i>\" you order her, passing the bottle to her. \"<i>You're going to give me balls, [master]? Are you sure?</i>\"\n\n");
		...
		public function giveAmilyAPinkEgg():void {
			...
			//Corrupt
				...
				outputText("You offer her a pink egg, explaining that it will remove her penis.  Amily looks completely devastated as she says, \"<i>Did fuck-slut not use her dick well enough?  Cum-bucket is so sorry [master], it won't happen again!</i>\"  Before you can respond she snatches the egg from your hand and begins to eat it, messily devouring it until she's licking her fingers.  Once she has finished, she drops her pants to watch as <b>her penis shrinks and blurs, eventually resolving itself once more into the dainty form of her clitoris</b>.\n\n");
				outputText("\"<i>Well, is there something else this worthless cum-collector can do to please her [master]?  Slut is so sorry she disappointed you,</i>\" she squeaks sadly, still trying to entice you into sex.");
			}
		...
		public function giveAmilyAWhiteEgg():void {
			...
			if(flags[kFLAGS.AMILY_FOLLOWER] == 1) {
				...
				//(If Nipples are 4 inches:
					...
					outputText("She shakes her head. \"<i>I'm sorry, but I think these are big and sensitive enough already. I'm a mouse, not a cow.</i>\" At that, she insistently walks away, clearly not going to take the egg. You return it to your pocket and decide to let her be.");
				}
			}
			//Corrupt!
			else {
				//(If Nipples are smaller than 4 inches:
				if(flags[kFLAGS.AMILY_NIPPLE_LENGTH] < maxSizeCorr) outputText("She looks at it and smiles knowingly. \"<i>Is [master] going to make this one's nipples into obscene toys?</i>\" she asks while accepting the egg.  She bites off the top and sucks down the contents in a practiced gulp.  Crushing the shell in her hand, she pulls at her top, allowing you to see her nipples swell and grow until they stop. She experimentally tweaks one, squeaking in shock at the sensation. \"<i>Thank you [master] for giving your slut such wonderful nipple-toys.  Would you like to abuse your slut's needy fuck-hole now?</i>\"\n\n");
				else outputText("She looks at it and moans lewdly, \"<i>My nipples are going to be sure huge for you [master].  No, nipples isn't right.  They're teats now.  Huge, swollen cow-teats on your mousey fuck-pet.  Lets make them bigger!</i>\" She bites off the top and sucks down the contents in a practiced gulp.  Crushing the shell in her hand, she pulls at her top, allowing you to see her nipples swell and hang even lower. She experimentally tweaks one, squeaking in shock at the sensation. \"<i>Thank you [master] for making your slut's body useless for anything but sex.  Would you like to abuse your slut's needy fuck-hole now?</i>\"\n\n");
				if(player.hasItem(consumables.WHITEEG)) {
					player.consumeItem(consumables.WHITEEG);
					flags[kFLAGS.AMILY_NIPPLE_LENGTH] += .25;
					flags[kFLAGS.AMILY_NIPPLE_LENGTH] = int(flags[kFLAGS.AMILY_NIPPLE_LENGTH] * 100) / 100;
		...
		public function giveAmilyABrownEgg():void {
			...
			if(flags[kFLAGS.AMILY_FOLLOWER] == 1) {
				...
				//(If Amily's butt is "delightfully jiggly":
					...
					outputText("She looks at the egg, and then shakes her head. \"<i>Sorry, but I've got more than enough junk in my trunk for my taste.</i>\" Sensing she won't back down on this, you put the egg back. \"<i>So, is there anything else you wanted?</i>\" She asks.");
				}
			}
			//IMPURE
			else {
				...
				//(If Amily's butt is "delightfully jiggly":
					...
					outputText("Amily scarfs down the egg and looks back expectantly, but it doesn't seem to make her already massive backside any larger.  She pouts and whimpers, \"<i>Slut is sorry, but her ass is as big and round as it can get [master]!</i>\"");
				}
			}
			doNext(amilyFollowerEncounter);
		}
		...
		public function giveAmilyAPurpleEgg():void {
			...
			//Corrupt
				...
				if(flags[kFLAGS.AMILY_HIP_RATING] < maxSizeCorr) {
					outputText("She looks at it thoughtfully. \"<i>[Master] wants my hips even wider?  Are you going to use me as your private breeding stock?  Mmm, that turns me on!</i>\" She takes the egg from you, her prominent front-teeth effortlessly biting off the top, whereupon she sucks down the contents in a practiced gulp. Crushing the shell in her hand, she almost loses her balance as her hips suddenly jut wider, the growth happening much faster than she expected. \"<i>Gods that made me wet!  Take me... let's fuck right now, please!</i>\" she begs.\n\n");
					if(player.hasItem(consumables.PURPLEG)) {
						player.consumeItem(consumables.PURPLEG);
						flags[kFLAGS.AMILY_HIP_RATING] += 1 + rand(2);
					}
		...
		public function get clothes():String {
			return flags[kFLAGS.AMILY_CLOTHING] ? "clothes" : amilyCorrupt() ? "sexy rags" : "rags";
		}
		...
		public function giveAmilySomePants():void {
			clearOutput();
			amilySprite();
			//If you played an early build that didn't initialize clothes.
			//(If Amily is wearing Tattered Rags:
			outputText("You offer her a set of comfortable clothes, asking if she'd like to wear these instead of her " + clothes + " she's wearing.\n\n");
			if(amilyCorrupt() || !flags[kFLAGS.AMILY_OWNS_BIKINI] && !flags[kFLAGS.GIVEN_AMILY_NURSE_OUTFIT]) //never given anything else
			...
			//(If Amily is wearing anything other than Tattered Rags:
				...
				outputText("\"<i>More new clothes? Ooh, you're spoiling me, [name]</i>!\" she teases you. Unabashed in the slightest at being naked in front of you, she strips down, doing her best to give you a little show as she does so. She then redresses herself in her new offering. \"<i>How do I look?</i>\" she giggles.\n\n");
		...
		private function makeTheMouseAMilkCowMoo():void {
			...
			if (flags[kFLAGS.AMILY_FOLLOWER] == 1) { //Pure
				...
				outputText("\"<i>Lactaid? You really want to try drinking mouse milk?</i>\" Amily teases, seductively, a little playfully.  You nod and assure that you do.  \"<i>Alright, my [master]!</i>\" She proclaims, taking the pink bottle from you and drinking the creaminess to be found within.");
			}
			//--LACTATION--
		...
		private function corruptAmilyOralSuckOff():void {
			...
			outputText("Amily's skills have been honed to near perfection.  She is familiar with your [cock] and is able to massage every single sensitive spot on your cock. Every time she bobs her head, she slurps noisily on your shaft and rakes her teeth teasingly; always careful so she won't hurt this most precious part of her [master].\n\n");
			//NO TEETH! OW
		...
		private function corruptAmilyLickPussiesLikeAPro():void {
			...
			outputText("\"<i>It's time to eat,</i>\" you declare and Amily instantly perks up after hearing it. You remove the lower half of your [armor] and thrust your crotch at Amily; displaying your quickly ");
			if(player.hasCock()) outputText("hardening " + Appearance.cockNoun(player.cocks[0].cockType) + " and ");
			...
			//[=Pussy=]
			outputText("You spread your pussy lips, ");
			//[(if squirter)
			if(player.wetness() >= 5) outputText("and a jet of fluids shoots out of your sensitive pussy.");
			else outputText("moisture wetting your finger.");
			...
			outputText("Amily's cold nose sends a chill of pleasure coursing through you, and you reward Amily's initiative with a small stream of juices that stick to her nose");
			//, [(if PC has a cock)
			if(player.hasCock()) {
			...
			if(player.vaginalCapacity() >= 45 && flags[kFLAGS.AMILY_NOT_FURRY]==0) {
				...
				outputText(vaginaDescript() + " , holding her " + ((flags[kFLAGS.AMILY_NOT_FURRY] == 0)?"nose":"mouth") + " closed; Amily's eyes bulge with surprise and you voice your pleasure with a long moan, \"<i>I'm not letting you breathe until you make me come slut. </i>\" With those words, you begin frigging yourself against Amily's ");
		...
		private function corruptAmilyScissorsLikeAPro():void {
			amilySprite();
			clearOutput();
			outputText("You grin as an interesting idea comes to your mind; you order Amily to lay down and spread her legs; she complies and you undress and gently set yourself between her legs, aligning your pussy with hers,");
			//[(if Amily has a cock)
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" Amily's " + amilyCock() + " throbs,");
			outputText(" and she smiles once she gets what you want to do.\n\n");

			outputText("You examine Amily's crotch closely, ");
			//[(if Amily has a cock)
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) {
				outputText("idly stroking her " + amilyCock());
				//(if Amily has balls)
				if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] > 0) outputText(" and " + amilyBalls() + ", which you shift to reveal her ");
				else outputText(" before dipping down to her ");
			}
			else outputText("eyeing her ");
			outputText(vaginaDescript() + ".  You lift a hand and stroke the slutty box with your fingers.  It doesn't take long before she is wet and ready, her little clit poking out as stiff as a cock");
			//[(if Amily has a cock)
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" almost as if trying to match its bigger brother above");
			...
			outputText("Making sure her legs are spread wide to receive you ");
			//[(if Amily has a cock)
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("and her cock ");
			outputText("is tipped out of the way ");
			...
			outputText("Amily moans");
			//[(if Amily's a squirter)
			if (flags[kFLAGS.AMILY_VAGINAL_WETNESS] >= 5) outputText(" and a jet of warm juices splashes against your " + vaginaDescript());
			// [Horsecock]
			...
			if(player.hasCock()) outputText(" and an aching " + cockDescript(0));
			...
			else outputText("  Wet squeltches and splats resound around you, as the results of Amily's orgasm flood what little space remains between the two of you.");
			//[(if Amily has a cock)
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" Her " + amilyCock() + " throbs and cum splashes on her breasts and face.");
			outputText("\n\n");
			...
			//[(if PC has a cock)
			if(player.hasCock()) outputText("  Amily's hands shoot out to grab " + sMultiCockDesc() + " and begin masturbating you; the corrupt succu-mouse doing all she can to give you the release that you crave.");
			outputText("\n\n");
			...
			//[(if PC's a squirter)
			if(player.wetness() >= 5) outputText(", drenching Amily's body in your fluids and marking her quite emphatically as your property");
			outputText(".\n\n");
		...
		private function corruptAmilysPussyGetsMotherfuckingFucked():void {
			amilySprite();
			clearOutput();
			outputText("You tell Amily that you're feeling playful and tell her you'd like to do some role-playing; you tell Amily to play the role of a demon out to seduce you. \"<i>But I could never do that [master], the only role I'm fit for is being your adoring slave!</i>\"\n\n");

			outputText("You grin and tell her she should at least try. Amily smiles and replies, \"<i>If that's what you want [master], I'd be glad to try,</i>\" Before starting, you tell Amily to try and act like a real demon.\n\n");

			outputText("You tell Amily to spread her legs and begin undressing, peeling off your [armor] piece by piece. Amily doesn't even bother to speak to you; she simply grins in delight, moisture already beginning to flow from her gaping pink vagina.\n\n");
			//(If pregnant:
			...
			outputText("  She flops down onto her " + amilyButt() + " and rolls backwards, spreading her legs out eagerly for you to have access and waving them like hungry, groping limbs, reaching for you in her impatience to start.  You grin in delight, but decide to tease the slutty "+((flags[kFLAGS.AMILY_NOT_FURRY]==0)?"mousette":"succubus")+" by staying just out of the reach of her "+((flags[kFLAGS.AMILY_NOT_FURRY]==0)?"foot-claws":"shapely feet")+".  \"<i>If you want it, then tell me just how much you want it slut,</i>\" you tease her mockingly, stroking your [cock] into erection. "+((flags[kFLAGS.AMILY_NOT_FURRY]==0)?" Just watch the heels, and don't dare injure your master.":""));
			//[(if PC has a vagina)
			if(player.hasVagina()) outputText("  Your other hand probes your moist fuck-hole for lube to help with your stroking.");
			outputText("\n\n");

			outputText("Amily doesn't even roll her eyes, she is that much of a helplessly loyal slut for you. \"<i>I want it, [master]! I want it more than anything else! I'm a cum-dumpster, a baby factory - I need cum in my hungry pussy, or I'll just fade away! Please, [master], pump me full of your hot, salty, gooey cum!");
			//(If not pregnant:
			if (!pregnancy.isPregnant) outputText("  I want you to fuck me full of cum, bloat me with seed until I'm a ball of skin over spooge with a head and limbs sticking out! I need to be full and round with the very stuff of life; fucking cum into me, [master]!");
			//(If pregnant:
			else outputText("  The babies need their cum, [Master]. They won't grow strong and healthy and slutty if you don't flood their womb with your hot baby-making juice! Please, cum and fill me with cum, for their sake?");
			outputText("</i>\"\n\n");

			outputText("It pleases you to see how much of an eager bitch she really is. You step close and take hold of her hip, then further tease her by rubbing your shaft along her slit, slickening it with her juices.");
			//[(if Amily has balls)
			if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] > 0) outputText("  Her balls do get in the way, but they form a nice cushion to rub your [cock] on, further stimulating you.");
			outputText("  Amily squeals in glee and wraps her legs around your " + hipDescript() + ". She is too well-trained to use them to slam you into her crotch, but you can feel the muscles in them quivering with the urge to pull you into proper penetrative position. You smile wickedly; as much as you want to plow her depths and fuck her raw, you also want to see just how far you can go before she breaks... \"<i>[Master]... please... why are you torturing yourself? Give yourself over to your loving mousy slut; lose yourself between my legs, let my needy cunt swallow your [cock] and milk you as only I can. No two-bit whore of a succubus will ever bring you the pleasure I will...</i>\" Amily moans pleadingly, her tail rising up to caress your [ass], her hands playing with her " + amilyTits() + ".\n\n");

			outputText("\"<i>Torturing myself you say? I think you're right. Maybe I should see if ");
			//[(if Jojo's corrupt)
			if(monk >= 5 && flags[kFLAGS.JOJO_DEAD_OR_GONE] == 0) outputText("Jojo wants to play,");
			//(else)
			...
			if(player.biggestCockArea() >= 61) {
				...
				if(flags[kFLAGS.AMILY_VAGINAL_WETNESS] >= 5) outputText("  Jets of fluid wash down whatever pre escapes your rigid tool.");
				outputText("  You have half a mind to brutalize the mousette then and there; yet for now you decide to let Amily have her way. She has earned this small reward for amusing you.");
				outputText("\n\n");
			}
			...
			if(flags[kFLAGS.AMILY_VAGINAL_WETNESS] >= 5) outputText("  Although she has yet to climax, her cunt is already bubbling and frothing madly around your [cock], causing you to squelch and slurp loudly with each thrust in and out of her wet depths. She's like a pot of sexual fluids on the boil, just waiting to geyser forth. All she needs is a little more encouragement.");
			outputText("\n\n");

			outputText("You feel it's time to end this. With one last vicious thrust, you cause Amily to dig slightly into the floor and cum. Painting her walls all the way to her womb, you unload.  Even her cervix is unable to stop the torrent you unleash upon her.");
			//[(If Amily is pregnant)
			if (pregnancy.isPregnant) outputText("  You wonder if your unborn children will appreciate their bath. Their mother certainly does.");
			//(If not squirter:
			if(flags[kFLAGS.AMILY_VAGINAL_WETNESS] < 5) outputText("  Her fluids slop wetly over your crotch, painting between your legs with her lubricant.");
			//(If squirter:
			else outputText("  A cascade of fluids pours from inside her; if you had allowed her to ride atop you, you'd be sopping with her femcum.");
			//(If Amily is herm:
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  It doesn't take long - seconds, at most - before her pulsing, twitching cock finally explodes, spewing gush after gush of herm-spunk all over her tits and face, which Amily eagerly laps up.");
			outputText("\n\n");

			outputText("You orgasm finally ends.");
			//[(if PC has large cum amount)
			if(player.cumQ() >= 1000) {
				//(if Amily is not pregnant)
				if (!pregnancy.isPregnant) outputText("  Amily's womb is so full of cum that it's bloated as if she was carrying a full litter of mousy sluts ready to birth.");
				//(else)
				else outputText("  Amily's belly looks ready to explode; besides the litter she's carrying, you've also filled her with enough cum to feed the little mousy sluts for week.");
			}
			outputText("  Spent, you slump on top of Amily.");
			//[(if PC has a pussy)
			if(player.hasVagina()) outputText("  Despite being neglected during the act, your " + vaginaDescript() + " leaks fluids to join Amily's on the floor.");
			outputText("  Amily pants loudly and sighs in satisfaction. Hesitantly, one hand dares to reach out and stroke your [face]. \"<i>Mmm... You are the light of my world, [master].");
			//(if not pregnant:
			...
			outputText("Once you've regained your breath you get up, extracting yourself from Amily's embrace. She gets up herself, a bit embarrassed by the way she acted. \"<i>Did I please you [master]?</i>\" she asks, awaiting your judgement expectantly. You smile and pat her head, telling her yes; she did please you. You comment that her theatrics weren't so bad.  She smiles and replies, \"<i>Thank you [master]. I loved this little play of ours... My favorite part was when I got used like a cocksleeve bitch in heat.</i>\" she says adoringly. You rub her hair and tell her she still has a job to do. Amily gasps and says, \"<i>Of course, forgive me [master],</i>\", and then she begins licking your [cock] clean.");
			//[(if PC has a pussy)
		...
		private function corruptAmilyCampBonesPCWithHerCock():void {
			...
			outputText("\"<i>Stay still, don't touch yourself and don't move.</i>\" You order Amily, she responds with an eager, \"<i>Yes [master]!</i>\" You continue looking at Amily's cock, carefully admiring your work.");
			//[(if Amily has balls)
			if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] > 0) outputText("  A pair of balls hangs under Amily's shaft; as you look, you have the slightest impression that her balls became rounder, fuller...");
			outputText("  Amily's shaft is ");
			if(flags[kFLAGS.AMILY_WANG_LENGTH] < 6) outputText("small");
			else if(flags[kFLAGS.AMILY_WANG_LENGTH] < 9) outputText("average");
			else outputText("big");
			...
			if(flags[kFLAGS.AMILY_WANG_LENGTH]*flags[kFLAGS.AMILY_WANG_GIRTH] > player.vaginalCapacity()) outputText("  You growl at her rough penetration, Amily stretches you out without regards for your pleasure; you'll definitely have to punish her for this later...");
			outputText("\n\n");

			outputText("Amily is overwhelmed by the feeling of your " + vaginaDescript() + ", and immediately goes berserk with lust. She doesn't speak, she grunts and snarls, thrusting into you as hard and fast as she can. There is nothing between you but the friction of flesh on flesh");
			//(squirter PC:
			if(player.wetness() >= 5) outputText(", the sounds of your fem-lube slurping and squelching as she smears it over herself and makes it splash out onto the ground beneath you");
			//(if Amily has balls:
			if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] > 0) outputText(" and the meaty smack-smack-smack of her balls slapping against your " + assDescript());
			outputText(".\n\n");
			...
			//[(if Amily has balls)
			if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] > 0) outputText("The slapping of her balls on your ass reverberates around you.  ");
		...
		private function corruptAmilyBuckFutter():void {
			...
			clearOutput();
			//[Anal - non-mutant]
			outputText("You order Amily to get on her hands and knees; you want to pound her ass. Amily's eyes light up in happiness and she replies excitedly, \"<i>Yes, [master]</i>!\" She quickly turns around and gets in position; her full ass swaying invitingly, while her tail stands erect to give you access. Her excitement at the prospect of having her ass fucked by you is made evident by the trickles of moisture that run down her legs");
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("; her " + amilyCock() + " stands erect, a tiny bead of pre forming on the tip as it throbs");
			outputText("\n\n");

			outputText("You undress and press your erect [cock] against Amily's buttcrack, rubbing your shaft on her ass and enjoying the grip of her soft yet firm ass; beads of pre-cum begin forming and sliding down your [cock], and you order Amily, \"<i>Prepare yourself for me.</i>\" Amily complies silently until her spaded tail digs into her pussy, drawing a moan from the excited mousette as she tail-fucks herself for a bit, before pulling her juice-slickened tail out. You distance yourself to allow her access and she rubs her slick tail-tip all over her asshole, lubing it up for you; then she rubs the still-wet spade along your [cock], slickening it and massaging your shaft all at once. \"<i>That's enough,</i>\" You say; her tail immediately releases your [cock] and goes back to its erect position.\n\n");

			outputText("You press your cockhead against Amily's throbbing ass and, with a groan, push inside. Amily moans lewdly, the pain of having her ass penetrated not even registering beyond the pleasure of the act. \"<i>Yesssss... [Master], fuck your mousy slut's ass! Fuck me until my ass is gaping wide. Oh! [master], I love being your fucktoy!</i>\" Amily screams in praise as you fill her insides.\n\n");
			...
			outputText("Having reached the limit, you waste no time and begin to brutally fuck the horny mousette's ass; drawing moans and screams of pleasure with each powerful movement and throb of your [cock]. Amily has no reservations about letting the world know that she's enjoying her predicament; she pushes back with every thrust and pulls away with every pull.  With such a brutal fucking you have no doubt that her ass is going to be sore for a while; yet she doesn't seem to care... pleasing her [master] is more important... besides, pain from such an act is pleasure for the corrupted mousette.\n\n");
			...
			//[(if PC has large cum amount)
			if(player.cumQ() >= 750) outputText("Her belly begins filling and inflating, not stopping even as she starts to look 6 months pregnant. Her ass clenches around you, trying to prevent the massive load you're still depositing from escaping, but inevitably, a few weak jets escape from around your [cock].  ");
			outputText("You spill your last jets with a couple more hard thrusts and then pull out of Amily's ass.");
			//[(if PC has large cum amount)
			if(player.cumQ() >= 1000) outputText("  Some cum backflows out of her ass in a jet, before she plugs herself with a hand, attempting to hold everything in.");
			else outputText("  Some cum begins leaking out of her ass, before she plugs herself with a hand.");
			outputText("\n\n");

			outputText("\"<i>Finish your work,</i>\" you order the tired mousette. She spins around");
			//[(if PC has large cum amount)
			if(player.cumQ() >= 750) outputText(" heavy belly jiggling along her breasts, ");
		...
		private function amilyLesboLetHerGo():void {
			...
			if(player.hasStatusEffect(StatusEffects.CampMarble) >= 0 || urtaLove()) {
				...
				outputText("\"<i>Are you? ...I see. Well, I'm happy that you, at least, found somebody. I... You're still welcome to come by and talk, but I'll respect your wishes.</i>\" Amily tells you. Evidently still quite embarrassed, she apologizes and then melts away into the ruins again.");
		...
		private function amilyPostConfessionGirlRemeeting():void {
			amilySprite();
			clearOutput();
			outputText("Amily looks happy to see you, as usual, but shy as well. \"<i>Ah... [name]... it's good to see you again.</i>\"\n\n");
		...
		public function hermilyOnFemalePC():void {
			amilySprite();
			clearOutput();
			outputText("Amily's efforts at leading you to a place to make love are a bit hampered by the erection tenting her pants, which she is clearly still having a bit of difficulty adjusting to. Finally, though, you have reached her current den, where you waste no time in removing your [armor].\n\n");
			...
			if(player.gender == 3) {
				...
				//else
				else outputText("\"<i>You know, it's not all bad, us both being this way... but remember that I want a turn at that, too.</i>\" She states, staring hungrily at your [cocks].\n\n");
			}
			...
			//(If first time:
			if(flags[kFLAGS.AMILY_HERM_TIMES_FUCKED_BY_FEMPC] == 0) outputText("\"<i>Er... are you really sure about this? I mean...</i>\" Amily murmurs uncertainly, until, irritated, you suddenly wrap your [legs] around her waist and pull her the last few inches needed to slam her dick into your needy pussy. She "+((flags[kFLAGS.AMILY_NOT_FURRY]==0)?"squeaks":"gasps")+" in shock and tries to pull out, but you still have your grip on her and pull her back, a process that repeats several times until the rhythm of it sinks in and Amily starts to thrust back and forth on her own.\n\n");
			else outputText("Amily grips your " + hipDescript() + ", gathering her courage, and then plunges her penis into your depths. Cautiously at first, she begins to thrust herself back and forth, growing faster and harder as her resolve builds.");
			player.cuntChange((flags[kFLAGS.AMILY_WANG_LENGTH] * flags[kFLAGS.AMILY_WANG_GIRTH]), true, true, false);
			outputText("\n\n");
		...
		private function amilyNewGenderConfrontation():void {
				...
				//[Female to Herm]
					...
					if(flags[kFLAGS.AMILY_WANG_LENGTH] == 0) {
						...
						//Medium Affection:
							...
							outputText("\"<i>I... don't take this the wrong way, [name], but... I'm not so sure we can be together any more while you have that. Be one thing or the other, not both.</i>\" Amily states. \"<i>But we can still talk, this time.</i>\"");
			...
			//Genderless tooo
				...
				//[High Affection]
					...
					if(player.hasCock()) outputText("bringing your [cock] to erection");
		...
		private function playerBirthsWifAmilyMiceInCamp():void {
			...
			outputText("You grunt in affirmation with a trace of irritation; ");
			//Repeat brood
			...
			outputText("\"<i>I hope you don't mind, [name], but I contacted them to come and take their new siblings back with them shortly after you told me you were pregnant.</i>\" Amily says, sheepishly. \"<i>We just can't really care for them here, and they'll be safer and happier with all their siblings.</i>\"\n\n");
		...
		public function amilyPopsOutKidsInCamp():void {
			...
			//CORRUPT!
				...
				outputText("\"<i>Yes! Ooh, yes, yes, yes! Ah, so good - that's it, babies, stretch your momma's dirty cunt for her! Out, out, little ones; [master] needs lots of new mousey sluts to worship " + player.mf("him","her") + " like " + player.mf("he","she") + " deserves!</i>\" Amily squeals in ecstasy, tail lashing at her surroundings like a whip and wings flapping madly. She is squatting in the clearing with her back to you, a mixture of feminine lubricant and amniotic fluid visibly pouring from her dilated cunt - she is quite evidently in labor, and loving every moment of it.");
				//(If Amily is herm:
				if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  She has hold of her cock with both hands and is furiously jacking herself off, spurts of cum spilling like a fountain; then spattering onto her heaving breasts, face and belly, even though her erection refuses to die away.");
				//[If Amily has balls:
				if(flags[kFLAGS.AMILY_HAS_BALLS_AND_SIZE] > 0) outputText("  Her testicles shake and quiver, swaying in the breeze as the pussy behind them readies itself to disgorge the life crammed into her swollen stomach above.");
				outputText("  Her eyes are rolled up in her head and her face is contorted into an expression of unholy orgasmic delight.\n\n");

				outputText("With a lewd moan, Amily pants, \"<i>Yes, it's coming! It's coming!!</i>\" Then she loses her balance and falls onto her back, her legs spreading wide; as if she wanted the whole world to see what's about to take place. And then it begins... A gush of fluids");
				//[(if Amily has a dick)
				if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" followed by a spurt of cum");
				outputText(" signals the coming of the first mouse. The head topped with wide mousy ears appears from between Amily's stretched lips, its small hands follow in suit;");
				//[(if Amily has balls)
				...
				outputText("Several children have their fill, but some are still in line.  \"<i>[Master]...</i>\" You hear Amily call between pants and moans of delight. You give her an inquisitive look. \"<i>Can I drink from you? Your sexy fucktoy bitch needs more food to turn into delicious milk for your newly born sluts.</i>\"\n\n");

				//(if PC is genderless)
				if(player.gender == 0) {
					outputText("You tell Amily that right now you can't feed her, so she'll have to make do with what she has. Amily pouts, but carries on. You chuckle and leave her with an order. \"<i>Make sure to train them into proper sluts, Amily, then let them go so they can gain some field experience,</i>\" You say, \"<i>Yes, [master].</i>\" Amily says. You turn and leave them to their own devices.");
					return;
				}
				...
				//[=Pussy=]
					...
					outputText("You smile and undress, showing Amily ");
					if(player.hasCock()) {
						outputText("your erect [cock] and ");
					}
				...
				outputText("Amily nuzzles your crotch affectionately before saying, \"<i>Look [master], our children are already eager to serve you.</i>\" You look down at her and she looks up at you. Then finally Amily turns to address the gathered children, \"<i>Sorry, my dears, but you're not yet ready to serve the [master]. If you want some of this...</i>\" She says, nuzzling your crotch once more, \"<i>You will need to have some experience first.</i>\"  The children all look down and whine in disappointment, sad that they won't get anywhere near their mother's treasure. \"<i>Now, now, don't despair, my beautiful budding sluts. If you go out into the world, I'm sure you'll gather the experience needed to serve the [master] in no time. Now give mommy a goodbye kiss.</i>\"\n\n");

				outputText("The mice quickly perk up and rush towards Amily, and she takes turns giving each of them a kiss; then sending each of them off towards the jungle with a playful slap on their little butts. When the last one has left, you congratulate Amily on being a good slut and giving birth to so many cute potential toys, \"<i>Thank you [master]!</i>\" she says happily. Then she nuzzles your crotch once more and adds, \"<i>If [master] wants to knock me up again, just say so. Your mousey cunt-slut is always ready to receive and deliver even more sluts to worship you and join your harem, my [master].</i>\" You pat her head and leave to attend to other affairs.");
		...
		public function makeChildren():void {
			...
			clearOutput();
			if(flags[kFLAGS.AMILY_ALLOWS_FERTILITY] == 1) {
				outputText("You ask Amily if she wouldn't mind going back on her herbs for now.  The mousegirl snickers and asks, \"<i>Had enough babies huh?</i>\" before she gives you a knowing wink.  Amily assures you, \"<i>Don't worry [name], we've brought more than enough life into the world.  I don't mind a break from it.</i>\"");
		...
		private function cookAmilyASnack():void {
			...
			if(flags[kFLAGS.TIMES_COOKED_AMILY_DRUG] > 0) {
				//After raping Amily for the first time, she is committed to the path of corruption.
			...
			//First Time
				...
				//(else)
					...
					if(player.hasItem(consumables.L_DRAFT)) outputText("Lust Draft ");
					else outputText("Fuck Draft ");
					outputText("and Goblin Ale inside, then you produce a wooden branch and begin stirring the contents until they are mixed together. Slowly you dip a finger and take an experimental lick; you're almost knocked back by the strong taste, your ");
					if(player.hasCock()) outputText(cockDescript(0) + " jumps up to attention");
					if(player.hasCock() && player.hasVagina()) outputText(", and your ");
					if(player.hasVagina()) outputText(vaginaDescript() + " nearly juices itself");
		...
		private function talkWithCORRUPTCUNT():void {
			...
			//Conversation: Minotaurs
				...
				outputText("\"<i>It doesn't surprise me in the least you got away untouched, [master],</i>\" Amily says flatteringly. \"<i>Minotaurs are dangerous brutes -  they're one of the biggest, strongest and toughest beasts around. I don't dare go near the mountains, not with those things roaming around. Sometimes they carry around huge axes, but most just make do with their fists,</i>\" she explains.\n\n");
			...
			//Conversation: Sand Witches
				...
				outputText("\"<i>I don't know... Maybe make me into some kind of milk-producing slave? That wouldn't do... if I became their slave I would never have been able to become your slave, [master],</i>\" she responds, grinning at you.\n\n");
				...
				outputText("You tell her no, when you want something inside her pussy you can shove it in yourself.  \"<i>[Master], you're going to make me horny saying things like that,</i>\" she says in mock embarassment. You laugh and dismiss her with a wave.\n\n");
			}
			//Conversation: Giant Bees
			else if(convo == 3) {
				...
				outputText("\"<i>Well... the Giant Bees you've probably seen [master]... the official name for them is the fertile caste, but folks tend to just call them handmaidens,</i>\" Amily explains.\n\n");
				...
				outputText("\"<i>Well [master], they mainly use this hypnotic thrumming from their wings; but someone as sharp and wise as you should have no problem shrugging it off. They aren't the kind to get violent; they will ask you to accept their eggs, if you resist their hypnosis, but they won't try to beat you into submission so they can lay their eggs. I wouldn't ever fight them; they're quick and pretty tough, thanks to that armor, and they've got this strange venom that saps your strength and arouses you at the same time. But I'm sure you wouldn't have any problems dealing with them [master]. If you ever fight one, get some of their honey; and if you get dirty while doing it, I'd be happy to give you a tongue bath until you're clean,</i>\" Amily says, slowly licking her lips from side to side.\n\n");

				outputText("Curious, you ask how she knows so much about these creatures.\n\n");

				outputText("Amily grins. \"<i>Well, they have been willing to trade honey in the past if you confront them peacefully... So I let them fill my ass with their eggs,</i>\" she explains. \"<i>If you want to [master]. I'd be happy to get my ass stuffed with whatever you want,</i>\" Amily adds, smiling while some of her juice running down her legs.\n\n");

				outputText("You smile and tell her you'll stuff her ass later. \"<i>Thank you, [master],</i>\" she says, smiling. You dismiss her with a wave.\n\n");
			}
			//Conversation: Fetish Cultists & Zealouts
			else if(convo == 4) {
				...
				outputText("Amily looks down to the floor and says, \"<i>Forgive your ignorant juice slave, [master]. I haven't had much contact with them so I can't tell you much. It looks like they have some weird magic that lets them change their clothes at random.</i>\"\n\n");

				outputText("\"<i>If you want [master], I could dress up for you, I'd love to wear any kind of kinky wear you would give me. Even if it was just ropes for tying me down.</i>\" Amily adds, smiling.\n\n");
			...
			//Conversation: Imps
				...
				if(player.biggestCockArea() < 24 && player.hasCock()) outputText("\"<i>[Master], can you grow a bigger cock to fuck your loyal cumdumpster?</i>\"");
				//(else if PC has a dick >= 24 area)
				else if(player.hasCock()) outputText("\"<i>[Master] I'm so horny, can you take your huge cock and jam it into your slutty mouse slave?</i>\"");
				//(else)
				else outputText("\"<i>[Master], can you grow a huge dick to fuck your needy cumslut?</i>\"");
				outputText(" Amily asks, rubbing her thighs together while moisture leaks from her sex.\n\n");

				outputText("You smile and say you'll think about it, then you dismiss her with a wave.");
			}
			//Conversation: Shark Girls
			else if(convo == 6) {
				...
				outputText("\"<i>Those are Shark Girls, [master]. I think they used to be a village of humans who lived on the lake... then the lake was contaminated, and they all turned into the lovely sharks they are now,</i>\" Amily says with a smile.\n\n");
			...
			//Conversation: Goblins
				...
				outputText("She looks to you pleadingly and says, \"<i>I love getting knocked up too, [master]. I could birth you many mice to worship you like you deserve; and then we could all fuck. Oh, [master]!");
				//[(if Amily's pregnant)
				if (pregnancy.isPregnant) outputText("  I can't wait to give birth to this batch of mice, just so I can get pregnant again.");
				//(else)
				else outputText("  Can you please knock me up? I don't care how, just order me to fuck until I get pregnant.");
				outputText("</i>\"\n\n");
			...
			//Conversation: What was life in her village like?
				...
				outputText("You decide to amuse yourself and ask Amily about her past. She rolls her eyes as if just the idea of breaching the subject was boring, but a quick glare puts her back in her place. \"<i>Umm... As you wish, [master],</i>\" she replies meekly.\n\n");

				outputText("\"<i>I don't really know why you are insterested in my past, [master]. What matters now is that I'm your slutty pet mouse, is it not?</i>\" she asks. You say yes, but it'd be amusing to know more about who she was. She smiles. \"<i>Very well, [master]. I'll do my best to entertain you. Is there anything in particular you wish to know?</i>\"\n\n");

				outputText("You rack your brains for a few moments, then ask her to tell you what life was like in her village.\n\n");

				outputText("\"<i>That is... Quite a boring subject [master]. But since you asked I'll tell you,</i>\" she says; then stops to think for a while. \"<i>It was a long time ago, but I can still remember like it was yesterday. The demons had already been around for a few years by the time I remember - I think I was born maybe shortly after they first appeared. In fact, I haven't celebrated my Day of First Breath in so long I don't really know how old I am. Not that it would matter, I'd much rather celebrate the day I became your personal cumbucket, [master]. Preferably with a nice load of cum,</i>\" she says, smiling seductively. Then she carries on, \"<i>Anyways, we had all that small village stuff. We fished in the lake, went swimming and sailed boats, gathered fruits and nuts and berries and mushrooms from the forest, we hunted birds and small game for meat. It was peaceful, really... But it was also boring... So boring... Everyone was a prude in the village and sex out in the open would get treated as some kind of taboo. It's moronic really,</i>\" Amily says, rolling her eyes.\n\n");

				outputText("\"<i>Fools... That's what we were.  We were fools living a fool's life. Back them the only thing that got inside the village was imps, and we, like the fools we were, believed we were safe. Haha, I feel like laughing, just remembering all this. It's no wonder my village fell the way it did... And I don't miss it; not for one instant. I'm much too happy here as your loyal cumslut, [master]. You saved me from a boring life as a villager, so I could live a glorious existence as a cumdumpster for you... Thank you, [master],</i>\" Amily says, smiling at you adoringly.\n\n");

				outputText("You approach her and pat her head, allowing her to nuzzle your crotch lovingly. You tell her not to worry; you will fuck her much more in the future. \"<i>Cum-pet can't wait for that, [master],</i>\" she replies. You dismiss her with a wave.\n\n");
			}
			//Conversation: Who was she before it was all lost?
			else if(convo == 9) {
				outputText("You decide to amuse yourself and ask Amily about her past. She rolls her eyes as if just the idea of breaching the subject was boring, but a quick glare puts her back in her place. \"<i>Umm... As you wish, [master],</i>\" She replies meekly.\n\n");

				outputText("\"<i>I don't really know why you are insterested in my past, [master]. What matters now is that I'm your slutty pet mouse, is it not?</i>\" she asks. You say yes, but it'd be amusing to know more about who she was. She smiles and asks, \"<i>Very well, [master]. I'll do my best to entertain you. Is there anything in particular you wish to know?</i>\"\n\n");
			...
			//Conversation: How did she survive?
				...
				outputText("\"<i>Well [master]... I ran as fast as I could to my house. I wanted to get my hunting knife and blowpipe. Useless tools... But back then I was a fool too,</i>\" she explains. \"<i>Then my parents showed up and told me to run. I didn't want to, but I obeyed... I was so scared... Don't know why, though? It's just sex... Wonderful, sweaty, slimy, creamy sex,</i>\" she says dreamily, licking her lips and shooting you a seductive grin. You tell her that you'll think about some way to reward her later. \"<i>I kept running as fast as I could, until I couldn't run anymore; then I hid and stayed hidden through the whole night. I only woke up in the morning, and I was ravenous, so I foraged something to eat and went back to the village. That's when I saw it was in ruins... I was lost and my life was in shambles... Thankfully, you eventually showed up to save me and pick up the pieces, [master],</i>\" she says adoringly.\n\n");

				outputText("You ask her what about her day-to-day? What was her routine?\n\n");

				outputText("\"<i>Boring.</i>\" she says rolling her eyes. \"<i>I prefer my new routine; I get to have wonderful sex with my [master] and drink a lot of cum. It's all a girl could ask for,</i>\" she says, smiling at you. \"<i>Anyways... I had enough knowledge to know what I could eat or not... I also salvaged some alchemy stuff to make poison darts, I was pretty weak before you gave me this delicious body, [master],</i>\" she continues, caressing her breasts for effect. You scold her, telling her she's not supposed to pleasure herself without permission. Amily quickly apologizes; you pat her head and tell her it's fine, you'll punish her later. \"<i>Thank you, [master],</i>\" She replies with a grin. \"<i>Other than that I only hid, hunted, foraged, ate and slept; that's all.</i>\"\n\n");

				outputText("Satisfied for the moment, you dismiss her with a wave and tell her you'll call for her when its time for her punishment. Amily excuses herself and leaves with a smile, pussy dripping");
				//[(if Amily has a dick)
				if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(", and cock hardening");
		...
		public function conquerThatMouseBitch():void {
			...
			clearOutput();
			//By HP:
			if(monster.HP <= monster.minHP()) outputText("Amily falls on the ground, unconscious. You drag the mousette towards an empty spot in her hideout. You smile as you strip her clothes off, then you find a rope to tie her up. You tie her hands together, then tie her hands to her ankles, ensuring she won't be able to fight you back. After that, you dispose of anything that could be used as a weapon, or could aid her in her escape, then sit down on a nearby stool to wait until she's recovered her consciousness. After all, it's much more fun if she's awake to see everything you'll do to her...\n\n");
			//By Lust:
			...
			if (flags[kFLAGS.AMILY_CORRUPTION] != 0) throw new Error("Svalkash fucked up after all, please report this and ping him on Discord.");
		...
		private function rapeCorruptAmily1():void {
			...
			if(player.cockTotal() > 0) {
				if(player.cockTotal() > 1) outputText(multiCockDescriptLight() + " are");
				else outputText(cockDescript(0) + " is");
				outputText(" already hard");
				if(player.hasVagina()) outputText(", and your ");
			}
			if(player.hasVagina()) outputText(vaginaDescript() + " is already wet");
		...
		private function rapeCorruptAmily1Male():void {
			...
			clearOutput();
			outputText("You smile and say, \"<i>Fine, but you're gonna have to work for it.</i>\" Amily's answer is to open her mouth wide. The invitation clear, you scoot closer to her mouth, and her lips touch the tip of your " + cockDescript(x) + "; Amily suddenly lifts her head and starts swallowing your dick");
			//[(if PC is huge)
		...
		private function rapeCorruptAmily2Male():void {
			...
			var x:Number = player.cockThatFits(61);
			if(x < 0) x = 0;
			outputText("You unceremoniously shove your " + cockDescript(x) + " into her maw. ");
			...
			//[(If PC is huge.)
			if(player.cockThatFits(61) < 0) outputText("  One of Amily's hands settles on your " + cockDescript(x) + ", stroking whatever she doesn't manage to swallow.");
			outputText("\n\n");
			...
			//[(If PC has large cum amount)
			if(player.cumQ() >= 750) outputText("  You feel your enormous load coming back through her throat and spilling from her mouth and nose, smearing your crotch as you pull away to let her breathe.");
			outputText("\n\n");
			if (!recalling) player.sexReward("saliva", "Dick");
		...
		private function rapeCorruptAmily2Female():void {
			amilySprite();
			clearOutput();
			outputText("You roughly grab ahold of Amily's ears and shove her face on your " + vaginaDescript() + ".");
			//[(if PC is a squirter)
		...
		private function rapeCorruptAmily3Male():void {
			...
			outputText("Amily complies without a word; then you drive your [cock] between her breasts and begin pumping vigorously.You blow your load then and there, right on Amily's breasts, covering them in a layer of white hot spunk, while Amily smiles contentedly.\n\n");

			//(if PC is < 60 Corruption)
			if(player.cor < 60 - player.corruptionTolerance && !recalling) {
			...
			outputText("\"<i>Enough teasing. Get to work now,</i>\" you order.  She complies by beginning to move and suckle on your [cock].\n\n");
		...
		private function rapeCorruptAmily3Female():void {
			...
			outputText("Amily springs up and rushes to nuzzle your still leaking vagina. \"<i>Thank you! Thank you! Thank you!</i>\" she says excitedly. Then, with a lick of her lips, she opens wide and engulfs your whole " + vaginaDescript() + ".");
			//[(if PC is a squirter)
		...
		private function rapeCorruptAmily3Epilogue():void {
			...
			outputText("\"<i>Please! Wait, [master], please!</i>\" Amily yells, you turn around and look back at her. \"<i>I want more. Please,</i>\" she begs.\n\n");
		...
		private function rapeCorruptAmily4Male():void {
			...
			outputText("Moaning, she drops on fours and crawls toward you to grovel at your [feet], rubbing her face on the very ground you stand on, and begging all the while.\n\n");

			outputText("\"<i>Please [master], make me your cumbucket. There is nothing I would love more than to become a receptacle for your lust. This would be like a dream come true! Please, oh please [master]! Knock me up and allow me to birth more mice to worship you like you deserve.</i>\"\n\n");
			...
			outputText(" She falls silent and you say. \"<i>Very well. From now on I am your one and only [master].  You will address me as such, always. My word is law, and you shall do whatever I tell you to, whenever I tell you to. Rise, it's time to break you into your new role as my cum-dumpster.</i>\"\n\n");
		...
		private function rapeCorruptAmily4Female():void {
			...
			//(if PC's corruption < 80)
			if(player.cor < 80 - player.corruptionTolerance && !recalling) {
			...
			outputText("Moaning, she drops on all fours and crawls toward you to grovel at your [feet], rubbing her face on the very ground you stand on, and begging all the while.\n\n");

			outputText("\"<i>Please [master], make me your cum-bucket. There is nothing I would love more than to become a receptacle for your lust. This would be like a dream come true! Please, oh please [master]!  Allow me to knock you up so you can birth more mice to worship you like you deserve.</i>\"\n\n");
		...
		private function rapeCorruptAmily4Epilogue():void {
			...
			outputText("She looks herself over, examining and enjoying her new sensitive body; every movement is accompanied with a pleasured sigh or a moan. She looks at you and kneels in front of you, a rush of fluids escapes her pussy as she does so. \"<i>[Master]... What are your orders?</i>\" Amily asks with a seductive tone.\n\n");
			...
			outputText("Amily grins and replies seductively, \"<i>Yes, [master]... Your orders are my pleasure.</i>\"\n\n");
			if (!recalling) {
		...
		public function stalkingZeAmiliez():void {
			...
			if(player.inte >= 50 && player.spe >= 65 || recalling) {
				amilySprite();
			...
			//(if PC's intelligence >= 50)
				...
				outputText("This is your cue to act; you quickly burst out of your hideout and swipe her blowpipe away. Amily jumps away in surprise and reaches for her knife, assuming a fighting stance. You ready your [weapon] and prepare to teach the foolish mouse a lesson.\n\n");
				//[Proceed to battle.]
			...
			//(if PC's speed >= 65)
				...
				outputText("Panicked, she takes her knife and prepares to fight you. You ready your [weapon] and prepare to teach the foolish mouse a lesson.\n\n");
				//[Proceed to battle.]
				startCombat(new Amily(),true);
			}
			else {
		...
		public function stalkingZeAmiliez2():void {
			...
			//(else)
				...
				if(player.hasCock()) {
					outputText(cockDescript(0) + " getting hard");
					if(player.hasVagina()) outputText(" and your ");
				}
				if(player.hasVagina()) outputText(vaginaDescript() + " growing wet");
				outputText(".\n\n");
				...
				if(player.cor < 45 - player.corruptionTolerance && !recalling) {
				...
				if(player.hasCock()) {
					outputText("erect " + cockDescript(0));
					if(player.hasVagina()) outputText(" and ");
				}
				if(player.hasVagina()) outputText(" dripping " + vaginaDescript());
		...
		public function stalkingZeAmiliez3():void {
			...
			if(player.hasCock()) {
				outputText("driving your [cock] into erection");
				if(player.hasVagina()) outputText(" and ");
			}
			if(player.hasVagina()) outputText(" moistening your " + vaginaDescript());
			...
			outputText("Amily plants her head on the ground and starts. \"<i>[Master], please let me drink more of your cum. Please! I need it so bad that I can't help but drool. I need it so bad that not being able to drink from you is like torture. Please let me drink!</i>\"\n\n");
		...
		public function amilyReturns():void {
			...
			outputText("\"<i>Thank you, [name]. I don't know how you did it, or if it had anything to do with me, but you managed to do something that I've never heard of anyone else having the will to do. You turned back from demonhood and repurified yourself again.</i>\"\n\n");
...
*/
		...
		public function amilyDefurryOfferAtCamp():void
		{
			...
			if (!amilyCanHaveTFNow())
			{
				outputText("Unfortunately, you can't do such a thing until you have acquired the necessary magical prerequisites.  In simple terms, you need a <b>golden seed</b> for a human face, a <b>black egg</b> to get rid of the fur, and some <b>purified succubus milk</b> to round things off.");
				doNext(amilyFollowerEncounter);
				return;
			}
			//EAT ZE ITEMS!
		...
		public function refuzzAmily():void {
			...
			if (!player.hasItem(consumables.MOUSECO, 2)) {
				outputText("Unfortunately, you can't do such a thing until you have two handfuls of mouse cocoa.");
				doNext(amilyFollowerEncounter);
				return;
			}
		...
		public function dateNightFirstTime():void {
			...
			outputText("\"<i>Hey there, lover,</i>\" Urta says happily as you approach the gates.  However, her gaze quickly shifts to the little mouse-girl at your side.  Urta wrinkles her brow in confusion for a moment before slow-dawning comprehension spreads across her face.  \"<i>So, [name], uh... who's your friend?</i>\"\n\n");

			outputText("Amily smiles, as friendly as she can be when placed in such an awkward situation, \"<i>Hi! My name's Amily - [name]'s mate.</i>\"  You give Amily's hand a little squeeze, proud of her for not overreacting to Urta's casual drop of the word \"<i>lover.</i>\"\n\n");
			...
			outputText("Urta scans the tavern's main room briefly, sees you, and strides over.  With a charming smile, Urta asks if she can have a seat.  You try to gently explain that you're on a date right now, but ");
			...
			outputText("The fox-girl grabs a chair from an adjacent table and brings it over, sitting reverse and leaning heavily on the back rest.  Before she can say anything more than a simple greeting, however, Amily grabs a bottle from ");
			if(flags[kFLAGS.ENCOUNTERED_VALA_AT_BAR] == 0) outputText("a passing waitress");
			else outputText("a rather alarmed Vala as she flutters by");
			outputText(". \"<i>Herr, ish gud,</i>\" Amily slurs, pawing off the bottle to a surprised Urta.\n\n");

			outputText("Well, this could be interesting.  If you get both the girls drunk, it might be easy (or inevitable) for something sexual to happen.  Or, you could take Amily home right now and make sure nothing untoward happens to either of your lovers.");
		...
		private function liqueurUpTheWaifus():void {
			...
			outputText("With a drunken grin, Urta laughs, \"<i>Oh, shit. Sorry, sweetie, I figured [name] woulda' blabbed about my third leg by now...</i>\" She leans over the table, nearly brushing noses with Amily as she adds, \"<i>Hey... Wanna give it a try?</i>\"\n\n");
			...
			if(player.hasCock()) outputText(", already feeling a strong stirring in your " + multiCockDescriptLight());
			else if(player.hasVagina()) outputText(", already feeling a little moisture in your loins");
			outputText(".\n\n");
			...
			outputText("and Amily wrapping her lithe little tail around Urta's massive endowment.  You strip off your [armor] and, looming over the girls, decide on how you want to go about this.");
		...
		public function threesomeAmilUrtaCAWKS():void {
			...
			/*if(silly()) {
				outputText("MUZZLESMUZZLES MUZZLE PAW FUR MURR HIR SHI YIFF YIFFITY YIFF PURR PURR MEW.");
			}*/
			...
			if(player.cockArea(x) > 60) outputText("filling her until she just can't take any more of you");
			else outputText("burying yourself up to the hilt in her sweet, wonderfully tight bum");
			...
			//if Futamily:
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText(" as her cock orgasms, too, spurting a nice, thick trail of mousecum all over Urta's face and tits");
			outputText(".\n\n");
		...
		public function urtaXAmilyCuntPussyVagSQUICK():void {
			...
			outputText("You groan as Urta slowly slides into you, filling your cunt until it's stretched to the limit to take your lover's massive equine dong.  She finally bottoms out, pushing into you until you feel her flared head pressing against your womb.");
		...
		public function pureAmilyPutsItInYourRectumDamnNearKilledEm():void {
			...
			outputText("Before she can misunderstand your intentions, you lean ");
			if(player.tallness >= 40) outputText("down ");
			...
			//(Merge):
			outputText("Working delicately, she carefully coats her shaft in a generous layer of the green ointment, until her cock glistens with an odd greenish sheen.  You take your position on the ground, lowering yourself down onto your knees and elbows, resting your chin on top of your hands and raising your [butt] into the air");
			if(player.tailType > Tail.NONE) outputText(", moving your tail out of the way");
			outputText(".\n\n");

			outputText("Amily ");
			if(flags[kFLAGS.AMILY_TIMES_BUTTFUCKED_PC] == 0) outputText("looks a bit puzzled as she contemplates the mechanics of this for a moment, and then with a shrug, ");
			outputText("takes her place behind you, running her shaft between your buttocks a bit to spread the slick lubricant around.\n\n");

			if(player.analCapacity() < 30) outputText("Bracing yourself a bit, you grimace a little as you feel Amily start to push against your " + assholeDescript() + ", just the tip sliding in with a bit of resistance.");
			else if(player.analCapacity() <= 60) outputText("You relax yourself, letting out a husky moan as you feel the tip of Amily's cock sliding into you.");
			else outputText("Amily lets out a slightly surprised squeak as your " + assholeDescript() + " offers her little resistance.");
			...
			outputText(".  Slowly, she begins to rock her hips back and forth, placing her hands on your " + hipDescript() + " for support.");
			...
			outputText("You begin to push yourself back against her slow thrusts, and, encouraged by your actions, she starts to pump a little harder.  Sliding herself down along your back, she begins to nuzzle into your neck, stroking your sides sensually as her " + amilyHips() + " rise and fall against you.");
			if(player.hasCock()) {
				outputText("  You can feel her hand slipping down underneath you, gripping your " + cockDescript(x) + " gently and starting to stroke and tease it with her fingertips.");
				if(player.cockTotal() > 1) outputText("She brushes along " + sMultiCockDesc() + " one at a time, stroking for a moment or two before switching off.");
			}
			outputText("\n\n");

			outputText("It isn't long before Amily's sensual thrusting and touching has you on the precipice of your orgasm, and you voice your pleasure loudly, digging your fingertips into the ground and moaning.");
			if(player.hasVagina()) {
				outputText("  Sexual fluids ");
				if(player.wetness() <= 1) outputText("drip ");
				else if(player.wetness() <= 3) outputText("dribble ");
				else outputText("gush ");
				outputText("down your thighs");
				if(player.hasCock()) outputText(" and p");
			}
			if(player.hasCock()) {
				if(!player.hasVagina()) outputText("  P");
				outputText("recum dribbles from your " + multiCockDescriptLight());
			}
			if(player.gender > 0) outputText(", forming a small puddle underneath you as you feel your climax approaching.");
			outputText("  With a tremendous shudder, you orgasm, your " + assholeDescript() + " clenching around her member.  At the same time, Amily moans into your ear, warm ribbons of herm-jizz filling your innards.");
			if(player.hasCock()) {
				outputText("  " + SMultiCockDesc() + " twitches with pleasure, soaking the ground beneath you with ");
				if(player.cumQ() < 250) outputText("a few streams");
				else if(player.cumQ() < 500) outputText("a small deluge");
				else outputText("a small lake");
				outputText(" of cum.");
			}
			outputText("\n\n");
		...
		private function fuckPureAmilysHeiny():void {
			...
			if(x < 0) x = 0;
			outputText("You pause and flash her a coy smirk, then you gently place the tip of a finger on her nose, slowly running it down along her body, over her lip, between her breasts, across her stomach, finally stopping to firmly grip her " + amilyButt() + ".");
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("  Her pants tent from arousal as her male organ reacts to your sensual touching, and you start to teasingly stroke it as well.");
			outputText("\n\n");
			...
			if(flags[kFLAGS.TIMES_FUCKED_AMILYBUTT] == 0) {
				...
				else if(player.tallness <= 40) outputText("down ");
				...
				outputText("You smile broadly, nodding, and assure her you'll be gentle as you begin to strip out of your [armor].\n\n");
			...
			outputText("She wipes her hand off on a discreet corner of her " + clothes + " and begins to shimmy out of them.");
			...
			outputText("You close your eyes and let out a shuddering moan into Amily's ear as your climax comes rushing forth, ");
			if(player.cumQ() <= 250) outputText("your " + cockDescript(x) + " twitching and throbbing as you paint her insides with a few strands of cum");
			else if(player.cumQ() <= 500) outputText(" the swell of your orgasm pushing past her sphincter as you unload a thick deluge into her back door");
			else outputText(" the volume of your virile jism filling her innards with warmth as she begins to gain a slight bulge in her abdomen");
			outputText(".  Being stimulated in such a way is not enough to give Amily a complete release, but the sensation of your warm cum filling her rear is enough to make ");
			if(flags[kFLAGS.AMILY_WANG_LENGTH] > 0) outputText("her own member twitch and dribble a small puddle of clear pre-cum onto the ground below her, joining with ");
