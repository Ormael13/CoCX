...
//const MARBLE_MILKED_BEFORE:int = 1165;
		...
		public function timeChange():Boolean
		{
			...
			if (player.hasStatusAffect(StatusAffects.CampMarble)) {
				...
				else if (!player.hasStatusAffect(StatusAffects.MarbleHasItem) && marbleScene.marbleAtCamp()) {
					// Off cooldown
					if (!player.hasStatusAffect(StatusAffects.MarbleItemCooldown)) {
						if (rand(10) == 0) {
							spriteSelect(41);
			...
			if (!player.hasStatusAffect(StatusAffects.Infested)) flags[kFLAGS.MARBLE_GROSSED_OUT_BECAUSE_WORM_INFESTATION] = 0;
			if (player.hasStatusAffect(StatusAffects.MarblesMilk) && player.findPerk(PerkLib.MarblesMilk) < 0) {
				...
				if (player.statusAffectv1(StatusAffects.MarblesMilk) <= 0) {
					...
					if (player.statusAffectv3(StatusAffects.Marble) > 0) {
						...
						if (player.hasStatusAffect(StatusAffects.BottledMilk)) {
			...
			if (player.statusAffectv3(StatusAffects.Marble) > 0 && player.findPerk(PerkLib.MarbleResistant) < 0 && player.findPerk(PerkLib.MarblesMilk) < 0 && player.statusAffectv2(StatusAffects.Marble) > 25) {
				...
				if (!player.hasStatusAffect(StatusAffects.MarblesMilk) && !player.hasStatusAffect(StatusAffects.BottledMilk)) {
			...
			if (player.hasStatusAffect(StatusAffects.MarbleWithdrawl)) {
				...
				else if (player.hasStatusAffect(StatusAffects.BottledMilk)) {
					outputText("\nYou no longer feel the symptoms of withdrawal.\n", false);
					needNext = true;
					player.removeStatusAffect(StatusAffects.MarbleWithdrawl);
					dynStats("tou", 5, "int", 5);
			...
			if (model.time.hours > 23) {
				...
				if (player.hasStatusAffect(StatusAffects.Marble) && player.statusAffectv2(StatusAffects.Marble) > 0) marbleStatusChange(0,-1);
			}
			return needNext;
		}
		...
		public function timeChangeLarge():Boolean {
			...
			if (player.statusAffectv3(StatusAffects.Marble) > 0 && player.statusAffectv2(StatusAffects.Marble) >= 100 && player.findPerk(PerkLib.MarblesMilk) < 0 && player.findPerk(PerkLib.MarbleResistant) < 0 && model.time.hours == 6) {
				...
				if (player.hasStatusAffect(StatusAffects.MarblesMilk)) {
					player.removeStatusAffect(StatusAffects.MarblesMilk);
				}
			...
			if (checkedMarbleMilk++ == 0 && model.time.hours == 6 && player.findPerk(PerkLib.MarblesMilk) >= 0) {
				...
				//Marble isn't at camp
					...
					if (!player.hasStatusAffect(StatusAffects.NoMoreMarble)) {
...
override public function marbleFollower():Boolean {
	return player.hasStatusAffect(StatusAffects.CampMarble);
}
...
public function encounterMarbleExploring2():void {
	...
	if(player.statusAffectv1(StatusAffects.Marble) < 60) {
		...
		if(!player.hasStatusAffect(StatusAffects.FuckedMarble)) {
...
private function AcceptMarblesSexualAdvances():void {
	...
	standardSex(true);
	if(!player.hasStatusAffect(StatusAffects.FuckedMarble)) player.createStatusAffect(StatusAffects.FuckedMarble,0,0,0,0);
...
public function addictedEncounterHappy(clearS:Boolean = true):void {
	...
	if(!player.hasStatusAffect(StatusAffects.MalonVisitedPostAddiction)) {
...
private function extendedMurbelFarmTalkz():void {
	...
	switch (flags[kFLAGS.MURBLE_FARM_TALK_LEVELS]) {
		...
		default:
			...
			if(!player.hasStatusAffect(StatusAffects.CampMarble)) outputText(", except that mission of yours that's so important to you");
			outputText(".</i>\"");
...
public function marbleStatusChange(affection:Number, addiction:Number, isAddicted:Number = -1):void {
	if(!player.hasStatusAffect(StatusAffects.Marble)) player.createStatusAffect(StatusAffects.Marble,0,0,0,40);
...
public function interactWithMarbleAtCamp():void {
	...
	if (player.hasStatusAffect(StatusAffects.MarbleHasItem)) gatherEvent = marbleGathered;
...
private function talkWithMarbleAtCamp():void {
	spriteSelect(41);
	if(!player.hasStatusAffect(StatusAffects.MarbleSpecials)) {
	...
	else if(player.hasStatusAffect(StatusAffects.FoundFactory) && flags[kFLAGS.MARBLE_CAMPTALK_LEVEL] < 2)
	{
...
private function marbleNomNoms():void {
	...
	if(flags[kFLAGS.MARBLE_DICK_TYPE] > 0) {
		...
		if(player.hasStatusAffect(StatusAffects.SlimeCraving) || player.lib + player.cor - player.inte > 35) {
