...
package classes.Scenes.NPCs {
	...
	public function BLUHBLUH():void {
		clearOutput();
		output("When you return to camp, you find Marble seated at the campfire, staring gloomily into the ashes. Curious, you approach your bovine lover and ask her what the matter is. She looks up at you, her ");
		if(flags[kFLAGS.MARBLE_BOVA_LEVEL] <= 1) output("human");
		else output("bovine");
		output(" head still in her hands, and sighs loudly. <i>\"I’m sorry, sweetie, I just... I’ve tried to ignore it, but I just don’t feel comfortable here.\"</i>");
		...
		{
			output("  ...Has she been fighting with ");
			if(amilyScene.amilyFollower() && !amilyScene.amilyCorrupt()) output("Amily");
			else if(followerKiha()) output("Kiha");
			else output("Izma");
			output("?");
		}
		...
		if(player.hasItem(consumables.PURHONY)) output("  You then recall that bottle of honey the bee-girl gave you, and ask if maybe it might work?");
		if(player.findStatusAffect(StatusAffects.PureCampJojo) >= 0) output("  You remember how Jojo helped you become pure, and suggest maybe his meditations could assist Marble?");
	...
	public function giveMarblePureQuestLabova():void
	{
		...
		{
			output("\n\nWithout ceremony, you pass the bottle of purified LaBova to Marble, who chugs it down. You wait for the usual reaction, but, to your surprise, nothing happens. You ask the cowgirl if she feels alright.");
	...
	public function visitRathazulToPurifyMarbleAfterLaBovaStopsWorkin():void
	{
		...
		{
			output("You spot the elderly alchemist's camp on the lake once more.\n\n");
		}
		output("As you approach the rodentine alchemist, you remember Marble’s problems and resolve to ask him if he can possibly help you. Once he finishes greeting you, you politely return the greeting and ask if he would be willing to lend his opinion on a problem you have.");
	...
	{
		clearOutput();
		output("Fortunately, Marble is at your camp rather than off at Whitney’s, and your bovine lover looks up at you hopefully as you approach. \"<i>Did he say he could help?</i>\" she asks, excitedly.");
	...
	{
		clearOutput();
		output("When you wake and join Marble ");
		if(companionsCount() > 2) output("and your other followers ");
		output("for breakfast this morning, you find a bleary-eyed Rathazul waiting at the campfire. Marble clucks her tongue and gives him a mug of something strong, which he accepts gratefully, noisily slurping down a mouthful. He takes a seat, staring into his mug, and then sighs softly.");
	...
	{
		clearOutput();
		output("You shake your head and tell Marble that you still need her at the camp; you’ll tell her when you can spare her.");
	...
	{
		clearOutput();
		output("You tell Marble that if she needs some time off to go and see her family for this matter, it’s okay with you.");
	...
	//Marble Gone Notice
	//9999
	//Marble isn’t here right now; she’s still off to see her family.
	...
	public function claraShowsUpInCampBECAUSESHESACUNT():void
	{
		...
		if(flags[kFLAGS.MARBLE_BOVA_LEVEL] <= 1) output("your mate has");
		else output("what your mate use to have");
		output("; cow legs, ears, horns, and large breasts.  However, she doesn’t have Marble’s brown fur, instead having black and white blotched fur across her legs and ears and a mane of white hair atop her head.  She is wearing a one piece thick cloth dress, with a great deal of fancy embroidery decorating it.  You also guess that she’s a bit smaller than Marble, in both the chest and height departments.");
		...
		if(player.inte >= 60) output(" with a odd smile on her face");
	...
	{
		clearOutput();
		output("You turn back to the camp and continue going about your morning routine.  By the time you’re ready to head out, the girls have come back from the farm, bearing a bottle of Clara’s milk.  The spotted bovine’s foul mood seems to have improved as well.");
		...
		if(companionsCount() >= 3)
		{
			...
			if(amilyScene.amilyFollower() && !amilyScene.amilyCorrupt())
			{
				...
				output("\n\nWhen you approach the semi-feral mouse about having tea with Clara, she smiles brightly. \"<i>I haven’t enjoyed something like that in ages... it kind of reminds me of the old days.</i>\" She trails off, looking a little sad, but then shakes it off. \"<i>Sure, I’m game.</i>\"");
			...
			{
				output("\n\nYou take a short walk over stream that Izma likes to bathe in and call her over.  She asks you what’s up and you let her know what Clara’s got brewing.  The tigershark snorts. \"<i>No thank you; I don’t trust anything that might have milk in it from one of those damn cows.</i>\" She snaps, showing her teeth in a predatory expression of dislike. You try to argue that she’s being rude and distrustful of Marble’s sister, but the tigershark won’t listen, instead pointedly pushing away from the shore and diving into the depths - such as they are - of the stream.");
	...
	public function dumbshitCowCuntsTryToDrugYouBecauseTheyreCunts(teaDrinkers:int):void
	{
		...
		//new page
		if(player.inte >= 60) addButton(0,"Next",9999);
		else if(player.inte >= 30) addButton(0,"Next",9999);
		else addButton(0,"Next",9999);
	}
	...
	public function partTwoOfLowIntelligencePCsFallingForTheIncrediblyObviousCowCuntsTrap():void
	{
		...
		//9999
		clearOutput();
		output("You become aware of a pleasant warm feeling throughout through your body, and the soft echo of a pleasant tune in your mind.  It feels like you’re on the top of the world, and nothing could be better.  Everything seems right.  For awhile you just float there on the sea of bliss and pleasantness.  You occupy yourself by thinking about how good this feels, and what it reminds you of.  It isn’t easy to figure it out since the tune keeps brushing the thoughts out of your mind, but you don’t mind; you just relax in the euphoric feeling and let your thoughts float along.");
		output("\n\nFinally you think of Marble’s face, then her chest, and realize that this feeling feels a lot like when you use to nurse her.  For some reason, this makes you feel anxious and worried.  Confused, you force yourself to focus, trying to get rid of the worry.  However, this only makes you feel more and more worried until your eyes fly open, and are met with a large cow-girl bust.  It isn’t the bust of the bovine you were imagining that you were staring at, but of a more recent acquaintance.");
		...
		if(flags[kFLAGS.MARBLE_BOVA_LEVEL] <= 1) output("face");
		else output("muzzle");
		output(", you try to figure out what the heck is going on.  You remember meeting Marble’s sister, then getting tea, and then...  What did Clara say to Marble?  You groan slightly, your body feeling an odd old craving.");
		...
		if(player.weaponName != "fists") output("your [weapon] is nearby, and ");
		output(" you’re still wearing your [armor].");
		//PC chooses fight or submit.
		menu();
		addButton(0,"Fight",9999);
		addButton(1,"Submit",9999);	
	}
	
	//Chose fight
	public function fightCowCuntAfterBeingTotesMcGoatsDrugged():void {
		clearOutput();
		output("You steal yourself and ");
		if(player.weaponName == "fists") output("raise your fists");
		else output("grab your " + player.weaponName);
		output(" and get ready to fight to free yourself from this mad milkmaid’s mammaries.  Realizing your intentions, Clara’s face turns from a crazed grin to an angry glare.  With barely contained fury, she screams at you, \"<i>Oh don’t you worry darling, I will <b>make<b> you love drinking my milk!</i>\" before standing up, hefting a nasty looking heavy mace, and charging at you.");
		//go to Clara’s fight
		startCombat(9999);
	}
	//Chose submit
	...
	{
		clearOutput();
		output("While Clara is handing out the cups, you can’t help but think how strange everything about this feels.  You can’t put your finger on what exactly is wrong, but neither can you put the unsettling feeling out of your mind.");
		...
		menu();
		addButton(0,"Drink Tea",);
		addButton(1,"Don't",9999);
	...
	{
		clearOutput();
		output("You ready yourself for battle as Clara pulls an angry looking heavy mace out and says, \"<i>Fine, we’ll doo this the hard way.</i>\"");
	...
	{
		clearOutput();
		output("You sigh and accept the cup from the cow-girl’s hands and down its contents.  In moments, your vision clouds over, and you fall into her soft arms.  Just before everything goes black, you hear a voice say, \"<i>Come on now, lets run away toogether.  I’ll make sure you’re addicted before you next wake up.</i>\"");
		//go to bad end
		//9999
	}
	...
	public function highIntelligenceOutcomesVsCowCuntDrugs(teaDrinkers:int):void
	{
		...
		output("\n\nDecision time, are you going to call her out on her strange behaviour in front of ");
		if(teaDrinkers <= 1) output("Marble");
		else output("your followers");
		output(", just not drink any tea yourself, or drink it anyway?");
	...
	public function highIntelligenceCallClaraOut():void
	{
		...
		if(9999 == 0)
		{
			...
			if(9999)
			{
				output("\n\n\"<i>I wouldn’t have done that if I were you.</i>\"  Jojo says, rising off the ground and retrieving his staff.  Then falling into a combat pose.  His face is contorted into an almost uncharacteristic visage of anger directed at the woman before you.");
			}
			//if (Kiha is in camp)
			if(9999)
			{
				output("\n\nThe dragon-girl Kiha wastes no time; unceremoniously spitting flames and swinging her axe with murderously brutal intent, radiating threat even as she waits the moment to spring into attack.");
			}
	...
	Attack: Clara suddenly snatches something from a pouch at her belt. "Try this, little cutie!" She snarls, and throws a vial of potion at you.
	{Use attack text for poison potion or weakening potion, same odds as to which is used.}
	...
	//Try to run away in camp
	"You can’t run, your followers will be defenseless."

	//Try to run away while in Clara’s base
	"Clara is blocking the only entrance, you’ll have to fight her."

	//Clara is becoming aroused
	"The anger in her motions is weakening."

	//Clara is somewhat aroused
	"Clara seems to be becoming more aroused than angry now."

	//Clara is very aroused
	"Clara is breathing heavily now, the signs of her arousal becoming quite visible now."

	//Clara is about to give in
	"It looks like Clara is on the verge of having her anger overwhelmed by her lusts."
	PC wins via health
	...
	//if Clara is being fought in camp, Marble will recover on the tenth turn of combat and will take Clara out from behind.  The fight instantly ends, proceed to victory scenes where Marble takes out Clara.
	*/


	
	public function loseToClara():void
	{
		//PC loses via health	
		if(player.HP < 1) output("Your body gives out from the strain, and you fall to the ground.  The last thing you hear before you pass out, is Clara’s triumphant laughter.");
		//proceed to loss scene
		//PC loses via lust
		else output("You just can’t fight it anymore, and desperately start to masturbate in front of Clara.  Laughing triumphantly, Clara walks up to you and forces a pill down your throat.  A moment later, you pass out.");
		//proceed to loss scene
		...
		output("\n\nYou dream of odd things, distorted images that you called out for help to.  Then you were dragged away.  You kept seeing more shapes that felt familiar and important somehow, but you couldn’t focus on them.  You’d call again and again, only to be dragged away by something else again.  Eventually your cries for help become weaker and halfhearted, you know what’s going to happen next, so why bother?  Suddenly you realize that it is a cow-girl that is dragging you away, her face looks like Marble’s at first, then it splits into huge, wicked smile, and you panic.");
		output("\n\nWith great effort, you open your eyes.  You feel incredibly weak, and the desire to go back to sleep is is almost overpowering.  It doesn’t help that your head is spinning like a top, but the adrenalin rush from your recent nightmare allows you to force back the lethargy and drag yourself up off the ground and take in your surroundings.");
		output("\n\nIt looks like you’re in an enclosed building with stone walls, similar to a cellar.  Given the amount of dust around the walls, this place hasn’t been used in a long time.  There is one entrance to the room, it's a hallway that turns a corner just outside the room and moves beyond of your view.  You were lying down on a bedroll, ");
		//(height check, <70/70-82/>82)
		if(player.tallness < 70) output("much too big for you.");
		else if(player.tallness <= 82) output("just your size.");
		else output("way too small for your stature.");
		output("  You also find that you aren’t wearing your " + player.armorName + ", and your " + player.weaponName + " is nowhere to be seen.  ");
		...
		if((pc.hasCock() && pc.smallestCockArea() <= 40) && pc.biggestTitSize() <= 25 && !pc.isTaur() && !pc.isNaga() && !pc.isGoo() && !pc.isDrider())
			output("Instead a plain but rather slim form fitting two piece garb adorns your body.  Whoever dressed you in that definitely wanted your bodily assets to be fully visible to anyone looking at you.");
		else output("You’re completely naked.");
	...
	public function losingToCowCuntsPageII():void
	{
		...
		{
			output("\"<i>Oh?  Wondering how I know about that?  Marble told me when she was explaining about you breaking free of the addiction.  Unlike her, I’m going to make sure you behave yourself.</i>\"  ");
		} 
		else
		{
			output("\"<i>Confused?  That was one of the things Marble found out when she was researching the nature of her addiction.  She said she didn’t want to trouble you with that knowledge, but I don’t see why.</i>\"  ");
		}
		output("This woman is nothing like her sister, she loves the power she has over you, and loves to show you just how much she has.");

		output("\n\nClara leans down and puts her face right in front of yours.  \"<i>I think that if you want to drink some of my milk, you’re going to have to do some things for me.</i>  Her voice is soft, but the meaning couldn’t be harder.  \"<i>From now on, you aren’t aloud to go anywhere without my say so, and I’m the only one you’re aloud to love.  Either emotionally or physically.  Understand?</i>\"  You shudder, and think of ");
		if(loversCount() == 1) output("Marble");
		else output("the others");
		output(".  Then your eyes drop down to Clara’s chest, and you think of your need.");

		output("\n\nIt feels like you’re ordering the deaths of everyone you’ve ever cared about, but you do nod your head.  \"<i>That’s not goood enough, " + pc.mf("lover-boy","lover-girl") + ".  I want to hear you say, ‘I love you, I love only you, and I will love only you.’  If you don’t, then there will be no milk for you.</i>\"  An exaggerated sad expression falls across her face, and it sounds like she is chastising a misbehaving child as she says that.");
	...
	public function cowCuntBadEndPartIII():void
	{
		...
		if(player.hairLength > 0) output("through your " + hairDescript());
		else output("across your scalp");
		output(", then they suddenly grip down tight.  \"<i>Let’s see if you can pick up the pace.  Drink faster, my love slave!</i>\"  You do as the voice asks, and loud moans of pleasure follow.");

		output("\n\nYou suck as hard as you possibly can: you need to!  You can’t possibly care about anything else than satisfying your need, and you want to do it in the best possible way for the subject of your love.  You don’t know how long this continues for, but eventually you’re pulled back abruptly from the tap from which the elixir of the gods flows.  You notice that your " + legDescript() + " is wet.  You look down and find that Clara’s dress is soaked, right in front of her womanhood.  \"<i>Time to switch nipples, " + player.mf("lover-boy","lover-girl") + "</i>\" Clara says breathlessly, calling your gaze back up.  \"<i>This time, we’re going to try it a bit slower.</i>\"");
		//new page
		menu();
	...
	public function cowCuntBadEndPartIV():void
	{
		...
		{
			output("At this point your " + multiCockDescriptLight() + " is rock hard, and Clara gives it a few strokes.  She does seem to lose interest surprisingly quick though.");
		}
		else if (player.hasVagina() && !player.hasCock())
		{
			output("You’ve become quite wet at this point, and Clara gives your " + vaginaDescript() + " a few rubs, before tugging on your " + clitDescript() + " once.");
		}
		else if (player.hasCock() && player.hasVagina())
		{
			output("Your arousal at this point is quite apparent on the whole of your genitals.  Clara gives you a few touches and caresses, but doesn’t seem all that interested.");
		}
		else
		{
			output("Or rather, your lack of thereof.  Clara doesn’t seem to mind too much, Marble must have told her about this part of you too.");
		}
	...
	public function badEndDickThatFitsClara():void
	{
		...
		if((pc.hasCock() && pc.smallestCockArea() <= 40) && pc.biggestTitSize() <= 25 && !pc.isTaur() && !pc.isNaga() && !pc.isGoo() && !pc.isDrider())
		{
			output("In seconds she’s freed you of the form fitting garment that you woke up in.  ");
		}
		output("You’re still erect from the arousal that nursing from Clara gave you.  The young cow-girl straddles your calves and gives your " + cockDescript(x) + " a slight flick with her finger and continues to smile broadly at you.  \"<i>One thing that I was really worried about was that you woouldn’t have a penis that I could actually fit inside me.  Thankfully, that isn’t the case, and that pretty pecker is just my size!</i>\"  She pulls apart the lips of her womanhood before continuing, \"<i>This will be my first time ever putting anything in here, take note just how special of a slave you are to me!</i>\"");

		output("\n\nWell, she certainly isn’t wasting any time, as the very next instant she has lowered her opened labia onto the tip of your " + cockDescript(x) + ".  The passage is already wet and slick, thanks to her previous orgasm.  Immediately, an expression of both pain and pleasure crosses over the bovine woman’s face and she starts to pant.  You lift up your hands, but a sharp command from Clara that you aren’t to do anything at all while she has the sex she wants.  You can’t do much else but just lie back and take it.");
		...
		output("\n\nFinally, Clara stops teasing you, and lowers herself down.  She gives several cute sounding gasps and long oos before the whole of your length is inside her.  At which point she promptly creams herself and cries out aloud, \"<i>Moo!</i>\"  Her orgasm is quite a wet one, liberally drenching your waist and crotch.");
		output("\n\nAny fears that you’re going to be left wanting are quickly dispelled, as no sooner had the once virgin woman finished her second orgasm, she has started to violently pump her body up and down.  The abrupt change in the kind of sex you’re having catches you off guard and blows your mind almost instantly.  Clara isn’t doing much better, as all that is coming out of her mouth at this point is just mindless pleasure-filled babbling.");
		output("\n\nThe sex is not much more than a blur of pleasurable sensations, lust addled nonsense, wet slapping of two bodies hitting one another, and the bouncing of two giant breasts.  It is worth noting that Clara does improve her sexual skills quite considerably during the interim.  Once you get to the end of the frantic lovemaking, she adds hip rotations, pelvic thrusts, and rubbing to her desperate pumping.");
	...
	public function cockTooBigForCowCuntageBadEndage():void
	{
		...
		if(pc.cockTotal() > 1) 
		{
			output("All of your penises are");
			else output("Your penis is");
			output(" too big for a virgin like me to fit!  How the hell am I suppose to get that inside me?</i>\"  Clara stands up and starts stomping around the room.  ");
		}
		//if (PC has at least one dick that will fit inside Marble’s current capacity)
		else if(pc.cockThatFits(9999) >= 0)
		{
			output("\"<i>’It’s not a flaw, it’s a feature,’ she said, ‘This is why we train ourselves to take big endowments,’ she said!  Fuck you Marble!  I’m not sticking that stupid big thing inside me.</i>\"  ");
		}
		else
		{
		...
		output("She continues ranting for several minutes before turning to you.  \"<i>YOOU!  Didn’t you ever stop to consider the needs of dainty girls like me before you started abusing transformatives?</i>\"  She sighs, apparently running out of steam.  \"<i>Who knew this plan would be so hard?");
	...
	public function defeatClaraCuntInAFight():void
	{
		...
		{
			output("Worried about the others, you head out of the cellar and drag Clara and her things with you.  Thanks to the strange way that travel works in this world, it doesn't take you long to find your way back to camp.  You catch your followers there just before they head out to go looking for you and dump your would be kidnaper before them.  Marble steps up next to you and looks down at her sister.");
		...
		if(loversCount() > 1)
		{
			output("I'm not even" + player.short + "'s only mate.  " + player.mf("He","She") + " can satisfy their needs, but can't satisfy mine?  "); //no new paragraph
		}
		output("The frustration of not being able to nurse, someone, anyone, but especially my mate is tearing me apart!</i>\"  Marble stops screaming and her head drops.  All that's coming out of her now is heavy breathing.");
		...
		output("\n\n\"<i>Really?  Didn't you say that the reason you where purifying yourself was so that " + player.mf("he","she") + " would nurse from you again?  Sounds pretty selfish to me after what you were saying about doing things for who you 'love'.</i>\"");
		...
		if((pc.hasCock() && pc.cockThatFits(20 >= 0) || pc.hasVagina()) output("use Marble's unstable state of mind to get her into a forceful threesome with Clara, ");
		//(if PC either has a cock that will fit Marble, or has a vagina)
		if((pc.hasCock() && pc.hasCockThatFits(9999) >= 0) || pc.hasVagina()) output("have sex in front of Clara so she can see what your love really is, ");
		//(if Marble is a futa)
		if(flags[kFLAGS.MARBLE_DICK_TYPE] > 0) output("egg Marble into violating Clara with her cock, ");
		output(" or try to calm Marble down so that she doesn't do anything to her sister.");
	...
	public function murbleSpanksCowCunt():void
	{
		...
		output("\n\nMarble picks her sister up off the ground and roughly tosses her onto one of the sitting logs around the firepit.  Clara continues to sob and beg for the abuse to stop, but it is all in vain.  Marble squats down slightly behind her, winds up, and brings down the first of many harsh smakes against her rear end.  Your mate doesn't let up on her punishment until every last inch of Clara's heinous hindquarters has been covered in blue welts.  By this point, the wannabe kidnapper and slaver is nothing but a slobbering, incoherent mess.");
	...
	{
		clearOutput();
		var x:int = pc.cockThatFits(20);
		output("You step forward and suggest to Marble that the two of you should work together to punish Clara.  She nods and asks what you have in mind.  Well, you were thinking that a sexual punishment from the two of you would be appropriate.");
		output("\n\n\"<i>What?  I don't what to have a threesome with my sister!  Only " + player.short + " should be able to touch me!</i>\"");
		...
		output("\n\nThe two of you strip yourselves and the bound malefactor down.  Then consider the naked tied-up girl, deciding how to go about this.  ");
		//if (PC has a cock under 20 area, and Marble has a cock)
		if((pc.hasCock() && x >= 0) && flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
		{
			//n is the index of the PC's biggest cock under 20 area.
			output("Eventually you pull her up and sandwich her between your bodies.  Marble expresses an aversion to anal, so you take the backside while she takes the front.  \"<i>Hey, you can't do this to me!  I don't want to loose my virginity to my sister!</i>\" Clara cries out indignantly.");
		...
		else if((pc.hasCock() && x >= 0) && flags[kFLAGS.MARBLE_DICK_TYPE] == 0)
		{
			//n is the index of the PC's biggest cock under 20 area.
		...
		else if(flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
		{
			...
			output("\n\nDamn it all, why did you have to do that?  Now I'll never be able to experience it for the first time with my perfect mate!  You're evil, sister.</i>\"  Marble looks like she is about to blow up once more at that comment....");
		}
		else 
	...
	public function NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG():void {
		...
		output("\n\nFrom there, you decide it is time to let your partner lead for a while.  She is quite happy to roll her massive melons around your face while you snuggle against the wonderfully smooth skin.  Abruptly, you feel the woman whose breasts you're trapped within turnaround.  \"<i>The fuck is this, sis?</i>\" the voice is spitting mad now.  You can hear that it's behind you, evidently someone wants to make sure the audience gets a full view of your sexual escapade.");
		...
		if(pc.hasCock() && x >= 0)
		{
			output("she drops herself onto your " + cockDescript(x) + ".  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
		...
		else if(pc.hasVagina() && flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
		{
			output("she drops herself onto the ground next to you.  Abruptly, she lifts you up and drops your womanhood onto the member you gave her.  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of penetration.  A shriek of dismay accompanies this act.");
		...
		{
			output("she drops herself onto your legs, and grabs one of them.  Instantly, your two womanhoods are being grinded against one another with great force and urgency.  Since your bovine mate isn't in the habit of wearing any form of undergarments, her skirt doesn't really do much to get in the way of tribadism.  A shriek of dismay accompanies this act.");
			output("\n\nThere has been a beast awakened in Marble, and there is nothing that can stop it.  Loud squelching noises can be heard coming from your combined pussies as she roughly scissors you.  What is essentially a high speed vibrator seems to be running on your " + vaginaDescript() + " right now, and its operator's hands are firmly holding your " + legsDescript() + " to make sure you stay in place.");
		}
	...
	public function futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock():void
	{
		...
		output("\n\nA loud, \"<i>MOOOO!</i>\" abruptly rises in crescendo out of the random moans, gasps, and gibbering mess that use to be Clara.  However, her sister doesn't stop with that orgasm and continues to pound away, bringing back the babbling wreck for a few more moments.  Then, Marble's own orgam rises out in her characteristic gasp and long low moan.  A much more subdued moo then the younger sister's.");
		output("\n\nThe futa in this equation pants for a few moments, then asks, \"<i>Have... you, ha... learned your lesson now?</i>\"  When the answer comes in a incoherent moan, it looks like Marble might be about to start round two!");
		//proceed to Calm Marble down
		//9999
	}
	//Calm Marble down
	...
	public function calmMurblesFatAssDown():void
	{
		...
		{
			output("tightly hugs herself against your body.  Gently stroking her hair, you're happy to be a shoulder for her to cry on, given both your affection for the gentle girl and the awareness that you're able to help her feel better.");
		} 
		//else if (PC taller than 5 feet)
		else if(player.tallness >= 60)
		{
			output("returns the hug.  Rocking back and forth against one another, you're happy to be here for her, given both your affection for the lovely lady and the warm feeling of being so close to one you care so much for.");
		}
		else
		{
			output("scoops you up, hugging you tightly into her chest.  Sandwiched between her massive mammaries, you're happy to serve as her temporary comfort doll, given both your affection for the big bovine and the pleasant accommodation of her bountiful breasts.");
		}
	...
	public function rathazulFinishingMarblesPotion():void
	{
		...
		{
			output("Then we'll be ready to purify you and your child");
			if(flags[kFLAGS.MARBLE_KIDS] > 0) output("ren");
			output(".  ");
		}
		output("So come back a little later.</i>\"  The rodent returns to his reactions.");
	...
	menu();
	addButton(0,"WaitItOut",waitItOut4MurbleStuff);
	addButton(1,"Leave",eventParser,13);
}

//Wait it out
function waitItOut4MurbleStuff():void
{
	//Leads straight to Formula is done
	//9999
...
{
	clearOutput();
	output("Rathazul tells you that the formula is ready, so you head over to Marble to let her know.");
	//Scene adapted from Rayfire's scene
	output("\n\nYou find her sitting on a ruined low stone wall at the edge of camp, still deep in thought.  It takes a few moments for you to scramble up next to her, and you take a seat just a short ways from her.  She gives a hard sigh and fiddles with her brown hair a bit before resuming her long thought. It's not till you tap her shoulder and call out her name that she turns to you with a startled look.  \"<i>S-Sweetie!</i>\" she calls out in a surprised manner and looks up at you, eyes lidded with dried tears.  You ask if she minds you sitting next to her and she agrees half-heartedly.  You both sit in silence for a few minutes before she brings up the courage to even utter a word to you.");
	output("\n\n\"<i>S-So sweetie, how is your condition, are you okay?\"</i>  you assure her you're okay, nothing you haven't dealt with before.  \"<i>Oh that's great, I've been worrying about that for a while now.</i>\"  She looks at you and gives a forced smile.  \"<i>Rathazul has finished the potion now I take it?  You came to pick me up?</i>\"  Marble starts to get up, but you stop her and grab her hands when you realize that they are trembling severely, both out of nerves and worry. It's in her face that you can can see that the poor bovine woman was putting up a front, masking the pain she feels while trying not to worry you.");
	output("\n\nYou calmly tell Marble that you're quite certain that she wouldn't have come out here to think on her own for so long if all she was worried about was your condition.  There is something else bothering her, something that she doesn't want you to worry about.  You're here for her just as she is there for you.  Just tell me what is wrong.");
	output("\n\nMarble's forced smile fades and her features resume the crestfallen look she had earlier, thinking about the right words to say to you.  Seeing as there is no other way around the conversation she says, \"<i>To be honest sweetie, you're right, I'm not alright.</i>\"  There is a moment's hesitation before she continues, \"<i>Oh " + player.name + ", so much has happened in such a short time and it's all that's on my mind. After all the things that Clara has said I just don't know what to do.</i>\"");
	...
	output("\n\n\"<i>The day when I met you, I felt that you were just one of the cutest things that I've ever seen.  I immediately took a liking to you after you helped me out with my breasts.  You spent time with me, talked to me, worked alongside me, and everyday my affection for you grew stronger.  I didn't think it was love at first, but everyday I looked forward to a visit from you.</i>\"  You give a warm smile hearing those words from her, happy to know your feelings were mutual as your relationship advanced.  \"<i>I was so happy when you agreed to drink the milk directly from my breasts.  It made me so happy to nurse someone like that again and remember the utter bliss I felt from that.</i>\"");
	...
	output("\n\nMarble bursts into tears and pulls you into a nearly back-breaking hug, calling out your name and declaring her love for you as she continues to embrace you.  She gives you a deep kiss and you return it wholeheartedly.  When she finally lets you go, she jumps down from the wall, giving you a delightful, beautiful smile.  \"<i>You really are too good for me " + player.short + ".  I feel alot better now.</i>\"");
	...
	output("\n\nWe could cut the monsters claws, you suggest.");
...
{
	clearOutput();
	output("The two of you arrive at Rathazul's workspace, and find the owner awaiting you, holding a bottle of whitish fluid with gold streaks running throughout.  \"<i>So is that the formula?</i>\"  Marble asks looking at the bottle.");
	...
	if(flags[kFLAGS.MARBLE_KIDS] > 0)
	{
		...
		output(" right away.  Give ");
		if(flags[kFLAGS.MARBLE_KIDS] == 1) output("her");
		else output("each");
		output(" a two drop dose,</i>\" the alchemist instructs you and hands over the bottle of his formula.");

		output("\n\nYou and Marble waste no time in heading over to the nursery and feed ");
		if(flags[kFLAGS.MARBLE_KIDS] > 1) output("each of ");
		output("your child");
		if(flags[kFLAGS.MARBLE_KIDS] > 1) output("ren");
		output(" two drops of the formula.  ");
		if(flags[kFLAGS.MARBLE_KIDS] > 1) output("They ");
		else output("She ");
		output("take");
		if(flags[kFLAGS.MARBLE_KIDS] == 1) output("s");
		output(" it easily enough, and while there are no outward effects, ");
		if(flags[kFLAGS.MARBLE_KIDS] == 1) output("your ");
		else output("each ");
		output("child does report feeling like something was lifted from ");
		if(flags[kFLAGS.MARBLE_KIDS] > 1) output("their");
		else output("her");
		output(" mind.");

		output("\n\nIt's with very light hearts and spirits you and the father of your children return to report the results of the formula.  \"<i>Excelent!  I will be sure to watch them closely to be sure that there are no lingering effects, but you and your family are likely now the first pure bovine-kin on Mareth in a long time.</i>\"");
	}
...
{
	clearOutput();
	output("Now there is only one more loose end in this whole effort to purify Marble: her sister.");
	...
	output("\n\n\"<i>Fuck off, mutant.</i>\" she dismisses her sister in an instant.  \"<i>I want to tell my destined mate what kind of a life [he/she] is missing out on!  I know you still want true lacta bovine milk.  Once you've tasted it, you know that you truly want it forever.  Come away with me, be my slave, and you'll be truly happy again!</i>\" a sick smile plays over her face, her madness clear for all to see.");
...
{
	clearOutput();
	output("You ask Marble for the bottle of Rthazul's formula.  She nods, hands it to you and says, \"<i>Yeah, I think that's the best choice too.</i>\"");
...
{
	clearOutput();
	output("You say that you're going to be keeping Clara a prisoner in the camp.  She deserves to be imprisoned so she'll no longer be a threat to anyone anymore.  Marble looks uncertain at you and asks if you're sure she needs to be locked up now that she's been purified.  You say you're certain, and your mate nods to you and says that she trusts your judgement on this matter.");
...
{
	clearOutput();
	output("You say that now that Clara is no danger to anyone anymore, she is free to go, so long as she promises to never bother you or your followers again.  Marble nods at this and says she agrees with your decision.");
...
{
	clearOutput();
	output("No, you'll offer her the mercy of letting her keep her precious corruption.  Marble tells you she will respect your decision on that matter.  Now the question is what to do with her.");
	output("\n\nYou guess you could also build a cage in the camp if you really want to keep an eye on her yourself.  That will make her another mouth to feed, and you'll have to hear her ranting every day.  Still, the saying does go, \"keep your enemies closer.\"");
	output("\n\nIf you're feeling especially generous, you could just let her go, with not more than a promise to never come back or try to kidnap you again.");
	//PC makes their choice: Tel'Adre, camp cage, let go."
	//9999
...
{
	clearOutput();
	output("You say that you're going to be keeping Clara a prisoner in the camp.  She deserves to be imprisoned so she'll be no threat to anyone anymore.  Marble nods to you and says that she trusts your judgement on this matter.");
...
{
	clearOutput();
	output("\"<i>It has been quite the morning, hasn't it sweetie?</i>\"  It certainly has been, but at least now, your lover is finally free of her corruption once and for all.");
	output("\n\nYou've gained x exp for helping Marble attain purity.");
	//increase exp by x
	//9999
