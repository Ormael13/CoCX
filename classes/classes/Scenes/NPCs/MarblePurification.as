...
package classes.Scenes.NPCs {
	...
	public function fightCowCuntAfterBeingTotesMcGoatsDrugged():void {
		clearOutput();
	 	outputText("You steal yourself and ");
	...
	public function loseToClara(combat:Boolean = true):void
	{
		...
		//PC wakes up laying down on a bed roll.  They feel very groggy and weak.  After a moment, they realize that they’ve got a strong craving for milk and that they need to drink it if they’re going to survive.  Clara then shows up and proceeds to demonstrate her control over the PC, taking great pleasure in it.
	 	...
	 	outputText("You dream of odd things, distorted images that you called out for help to.  Then you were dragged away.  You kept seeing more shapes that felt familiar and important somehow, but you couldn’t focus on them.  You’d call again and again, only to be dragged away by something else again.  Eventually your cries for help become weaker and halfhearted, you know what’s going to happen next, so why bother?  Suddenly you realize that it is a cow-girl that is dragging you away, her face looks like Marble’s at first, then it splits into huge, wicked smile, and you panic.");
	...
	{
		clearOutput();
	 	outputText("It is then that you realize with a shock that a familiar urge is welling up inside you.  No, it is much stronger then you remember it being before.  You want breastmilk.  You <i>need</i> breastmilk, and if you don’t get it, you’ll die.  To make matters worse, you know who’s breastmilk it is too.  Clara, Marble’s younger sister, has fatally addicted you to her milk.");
	...
	public function cowCuntBadEndPartIV():void
	{
		...
		menu();
		if(!pc.hasCock()) addButton(0,"Next",cocklessPCsGetAbusedByCowCunts);
		else if(pc.cockThatFits(20) < 0) addButton(0,"Next",cockTooBigForCowCuntageBadEndage);
	...
	{
		clearOutput();
	 	outputText("Clara straddles head with her legs and looks down at you.  \"<i>Now you’re going to lick me until I’m satisfied.  Also, don’t you dare doo anything unless I tell you to, got it?</i>\"  She doesn’t wait for a response.  \"<i>Goood, now let's get started.</i>\"  The cow-girl promptly drops her snatch onto your face.");
	...
	public function defeatClaraCuntInAFight():void
	{
		...
		if((player.hasCock() && player.cockThatFits(9999) >= 0) || player.hasVagina()) outputText("have sex in front of Clara so she can see what your love really is, ");
		//(if Marble is a futa)
	...
	//does not work with centaur, naga, drider, or goo bodies.  However, I don't particularly want to just say you can't do this scene if you have them.
	{
		...
		if((player.hasCock() && x >= 0) && flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
		{
			//n is the index of the PC's biggest cock under 20 area.
		 	outputText("Eventually you pull her up and sandwich her between your bodies.  Marble expresses an aversion to anal, so you take the backside while she takes the front.  \"<i>Hey, you can't do this to me!  I don't want to loose my virginity to my sister!</i>\" Clara cries out indignantly.");
	...
	public function NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG():void {
		...
		var x:int = player.cockThatFits(9999);
	...
	function theFormulaIsDone():void
	{
		...
		//Scene adapted from Rayfire's scene
	 	...
	 	outputText("\n\nMarble's forced smile fades and her features resume the crestfallen look she had earlier, thinking about the right words to say to you.  Seeing as there is no other way around the conversation she says, \"<i>To be honest sweetie, you're right, I'm not alright.</i>\"  There is a moment's hesitation before she continues, \"<i>Oh " + player.short + ", so much has happened in such a short time and it's all that's on my mind. After all the things that Clara has said I just don't know what to do.</i>\"");
	...
	{
		clearOutput();
	 	outputText("You ask Marble for the bottle of Rthazul's formula.  She nods, hands it to you and says, \"<i>Yeah, I think that's the best choice too.</i>\"");
	...
	function purifiedSuccubusMilkForPureMarbles():void
	{
		...
		addButton(1,"No",eventParser,9999);
	}
	//if yes
	...
	function nursingFromPureMarble():void
	{
		...
		{
		 	outputText("Feeling a bit thirsty, you ask Marble if theres any chance she has some milk for you.  She gives you a sad look and informs you that no, she hasn't yet built up enough milk for you to have more.  Purifying her has made it take a bit longer for her to produce milk.");
			//9999
		}
		else
		{
	...
	public function pureMarbleDecidesToBeLessOfABitch():void
	{
		...
		clearOutput();
	 	outputText("While checking your traps to insure that they're still working, you hear a voice call out from outside the camp, \"<i>Sweetie!</i>\"  You look up and see the smiling face of Marble");
		//if (PC has at least one child with Marble)
