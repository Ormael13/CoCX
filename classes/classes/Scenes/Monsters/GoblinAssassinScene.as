	...
	public class GoblinAssassinScene extends BaseContent
	{
		...
		public function goblinAssassinEncounter():void {
			...
			//Repeat Intro - Cell Chambers
				...
				output("A familiar sight catches your attention as a shadowy blur lands in front of you - another goblin assassin appears to be lurking around these parts. You ready your [weapon] as she straps on a belt loaded with various types of needles and assumes a combat stance.");
				//[Initiate combat encounter – goblin assassin]
			}
...
//[LOSS SEXAHNZ]
		public function gobboAssassinBeatYaUp():void {
			...
			if(player.cockThatFits(monster.vaginalCapacity()) >= 0 && player.hasCock() && (!player.hasVagina() || rand(2) == 0)) 
			{
				...
				if(pc.balls > 0) output("[sack], giving it a gentle caress before vanishing.");
				else if(pc.hasVagina()) output(vaginaDescript() + ", the tips giving your soaked mound only the barest ghost of a touch.");
				...
				if(pc.femininity < 40) output("almost ");
				output("girlish moan.  The need thrumming through [eachCock] is telling you - no, demanding - that you do what she says and fuck her.  Like a beast in rut, you groan.  You look the short-but-stacked aggressor in her big, liquid blue eyes and make a decision entirely rationalized by your aching, drug-fueled needs.  Still pumping your shaft like a man possessed, you whimper, \"<i>Yes... I... I came to have sex with you.</i>\"");
				output("\n\nVeridian skin caresses you just above (the sheath/your base) for a brief moment but vanishes before you can gain any satisfaction from the act.  \"<i>I’m sorry, that was too slow,</i>\" the diminutive fighter teases, a moment before she slams a pair of needles into your [legs].  [EachCock] rises and imeediately spurts a trickle of white pre-cum, unable to to resist the potent chemical concoction flooding your bloodstream, so perfectly tailored to inflame your passions past the point of reason.  You fail to notice the removal of the spent plungers, but you sure as hell notice the return of the lithe green woman’s probing caresses.  She whispers, \"<i>Now tell me how other girls don’t matter.  This was all just a ruse to get you some green love, wasn’t it?</i>\"");
				...
				output("  The emerald assassin glances over her shoulder at you and urges, \"<i>Come on now.  Fill me up.  Let out all that spunk I’ve been making build up ");
				if(pc.balls > 0) output("in your balls");
				else output("inside you");
				output(".  Pay your ‘fine’, and I might let you go.</i>\"");
				if(pc.balls > 0) output("  A hand squeezes your [sack] encouragingly");
				else output("  Hands squeeze your [butt] encouragingly");
				output(" as the goblin impales herself to the hilt with a sloppy squelch.  You orgasm, cumming hard inside her.  Your back arches, your body clenches, and rope after rope of seed sprays inside the goblin’s baby-hungry womb.  Her pussy seems to drink it down, wicking the jizz away from you just as fast as you can put it out.");
				if(pc.cumQ() >= 750) {
			...
			else if(player.biggestCockArea() > monster.vaginalCapacity() && player.hasCock() && (!player.hasVagina() || rand(2) == 0))
			{
				...
				output("You tear through your [armor] in a rush, anything to release [eachCock] from its hellish confinement.  You’re so aroused, so horny.  Any passing thoughts of modesty are immediately discarded and forgotten, washed away under the wave of your all-consuming lust as your fingers slide home around [oneCock] and begin to play with the turgid tool.  You immediately buck from the sensation of incredible, pent-up need, nearly cumming on the spot.  Looking up, you meet the emerald-skinned woman’s eyes pleadingly.");
				...
				if(pc.biggestCockLength() < pc.tallness/2) output("sitting on your face, the " + cockHead(x) + " of your maleness jutting against her drooling lips.");
				else output("sitting on the ground ‘above’ you, your " + cockHead(x) + " reaching all the way up to spear her moist box, judging by what you’re feeling anyway.");
			...
			//iii. Lust Loss - Female
				...
				output("Defeated by your own needs, your fingers dart to your [armor], rapidly disassembling it in your hurry to expose your aching, tender puss to the goblin’s eyes.  The assassin watches you with an unimpressed, bemused expression, though she remains rooted in place, her gaze locked onto you as you succumb to your lusts.  Knowing that you have an audience, you twist and contort your body to present your [chest] and [vagina] to the petite greenskin.  Glistening moisture forms on your exposed nether-lips, tempting you to slide your fingers into their slick warmth.  As if you could resist.");
				...
				if(pc.hasFuckableNipples()) output("piston in and out of your [nipple], masturbating it just as hard as the real thing.");
