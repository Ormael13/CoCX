...
use namespace kGAMECLASS;
	...
	public class ImpScene extends BaseContent
	{
		...
		public function impVictory():void {
			...
			if (player.tailType == AppearanceDefs.TAIL_TYPE_MANTICORE_PUSSYTAIL) addButton(7, "Tail Rape", uniquuuesexscene.manticoreTailRapeScene);
			if (player.canOvipositBee()) addButton(8, "Oviposit", putBeeEggsInAnImpYouMonster);
			if (player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_PLANT_FLOWER) addButton(9, "Get Pollinated", uniquuuesexscene.alrauneGetPollinatedScene);
		...
		private function centaurOnImpFemale(vape:Boolean = false):void {
			...
			//Not gaping?
			if(player.vaginas[0].vaginalLooseness <= AppearanceDefs.VAGINA_LOOSENESS_GAPING) {
		...
		*/
		...
		private function sexAnImpLord():void {
			...
			if(player.lust >= 33 && flags[kFLAGS.SFW_MODE] <= 0) {
				...
				if (player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_PLANT_FLOWER) addButton(5, "Get Pollinated", uniquuuesexscene.alrauneGetPollinatedScene);
			}
			addButton(14,"Leave",cleanupAfterCombat);
		}
		...
		private function loseToImpLord():void {
			...
			if(player.cockTotal() == 1) {
				...
				outputText("\n\nYou feel your [cocks] grow harder than usual and throb.  You go to stroke yourself but they are far too sensitive. Any stroking you can do is far too little stimulation and anything else is to painful too withstand.  You whimper and curse in desperation.  Your lust clouded mind can only think of one solution; you bend over and reveal your " + assholeDescript() + " to the grinning imp.  The humiliation keeps you from looking back to see the imp's reaction, but you can tell by his chuckle that this is exactly what he wanted.");
			}
		...
		public function impPackEncounter():void {
			clearOutput();
			outputText("During your searching thou current location you suddenly hear sound of many wings flapping.  Turning around you notice a large group of imps flying toward you.  In no time their catch up to you and surrounds.  No way around it, you ready your [weapon] for the fight.");
		...
		public function impPackGetsRapedByFemale():void {
			...
			if(player.vaginas[0].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLICK) outputText("and coating it with feminine drool ");
			...
			if(player.vaginas[0].vaginalWetness >= AppearanceDefs.VAGINA_WETNESS_SLAVERING) outputText("Cunt juices splatter him as you squirt explosively, leaving a puddle underneath him.  ");
