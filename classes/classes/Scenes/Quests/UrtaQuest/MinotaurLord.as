package classes.Scenes.Quests.UrtaQuest
{
	...
	public class MinotaurLord extends Monster
	{
		...
		override protected function performCombatAction():void
		{
			...
			else if (!hasStatusAffect(StatusAffects.Timer)) minotaurLordEntangle();
			else if (hasStatusAffect(StatusAffects.MinotaurEntangled)) minotaurCumPress();
			else {
		...
		private function minotaurDrankMalk():void //Only procs during Urta's quest.
		{
			...
			if (statusAffectv1(StatusAffects.MinoMilk) < 3) {
				...
				addHP(300);
				lust += 10;
				if (!hasStatusAffect(StatusAffects.MinoMilk))
