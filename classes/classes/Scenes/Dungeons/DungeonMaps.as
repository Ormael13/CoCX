...
public class DungeonMaps extends DungeonAbstractContent
	{
		...
		public function chooseRoomToDisplay():void {
			for each (var map:DungeonMap in maps)
				if (map.playerIsHere()) {
					rawOutputText(map.parseMap());
					return;
				}
			if (dungeonLoc >= DUNGEON_FACTORY_FOREMANS_OFFICE && dungeonLoc < DUNGEON_CAVE_ENTRANCE) { //Factory
				outputText("Factory, 2F");
