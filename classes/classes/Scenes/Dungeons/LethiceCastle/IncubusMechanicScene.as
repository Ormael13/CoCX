	...
	public class IncubusMechanicScene extends BaseContent
	{
		...
		public function approachIncubus():void {
			...
			if (flags[kFLAGS.LETHICE_CASTLE_INCUBUS_GREETED] == 0) {
				...
				outputText("The mechanic doesn't seem like he'd stop you from turning around and walking back into the complex, but if you want to use the lift, you're going to need to accede to his demands or fight him. What do you do?");
				if (flags[kFLAGS.FACTORY_INCUBUS_DEFEATED] == 0) flags[kFLAGS.FACTORY_INCUBUS_DEFEATED] = 1; //Set to 1, incubus no longer in factory.
				flags[kFLAGS.LETHICE_CASTLE_INCUBUS_GREETED] = 1;
			}
			else {
				if (flags[kFLAGS.LETHICE_CASTLE_INCUBUS_DEAL] != 2) {
					outputText("\"<i>Back again?</i>\" the incubus queries while he reclines against the softly thrumming machinery. \"<i>I was just doing some calibrations on my baby here. Now she purrs like a kitten.</i>\" He affectionately pats the metal devices that power the lift. \"<i>If you want to use her, you're going to need to pay up, [name]. 500 gems seems fitting recompense, but between you and me, I'd rather have you on your knees polishing my python here.</i>\" The mechanic's cock suddenly rears up, swaying back and forth in a decidedly snake-like motion, the light glistening off his purple-hued pecker as it stiffens encouragingly.");
					...
					outputText("As soon as the incubus mechanic spots you approaching, his wriggling, exotic cock settles slightly, growing longer and thicker in anticipation. The demonic, corrupted nubs that encircle its girth pulsate as the whole of his dong floods with arousal, and you cannot help but imagine the feel and taste of it on your tongue once more, the corrupted taste making your mouth salivate with such intensity that you're forced to swallow to keep yourself from drooling all over yourself. Why does his dick taste so goddamn good?");
					outputText("\n\nThe grinning demon offers, \"<i>If you want to use the lift, you'll need to pay the toll.</i>\" He strokes himself enticingly, no longer offering you gems, only the musky aroma of his tumescent cock.");
					outputText("\n\nIt appears that your only options are to walk away, suck his cock, or fight him.");
				}
			}
			dynStats("lus", 10);
			menu()
			if (player.gems >= 500 && flags[kFLAGS.LETHICE_CASTLE_INCUBUS_DEAL] <= 0) addButton(0, "Pay Toll", payToUseLift)
			addButton(1, "Suck Dick", suckIncubusDick);
			addButton(2, "Fight", initiateFightIncubus);
			addButton(4, "Leave", eventParser, 1);
		}
		
		public function payToUseLift():void {
			clearOutput();
			spriteSelect(30);
			outputText("You unceremoniously fill a small pouch with 500 gems and toss it to the incubus.");
			...
			flags[kFLAGS.LETHICE_CASTLE_INCUBUS_DEAL] = 1;
			player.gems -= 500;
			statScreenRefresh();
			kGAMECLASS.dungeons.lethicecastle.exitDungeonByLift();
		}
		...
		public function suckIncubusDick():void {
			...
			if (player.hasVagina()) {
				...
				if (player.lust < 33) outputText("even more soaked than normal");
				else if (player.lust >= 33 && player.lust < 66) outputText("wet with fresh lust");
			...
			flags[kFLAGS.LETHICE_CASTLE_INCUBUS_DEAL] = 2;
			dynStats("lib", 5, "cor", 5, "lus", 100);
			kGAMECLASS.dungeons.lethicecastle.exitDungeonByLift();
		}
		
		public function initiateFightIncubus():void {
			clearOutput();
			spriteSelect(30);
			outputText("\"<i>No way! I'm not paying the toll! You're going down!</i>\" you yell. \"<i>Very well. But if I beat you up, you're going to have a bad time,</i>\" the incubus says with an intimidating look.");
			startCombat(new IncubusMechanic(), true);
		}
		
		public function incubusDefeated():void {
			clearOutput()
			flags[kFLAGS.LETHICE_CASTLE_INCUBUS_DEFEATED] = 1;
			outputText("The mechanic collapses, his overalls "); 
			if (monster.HP <= 0) outputText("falling off his prone from, shredded.")
			...
			cleanupAfterCombat();
			if (player.hasCock()) {
				if (player.smallestCockArea() > 120) outputText("\n\n<b>Unfortunately, your manhood is too big to fit into the incubus's ass!</b>")
			}
			menu();
			addButton(0, "Kill Him", killIncubus);
			addButton(1, "Let Him Go", letIncubusGo);
			if (player.hasCock() && player.smallestCockArea() <= 120 && player.lust >= 33) addButton(2, "Buttfuck", fuckIncubusButt);
			if (player.hasVagina() && player.lust >= 33) addButton(3, "Female Rape", rapeIncubusAsFemale);
		}
		
		public function killIncubus():void {
			clearOutput();
			flags[kFLAGS.LETHICE_CASTLE_INCUBUS_DEFEATED] = 2;
		...
		public function letIncubusGo():void {
			clearOutput();
			...
			outputText("The incubus ");
			if (monster.HP <= 0) outputText("slowly struggles to his feet");
			...
			outputText("The incubus mechanic flies away for good, though you can’t can say whether he will wreak havoc or live quietly.");
			doNext(1);
		}
		
		public function fuckIncubusButt():void {
			clearOutput();
			flags[kFLAGS.LETHICE_CASTLE_INCUBUS_DEFEATED] = 3;
			var x:int = player.cockThatFits(120);
			outputText("The incubus may have done his best to seduce you with his " + (monster.lust >= 80 ? "throbbing hard": "gently bobbing") + " dick, but you've got a better idea. You got more than one look at the curvature of his butt while you were fighting, tempting your burgeoning bulge to the point of distraction. You hastily remove your [armor] and smile. [EachCock] springs up, full and hard. You take " + (player.cocks.length > 1 ? "one": "it") + " in hand, pumping slowly as you eye your tainted prey. He whimpers " + (monster.lust >= 100 ? "in need,": ",but") + " his cock seems to get even bigger and harder. The poor guy doesn't realize what a reaming he's in for.");
			...
			outputText("\n\nYou grab " + oMultiCockDesc() + " with one hand and the incubus' waist with the other. Your heart is hammering in anticipation. Growling in the back of your throat, you align " + (silly() ? "tab C with slot buttfuck": "your eager tool with the corrupted anus") + " and steadily push. At first, his asshole shuts you out. The sphincter is just too tightly clenched for anything, let alone something so full and thick, to fit inside. You give up on the push and begin rocking your [hips] back and forth, getting his soon-to-be-stretched hole used to the idea of having you inside him. A droplet of pre-cum escapes your " + cockHead() + " as you work, smearing over him.");
			...
			if (player.cumQ() < 150) outputText("\n\nYou pull" + (player.hasKnot() ? ", popping your knot": "") + " out after depositing your meager load into your demonic fuck-hole. One slick strand of creamy spunk is leaking out over the demon's ballsack.");
			else if (player.cumQ() >= 150 && player.cumQ() < 350) outputText("\n\nYou pull" + (player.hasKnot() ? ", popping your knot": "") + " out of the demon's creampied fuck-hole with a smile. His stretched-out ass looks virtually glazed with cum, and you can see small rivers of it trickling down over his balls and slowly shrinking cock.");
			else if (player.cumQ() >= 350 && player.cumQ() < 1000) outputText("\n\nYou pull" + (player.hasKnot() ? ", popping your knot": "") + " out of the demon's well-fucked asshole with a smile. As soon as the bulk of your dick unplugs him, a river of white spills out, washing over his balls and cock to drip down into his puddle. You smirk at the white-glazed demon. His butt is still full of your spunk.");
			else outputText("You pull" + (player.hasKnot() ? ", popping your knot": "") + " out after depositing a massive, belly-bulging load directly into the demon's guts. A tidal wave of goo bursts out of his over-pressurized asshole to fall on the ground, and as it dies off, small rivers of ivory spunk escape the lake in his ass to run down his balls and cock. His belly is {pudgy/swollen/absolutely gravid} with the distended weight of the cum packing his body. From time to time, fresh streamers of sperm squirt out of his thoroughly cream-pied backdoor.");
		...
		public function rapeIncubusAsFemale():void {
			clearOutput();
			flags[kFLAGS.LETHICE_CASTLE_INCUBUS_DEFEATED] = 3;
			...
			outputText("\n\nAbruptly, you drop your [butt] down on top of him, sitting square on top of his chest" + (player.hasLongTail() ? ", tail draped across his face": "") + ", and casually begin to undress. Looking over your shoulders, you see hope in his soulless eyes. You tease, \"<i>I wouldn't want to stain my clothes when I finish you off, would I?</i>\" His dick wilts a little at that, and he looks about to faint. Giggling, you toss aside your [armor] and grab hold of his mostly-hard cock, feeling the corrupted, textured nodules that encircle his shaft flaring up at your gentle squeeze. He firms up nicely in your grip, filling back to a pulsating, rigid hardness in seconds.");
			outputText("\n\n\"<i>Hey, you can transform your dick, right?</i>\" you call over your shoulder. The demons voice comes back as a dry, irritated rumble. \"<i>That and more, mortal. Would it please you to shape my cock for your pleasure?</i>\"");
			outputText("\n\n");
			if (player.findPerk(PerkLib.BimboBrains) >= 0 || player.findPerk(PerkLib.FutaFaculties) >= 0 || player.lust >= 33) outputText("Giggling, you answer, \"<i> Like totally! Let's give you a cock makeover!</i>\"");
			else outputText("Nodding and languidly stroking his fantastically useful phallus, you answer, \"<i>Maybe. I still haven't decided what to do with you. </i>\"");
			outputText("\n\nWhat kind of dick would you like the incubus to have when you fuck him?");
			menu();
			addButton(0, "Demon", chooseCockType, 0);
			addButton(1, "Horsecock", chooseCockType, 1);
			addButton(2, "Dog cock", chooseCockType, 2);
		}
		
		public function chooseCockType(type:Number):void {
			clearOutput()
			if (type == 0) {
			...
			if (type == 1) {
				...
				monster.createCock(22, 4, CockTypesEnum.HORSE);
			}
			if (type == 2) {
				player.removeCock(0, 1);
				...
				monster.createCock(12, 3, CockTypesEnum.DOG);
			}
			doNext(rapeIncubusAsFemalePart2);
		}
		
		public function rapeIncubusAsFemalePart2():void {
			clearOutput();
			outputText("You dare not delay any further. Your [vagina] is so hot and slick with lube that it feels almost like it's steaming. It aches for penetration. Lifting yourself up off your [legs], you guide your wanton honeypot up until it's poised just above the " + eCockDescript() + "");
			if (player.averageVaginalWetness() > 3) outputText(", letting your free-flowing lubricant wash over it, lubricating it with your glorious girlcum");
			else if (player.averageVaginalWetness() > 1) outputText(", letting your oh - so - juicy vagina drip and dribble, soaking it with your copious girlcum");
			...
			if ((player.vaginalCapacity() * 0.75) > monster.biggestCockArea()) { //Easy fit
				...
				if (monster.horseCocks() > 0) outputText(", your belly bulging with his equine shape");
				else if (monster.dogCocks() > 0) outputText(", your lips slurping noisily about his knot");
				outputText(".");
			}
			else if (player.vaginalCapacity() > monster.biggestCockArea()) { //Decent fit
			...
			else if (player.vaginalCapacity() * 1.5 > monster.biggestCockArea()) { //Too tight!
			...
			player.cuntChange(monster.biggestCockArea(), true, true, true);
			outputText("\n\nYou hold yourself atop him and let your abdominal muscles flutter around the incubus' " + eCockDescript() + " like an organic sextoy, getting used to holding him inside you. Turning to look back over your shoulders, you place a hand upon his chest feel his well-defined muscles. You stop to admire his nipple, tugging at it a little bit. He growls in irritation, but you just squeeze his dick with your " + (player.vaginalCapacity() < monster.biggestCockArea() ? "well-stretched": "stuffed")+ " twat to silence him.");
			outputText("\n\n\"<i>Are all demons controlled this easily?</i>\" you muse out loud. \"<i>A few squeezes from a cunt and you're content to let me do whatever I want to you. I could probably slap a collar around your neck and take you back to camp without hearing a word of complaint. Perhaps 'please fuck me,' but I doubt I'd hear any legitimate desire to come back here.</i>\" You lift yourself up an inch and glide back down, teasing him. \"<i>Poor boy, they keep you so pent up, don't they?" + (flags[kFLAGS.FACTORY_INCUBUS_BRIBED] > 0 ? "Nothing but hentai mags for you to drain your balls with...": "") + "</i>\"");
			...
			if (monster.horseCocks() > 0) outputText(", the head flaring to seal itself against your cervix");
			else if (monster.dogCocks() > 0) outputText(", the knot popping in just in time to start bloating, locking you in place}, and jism begins to shoot out in waves.");
			if (!player.isPregnant()) { 
				outputText("Your empty womb drinks up the virile, seed - filled sludge with abandon. ");
				if (monster.horseCocks() > 0) outputText("After all, that bestial horse-cock doesn't really leave it anywhere else to go. The demonic animal-cunt just continues spurting straight on in to have its way with your eggs, and you're sure there isn't a contraceptive this side of Mareth that could stop you from having his imps.");
				else if (monster.dogCocks() > 0) outputText("After all, that immense knot doesn't really leave it anywhere else to go. Once every fold is bathed in alabaster doggie-cream, the continued ejaculations start pouring in after your eggs. His knot is so big and stretching you so wide; you don't think he would pop out even if he filled your belly to look nine months pregnant. Worse, or perhaps better, you're sure with how thick and virile his seed feels that you'll be getting pregnant with his imps from this. ");
			}
			else {outputText("Your blocked womb deflects the seed, forcing it all to stay in your channel ");
				if (player.vaginalCapacity() < 120) outputText("and drip out over his balls, much to his chagrin");
				else outputText("and stretch you further.");
			}
			outputText("\n\nThe feeling triggers your own climax. Your [vagina] clenches down around his " + eCockDescript() + " and gushes out around it, splattering the demon's crotch and legs the whole way down to his hooves. Arcs of femcum actually jet across the air{, even though you aren't normally a squirter}. The pleasure is so great that you can feel electric impulses arcing between neurons, short-circuiting your throughs into involuntary ecstasy. Your eyes roll back, and your [hips] twitch and shake, vibrating around the object that brought you to this perfect " + (!player.isPregnant() ? "insemination": "tryst") + ".");
			outputText("\n\nYou both cum for what feels like hours" + (player.hasCock() ? ", shooting waves after waves of semen all over the incubus": "") + ". Yet, all good things must come to an end. You do have a queen bitch to depose, after all." + (monster.dogCocks() > 0 ? " When you try to rise, you find that his knot is still stubbornly inflated, trying to hold all his fertile sperm deep inside you. It won't come out! Glaring at him, you put as much of your gear on as possible and start dragging him through the dungeon after you until he has the good sense to shift his cock into another shape. He weakly slides out, panting something about seeing you at your camp. He's gone by the time your gear is properly settled.": "When you rise, it amazes you just how many inches of cock are sliding out of your well-used snatch. The " + eCockHead() + " eventually slips free, and you watch in awe at how his musky semen runs out from your gaped, thoroughly creamed lips. By the time you dress, he's vanished, but you recall hearing him panting something about seeing you at camp.") + "");
			...
			dynStats("lib", 5, "cor", 5);
			player.orgasm();
			cheatTime(1 / 2, true);
		}
		
		//Decide bad end!
		public function loseToIncubus():void {
			if (silly()) {
				clearOutput();
				outputText("\"<i>You've lost,</i>\" the incubus laughs. \"<i>Now I get to decide what Bad End you'll receive!</i>\"");
				if (player.hasCock()) doNext(incubusLossMaleBadEndPart1);
				else doNext(incubusLossFemaleBadEndPart1);
			}
			else {
				if (player.hasCock()) incubusLossMaleBadEndPart1();
				else incubusLossFemaleBadEndPart1();
			}
		}
		...
		public function incubusLossMaleBadEndPart1():void {
			...
			if (player.lust >= player.maxLust()) {
				...
				else if (player.biggestCockLength() < 12) outputText("\n\n\"<i>You're so… average. I suppose you'll do, if the succubi feel like having a drab, everyday orgasm.</i>\" The incubus shrugs. \"<i>A dick will do…</i>\"");
				else if (player.biggestCockLength() < 18) outputText("\n\n\"<i>Huh….</i>\" The incubus seems pleasantly surprised by your endowment. \"<i>I gotta say, that'll probably work good for what I have in mind. Maybe you'll even be popular with the succubi.</i>\" He laughs.");
				else {
					outputText("\n\n\"<i>Seriously?</i>\" the incubus asks. \"<i>How am I supposed to work with something so gargantuan?</i>\" He disdainfully pulls a vial labeled \"Reducto\" from his pocket and upends it over your crotch. The effect is immediate. [pc.EachCock] shrinks from ridiculously large down to horse-sized. <i>\"There, that's better.\"</i>");
					player.cocks[player.biggestCockLength()].cockThickness = 3;
					player.cocks[player.biggestCockLength()].cockLength = 18;
				}
				outputText("\n\nYou hump against your palm and lick your lips encouragingly. The sheer need thrumming through your veins will allow nothing less. You want to fuck and be fucked. To be used and abused. To sink {your/a} length inside any hold and thrust until you're quivering in orgasmic release. That gorgeous demon-cock taunts you, swaying just out of reach.");
			}
			else {
				...
				outputText("\n\nLeering down at you, the incubus swiftly tears away your [armor], his fingers suddenly razor-sharp claws. He is precise in his attentions, leaving your " + player.skinDesc + " pristine and unbroken. You get the impression he doesn't want damaged goods. He regards your " + multiCockDescriptLight() + " with interest, judging them.");
				if (player.biggestCockLength() < 6) outputText("\n\n\"<i>That's it? You expect to please anyone with such a pitiful endowment? I suppose there may be someone into that, somewhere.</i>\" The incubus sighs. <i>\"Your flaws only make my job harder.</i>\"");
				else if (player.biggestCockLength() < 12) outputText("\n\n\"<i>You're so… average. I suppose you'll do, if the succubi feel like having a drab, everyday orgasm.</i>\" The incubus shrugs. \"<i>A dick will do…</i>\"");
				else if (player.biggestCockLength() < 18) outputText("\n\n\"<i>Huh….</i>\" The incubus seems pleasantly surprised by your endowment. \"<i>I gotta say, that'll probably work good for what I have in mind. Maybe you'll even be popular with the succubi.</i>\" He laughs.");
				else {
			...
			outputText("\"<i>Now, you may have been a champion before, but by right of battle, you're little more than my property. It's time to make your appearance match your new station,</i>\" the tainted mechanic explains.");
			doNext(incubusLossMaleBadEndPart2);
		}
		public function incubusLossMaleBadEndPart2():void {
			clearOutput();
			outputText("First, he reaches into his pocket and produces a shining, golden ring. It gleams with unnatural brightness in his hand, far too thick to be worn on a finger and yet far too narrow to be a collar of any kind. The demon twirls it over his knuckles and kneels beside you, letting his member flop against your cheek. A trickle of corrupted pre-seed oozes out onto your cheek, and you cannot help but turn your head to suck his crown into your mouth. As you suckle and polish the phallus, your throat works to swallow everything he gives you, and he feeds you plenty.  ");
			...
			if (player.cocks.length > 1) {
				...
				outputText("\n\n<b>The ring is being forced down your bundled shafts. Below, your dicks remain as they are. Above, they're being merged together into one impressive tool. {The skin of your transforming length is darkening to a purplish hue, and in the wake of the magical cock-ring, nodules are slowly emerging, growing into an imitation of the cock you're still suckling. }The incubus chuckles, easily pushing the ring the rest of the way down, smiling at the way your demonic tip pulsates and leaks pre-cum. Once it's nestled firmly against your groin, your new master withdraws his hands (and his cock). <b>Your singular, demonic cock is impossibly hard.</b>");
			}
			else {
			...
			doNext(incubusLossMaleBadEndPart3);
		}
		public function incubusLossMaleBadEndPart3():void {
			clearOutput();
			...
			player.orgasm();
			dynStats("lib=", 100, "cor", 50, "lus=", 1000);
			doNext(incubusLossMaleBadEndFinale);
			
		}
		public function incubusLossMaleBadEndFinale():void {
			clearOutput();
		...
		//Bad end for females and genderless
		public function incubusLossFemaleBadEndPart1():void {
			...
			if (!player.hasVagina()) {
				if (player.lust >= player.maxLust()) {
					...
					outputText("\n\nSoon, those licks turn to sucks, and those sucks into a devilishly pleasurable blowjob. One hand winds to your steaming box while the other gropes at your [fullChest], {pinching and tugging on {nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to/your [nipples] one after another}./slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.} Alas, all good things must come to an end, and your lovely, lovely little lollipop is rudely pulled from your questing lips with a noisy pop.");
				...
				if (player.lust >= player.maxLust()) {
					...
					outputText("\n\nSmiling, the overall-clad corruptor drags his thick tool up your cheek and across your lips, leaking his slime as he goes, and he gleefully watches you try to maintain your disdain in the face of his lascivious influence.");
					outputText("\n\nGods, your pussy is wet, but you've got to fight it! You have to hold on, maybe bite him or something to give you the time to recover and get back on the offensive! Yeah… biting might do the trick. You inhale to steel yourself for the oral assault, getting a nice, long whiff of his supremely potent pheromones in the process. Your [clit] gets so stiff that you start writhing your [hips] in order to squish your cunt-lips tight around. <i>Mmmm, gotta attack…</i> you think, but your heart just isn't in it anymore. You open your mouth, letting him push his tip inside, so that you can bite him, of course. Oh, the flavor is so intense… so strong and masculine and perfect on your tongue that you decide NOT to chomp down. Not yet. You can spend a little time recovering first, with his wonderfully tasty, throbbing-hot demon-cock on your happily licking tongue.");
					outputText("\n\nSoon, those licks turn to sucks, and those sucks into a devilishly pleasurable blowjob. One hand winds under your [armor] to tend to your steaming box while the other gropes at your [fullChest], {pinching and tugging on {nipple after nipple, your myriad breastly endowments providing you with oh so many buttons to tend to/your [nipples] one after another}./slipping inside a [nipple] to thrust just like your other hand. You work your upper and lower pussies like a woman possessed.} Alas, all good things must come to an end, and your lovely, lovely little lollipop is rudely pulled from your questing lips with a noisy pop.");
					outputText("\n\nYou give a mewl of disappointment and open your eyes to see the incubus standing back a few feet, smirking at you, his cock hard, jutting, and leaking. That simply won't do! A dick like that… it needs to be sucked… tended to… until it can cum! You struggle out of your [armor] and up onto your [feet], staggering towards it with your hand still firmly lodged in your [vagina]. You've got to tempt him back to you, or catch him at the very least.");
				}
			}
			dynStats("lus=", 100);
			doNext(incubusLossFemaleBadEndPart2);
		}
		
		public function incubusLossFemaleBadEndPart2():void {
			clearOutput();
			...
			outputText("\n\nThe source of your lust is soon bending double to fasten more bindings around your waist and [hips], attacking them to more chains by the sounds of it. Soon after, the devices lift you up off the ground that you{ don't even need to support yourself anymore/r coils don't get sore/knees don't get sore}. How wonderful! If you move just right, you can make the whole contraption sway slightly to get a proper dick-sucking rhythm going. His pearly pre-cream oozes onto your tongue, and you swallow as much of it as you can.");
			...
			outputText("\n\nThe demon must read your question in your eyes, because he nods understandingly. \"<i>Yes, little mortal, this draught is going to make you wetter and hornier than you can possibly image. That cute little twat you've got is going to my own personal lubricant-fountain. I suspect you may even manage to learn how to bring yourself off by shifting your [hips] to make your lips rub your [clit] just right.</i>\" He shrugs. \"<i>I don't really care, so long as that cunt of yours stays wet enough to shame the goo-girls at the lake.</i>\" His hand ruffles your hair once more before he steps back to watch you drink this the milky, medicinal goo.");
			outputText("\n\nYou can already feel the first few swallows affecting you. Your [skin] is flushing, worse than before, and your nether-lips are getting so wonderfully slippery that you can feel your moisture beading on your mons. Your [clit] fares little better, swelling up nearly double it's old size{, even if that makes it truly monstrous indeed}. It drips long, sticky strands of girlish lust as you swallow, only becoming more soaked as time passes as your body becomes even more productive. A plaintive whimper escapes around the edges of your phallic gag, but your cry falls on deaf ears. The incubus is too busy fitting pipes together beneath you to care.");
			...
			dynStats("lib=", 100, "sens=", 100);
			doNext(incubusLossFemaleBadEndFinale);
		}
		
		public function incubusLossFemaleBadEndFinale():void {
			clearOutput();
