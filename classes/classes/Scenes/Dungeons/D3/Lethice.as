	...
	public class Lethice extends Monster
	{
		...
		override public function get long():String
		{
			...
			else
			{
				...
				{
					str = str + ", her wings in tatters";
				}
				str = str + ", Lethice regards you with a snarl and a crack of her flaming whip. Her yellow-black eyes are wide with fury, the pointed tips of her teeth bared in a snarl. Gorged with lethicite, the enraged demon queen is determined to finish you. Her endurance can’t possible hold for too much longer. You can beat her!";
			...
			if(player.findStatusAffect(StatusAffects.LethicesRapeTentacles) >= 0)
			{
				...
				{
					str = str + " A blast of white-fire could probably dispel them, or you could rely on your";
					if(player.canFly())
					{
						str = str + " flight";
					}
					...
					str = str + " to stay safe. Either way, you’re free for now.</b>";
				}
			}
		...
		{
			var atks:Array = [this.demonicArouse,this.demonfire];
		...
		private function demonicArouse():void
		{
			...
			if(player.findStatusAffect(StatusAffects.MinotaurKingsTouch) >= 0)
			{
				l = l * 1.25;
			}
			game.dynStats("lus",l);
		...
		private function phase1Ends(hpVictory:Boolean) : void
		{
			...
			pronoun2 = "them";
			pronoun3 = "their";
			if(findStatusAffect(StatusAffects.PhysicalDisabled) >= 0)
			{
				removeStatusAffect(StatusAffects.PhysicalDisabled);
			}
			if(findStatusAffect(StatusAffects.AttackDisabled) >= 0)
			{
				removeStatusAffect(StatusAffects.AttackDisabled);
			}
			combatRoundOver();
		}
		...
		{
			var atks:Array = [this.demonLustMagic,this.dirtyDancing,this.hornyPoke,this.crushingBodies];
			if(rand(10) == 0 && player.findStatusAffect(StatusAffects.Blind) < 0)
			{
		...
		private function phase2Ends(hpVictory:Boolean) : void
		{
			...
			{
				game.addButton(0,"DemonFuck",this.p2DemonFuck,hpVictory);
			}
			if(player.findStatusAffect(StatusAffects.KnowsHeal) >= 0)
			{
				game.addButton(1,"Heal",this.p2Heal);
			}
			game.addButton(2,"Next",this.p2Next);
		}
		...
		private function p2DemonFuck(hpVictory:Boolean) : void
		{
			...
			game.menu();
			game.addButton(0,"OralFinish",this.oralFinish);
			if(player.hasCock())
			{
				game.addButton(1,"FuckDemon",this.fuckDemon);
			}
			game.addButton(2,"RideCock",this.rideCock);
		}
		...
		private function beginPhase3(doLethNext:Boolean) : void
		{
			...
			if(doLethNext)
			{
				game.addButton(0,"Next",this.p2Next);
			}
			else
			{
				combatRoundOver();
		...
		private function phase3():void
		{
			...
			{
				atks = [this.parasiteThrowingStars,this.whiptrip,this.sonicwhip];
