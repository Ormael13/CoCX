...
/**
	...
	public class SuccubusGardenerScenes extends BaseContent
	{
		...
		public function fuckUpTheGardener(hpVictory:Boolean):void
		{
			...
			addButton(2, "Fuck Her", player.gender == 0 ? curry(tentaFail, -1, true) : //genderless
				curry(sceneHunter.selectGender,
					curry(sceneHunter.callBigSmall, tentaFail, 30, -1, "length"), //dick selector
					curry(tentaFail, -1) //vag call
				)
			);
		}
		...
		private function tentaFail(cock:int, createdVag:Boolean = false):void {
			...
			if (cock < 0) outputText(" cunt-kissers");
			...
			if (cock >= 0) outputText(" [oneCock]");
			else if (!createdVag) outputText(" your [vagina]");
			...
			if (cock >= 0) outputText(" blow a wad down this slut's throat");
			else if (!createdVag) outputText(" cream yourself on this slut's face");
			...
			if (cock < 0) outputText(" Its outline is clearly phallic, but how could such a huge thing ever fit inside you, let alone anyone?");
			else {
				outputText(" Its tip oozes lubricants, but the gaping pussy at its tip is big enough to hold six minotaurs' monster-cocks.");
				if (player.cocks[cock].cockLength < 36) outputText(" How could you ever hope to fill it?");
			...
			//Dicks
				...
				outputText("\n\nThat monstrous, hungry-looking pussy wastes no time in descending towards your [cocks].");
				if (player.cocks[cock].cockLength < 12) outputText(" The ease with which you slide inside is no surprise, given the disparity in size.");
				else if (player.cocks[cock].cockLength < 30) outputText(" The ease with which you enter is no real surprise, given that even your bloated length" + ((player.cocks.length > 1) ? "s are" : " is") + " small by comparison.");
				else if (player.cocks[cock].cockLength < 48) outputText(" You slide in with ease, perfectly matched to the hungry fuck-tunnel in size.");
				...
				if (player.cocks[cock].cockLength < 12) outputText(" than a giant that looks that loose should. You'd swear it was tight little twat from how firmly it's squeezing you!");
				else if (player.cocks[cock].cockLength < 30) outputText(" than you had thought at a glance. Not only is it the perfect size, but it seems to hug and touch every part of your maleness" + ((player.cocks.length > 1) ? "es" : "") + " just right.");
				else if (player.cocks[cock].cockLength >= 48) outputText(" than you would expect given how tightly-stretched it looks. You'd think it'd be pressing down painfully, but it feels tailor made to take you instead.");

				//Dicks need to grow
				if (player.cocks[cock].cockLength < 30) {
				...
				// Dicks big enough
					...
					outputText("\n\nGlowing orange goo leaks out around your base" + ((player.cocks.length > 1) ? "s" : "") + ". That must be what passes for the tentacle's girlcum. It feels like normal lubricant, at least. It doesn't burn or numb you, and it certainly doesn't seem to be making you grow anymore. Wait... you close your eyes and focus on the feel of it. It's... it's better than normal lubricant! You're not sure why or how, but it feels subtly slicker than a normal pussy, and not in a bad 'no friction' kind of way. Every fold and silken caress still rubs your nerves just right. How can it be so slick and yet so powerfully soft and gripping?");

					outputText("\n\nThe inside of the tentacle contracts slightly, yet the accompanying pleasure is anything but slight. It feels like the whole world tightens against your [cocks], cradling them in cotton softness girded with velvet silk. You moan and cum, flooding the tube with the creamy goo of your release. Your body tries to thrust, but you're held completely immobile, forced to climax to the tentacle's tune. The ropes of pure pleasure are soon squirting to the timing of the gloriously textured fuck-tunnel, and it is only when your orgasm ends that you realize what the orange goo is doing to you; it's making you more sensitive.");

					outputText("\n\nYou don't go soft after you finish squirting. You don't even get to catch your breath. The orange channel might be holding still as it digests your load, but your dick" + ((player.cocks.length > 1) ? "s are" : " is") + " still getting more sensitive. You don't even need motion for the clutching walls to arouse you anymore, and once it starts moving... you're not sure you'll be able to handle it.");
		...
		public function surrenderToTheGardener(hpVictory:Boolean = false):void
		{
			...
			sceneHunter.selectLossMenu([
					[0, "Dick", curry(maleLoss, hpVictory)],
					[1, player.hasVagina() ? "Vagina" : "???", curry(femGenderlessLoss, hpVictory), "Req. a vagina.", player.hasVagina()]
				],
				"Well, shit. Your adventure is over. But at least you are in good hands - the succubus will treat you nice and definitely find some uses for your dick... or do you want her to use your other parts instead?\n\n"
			);
		}
		...
		private function maleLossPet():void
		{
			...
			outputText("\n\nThe first flows splatter on impact. You're lucky enough to have a few flecks land on your lips, and you tentatively test them with your tongue. They're delicious! You lick and lap at every stray drop");
			...
			outputText("\n\nYay! You plod along after her, low to the ground and sniffing at the air. Mistress has a very unique aroma, now that you take a chance to appreciate it, like earth and berries and sex all mixed up into one scent. You've never sampled its ilk before. Other aromas bring themselves to your attention, some wafting from side passages and other mere traces on the ground, but you're aware of them all, intrigued by them all and wishing that you could run off to see what every single one was. Maybe after you discover your new home, she'll let you go for a run.");
		...
		private function maleLossPetII():void
		{
			...
			if (player.skinType != Skin.FUR) outputText(" Was that there before?");
		...
		private function maleLossFeeder():void
		{
			...
			//[Next]
			doNext(maleLossFeederII);
		}

		private function maleLossFeederII():void
		{
			clearOutput();
			outputText("Trapped in a hellish loop of constant yet unsatisfied orgasm, you're pulled into the sea of tentacles. Your eyes have rolled back too far to see the grinning demoness or her confidently swaggering derrière, but if they could, they'd grow wide with worry once the twisting, dripping vines blocked her out. Hunger and thirst soon compete with pleasure for your attention. The swelling of your belly has already receeded, the fluid inside long side converted to ivory jets of release.");
