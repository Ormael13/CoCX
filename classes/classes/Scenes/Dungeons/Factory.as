	...
	public class Factory extends DungeonAbstractContent
	{
		...
		private function goDemon():void {
			...
			if(player.gender == 1) {
				...
				if(player.cockTotal() > 1) {
					...
					if(player.balls > 0) outputText("your " + ballsDescriptLight() + " and ");
			...
			//[HERM ENDING]
				...
				outputText("She effortlessly swings her lissomelegs onto the table as she pulls herself up, mounting you in a single swift motion.  You can feel waves of heat rolling off her sex, bathing your " + cockDescript(0) + " in her warmth.  ");
		...
		private function chooseBreasts():void {
			...
			//Multiple Rows...
				...
				outputText("Your top " + breastDescript(0) + " tingle pleasantly as the magic takes effect.  You watch with fascination as they begin to swell up, like sponges exposed to water.  The top of your " + player.armorName + " is pulled tight by change, until you're chest seems ready to burst free.  <b>You've gained 4 cup sizes!</b>  ");
		...
		private function chooseDick():void {
			...
			if(player.cocks.length > 1) {
				...
				if(player.demonCocks() == player.cockTotal()) {
					outputText("Your " + multiCockDescriptLight() + " leap forwards, taking to the dark magic with ease.  Inch after inch of new length erupts from your groin as your " + multiCockDescriptLight() + " get longer and thicker.  They pulsate, as if promising dark pleasure as they settle into their new enhanced size.");
					temp = player.cocks.length;
					while(temp > 0) {
						temp--;
				...
				//Not yet full of demoncocks...
					...
					outputText("You smile, watching your " + multiCockDescriptLight() + " grow longer.  Inches of newfound dick-flesh erupt from your crotch in response to omnibus' dark magics.  Before you can play with your transformed pleasure tools, a wave of burning desire passes through you.  You watch");
					if(player.cor < 80) outputText(" in horror");
					else outputText(" curiously");
					outputText(" as the skin of your " + multiCockDescriptLight() + " turns shiny and purplish-black.  Corrupt nodules begin to spring up over the entire length of each dick.  ");
					temp = player.cocks.length;
					while(temp > 0) {
						temp--;
		...
		private function normalGroin():void {
			...
			//Remove multiple.
			if(player.cocks.length > 1) {
				outputText("Your " + multiCockDescriptLight() + " shiver and retract back towards your body.  When the process finishes you are left with only your " + cockDescript(0) + ".  ");
		...
		public function doBadEndSuccubusPart1():void {
			...
			//Tits – regular
			if(player.biggestLactation() < 1) outputText("Your " + nippleDescript(0)  + "s begin prodding painfully against your " + player.armorName + ", every touch serving to make them harder and more erect.  ");
			//Tits – lactating
			if(player.biggestLactation() >= 1 && player.biggestLactation() < 3) outputText("Your " + nippleDescript(0) + "s get painfully hard as you feel milk begin backing up inside your " + allBreastsDescript() + ".   The succubus glances down mischieviously as her hands begin to grope you through your " + player.armorName + ", squeezing out a few drops of milk.  ");
			//Tits – megalactating
			if(player.biggestLactation() >= 3) outputText("Your " + nippleDescript(0) + "s get painfully hard as milk begins drooling down your over-productive chest, making your " + player.armorName + " slide across your leaky milk-spouts in an agonizingly pleasurable way.  ");
			//Cock – single
			if(player.cocks.length == 1) {
				if(player.cockArea(0) < 30) outputText("Swooning from sudden blood loss, you struggle to maintain the kiss as your body takes your " + cockDescript(0) + " to full hardness in seconds.  ");
				//Cock – single big
			...
			if(player.cocks.length == 0) {
				...
				if(player.vaginas.length > 0) outputText("a vacuum pump to your clit, and a pump many times bigger than your " + cockDescript(0) + " to it.  ");
		...
		private function doBadEndSuccubusPart2():void {
			...
			if(player.cocks.length > 0) {
				...
				//either or:
				outputText("Dimly, you feel a needle lodged in your taint, pumping your prostate full of something.  Your " + cockDescript(0) + " begins growing mid-orgasm, the skin turning a deep purple even as small nodule-like bumps form all over it, rapidly becoming a bloated parody of it's demonic counterparts.  ");
			}
			//Puss Orgasmz
		...
		private function doBadEndOmnibusPart2():void {
			...
			if(player.cocks.length > 1) {
				...
				outputText("Overloaded with pleasure, you feel your " + multiCockDescriptLight() + " pulse and cum, creating translucent green cum balloons the size of ");
			...
			if(player.cocks.length == 1) {
				...
				if(player.averageCockLength() < 15) outputText("With one swift motion, the beast engulfs your " + cockDescript(0) + " in its slimy maw.  ");
				else outputText("Distending obscenely, the beast starts engulfing your " + cockDescript(0) + " in its slimy maw, progressing along its entire length until you can no longer see your pleasure tool.  ");
			...
			if(player.vaginas.length == 1 || player.gender == 0) {
				...
				outputText("She giggles and reaches down.  Too aroused to care, you just keep fucking her and moaning in delight.  Her hands come up and begin to massage and rub your " + allBreastsDescript() + " taking special care to tweak and tug on your nipples.  They become as hard as ");
