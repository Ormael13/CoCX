...
//------------
	public function msMenu():void {
		...
		if (player.raijuScore() >= 7 && player.findPerk(PerkLib.ElectrifiedDesire) >= 0) {
			addButton(button++, "OrgasmicLightningStrike", OrgasmicLightningStrike).hint("Sorry Liadri not wrote tooltip for this yet.");
		}
...
//This is now automatic - newRound arg defaults to true:	menuLoc = 0;
		...
		if (player.findPerk(PerkLib.LightningAffinity) >= 0) damage *= 2;
		if (player.findPerk(PerkLib.ElectrifiedDesire) >= 0) damage *= (1 + player.lust100);
		damage = Math.round(damage);
		//Shell
		if(monster.hasStatusEffect(StatusEffects.Shell)) {
			outputText("As soon as your magic touches the multicolored shell around " + monster.a + monster.short + ", it sizzles and fades to nothing.  Whatever that thing is, it completely blocks your magic!\n\n");
