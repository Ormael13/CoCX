...
//Gnoll Rape
		...
		public function defeatHyena():void
		{
			...
			if (player.lust >= 33 && flags[kFLAGS.SFW_MODE] <= 0) {
				...
				if (uniquuuesexscene.pcCanUseUniqueSexScenev2(true, null) > 0) addButton(13, "U.Sex Scenes", curry(uniquuuesexscene.pcCanUseUniqueSexScenev2, false, [defeatHyena])).hint("Other non-typical sex scenes.");
		...
		private function dickInGnollAss():void
		{
			...
			if (rand(2) == 0) {
				...
				if (player.cockTotal() > 1 && player.cockArea(x) < monster.analCapacity() && y != -1) {
					...
					//(if cockTotal>4)
					if (player.cockTotal() > 4 && player.cockArea(0) + player.cockArea(1) + player.cockArea(2) + player.cockArea(3) < monster.analCapacity()) outputText("  And then the next.");
					//(if cockTotal>5)
