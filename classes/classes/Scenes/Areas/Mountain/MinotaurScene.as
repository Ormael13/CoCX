...
*/	//Hungry for cum?  Grab a snickers.
	...
	//No rape, no feeder
		...
		if(monster.lust >= monster.eMaxLust()) outputText("You smile in satisfaction as the " + monster.short + " drops down on all fours and begins masturbating feverishly.");
...
private function minotaurGetsRapedByHerms():void {
	...
	if(player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS_GAPING) outputText("The intense feelings seem to grow more and more powerful as he stays inside you, his drug-like pre-cum making your " + vaginaDescript(0) + " tingle from the inside-out.");
...
public function getRapedByMinotaur(autoRape:Boolean = false):void {
	...
	if(player.cockTotal() == 1) outputText("The bull-man bends forward a little, and grabs your " + cockDescript(0) + " in a crushing grip. He makes short jerking motions as he keeps thrusting into you.");
	if(player.cockTotal() > 1) 	outputText("The bull-man bends forward a little, and grabs one of your " + cockDescript(0) + "s in a crushing grip. He makes short jerking motions as he keeps thrusting into you.");
	if(player.vaginas.length > 0) {
...
private function getOralRapedByMinotaur():void {
	...
	if(player.cockTotal() >= 1) {
		outputText("  Your " + multiCockDescriptLight() + " slap");
...
private function minoGetsTitFucked():void {
	...
	if(monster.lust >= monster.eMaxLust()) outputText("You push his hands away and y");
	else outputText("Y");
	...
	if(player.totalCocks() > 0) outputText("  Your " + cockDescript(0) + " grows harder and more aroused than ever before while your ");
...
private function minotaurBlowjob():void {
	...
	outputText("revealing your " + multiCockDescriptLight() + " to the defeated bull.  The beast looks to you in both arousal and shock, mouth parting slightly at the idea of another creature taking charge.  Within the musky stench of its dripping shaft you nearly lose yourself, and ");
