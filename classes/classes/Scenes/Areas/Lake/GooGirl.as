		...
		private function gooGalAttack():void
		{
			...
			//everyone else
				...
				if (findPerk(PerkLib.Acid) >= 0) {
					...
					if (!player.hasStatusAffect(StatusAffects.AcidSlap)) player.createStatusAffect(StatusAffects.AcidSlap, 0, 0, 0, 0);
				}
...
//Engulf – 
		private function gooEngulph():void
		{
			...
			if (!player.hasStatusAffect(StatusAffects.GooBind)) player.createStatusAffect(StatusAffects.GooBind, 0, 0, 0, 0);
			combatRoundOver();
		}
