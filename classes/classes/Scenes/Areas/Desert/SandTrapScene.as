...
private function useSandTarpsHands():void {
    ...
    function dickF():void {
		...
		if(player.cockLength(x[0]) < 6) {
			// balls:
			if(player.balls == 0) outputText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + player.cockDescript(x[0]) + " to stimulate you even further.");
			else outputText("\n\nThe sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " + player.cockDescript(x[0]) + " to incite you even further, whilst it uses its free hand to gently cup your [balls].");
        ...
        case 1:
            ...
            break;
        case 2:
            outputText("  As the sandtrap begins to pump you faster, your disregarded " + player.cockDescript(x[1]) + " bobs in hard, eager concert with your " + player.cockDescript(x[0]) + ".  You gasp as the trap slows down momentarily to grasp your second sex.  It is clumsy at first as it moves its lubricated palms in tandem, then with increasing confidence as it finds a position and rhythm that suits it, until it is jerking you off furiously, clutching your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
            outputText("\n\nThe warm, oily double sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery orgasm.");
            break;
        case 3:
            outputText("\n\nAs the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your " + player.cockDescript(x[0]) + ".  You gasp as the trap slows down momentarily to reach one of its hands down to grasp your second sex, and then stops entirely as  one of its hands clutching your ass releases and snakes back around to take hold of your " + player.cockDescript(x[2]) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking all three of your cocks in rhythm.");
    ...
    function hermF():void {
		...
		//Cock 6-16 inches: 
			...
			if(player.balls == 0) outputText("\n\nThe sandtrap glides its hand up and down your " + player.cockDescript(x[0]) + ", picking up a warm smooth rhythm.");
			else outputText("\n\nThe sandtrap glides its hand up and down your " + player.cockDescript(x[0]) + ", picking up a warm smooth rhythm as it gently cups your [balls] with its free hand.]  It slows down intermittently to move its fingers around your length, letting every inch of your dick feel the tips of its delicate fingers.");
		}
		else {
        ...
        case 4:
            ...
            break;
        default:
            outputText("\n\nAs the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " + player.cockDescript(x[0]) + ".  You gasp as the trap slows down momentarily to reach both of its hands clutching your ass back around to take hold of your " + player.cockDescript(x[1]) + " and " + player.cockDescript(x[2]) + ".  It is clumsy at first as it works its way around the rather complex arrangement you have down below, but it moves its lubricated palms with increasing confidence, until it is jerking three of your cocks whilst fingering you in rhythm.  You laugh into the sky with an almost hysterical edge at how mindbogglingly good getting a quadruple handjob feels, and the trap giggles slightly as well despite itself.  Hazily you wonder if it is too much to wish that this creature, with its excellent wrist action, had just one or two more hands to its name.");
...
private function rideDatSandTarpLikeIts1999():void {
	...
	outputText("\n\n\"<i>You like that, don't you, sand sissy?</i>\"  It is breathing heavily but it still won't look into your face. \"<i>Look me in the eye!</i>\" As it finally does so you push your oil-covered fingers into its mouth and it moans around them as you swab its tongue and cheeks with its own grease.  You press your body against it, your [allbreasts] pushing into its own flat chest; it is powerless to resist you as its own pheromones take effect upon it, and you slowly push it backwards onto the flat of its back on the sloping sand.  You sit your moist " + player.vaginaDescript(0) + " at the base of its pseudo-cock and then slowly lead yourself back, luxuriating in pushing your " + player.clitDescript() + " up this oily slide.   It moans and shifts itself upwards needily, but you tut mockingly and stop your slow slide inches away from its tip.");
	outputText("\n\n\"<i>Ask for it.</i>\"");
...
private function stickWangInSandgina():void {
	clearOutput();
	spriteSelect(SpriteDb.s_sandtrap);
	sceneHunter.callFitNofit(scene, 35, "length");
    ...
    function scene(x:int):void {
        ...
        //Megacock: 
            ...
            if(player.balls > 0) outputText("  Your [balls] slap against the creature's wet opening.  ");
...
private function nagaThreesomeWithSandTrap():void {
	...
	function dickF():void {
		...
		if(player.cocks[0].cockLength < 14) {
			...
			if(player.balls > 0) outputText("slapping your [balls] into its nose, forcing your cock all the way to the top of its tight throat.");
		}
		//14 inches or more: 
		else {
