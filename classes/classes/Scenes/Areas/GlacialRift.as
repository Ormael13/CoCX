...
package classes.Scenes.Areas 
{
	...
	public class GlacialRift extends BaseContent
	{
		...
		public function exploreGlacialRift():void {
			...
			if (player.faceType == 24 && player.earType == 18 && player.armType == 10 && player.lowerBody == 33 && player.tailType == 29 && player.skinType == 1 && player.hairColor == "glacial white" && player.furColor == "glacial white" && player.hasKeyItem("Fenrir Collar") < 0) choice[choice.length] = 7; //Fenrir ruined shrine
			...
			switch(select) {
				...
				case 7: //Fenrir ruined shrine!
					if (flags[kFLAGS.FENRIR_COLLAR] == 1) {
						outputText("On your way to the glacial rift you find your way back to the temple again and the menacing voice of Fenrir echoes.", true)
				...
				case 8: //True Ice Golems
					outputText("As you take a stroll, out of the nearby glaciers emerge a group of golems. Looks like you have encountered some true ice golems! You ready your " + player.weaponName + " for a fight!", true);
					startCombat(new GolemsTrueIce());
					break;
				default:
					outputText("You spend an hour trudging through the bleak and bitingly cold glaciers but you don’t find anything interesting. But on your way back you feel you're a little more used to traveling throu this harsh area.", true);
					dynStats("tou", .5);
					doNext(camp.returnToCampUseOneHour);
			}
		}
		...
		private function leaveShrine():void {
			outputText("You're not sure if putting this collar on is the brightest idea especially considering it's an item belonging to an evil deity. You decide to leave and go back when you're ready to make a choice however Fenrir still manage to reply a sinister promise as you leave the cave.", true)
