...
public class BehemothScene extends BaseContent
	{
		...
		public function talkToBehemothMenu():void {
			...
			if (timesSexed() >= 2) addButton(3, "His Cum", talkAboutHisCum).hint("Talk to the behemoth about the special properties of his cum and how he managed to cum that much despite his ball size.");
		...
		private function talkAboutYourself():void {
			clearOutput();
			outputText("You tell the behemoth about your origins, where you originally lived in, how you've come to be the champion of Ingnam and sent through the portal. ");
			outputText("\n\n\"<i>Ingnam? That sounds like a nice place. I used to live in another world too and like you, I've gone to this world through a portal.</i>\"");
		...
		private function talkAboutSadStory():void {
			...
			outputText("\n\n\"<i>When my second birthday comes, my father told me that I was meant to be sent through the portal. I was like \"WHAT?\", and my father claimed that should the sacrifice fail, my homeworld would be doomed. Of course, I don't believe that, but I got sent into the portal anyway and I couldn't come back. The portal wouldn't let me go back at all. For the first two weeks, I check up on the portal if it opens but to no avail. Luckily, before I got sent, I was given two hundred gems. So I spend some time exploring the realms of this world.</i>\"");
		...
		private function behemothSexMenu(victory:Boolean = false):void {
			...
			menu();
			addButtonIfTrue(0, "Anal Fuck", analPitchBehemoth,
				"<b>Unfortunately, your cock is too big to fit in his butthole.</b> Req. a cock with area smaller than 72.",
				player.cockThatFits(72) >= 0, "Anally penetrate the behemoth.");
			addButton(1, "GetAnalFucked", analCatchBehemoth).hint("Have him penetrate you anally.");
			addButtonIfTrue(2, "GetVagFucked", vagCatchBehemoth, "You need a vagina for this.", player.hasVagina(), "Have him penetrate you vaginally.");
			addButton(3, "Suck His Cock", suckThatBigCock).hint("Suck his wonderful cock and get stuffed with cum!");
			addButtonIfTrue(4, "Cum Bath", haveACumBathLiterally, "Have sex with the behemoth enough times to unlock this!", timesSexed() >= 3, "Have a (literal) cum bath!");
		...
		private function haveACumBathLiterally():void {
			...
			if (flags[kFLAGS.BEHEMOTH_TENT] <= 0) {
				...
				outputText("\n\nWhat catches your eye the most is the empty metal basin at the center that looks like it could hold two people. \"<i>Do you like my tent?</i>\" The behemoth asks. You tell him that yes, you like his tent. He smiles at you and gives you thanks. ");
		...
		private function watersportsWithBehemoth():void {
			...
			outputText("\n\nYou follow the behemoth all the way to his tent, he gestures for you to enter, and you enter along with him.");
			if (flags[kFLAGS.BEHEMOTH_TENT] <= 0) {
				outputText("\n\nThe tent is nicely decorated with a bed at one corner with an end table beside it. A bookshelf holds various books, implying that the behemoth is actually an intelligent race, capable of reading and writing. At another corner you see a chair and a table where the behemoth would sit and eat or study.");
				outputText("\n\nWhat catches your eye the most is the empty metal basin at the center that looks like it could hold two people. \"<i>Do you like my tent?</i>\" The behemoth asks. You tell him that yes, you like his tent. He smiles at you and gives thanks to you. ");
				flags[kFLAGS.BEHEMOTH_TENT] = 1;
			}
		...
		private function watersportsBucket():void {
			...
			if (player.isTaur()) {
				...
				awardAchievement("Urine Trouble", kACHIEVEMENTS.GENERAL_URINE_TROUBLE, true, true, false);
				outputText("\n\n\"<i>I love you,</i>\" the behemoth says with a smile. You smile back at him, and you eventually finish peeing. The behemoth is still peeing, and the bucket is more than halfway full of mixed urine.");
