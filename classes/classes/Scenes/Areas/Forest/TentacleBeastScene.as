...
private function tentacleEntice():void {
	...
	//Spoiler for Entice Attack-Female: 
		...
		if(rand(2) == 0) {
			//GENDERLEZZ
			if(player.gender == 0) outputText("You brazenly turn your back on the creature and, glancing over your shoulder, begin bending over and presenting your [butt] to the beast. It pauses and observes while you bend over further, presenting a full view of both your back door and your " + assholeDescript() + ". You shift from side to side and observe the beast match your movements. You have obtained its attention to say the least.\n\n");
			//CHICKS
...
internal function tentacleLossRape():void {
	...
	if (player.isAlraune()){
		outputText("The tentacle beast at first is confused, mainly because it’s body fails to detect actual mammal matter. It occurs to you that you both, are in theory, plants. Well there's no such thing as restraints amongst flowers, what’s this moron waiting for?");
	...
	if(player.biggestLactation() >= 3.5 && player.gender > 0) {
		...
		player.sexReward("Detault","Nipples")
		dynStats("tou", .5, "spe", -.5, "int", -.5, "lib", 1, "sen", 1, "cor", 1);
		player.boostLactation(.5);
		monster.HP = 0;
		if (player.HP == 0) player.HP++;
