...
internal function tentacleLossRape():void {
	...
	//Centaur madness!
		...
		if(player.hasCock()) {
			...
			// has cock, not engulfable: 
				...
				//[With testicles: 
				if(player.balls > 0) outputText("The sucking reaches all the way to your " + ballsDescriptLight() + ", a spectacularly strange sensation that nevertheless feels wonderful.");
				outputText("\n\n");
			}
		}
		// has vagina: 
		...
		if(player.biggestTitSize() >= 1 && player.lactationQ() <= 0) {
			outputText("Roving tentacles latch onto your " + allBreastsDescript() + "; tiny spikes jabbing into each " + nippleDescript(0) + " and injecting some sort of hot fluid.");
		...
		// Breasts > Manly, lactating, enough to overfill: 
			...
			if(player.hasCock()) {
				outputText("Your " + cockDescript(0) + " explodes inside the creature, ");
		...
		if(player.hasCock()) {
			...
			outputText("The creature's desires are soon fulfilled as your " + cockDescript(0) + " starts to swell.  ");
			//[has testicles: 
			if(player.balls > 0) outputText("Your " + ballsDescriptLight() + " tighten up against you in preparation for their inevitable release, ready to spray their boiling load into the beast.  ");
	...
	if(player.gender == 1) 
	{
		...
		if(player.cockTotal() > 1) outputText("at the base of your " + multiCockDescriptLight() + ".");
...
private function tentacleRapeContinuation():void {
	...
	if (player.gender == 1) {
		...
		if (player.cockTotal() == 1)
			outputText("using a sucking-stroking motion on your helpless " + multiCockDescriptLight() + ". The swelling of the ass tentacle pressures your prostate in a paradoxically pleasurable and painful manner. You realize, much to your terror, that this beast is MILKING you of your semen!");
		else
			outputText("using a sucking-stroking motion on your " + multiCockDescriptLight() + ".  The swelling of the ass tentacle pressures your prostate in a paradoxical pleasurable and painful manner.  You realize, much to your terror, that this beast is MILKING you of your semen!");
