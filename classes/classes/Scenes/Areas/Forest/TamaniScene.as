...
private function tamaniFirstTimeConsentual():void {
	...
	if(player.balls > 0) outputText("Your " + ballsDescriptLight() + " swell with seed, spurring your desire to new heights.");
	...
	if(player.cockArea(0) <= 90) {
		...
		//[CORRUPT]
		if(player.cor > 66) outputText("  Your " + cockDescript(0) + " twitches at the thought, ready and wanting more.");
	}
	//(TOO BIG)
	else {
		...
		if(player.balls > 0 && player.hoursSinceCum > 200) outputText(", visibly draining your " + ballsDescriptLight() + " down to their normal size");
...
private function tamaniMaleRepeatEncounter():void {
	...
	if (player.findPerk(PerkLib.SoulSense) >= 0 && flags[kFLAGS.SOUL_SENSE_TAMANI] < 4) flags[kFLAGS.SOUL_SENSE_TAMANI]++;
	if (flags[kFLAGS.SOUL_SENSE_TAMANI] == 4) {
...
internal function tamaniSexLetHer():void {
	...
	if(player.isTaur()) {
		...
		//If >18 Inches 
			...
			//High cum: 
				...
				if(player.balls > 0) outputText("Your " + ballsDescriptLight() + " ache and you feel very thirsty indeed.");
	...
	//Not-taurs
		...
		if(player.cockArea(0) <= 90) {
			...
			if(player.biggestTitSize() >= 2) outputText("Your " + allBreastsDescript() + " bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.");
			...
			if(player.balls > 0) outputText("Your " + ballsDescriptLight() + " twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.");
		...
		//(Doesnt fit)
			...
			outputText("The kinky goblin dominatrix on your " + cockDescript(0) + " leans even further forwards, angling the dildo in your " + assholeOrPussy() + " upwards to squeeze your ");
...
internal function tamaniSexLost():void {
	...
	if(player.balls > 0) outputText("cups your " + ballsDescriptLight() + " tightly");
...
internal function tamaniSexWon():void {
	...
	if(player.cockArea(x) <= 90) {
		...
		outputText("It's too tempting of a target to resist.  You open your " + player.armorName + " and allow your " + multiCockDescriptLight() + " to flop free.  You're already hard from the enticing display, and in a moment you're pressing against her lust-slicked pussy");
...
private function tamaniPregnantFuck():void {
	...
	//(Doesn't Fit)
		...
		else outputText("jacking the base of your " + cockDescript(0) + " with her feet");
...
internal function getRapedByTamaniYouHypnoSlut():void {
	...
	//[DOESNT FIT]
		...
		outputText(cockDescript(0) + ".  Her warmth spreads over the " + player.cockHead() + " of your " + cockDescript(0) + ", but will go no further.  You tug harder on her bouncing backside, but you're simply too large to fit inside your wife's beautiful pussy.  She turns around and places a small hand ");
