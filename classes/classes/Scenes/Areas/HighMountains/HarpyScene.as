...
//*If genderless, rape options are Finger Her*
		...
		public function harpyLossU():void
		{
			...
			//No fitu
				...
				clearOutput();
				sceneHunter.print("Check failed: vagina or cock fitting " + monster.vaginalCapacity() + " area.")
				outputText("Though you've been defeated by the harpy, it doesn't seem she wants anything to do with you, and gives you a whack upside your head before departing.");
				cleanupAfterCombat();
			}
		}
...
//Requires pussy or cock small enough for harpy!
		private function harpyLossLust():void
		{
			...
			if (player.hasVagina() && (x < 0 || sceneHunter.uniHerms && rand(2) == 0)) {
				...
				outputText("When you recover a few hours later, the harpies are gone. All they've left you with is a sloppy cunt, and a few downey feathers clinging to your nipples and inner thigh.");
			...
			//(Reminds me of what my own Succubus does in my novel)
				...
				if (player.hasBalls()) outputText("You can feel her little tongue rolling across your balls and the base of your ");
				else outputText("You can feel her little tongue rolling across the base of your ");
				outputText(cockDescript(x) + " whenever the one on top of you doesn't take you up to the hilt.\n\n");

				outputText("You were just about to climax, when the harpies keeping you airborne decide they want their own turn. Spurned, they let go of you, sending you to plummet back to earth with one of the harpies still fucking on top of you. This time, the other three don't seem bothered to save you anymore.\n\n");
...
//No genderless folks.
		private function harpyDamageLoss():void
		{
			...
			var y:Number = player.findCock(2, -2, monster.vaginalCapacity());
			if (player.hasCock() && x < 0 || y < 0)
				sceneHunter.print("Failed: fitting cocks, up to 2");
			if (sceneHunter.uniHerms) {
				var choices:Array = [];
				if (x >= 0) choices[choices.length] = 0; //single-cock, maybe herm
				if (y >= 0) choices[choices.length] = 1; //multi-cock, not herm
				if (player.hasVagina()) choices[choices.length] = 2; //vagina
				var choice:int = choices[rand(choices.length)];
				if (choices.length > 1)
					sceneHunter.print("There're some random variations here. Have fun! Current: " + choice);
			}
			else {
				if (y >= 0) choice = 1;
				else if (x >= 0) choice = 0;
				else choice = 2;
			}
			...
			if (choice == 0 || choice == 1) {
				...
				//[(Male)
				if (!player.hasVagina() || choice == 1) { //with multi - forced
					...
					if (player.hasBalls()) outputText(", craning their necks to slurp and massage your [balls] eagerly, trying to coax up your cum production as much as they can");
				...
				//[(Herm)
					...
					//(If balls)
					if (player.hasBalls()) outputText("their hands moving up to grope and fondle your [balls], massaging them to boost your cum production as much as they can, ");
			...
			if (choice == 0) {
				...
				if (player.hasBalls()) outputText(", her huge feathery bottom coming to rest on your " + ballsDescriptLight());
			...
			//[(Female)
				...
				//{(If breasts)
				if (player.biggestTitSize() >= 2) outputText("Their hungry feminine mouths creep beneath the body of the harpy atop you and grab hard onto your " + breastDescript(0) + ".  They pull them towards their mouths, those gold lips wrapping themselves around your " + nippleDescript(0) + "s to suckle gently.  ");
				//{(If lactating)
		...
		private function victoryHarpyGetsHerPussyRaped():void
		{
			...
			if (y >= 0) outputText("and viciously-clenching ass ");
			...
			//(If balls)
			if (player.hasBalls()) outputText("You eventually bottom out, your [balls] coming to rest between her powerful, gelatinous thighs, her soft feathers tickling at your " + sackDescript() + " maddeningly.\n\n");
			//(If balls-less)
			...
			outputText("Grinning and massaging your conquest's round, soft ass in anticipation, you gently begin to thrust your " + hipDescript() + ", moaning loudly as her rough, grasping walls drag over the sensitive flesh of your [cocks].  ");
			outputText("Taking care not to waste your opportunity with an immense butt by cumming too soon, you slowly push and pull her fat, quivering cheeks up and down your " + cockDescript(x) + ", enjoying the feeling of her rough walls and clutching, gasping convulsions massage your manhood");
			if (y > 0) outputText("s");
			outputText(" along ");
			if (y >= 0) outputText("their");
			else outputText("the");
			outputText(" whole length, already bringing you close to the edge of a bone-shaking orgasm.");
			if (player.biggestTitSize() > 0) {
				outputText("\n\nYou laugh in glee as the bird's groping, clutching hole");
				if (y >= 0) outputText("s massage");
			...
			//[(Herm)
			if (player.hasVagina()) {
				outputText("  Your " + vaginaDescript(0) + " spurts fluids out, dribbling down the inside of your thighs as you continue to pump in and out of the helpless harpy.  You love every second of it.");
			}
			outputText("\n\nIt seems the exposed girl is enjoying herself just as much.  Every tiny motion of your " + hipDescript() + " causes her to writhe and moan, her moans punctuated with birdlike squawks as she pushes her huge egg-bearing hips back at you in sheer, undisguised desire to have you fuck her harder and deeper.\n\n");
			...
			outputText(".  ");
			//{(If balls)
			if (player.hasBalls()) outputText("Your " + sackDescript() + " slaps wetly into her immense, wobbling thighs.  ");
		...
		private function winAndRapeHarpyAnally():void
		{
			...
			//(if balls)
			if (player.hasBalls()) outputText("  Your [balls] come to rest against her thick, wiggly rear.");
			outputText("\n\n");
			...
			//[(Herms/Shemales)
				...
				//(If herm)
				if (player.hasVagina()) {
					outputText("  Her struggles are driving you wild, and your " + vaginaDescript(0) + " is seeping fluids, growing more and more aroused as she bounces and slamming that immense, rippling rear into you, the force squishing your moist lips together, releasing yet more sticky juices and adding an extra little tone of pleasure.");
					//{(If balls)
					if (player.hasBalls()) outputText("  She even makes your " + ballsDescript() + " slap against your " + vaginaDescript(0) + ", a hot, moist SPLAT accompanying every tiny motion the harpy makes.");
				}
				outputText("\n\n");
			}
			outputText("With an ear-splitting squawk, the impaled harpy's drooling pussy absolutely explodes.  It fires huge streams of her sticky feminine cum onto your ever-thrusting " + hipDescript() + ", her muscles locking in ecstasy, before the exhausted bird-girl collapses over you, ");
			//(if breasts)
			if (player.biggestTitSize() >= 2) outputText("her chest mashing against yours as she becomes still, ");
		...
		private function WinOnHarpyAndOralRape():void
		{
			...
			//(if pussy)
			if (player.hasVagina()) {
				outputText(", fluids seeping from your plush " + vaginaDescript(0) + " all the while");
			}
			//(if cock)
			if (player.hasCock()) {
				outputText(", ");
			...
			function singleF():void {
				...
				//(If balls)
				if (player.hasBalls()) outputText("  Her hands creep down to softly squeeze and massage your [balls], the harpy's eager head bobbing back and forth along the sweet tasting shaft of your [cock], sending waves of powerful, overwhelming pleasure resonating through your body.");
				outputText("\n\n");
				...
				//(If balls)
				if (player.hasBalls() && player.cocks[0].cockLength <= 16) outputText("She even deep throats you, taking your length into her expertly-controlled mouth, that tongue creeping out to tickle at your [balls].  ");
			...
			function multiF():void {
				...
				//(If balls)
				if (player.hasBalls()) outputText("  She moves that hand to softly squeeze and pet at your [balls], massaging those bloated cum-factories with expertise. You can feel a distinct rumble as your " + sackDescript() + " really ups its production, almost feeling heavier.");
				//(If herm)
				...
				//(if balls)
				if (player.hasBalls()) outputText(", dribbling all over your " + sackDescript());
		...
		private function harpyScissorSurprise():void
		{
			...
			if (monster.lust >= monster.maxOverLust()) outputText(" again");
...
//Fuck a harpy with the players throbbing clit.
		private function clitFuckAHarpy():void
		{
			...
			if (monster.lust >= monster.maxOverLust()) outputText("moan and masturbate, dipping her fingers into her honey-pot with wild abandon.");
