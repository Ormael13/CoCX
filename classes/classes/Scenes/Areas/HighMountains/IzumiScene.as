package classes.Scenes.Areas.HighMountains 
{
	...
	public class IzumiScene extends BaseContent
	{
		...
		public function actsLikeACow():Boolean
		{
			...
			if (player.findStatusEffect(StatusEffects.Dysfunction)) return true;
			return false;
		}
		...
		{
			var affectIndex:int = player.findStatusEffect(StatusEffects.IzumisPipeSmoke);

			if (affectIndex >= 0)
			{
				player.statusEffect(affectIndex).value1 -= hours;

				if (player.statusEffect(affectIndex).value1 <= 0)
				{
		...
		protected function smokeEffectWearsOff():void
		{
			...
			if (affectIndex >= 0)
			{
				player.spe += Math.abs(player.statusEffect(affectIndex).value2);
				player.sens -= player.statusEffect(affectIndex).value3;
				player.lib -= player.statusEffect(affectIndex).value4;
		...
		protected function surrenderFemaleNonExhibitionVariant():void
		{
			...
			else
			{
				...
				if ((player.findStatusEffect(StatusEffects.BreastsMilked) > 0) && (player.lactationQ() > 750))
				{
			...
			if (x == 1) player.orgasmTits(); else player.orgasmVaginal();
		...
		public function touchThatFluffyHorn():void
		{
			...
			if (player.hasCock())
			{
				...
				if (player.countCocksOfType(CockTypesEnum.TENTACLE) >= 8) addButton(5, "Tentacruel", tentacruelRevengeFux);
			}
		}
		...
		protected function tentacruelRevengeFux():void
		{
			...
			if (player.countCocksOfType(CockTypesEnum.TENTACLE) >= 10) outputText("  You decide to test the theory, snaking another prehensile appendage into each of her open, outstretched palms.  Sure enough, after a few fumbling attempts, Izumi manages to grasp the concept of a handjob and awkwardly starts to jerk them off.");
