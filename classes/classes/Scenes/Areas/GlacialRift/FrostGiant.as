	...
	public class FrostGiant extends Monster
	{
		...
		public function giantGrab():void {
			...
			if (combatMiss() || combatEvade() || combatFlexibility() || combatMisdirect()) {
				...
				createStatusAffect(StatusAffects.Stunned, 1, 0, 0, 0);
			}
			else {
				...
				player.createStatusAffect(StatusAffects.GiantGrabbed, 2, 0, 0, 0);
				if (player.findStatusAffect(StatusAffects.GiantStrLoss) < 0) player.createStatusAffect(StatusAffects.GiantStrLoss, 0, 0, 0, 0);
			}
			combatRoundOver();
		}
		...
		public function giantBoulderThrow():void {
			...
			outputText("\n\nWith a grunt and a shove, the giant throws the boulder directly at you!")
		...
		public function giantBoulderHit(mode:int = 0):void {
			...
			else outputText(", but you do look up just in time to nearly avoid the large boulder he chucked your way. Scrambling to react, you jump to the side, only to realize you chose the wrong side. The boulder hits you in the back, propelling you.  Battered, beaten, bruised, you struggle to stand, when the giant picks you up, laughs in his deep, mighty bellow, and punts you over a mountain. You land several feet deep in a snowbank, and see something flying toward you before passing out. ");
