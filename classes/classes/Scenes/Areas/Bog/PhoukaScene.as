	...
	public class PhoukaScene extends BaseContent {
		...
		{ //In old Irish legends the Phouka are responsible for Will-o’-the-wisps. In the deep woods it’s Kitsune, but in the bog it’s a Phouka
			...
			if (flags[kFLAGS.PHOUKA_ENCOUNTER_STATUS] != 0) {
				...
				outputText("It seems to be moving around in one of the dense parts of the bog, where the canopy of dead trees and creeping vines create shadows as deep as twilight even at noon.\n\nYou can't make out a figure holding the light, but you decide to follow.  Spurred on by the though that it may be someone else like yourself, or perhaps some agent of the demons, you keep trying to catch up with the light up ahead.  This trip into the bog is about to pay off.");
		...
		protected function phoukaDrinkWhilePregnant(playerOfferedTheBooze:Boolean):void
		{
			...
			if (acceptable < 0) {
				...
				outputText("  Sure, some of the elders around the village said a beer or some light cider was alright for a woman heavy with child, but you doubt that whiskey would be acceptable.  You politely refuse the drink, hoping the phouka won't be offended.");
			}
			else {
		...
		{ //In every path that leads here the character learns the creature is called a phouka, so no phoukaName() calls needed
			...
			if (player.tou < 30) {
				outputText("\n\nYou quickly end up drunk and begin a rambling story about your adventures.  The phooka is a good listener, though he asks for lots of detail whenever you bring up sex or potential sex of any kind.  You realize you're in real trouble, turned on and watching the phouka stroking his cock, which is still hard despite the alcohol he's been downing.  He notices your interest and says <i>“Now that you're relaxed lets go for the main course.  I'll make sure you'll remember it even once you're sober.”</i>");
		...
		protected function phoukaDrinkTalk(playerNotDrinking:Boolean):void
		{
			...
			if (playerIsInebriated) {
				...
				addButton(0, "Leave", phoukaDrinkLeave);
				if (player.lust >= 33) {
					addButton(1, "Mount Phouka", phoukaSexFaerieNonCombat); //Works for all
		...
		protected function phoukaDrinkLeave():void
		{
			if (player.lust < 33)
				...
				outputText("\n\nIf you wanted");
			outputText(" you could take advantage of the phouka’s predicament, but right now you’d rather go and check on the portal.  Thanks to the whiskey the water [if (isNaga = true)soaking into your scales][if (isNaga = false)soaking the lower half of your legs] doesn't matter to you quite so much as you slog back towards camp.");
			doNext(13);  //Return to camp, 1 hour used
		}
		...
		public function phoukaHalloween():void
		{
			...
			if (player.str < 40) {
				...
				outputText("You don't know why they said this was difficult.  You have no problem pumping the treacle, in fact they have to stop you part way through to change barrels.");
			}
		...
		{
			outputText("\n\nFeeling you're better off not finding out you excuse yourself, telling them they can enjoy the treacle amongst themselves.  As you walk away you just catch the boss telling the others <i>“Watch out fer that one, [he]'s a smart one.”</i>");
			doNext(13);  //Return to camp, 1 hour used
		}
		...
		protected function phoukaLeaveOnLustWin():void
		{
			outputText("\n\nThe lusty " + phoukaName() + " continues to pound his cock into the earth as you prepare to go, oblivious to your presence.");
		...
		internal function phoukaPlayerWins(hpVictory:Boolean):void
		{
			...
			if (hpVictory) { //You win by physical damage, the phouka cheats and runs
				...
				if (player.lust < 33) {
					...
					addButton(1, "Mount Phouka", phoukaSexFaeriePostCombat); //Works for all
		...
		public function phoukaPregBirth():void
		{
			...
			if (flags[kFLAGS.PREGNANCY_CORRUPTION] > 6) { //You’ve been drinking like a fish, haven’t you?
				...
				if (player.fertility < 1) player.fertility = 1;
				fatigue(75);
		...
		{ //Whether by horse, bunny or (male) faerie sex it all ends up here if the PC has a vagina
			...
			player.knockUp(player.PREGNANCY_FAERIE, 200);
			if (phoukaForm == PHOUKA_FORM_FAERIE) {
