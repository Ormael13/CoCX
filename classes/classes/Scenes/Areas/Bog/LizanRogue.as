	...
	public class LizanRogue extends Monster
	{
		...
		public function blowGun():void {
			if (player.getEvasionRoll()) {
				...
				if (!player.hasStatusAffect(StatusAffects.LizanBlowpipe)) player.createStatusAffect(StatusAffects.LizanBlowpipe, 5, 0, 5, 0);
				else {
		...
		public function immaHurtYouBadly():void {
			if (player.getEvasionRoll()) {
				...
				if (!player.hasStatusAffect(StatusAffects.LizanBlowpipe)) player.createStatusAffect(StatusAffects.LizanBlowpipe, 0, 5, 0, 5);
				else {
		...
		public function tongueAttack():void {
			if (player.getEvasionRoll()) {
				...
				if (!player.hasStatusAffect(StatusAffects.Blind)) player.createStatusAffect(StatusAffects.Blind, 1 + rand(2), 0, 0, 0)
			}
			combatRoundOver();
		}
