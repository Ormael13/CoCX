...
public function letKatKnotYourCuntPussyFuck():void {
	...
	if (player.isRace(Races.HARPY) || player.sharkScore() >= 4 || player.catScore() >=4 || player.dogScore() >= 4 || player.bunnyScore() >= 4) outputText("  Mmm... yeah, that's it, moan for me, you little slut; who's the alpha, huh?  Katherine's your alpha - go on, say it!</i>\" she yells, pounding herself into you with greater force, her claws extending just far enough to begin biting into your flesh, pinpricks of pain to counter the pleasure.");
	else outputText("</i>\"");
	outputText("\n\n");
...
public function getPenetrated():void {
	...
	if(player.isRace(Races.HARPY) || player.sharkScore() >= 4 || player.catScore() >=4 || player.dogScore() >= 4 || player.bunnyScore() >= 4)
