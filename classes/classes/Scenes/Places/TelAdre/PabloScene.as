package classes.Scenes.Places.TelAdre 
{
	...
	public class PabloScene extends TelAdreAbstractContent
	{
		...
		public function pabloSexMenu():void {
			...
			if (player.hasVagina()) addButton(1, "Use Pussy", PabloPussPussSubMenus);
			else addButtonDisabled(1, "Use Pussy", "You need to have a pussy for that.");
			addButton(2, "Use Ass", pabloButtsecksSubMenus);
		}
		
		private function pabloDickingTimeSubMenus():void {
			clearOutput();
		...
		private function pabloDickingTime(inCharge:Boolean = false, anal:Boolean = false):void {
			...
			if (anal) {
				...
				flags[kFLAGS.PABLO_GOT_DICKED_ANALLY]++;
			}
			//Vaginal
			else {
				if (inCharge) {
		...
		private function PabloPussPussSubMenus():void {
			clearOutput();
		...
		private function pabloButtsecksSubMenus():void {
			clearOutput();
			outputText("How shall your " + player.assDescript() + " be used? Do you take charge or let him lead?");
			menu();
			addButton(0, "Take Charge", pabloButtSecks, true, null, null, "You'll take charge and give him vaginally.", "Take Charge");
			addButton(1, "Pablo Leads", pabloButtSecks, false, null, null, "You'll let Pablo lead and he'll take your dick vaginally.", "Pablo Leads");
		}
