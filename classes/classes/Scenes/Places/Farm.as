...
public function farmExploreEncounter():void {
	...
	if(player.statusAffectv1(StatusAffects.MetWhitney) < 2) {
		...
		if(player.findStatusAffect(StatusAffects.MetWhitney) < 0) {
			...
			if(player.statusAffects[player.findStatusAffect(StatusAffects.MetWhitney)].value1 == 2) outputText("<b>You've been to the farm enough to easily find it.  You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.\n\n", false);
		}
		menuLoc = 2;
