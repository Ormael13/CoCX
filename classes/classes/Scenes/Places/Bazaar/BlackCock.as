	...
	public class BlackCock extends BazaarAbstractContent
	{
		...
		public function enterTheBlackCock():void {
			...
			if (model.time.hours >= 19) {
				...
				addButton(2, "Satyrs", satyrGangIntro);
			}
		...
		private function addFoodToGo(item:ItemType, price:int):void {
			...
			for (var i:int = 0; i < 14; i++) {
				if (mainView.buttonIsVisible(i)) button++;
			}
			outputText("\n" + capitalize(item.longName) + " - " + price + " gems");
			addButton(button, item.shortName, createCallBackFunction2(orderFoodToGo, item, price));
		}
		...
		private function addFoodPlate(foodName:String, gems:int, tooltip:String, foodEatDesc:String):void {
			...
			var fullFoodName:String = "";
			switch(foodName) {
				case "P. Potatoes":
					fullFoodName = "Party Potatoes";
					break;
				case "Prot. Shake":
					fullFoodName = "Protein Shake";
					break;
				case "R.Tk. Dinner":
					fullFoodName = "Roast Turkey Dinner";
					break;
				case "G.Ham Dinner":
					fullFoodName = "Glazed Ham Dinner";
					break;
				case "Mystery Meat":
					fullFoodName = "Mystery Meat";
					break;
				default:
			}
			outputText("\n" + (fullFoodName != "" ? fullFoodName : foodName) + " - " + gems + " gems");
			addButton(button, foodName, createCallBackFunction2(orderFoodPlate, foodName, gems, foodEatDesc));
			mainView.bottomButtons[button].toolTipText = tooltip;
		}
		...
		private function eatFoodNomNom(foodName:String, foodEatDesc:String):void {
			...
			switch(foodName) {
				...
				case "Giant Burger":
					...
					player.modThickness(100, 4);
					if (rand(3) == 0 && player.hairLength < 16) player.hairLength += 0.5;
					break;
				case "P. Potatoes":
					//player.refillHunger(70);
					player.modThickness(100, 2);
					if (rand(2) == 0) player.buttRating++;
					break;
				case "Spicy Chilli":
					//player.refillHunger(60);
					player.modThickness(100, 1);
					if (rand(2) == 0) player.hipRating++;
					break;
				case "Prot. Shake":
		...
		private function checkStaff():void {
			...
			if (flags[kFLAGS.BLACK_COCK_MET_HARRY] == 0) {
				...
				addButton(2, "Sneak", meetHarryRoswell);
			}
			else {
			...
			addButton(4, "Back", enterTheBlackCock);
		}
		...
		private function meetAnitaRoswell():void {
			...
			menu();
			addButton(0, "Song", listenToAnitaSong);
		...
		private function rompUpAnitaOffer():void {
			...
			menu();
			if (player.canFly() && player.gender > 0) addButton(0, "Eagle Fuck", eagleFuckAnita);
			else outputText("\n\n<b>If you have wings, you could engage in an exotic sex session with Anita.</b>");
			if (player.hasCock()) addButton(1, "Vaginal", vaginalFuckAnita);
			else outputText("\n\n<b>If you have a cock, you could fuck her vaginally.</b>");
			if (player.isBiped() || player.isNaga() || player.isGoo()) addButton(2, "Sixty Nine", sex69Anita);
			else outputText("\n\n<b>Unfortunately, your tauric body makes it impossible to engage in a sixty-nine session with Anita.</b>");
			addButton(4, "Nevermind", nevermindToAnita);
		}
		...
		private function meetAndyMathis():void {
			...
			menu();
			addButton(0, "Stash", checkAndyStash);
			if (player.lust >= 33) addButton(1, "Sex", rompUpWithAndyTheSatyr);
		...
		private function smokeSomeHookah():void {
			...
			switch(rand(2)) {
				...
				case 1:
					...
					if (player.humanScore() >= 5) outputText("You nod, telling him that you look the same as when you walked through the portal.");
			...
			menu();
			addButton(0, "Grab It", grabAndysDongAfterSmoking);
		...
		private function stayAfterNoSmoke():void {
			...
			menu();
			if (player.hasCock()) addButton(0, "Suck my Cock");
			if (player.hasVagina()) addButton(1, "Lick my Pussy", haveAndyLickYouOut);
			addButton(2, "Rim my Ass", haveAndyRimYouOmgItsGross);
		...
		private function rompUpWithAndyTheSatyr():void {
			...
			addButton(1, "Get Fucked", haveAndyFuckYouPrompt);
		}
		...
		private function orderFridasCake():void {
			...
			if (player.findStatusAffect(StatusAffects.Fullness) >= 0) {
				outputText("<b>You are too full to consider ordering that.</b>");
				doNext(enterTheBlackCock);
				return;
			}
		...
		private function satyrGangIntro():void {
			...
			if (player.lust >= 33) {
				if (!player.isTaur()) {
					addButton(0, "Let Them", letTheSatyrsHaveTheirWayWithYou, false);
					mainView.bottomButtons[0].toolTipText = "Let the satyrs have their way with you. This is going to be rough and intense.";
					if (player.hasVagina()) addButton(5, "No Rimjobs", letTheSatyrsHaveTheirWayWithYou, true);
					mainView.bottomButtons[5].toolTipText = "Let the satyrs have their way with you. This is going to be rough and intense. Also signal that you don't want to rim one of the satyr's asses.";
				}
				else outputText("\n\n<b>You doubt you'll be able to participate into the orgy with your tauric body.</b>");
			}
			else outputText("\n\n<b>You're not horny enough to consider that.</b>");
			addButton(1, "No Thanks", dontLetTheSatyrsFuck);
		}
		...
		private function letTheSatyrsHaveTheirWayWithYou(noRimjob:Boolean = false):void {
			...
			if (!player.isTaur()) {
				...
				outputText("\n\nFinally exhausted, you slump on one of the satyrs, embracing his warm body. The rest of the satyrs are finally spent as well. When you've finally recovered from the ordeal, you get yourself cleaned up, redress yourself and return to your camp.");
			}
			player.orgasm();
			fatigue(25);
		...
		public function satyrTFs():void
		{
			...
			if (rand(3) == 0 && changes < changeLimit && player.hasCock() && player.balls <= 0) {
				outputText("Without warning your body begins to tremble as just below [eachCock] you feel a warm trickling sensation of fluid sliding down your body. Before you can check it, the sensation becomes ovewhelming as [eachCock] grows hard and ejaculates into your [armor]. Once you've recovered from your intense orgasm you remove your [armor] to clean yourself and find a <b>new pair of balls</b> hanging just below [eachCock].");
				player.balls = 2;
				player.ballSize = 1;
				player.orgasm();
				changes++;
			...
			if (rand(4) == 0 && changes < changeLimit && player.earType != EARS_ELFIN) {
			...
			if (rand(3) == 0 && changes < changeLimit && player.cockTotal() == 1 && player.countCocksOfType(CockTypesEnum.HUMAN) == 0) {
				outputText("\n\nYou feel a stirring in your loins as your cock grows rock hard. You pull it out from your [armor], to take a look. It seems you now <b>have a human dick again</b>.");
			...
			if (rand(3) == 0 && changes < changeLimit && player.cockTotal() > 1 && (player.cockTotal() - player.countCocksOfType(CockTypesEnum.HUMAN)) > 0) {
				outputText("\n\nOne of your penises begins to feel strange. You pull it out from your [armor], releasing a plume of thick smoke. When you look down you see it has <b>become a human dick</b>.");
		...
		public function rhinoTFs():void
		{
			...
			if (rand(4) == 0 && changes < changeLimit && player.gills) {
				outputText("\n\nYou feel your gills tighten, the slits seeming to close all at once. As you let out a choked gasp your gills shrink into nothingness, leaving only smooth skin behind. Seems you won't be able to stay in the water quite so long anymore. <b>You no longer have gills!</b>");
				player.gills = false;
			}
			// Rhino TFs
			//------------
			...
			if (rand(3) == 0 && changes < changeLimit && player.isBiped() && player.tailType != TAIL_TYPE_RHINO) {
				if (player.tailType > 0) outputText("\n\nYou [tail] suddenly goes numb. Looking back you see it changing, twisting and reforming into a long ropy tail with a little " + player.hairColor + " tuft at the end. <b>You now have a rhino tail.</b>");
				else outputText("\n\nYou feel an odd itchy sensation just above your [ass]. Twisting around to inspect it you find a long ropy tail with a little " + player.hairColor + " tuft on the end. <b>You now have a rhino tail.</b>");
			...
			if (rand(3) == 0 && changes < changeLimit && player.balls > 0 && player.ballSize < 4) {
				...
				if (player.ballSize > 2) outputText("\n\nA sudden onset of heat envelops your groin, focusing on your " + player.sackDescript() + ". Walking becomes difficult as you discover your " + simpleBallsDescript() + " have enlarged again.");
		...
		public function echidnaTFs():void
		{
			...
			if (rand(3) == 0 && changes < changeLimit && player.hasCock() && player.countCocksOfType(CockTypesEnum.ECHIDNA) < player.cockTotal()) {
				outputText("\n\n");
				if (player.cockTotal() == 1) outputText("Your [cock] suddenly becomes rock hard out of nowhere. You pull it out from your [armor], right in the middle of the food tent, watching as it begins to shift and change. It becomes pink in color, and you feel a pinch at the head as it splits to become four heads. " + (player.hasSheath() ? "" : "The transformation finishes off with a fleshy sheath forming at the base.") + " It ejaculates before going limp, retreating into your sheath.");
				else outputText("One of your penises begins to feel strange. You pull the offending cock out from your [armor], right in the middle of the food tent, watching as it begins to shift and change. It becomes pink in color, and you feel a pinch at the head as it splits to become four heads. " + (player.hasSheath() ? "" : "The transformation finishes off with a fleshy sheath forming at the base.") + " It ejaculates before going limp, retreating into your sheath.");
