	...
	public class BlackCock extends BazaarAbstractContent
	{
		...
		private function applyAndysSmokeEffect():void {
			if (player.findStatusAffect(StatusAffects.AndysSmoke) >= 0) {
				if (player.statusAffectv1(StatusAffects.AndysSmoke) > -15) { //Maximum speed loss is -15.
					player.addStatusValue(StatusAffects.AndysSmoke, 1, -5);
					dynStats("spe", -5);
				}
				if (player.statusAffectv1(StatusAffects.AndysSmoke) < 15) { //Maximum intelligence boost is 15.
					player.addStatusValue(StatusAffects.AndysSmoke, 1, 5);
					dynStats("inte", 5);
				}
		...
		private function grabAndysDongAfterSmoking():void {
			...
			if (player.hasCock()) { //Male/herm
				...
				if (player.vaginas[0].looseness >= 2) {
		...
		private function stayAfterNoSmoke():void {
			...
			addButton(2, "Rim Ass", haveAndyRimYouOmgItsGross, "Have Andy rim your ass. \n\nNOTE: Contains rimjob! You have been warned.");
		...
		private function haveAndySuckYouOff():void {
			...
			outputText("\n\nYou wag your dick at him to display your impatience; he gets the message. It's as if he has snapped out of a trance the way he suddenly tugs his shirt off and kneels before you with the reverence of a servant bowing before his {king/queen}. With patient hands he reaches forward, taking your [cock] out of your hand and holding it out with a firm grip as his lips descend.");
			//Cocks check
		...
		private function haveAndyFuckYou():void {
			...
			if (player.isTaur()) { //Tauric lower body
				...
				//Looseness check
				if ((player.hasVagina() && player.vaginas[0].looseness < 2) || (!player.hasVagina() && player.ass.analLooseness < 2)) {
				...
				else if ((player.hasVagina() && player.vaginas[0].looseness < 4) || (!player.hasVagina() && player.ass.analLooseness < 4)) {
		...
		private function letTheSatyrsHaveTheirWayWithYou():void {
			...
			if (!player.isTaur()) {
				...
				outputText("\n\nOne of the satyrs holds up a beer and yells something about a toast in your name. Cold beer splashes down your [fullChest] as the group of drunk satyrs give a barely audible toast as Qthey all whip out their human dicks, already dripping with need.[pg]");
		...
		public function echidnaTFs():void {
			outputText("\n\n<b>Echidna transformations are not yet implemented. Wait until the next snapshot!</b>");
			/*var changes:int = 0;
			var changeLimit:int = 3;
			if (player.findPerk(PerkLib.HistoryAlchemist) >= 0) changeLimit++;
			if (player.findPerk(PerkLib.TransformationResistance) >= 0) changeLimit--;
