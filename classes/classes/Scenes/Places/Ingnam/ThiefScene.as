package classes.Scenes.Places.Ingnam
{
	...
	public class ThiefScene extends BaseContent
	{
		...
		private function rapeThiefAnally():void {
			...
			outputText("\n\nYou " + (!player.armor.isNothing || player.lowerGarment != UndergarmentLib.NOTHING ? "remove your [armor], " : "") + "get the thief into position and you slowly slide your " + cockDescript(x) + " into his rear hole. Despite the thief's protestations, he seems to find pleasure.");
		...
		private function getLicked():void {
			clearOutput();
			outputText("You drag the unconscious thief into one of the alleys. The thief wakes up and realize what you're going to do. You " + (!player.armor.isNothing || player.lowerGarment != UndergarmentLib.NOTHING ? "remove the bottom half of your [armor]" : "") + " to reveal your [vagina]" + (player.hasCock() ? " and " + player.cockDescript(): "") + ".");
