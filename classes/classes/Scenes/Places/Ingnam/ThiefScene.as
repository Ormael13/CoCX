package classes.Scenes.Places.Ingnam 
{
	...
	public class ThiefScene extends BaseContent
	{
		...
		private function rapeThiefAnally():void {
			...
			outputText("\n\nYou remove the bottom half of your [armor], get the thief into position and you slowly slide your " + cockDescript(x) + " into his rear hole. Despite the thief's protestations, he seems to find pleasure.");
			...
			outputText("\n\nEventually, you can hold back no more and you unload your seed into his depths. The thief achieves orgasm as well, cumming all over the ground before falling on ground, dazed. You slide your " + player.cockDescript(x) + " out, redress yourself and leave the ravaged thief to recover.");
			player.orgasm();
			cleanupAfterCombat();
		}
		...
		private function getLicked():void {
			clearOutput();
			outputText("You drag the unconscious thief into one of the alleys. The thief wakes up and realize what you're going to do. You remove the bottom half of your [armor] to reveal your " + player.vaginaDescript() + (player.hasCock() ? " and " + player.cockDescript(): "") + ".");
			...
			outputText(" all over his face. Still disoriented, the thief collapses back on the ground. You re-dress and make a hasty exit while the thief is recovering from his ordeal.");
			player.orgasm();
			cleanupAfterCombat();
		}
