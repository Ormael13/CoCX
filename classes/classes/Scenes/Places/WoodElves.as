package classes.Scenes.Places{
	...
	public class WoodElves extends BaseContent implements SaveableState {
		...
		public function GroveLayout():void {
			...
			if(player.elfScore() < 11 && !player.isRace(Races.WOODELF)) outputText("<i> Well... you did change a little, but that doesn't matter. You're still, and forever will be family!.</i>");
				outputText("\"" +
				"\n\nUnsurprisingly, Elenwen is not so far behind, a little confused." +
						"\n\n\"<i>Alyssa, what's going on? </i>");
				if(player.elfScore() < 11 && !player.isRace(Races.WOODELF)) outputText("<i>Who's this?</i>");
				...
				if(player.elfScore() < 11 && !player.isRace(Races.WOODELF)) outputText("<i> Doesn't matter if you've changed, t</i>");
		...
		public function GroveLayout2(isFirstTime:Boolean = false):void {
			...
			if ((player.elfScore() <=10 && !player.isRace(Races.WOODELF))) addButtonDisabled(0,"River","You need to be an elf in order to go bath with the girls.");
			...
			if ((player.elfScore() <=10 && !player.isRace(Races.WOODELF))) addButtonDisabled(1,"Tent","You need to be an elf.");
