...
public function encounterMarae():void {
	...
	if(player.hasStatusAffect("DungeonShutDown") < 0) {
		...
		if(player.hasStatusAffect("Met Marae") < 0) {
			player.createStatusAffect("Met Marae",0,0,0,0);
	...
	//Dungeon inoperable
		...
		//Corrupt!
			...
			if(player.hasStatusAffect("Met Corrupt Marae") < 0) {
				player.createStatusAffect("Met Corrupt Marae",0,0,0,0);
...
private function maraeBadEnd():void {
	...
	if(player.statusAffectv3("Marble") > 0) outputText("Any thought or need to drink Marble's milk vanishes from your mind and body.  ", false);
...
private function maraeStealLethicite():void {
	...
	outputText("", true);
	//(SUCCESS) 
	if((player.spe > 35 && (rand(player.spe/3 + 30) > 20)) || (player.spe > 35 && player.hasPerk("Evade") >= 0 && rand(3) < 2))
	{
	...
	//(FAIL) 
		...
		{
			outputText("She extends a hand expectantly, watching with detached concentration while a tentacle lowers from the tree into her palm.   Hips swaying sexually, she advances, peeling back the tentacles outer layer.  It opens up to reveal a wet, gummy mouth.  She giggles and bumps the opening against your ", false);
			if(player.cocks.length > 1) outputText("largest ", false);
			outputText(cockDescript(0), false);
			outputText("'s tip.  Immediately a powerful suction draws your " + cockDescript(0) + " inside the tentacle-maw, burying you up to the base in squirming pleasure.   Marae watches the plant go to work, teasing squeezing until you orgasm.  It takes mere moments for the gifted tentacle to achieve its goal.  Your cum makes a tasty treat for the plant-beast, and it sucks and sucks until your body feels empty and drained.\n\n", false); 
...
public function level2MaraeEncounter():void {
	...
	//(FAIL-STOLE)
	else if(player.hasPerk("Marae's Gift - Stud") >= 0 || player.hasPerk("Marae's Gift - Fertility") >= 0) {
	...
	if(player.hasPerk("Marae's Gift - Fertility") >= 0 || player.hasPerk("Marae's Gift - Stud") >= 0) outputText("second ", false);
...
private function MaraePt2RoundIIIPrizes():void {
	...
	//[HERMS]
		...
		if(player.hasPerk("Marae's Gift - Fertility") < 0 && player.hasPerk("Marae's Gift - Stud") < 0) {
			...
			if(player.hasPerk("Marae's Gift - Fertility") >= 0) outputText("<b>(New Perk Gained: Marae's Gift - Fertility)</b>", false);
			else outputText("<b>New Perk Gained: Marae's Gift - Stud)</b>", false);
		}
		//(HAZ BREEDER)
		else if(player.hasPerk("Marae's Gift - Fertility") >= 0) {
