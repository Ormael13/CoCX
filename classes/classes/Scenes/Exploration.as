...
package classes.Scenes
{
	...
	public class Exploration extends BaseContent
	{
		...
		public function doExplore():void
		{
			...
			if (player.exploredDesert > 0) addButton(1, "Desert", exploreDesert);
			if (player.exploredForest > 0) addButton(2, "Forest", exploreForest);
			if (player.exploredLake > 0) addButton(3, "Lake", exploreLake);
			addButton(4, "Next", explorePageII);
			if (flags[kFLAGS.TIMES_EXPLORED_PLAINS] > 0) addButton(5, "Plains", explorePlains);
			if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00272] > 0) addButton(6, "Swamp", exploreSwamp);
			if (player.hasStatusAffect("exploredDeepwoods") >= 0) addButton(7, "Deepwoods", exploreDeepwoods);
			if (player.exploredMountain > 0) addButton(8, "Mountain", exploreMountain);
...
//Try to find a new location - called from doExplore once the first location is found
		...
		public function exploreDeepwoods():void
		{
			...
			if (player.statusAffectv1("exploredDeepwoods") % 10 == 0 && kGAMECLASS.isHalloween()) {
			...
			if (chooser == 3) {
				kGAMECLASS.akbalScene.supahAkabalEdition();
			}
			else if (chooser == 4) {
				if (rand(3) == 0) kGAMECLASS.kitsuneScene.kitsuneShrine();
				else kGAMECLASS.kitsuneScene.enterTheTrickster();
			}
		}
		...
		public function exploreForest():void
		{
			...
			if (chooser == 1) {
				...
				if (kGAMECLASS.monk == 0) {
					...
					if (player.gender > 0) {
						trace("Gender != 0");
						simpleChoices("Accept", 2003, "Rape Him", kGAMECLASS.jojoRape, "BWUH?", 0, "Decline", 13, "", 0);
					}
					else {
						trace("Gender == 0");
				...
				if (kGAMECLASS.monk == 1) {
					...
					if (player.hasStatusAffect("infested") >= 0) {
						...
						if (player.gender > 0) simpleChoices("Purge", 2083, "Meditate", 2003, "Rape", kGAMECLASS.jojoRape, "", 0, "Leave", 13);
					...
					if (player.gender > 0) simpleChoices("Yes", 2003, "No", 13, "BWUH", 0, "Rape Him", kGAMECLASS.jojoRape, "", 0);
...
//Explore High Mountain
		public function exploreHighMountain():void
		{
			...
			//Basilisk!
			if (chooser == 1) {
				kGAMECLASS.basiliskScene.basiliskGreeting();
				return;
			}
			//Sophie
			if (chooser == 2) {
		...
		public function exploreMountain():void
		{
				...
				if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00141] < 3) {
					...
					if (player.faceType == FACE_DOG && (player.dogCocks() >= 2 || (player.hasVagina() && player.pregnancyType == 6 && player.pregnancyIncubation > 0)) && player.cor >= 60 && player.tailType == TAIL_TYPE_DOG && (player.lowerBody == LOWER_BODY_TYPE_DOG || player.hairColor == "midnight black")) {
						...
						if (flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00141] == 0) {
							kGAMECLASS.hellHoundScene.HellHoundMasterEncounter();
							return;
						}
			...
			if (chooser == 3) {
				...
				if (player.hasStatusAffect("wormsOn") >= 0 && rand(2) == 0) {
					...
					if (player.hasStatusAffect("wormsHalf") >= 0 && rand(2) == 0) {
						kGAMECLASS.hellHoundScene.hellhoundEncounter();
						return;
					}
					kGAMECLASS.infestedHellhoundScene.infestedHellhoundEncounter();
					return;
				}
				kGAMECLASS.hellHoundScene.hellhoundEncounter();
			}
...
//Explore Lake
		public function exploreLake():void
		{
			...
			//Chance of dick-dragging! 10% + 10% per two foot up to 30%
				...
				if (player.hasStatusAffect("FactoryOverload") >= 0 && rand(10) == 0) {
					kGAMECLASS.gooGirlScene.spyOnGooAndOozeSex();
					return;
				}
				//Else pretty rare.
				else if (rand(25) == 0) {
				...
				if (rand(100) <= girlOdds) {
					kGAMECLASS.gooGirlScene.encounterGooGirl();
				}
				//OOZE!
				else {
			...
			//Find whitney or equinum
				...
				kGAMECLASS.fetishCultistScene.fetishCultistEncounter();
			}
			else if (select == 6) {
		...
		public function boatExplore():void
		{
			...
			switch (selector) {
				...
				case 3:
					...
					return;
				case 4:
					kGAMECLASS.fetishZealotScene.zealotBoat();
					return;
				case 5:
		...
		private function exploreBog():void
		{
		...
		public function exploreSwamp():void
		{
			...
			switch (choice) {
				case 0:
					kGAMECLASS.femaleSpiderMorphScene.fSpiderMorphGreeting();
					break;
				case 1:
					kGAMECLASS.maleSpiderMorphScene.greetMaleSpiderMorph();
					break;
				case 2:
					kGAMECLASS.corruptedDriderScene.driderEncounter();
					break;
				case 3:
		...
		public function explorePlains():void
		{
			...
			var choices:Array = [plainsLoot, plainsLoot,
				kGAMECLASS.gnollSpearThrowerScene.gnoll2Encounter,
				kGAMECLASS.gnollScene.gnollEncounter,
				kGAMECLASS.bunnbunbunMeet, kGAMECLASS.bunnbunbunMeet];
...
//AND THE TOTAL COMBINED WIDTH OF ALL THEIR COCKS IS TWELVE INCHES OR GREATER]
		private function bigJunkDesertScene():void
		{
			outputText("", true);
...
//[RANDOM SCENE IF CHARACTER HAS AT LEAST ONE COCK LARGER THAN THEIR HEIGHT, AND THE TOTAL COMBINED WIDTH OF ALL THEIR COCKS IS TWELVE INCHES OR GREATER]
		private function bigJunkForestScene(lake:Boolean = false):void
		{
			outputText("", true);
			var x:Number = player.longestCock();
...
//addButton(0,"Join",joinBeingAMinoCumSlut);
		private function joinBeingAMinoCumSlut():void
		{
			...
			dynStats("lib", .5, "sen", -3, "lus=", 0, "cor", 1);
