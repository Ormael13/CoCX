package classes 
{
	...
	public class CharCreation extends BaseContent {
		...
		private function rarePerks():void {
			...
			if (player.ascensionPerkPoints >= 5 && player.findPerk(PerkLib.AscensionUnlockedPotential) < 0) addButton(4, "UnlockPotent", perkUnlockedPotential, null, null, null, "Perk allowing to have increase passive gains of max hp, fatigue and lust at each lvl-up.\n\nCost: 5 point");
		...
		private function ascensionPermeryMenu(page:int = 1):void {
			...
			menu();
			if (page == 1) {
				if (player.perkv4(PerkLib.CorruptedKitsune) < 1) addButton(0, "CorruptedKitsune", permanentizePerk, PerkLib.CorruptedKitsune);
				else addButtonDisabled(0, "CorruptedKitsune", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.CorruptedNinetails) < 1) addButton(1, "CorruptedNinetails", permanentizePerk, PerkLib.CorruptedNinetails);
				else addButtonDisabled(1, "CorruptedNinetails", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.DarkCharm) < 1) addButton(2, "DarkCharm", permanentizePerk, PerkLib.DarkCharm);
				else addButtonDisabled(2, "DarkCharm", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.DragonFireBreath) < 1) addButton(3, "DragonFireBreath", permanentizePerk, PerkLib.DragonFireBreath);
				else addButtonDisabled(3, "DragonFireBreath", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				addButton(4, "Next", ascensionPermeryMenu, page + 1);
				if (player.perkv4(PerkLib.DragonIceBreath) < 1) addButton(5, "DragonIceBreath", permanentizePerk, PerkLib.DragonIceBreath);
				else addButtonDisabled(5, "DragonIceBreath", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.EnlightenedKitsune) < 1) addButton(6, "EnlightenedKitsune", permanentizePerk, PerkLib.EnlightenedKitsune);
				else addButtonDisabled(6, "EnlightenedKitsune", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.EnlightenedNinetails) < 1) addButton(7, "EnlightenedNinetails", permanentizePerk, PerkLib.EnlightenedNinetails);
				else addButtonDisabled(7, "EnlightenedNinetails", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.FerasBoonAlpha) < 1) addButton(8, "FerasBoonAlpha", permanentizePerk, PerkLib.FerasBoonAlpha);
				else addButtonDisabled(8, "FerasBoonAlpha", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.FerasBoonBreedingBitch) < 1) addButton(10, "FerasBoonBreedingBitch", permanentizePerk, PerkLib.FerasBoonBreedingBitch);
				else addButtonDisabled(10, "FerasBoonBreedingBitch", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.FerasBoonMilkingTwat) < 1) addButton(11, "FerasBoonMilkingTwat", permanentizePerk, PerkLib.FerasBoonMilkingTwat);
				else addButtonDisabled(11, "FerasBoonMilkingTwat", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.FerasBoonSeeder) < 1) addButton(12, "FerasBoonSeeder", permanentizePerk, PerkLib.FerasBoonSeeder);
				else addButtonDisabled(12, "FerasBoonSeeder", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.FireLord) < 1) addButton(13, "FireLord", permanentizePerk, PerkLib.FireLord);
				else addButtonDisabled(13, "FireLord", "This perk is already made permanent and will carry over in all subsequent ascensions.");
			}
			if (page == 2) {
				if (player.perkv4(PerkLib.Flexibility) < 1) addButton(0, "Flexibility", permanentizePerk, PerkLib.Flexibility);
				else addButtonDisabled(0, "Flexibility", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.Hellfire) < 1) addButton(1, "Hellfire", permanentizePerk, PerkLib.Hellfire);
				else addButtonDisabled(1, "Hellfire", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.InkSpray) < 1) addButton(2, "InkSpray", permanentizePerk, PerkLib.InkSpray);
				else addButtonDisabled(2, "InkSpray", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.LizanRegeneration) < 1) addButton(3, "LizanRegeneration", permanentizePerk, PerkLib.LizanRegeneration);
				else addButtonDisabled(3, "LizanRegeneration", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				addButton(4, "Next", ascensionPermeryMenu, page + 1);
				if (player.perkv4(PerkLib.Lustzerker) < 1) addButton(5, "Lustzerker", permanentizePerk, PerkLib.Lustzerker);
				else addButtonDisabled(5, "Lustzerker", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.MagicalFertility) < 1) addButton(6, "MagicalFertility", permanentizePerk, PerkLib.MagicalFertility);
				else addButtonDisabled(6, "MagicalFertility", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.MagicalVirility) < 1) addButton(7, "MagicalVirility", permanentizePerk, PerkLib.MagicalVirility);
				else addButtonDisabled(7, "MagicalVirility", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.MaraesGiftButtslut) < 1) addButton(8, "MaraesGiftButtslut", permanentizePerk, PerkLib.MaraesGiftButtslut);
				else addButtonDisabled(8, "MaraesGiftButtslut", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				addButton(9, "Previous", ascensionPermeryMenu, page - 1);
				if (player.perkv4(PerkLib.MaraesGiftFertility) < 1) addButton(10, "MaraesGiftFertility", permanentizePerk, PerkLib.MaraesGiftFertility);
				else addButtonDisabled(10, "MaraesGiftFertility", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.MaraesGiftProfractory) < 1) addButton(11, "MaraesGiftProfractory", permanentizePerk, PerkLib.MaraesGiftProfractory);
				else addButtonDisabled(11, "MaraesGiftProfractory", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.MaraesGiftStud) < 1) addButton(12, "MaraesGiftStud", permanentizePerk, PerkLib.MaraesGiftStud);
				else addButtonDisabled(12, "MaraesGiftStud", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.MilkMaid) < 1) addButton(13, "MilkMaid", permanentizePerk, PerkLib.MilkMaid);
				else addButtonDisabled(13, "MilkMaid", "This perk is already made permanent and will carry over in all subsequent ascensions.");
			}
			if (page == 3) {
				if (player.perkv4(PerkLib.OneTrackMind) < 1) addButton(0, "OneTrackMind", permanentizePerk, PerkLib.OneTrackMind);
				else addButtonDisabled(0, "OneTrackMind", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.PureAndLoving) < 1) addButton(1, "PureAndLoving", permanentizePerk, PerkLib.PureAndLoving);
				else addButtonDisabled(1, "PureAndLoving", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.PurityBlessing) < 1) addButton(2, "PurityBlessing", permanentizePerk, PerkLib.PurityBlessing);
				else addButtonDisabled(2, "PurityBlessing", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				if (player.perkv4(PerkLib.SensualLover) < 1) addButton(3, "SensualLover", permanentizePerk, PerkLib.SensualLover);
				else addButtonDisabled(3, "SensualLover", "This perk is already made permanent and will carry over in all subsequent ascensions.");
				addButton(9, "Previous", ascensionPermeryMenu, page - 1);
			}
			addButton(14, "Back", ascensionMenu);
		}
