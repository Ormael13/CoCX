package classes.Monsters
{
	...
	public class Kitsune extends Monster
	{
		...
		private function kitsuneEntwine():void
		{
			...
			createStatusAffect("PCTailTangle", 0, 0, 0, 0);
		...
		public function kitsuneStruggle():void
		{
			...
			//Success
			if (rand(20) + player.str / 20 + statusAffectv1("PCTailTangle") >= 12) {
				...
				removeStatusAffect("PCTailTangle");
				doAI();
			}
			//Failure - +5-10 LUST
			else {
				...
				addStatusValue("PCTailTangle", 1, 3);
				combatRoundOver();
			}
		}
...
//Success:
		private function illusionKitsuneAttack():void
		{
			...
			if (rand(100) < resist) {
				...
				createStatusAffect("Illusion", 0, 0, 0, 0);
				spe += 20;
			}
			combatRoundOver();
		}
...
//PCs with "Religious" background and < 20 corruption have up to 20% resistance to sealing at 0 corruption, losing 1% per corruption.
		private function kitsuneSealAttack():void
		{
			...
			if (select == 0) {
				...
				player.createStatusAffect("Sealed", 4, 0, 0, 0);
			}
			else if (select == 1) {
				//Tease:
				...
				player.createStatusAffect("Sealed", 4, 1, 0, 0);
			}
			//Spells:
			else if (select == 2) {
				...
				player.createStatusAffect("Sealed", 4, 2, 0, 0);
			}
			//Items:
			else if (select == 3) {
				...
				player.createStatusAffect("Sealed", 4, 3, 0, 0);
			}
			//Run:
			else if (select == 4) {
				...
				player.createStatusAffect("Sealed", 4, 4, 0, 0);
			}
			//P.Special:
			else if (select == 5) {
				...
				player.createStatusAffect("Sealed", 4, 5, 0, 0);
			}
			//M.Special:
			else {
				...
				player.createStatusAffect("Sealed", 4, 6, 0, 0);
			}
			if (resist >= rand(100)) {
...
//Tease Texts:
		...
		override protected function performCombatAction():void
		{
			...
			if (player.hasStatusAffect("Sealed") < 0) moves.push(kitsuneSealAttack);
			if (player.hasStatusAffect("Sealed") < 0) moves.push(kitsuneSealAttack);
			if (hasStatusAffect("PCTailTangle") < 0) moves.push(kitsuneEntwine);
			if (hasStatusAffect("Illusion") < 0) moves.push(illusionKitsuneAttack);
