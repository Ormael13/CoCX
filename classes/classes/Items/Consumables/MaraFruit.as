	...
	override public function useItem():Boolean {
		...
		if (player.gender == 2 || (player.gender == 3 && player.mf("m", "f") == "f")) {
			...
			if (player.vaginas.length > 0) {
				...
				while (temp > 0) {
					temp--;
					if (player.vaginas[0].vaginalWetness < AppearanceDefs.VAGINA_WETNESS_SLAVERING) player.vaginas[temp].vaginalWetness++;
				}
				changes++;
			}
			//Fertility boost
		...
		//Skin
		if (!player.hasPlainSkinOnly() && !player.isGargoyle() && changes < changeLimit && rand(4) == 0 && player.faceType == AppearanceDefs.FACE_HUMAN) {
		...
		if (player.skin.hasPlainSkinOnly() && (player.skinTone == "leaf green" || player.skinTone == "lime green" || player.skinTone == "turquoise") && changes < changeLimit && rand(3) == 0) {
			...
			if (player.gender <= 1 || (player.gender == 3 && player.mf("m", "f") == "m")) {
				if (player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_PLANT_ROOT_CLAWS) {
					outputText("\n\n");
					...
					mutations.setLowerBody(AppearanceDefs.LOWER_BODY_TYPE_PLANT_ROOT_CLAWS);
					player.legCount = 2;
				}
			}
			//Females/futa get high heels
			else {
				if (player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_PLANT_HIGH_HEELS && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_PLANT_FLOWER) {
					outputText("\n\n");
					if (player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_HUMAN) outputText("You watch, spellbound, while your legs gradually change its entire outer structure into a plain human-like form. ");
					...
					mutations.setLowerBody(AppearanceDefs.LOWER_BODY_TYPE_PLANT_HIGH_HEELS);
					player.legCount = 2;
				}
			}
			changes++;
		...
		//Arms
		if ((player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_PLANT_HIGH_HEELS || player.lowerBody == AppearanceDefs.LOWER_BODY_TYPE_PLANT_ROOT_CLAWS) && !InCollection(player.armType, AppearanceDefs.ARM_TYPE_GARGOYLE, AppearanceDefs.ARM_TYPE_PLANT) && changes < changeLimit && rand(3) == 0) {
			...
			mutations.setArmType(AppearanceDefs.ARM_TYPE_PLANT);
			changes++;
		}
		//Wings slot aka tentacle cockvine wings
		if (player.armType == AppearanceDefs.ARM_TYPE_PLANT && player.wingType != AppearanceDefs.WING_TYPE_PLANT && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_PLANT_FLOWER && changes < changeLimit && rand(3) == 0) {
			...
			mutations.setWingType(AppearanceDefs.WING_TYPE_PLANT, "cockvine");
			changes++;
		}
		//Ears
		if (player.earType != AppearanceDefs.EARS_ELFIN && !player.isGargoyle() && changes < changeLimit && rand(3) == 0) {
			...
			changes++;
			mutations.setEarType(AppearanceDefs.EARS_ELFIN);
		}
		//Face
		if (player.faceType != AppearanceDefs.FACE_HUMAN && changes < changeLimit && rand(4) == 0 && player.earType == AppearanceDefs.EARS_ELFIN) {
			changes++;
			mutations.setFaceType(AppearanceDefs.FACE_HUMAN);
		...
		//Leaf Hair
		if (player.hairColor == "green" && (player.hairType != AppearanceDefs.HAIR_LEAF && player.hairType != AppearanceDefs.HAIR_GRASS) && !player.isGargoyle() && rand(3) == 0 && changes < changeLimit)
		{
			if (rand(2) == 0) {
				...
				mutations.setHairType(AppearanceDefs.HAIR_GRASS);
			}
			else {
				...
				mutations.setHairType(AppearanceDefs.HAIR_LEAF);
			}
			changes++;
		}
		//green hair
		...
		//Horns
		if ((player.hairType == AppearanceDefs.HAIR_LEAF || player.hairType == AppearanceDefs.HAIR_GRASS) && changes < changeLimit && rand(2) == 0) {
			...
			if (player.gender <= 1 || (player.gender == 3 && player.mf("m", "f") == "m")) {
				if (player.horns == 0) {
					...
					mutations.setHornType(AppearanceDefs.HORNS_OAK, 4 + rand(4));
				}
				if (player.horns > 0 && player.hornType != AppearanceDefs.HORNS_OAK) {
			...
			//Females/futa get orchid flowers
				...
				if (player.horns == 0 && player.hornType == AppearanceDefs.HORNS_NONE) {
					...
					mutations.setHornType(AppearanceDefs.HORNS_ORCHID, 2);
				}
		...
		//
		if (player.plantScore() >= 7 && player.wingType == AppearanceDefs.WING_TYPE_PLANT && player.hasStatusEffect(StatusEffects.AlrauneFlower) && changes < changeLimit && rand(2) == 0) {
			...
			mutations.setWingType(AppearanceDefs.WING_TYPE_NONE, "non-existant");
			mutations.setLowerBody(AppearanceDefs.LOWER_BODY_TYPE_PLANT_FLOWER);
			player.legCount = 12;
			changes++;
		}
