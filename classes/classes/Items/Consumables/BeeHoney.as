...
public class BeeHoney extends Consumable
    {
		...
		override public function useItem():Boolean {
			...
			if (changes < changeLimit && player.hairType == 4 && Utils.rand(2) == 0) {
				...
				mutations.setHairType(AppearanceDefs.HAIR_NORMAL);
				changes++;
			}
			...
			//Appearance Stuff
			//Hair Color
			if (changes < changeLimit && (player.hairColor != "shiny black" && player.hairColor != "black and yellow") && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_GARGOYLE && player.hairLength > 10 && Utils.rand(5) == 0) {
			...
			//Horns
			if (changes < changeLimit && player.horns > 0 && player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_GARGOYLE && Utils.rand(3) == 0) {
				outputText("\n\nYour horns crumble, falling apart in large chunks until they flake away to nothing.");
				mutations.setHornType(AppearanceDefs.HORNS_NONE);
				changes++;
			}
			// Chitin
			if (changes < changeLimit && (player.skin.coverage != Skin.COVERAGE_LOW
				|| player.skin.coat.type != AppearanceDefs.SKIN_COAT_CHITIN
				|| player.skin.coat.pattern != AppearanceDefs.PATTERN_BEE_STRIPES)) {
				player.skin.growCoat(AppearanceDefs.SKIN_COAT_CHITIN,{
					color:"yellow",
					color2:"black",
					pattern:AppearanceDefs.PATTERN_BEE_STRIPES
				},Skin.COVERAGE_LOW);
			...
			if (changes < changeLimit
				&& player.lowerBody != AppearanceDefs.LOWER_BODY_TYPE_BEE
				...
				mutations.setLowerBody(AppearanceDefs.LOWER_BODY_TYPE_BEE);
				player.legCount = 2;
				changes++;
			}
			...
			//Gain oviposition!
			if (changes < changeLimit && player.findPerk(PerkLib.BeeOvipositor) < 0 && player.tailType == AppearanceDefs.TAIL_TYPE_BEE_ABDOMEN && Utils.rand(2) == 0) {
			...
			//Venom Increase
			if (changes < changeLimit && player.tailType == AppearanceDefs.TAIL_TYPE_BEE_ABDOMEN && player.tailRecharge < 15 && Utils.rand(2)) {
			...
			if (changes < changeLimit && player.wingType == AppearanceDefs.WING_TYPE_BEE_LIKE_SMALL && Utils.rand(4)) {
				changes++;
				mutations.setWingType(AppearanceDefs.WING_TYPE_BEE_LIKE_LARGE, "large bee-like");
			...
			if (changes < changeLimit && player.wingType == AppearanceDefs.WING_TYPE_NONE && Utils.rand(4)) {
				changes++;
			//	if (player.wingType == CoC.WING_TYPE_SHARK_FIN) outputText("\n\nYou feel an itching on your large back-fin as something begins growing there.  You twist and contort yourself, trying to scratch and bring yourself relief, and failing miserably.  A sense of relief erupts from you as you feel something new grow out from your fin.  You hastily remove the top portion of your [armor] and marvel as a pair of small bee-like wings sprout from your back, replacing the fin that once grew there.  Tenderly flexing your new muscles, you find you can flap them quite fast.  Unfortunately you can't seem to flap your little wings fast enough to fly, but they would certainly slow a fall.  A few quick modifications to your [armor] later and you are ready to continue your journey with <b>your new bee wings</b>.");
				...
				mutations.setWingType(AppearanceDefs.WING_TYPE_BEE_LIKE_SMALL, "small bee-like");
			}
			//Melt demon wings!
			if (changes < changeLimit && (player.wingType == AppearanceDefs.WING_TYPE_BAT_LIKE_TINY || player.wingType == AppearanceDefs.WING_TYPE_BAT_LIKE_LARGE || player.wingType == AppearanceDefs.WING_TYPE_BAT_LIKE_LARGE_2)) {
				changes++;
			...
			if (Utils.rand(4) == 0 && player.hasGills() && changes < changeLimit) {
				if (player.gillType == AppearanceDefs.GILLS_ANEMONE) outputText("\n\nYour chest itches, and as you reach up to scratch it, you realize your gills have withdrawn into your skin.");
				...
				mutations.updateGills(AppearanceDefs.GILLS_NONE);
				changes++;
			}
