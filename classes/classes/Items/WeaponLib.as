...
public final class WeaponLib extends ItemConstants
	{
		...
		public const A_STAFF:Weapon = new Weapon("A.Staff", "A. Staff", "amethyst staff", "an amethyst staff", "smack", 2, 240, "This staff is made of very old wood and seems to tingle to the touch.  The top has amethyst embedded in it, and the wood is worn smooth from lots of use.  It probably belonged to a wizard at some point and would aid magic use.", "Staff", WT_STAFF).withBuffs({'spellpower':+0.2}) as Weapon;
		...
		public const DAGWHIP:Weapon = new Weapon("DagWhip","DaggerWhip","dagger whip","a dagger whip","whip-like slash",5,200,"A small blade that is tied to the wrist by an 8 ft. cloth.  Could be used to attack few enemies at once.", "Small, Whipping", WT_DAGGER);
		...
		public const E_STAFF:Weapon = new Weapon("E.Staff", "E.Staff", "eldritch staff", "an eldritch staff", "thwack", 10, 800, "This eldritch staff once belonged to the Harpy Queen, who was killed after her defeat at your hands.  It fairly sizzles with magical power.", "Staff", WT_STAFF).withBuff('spellpower', +0.6) as Weapon;
		...
		public const L_STAFF:Weapon = new Weapon("L.Staff", "Lthc. Staff", "lethicite staff", "a lethicite staff", "smack", 14, 1337, "This staff is made of a dark material and seems to tingle to the touch.  The top consists of a glowing lethicite orb.  It once belonged to Lethice who was defeated in your hands.", "Staff", WT_STAFF).withBuff('spellpower', +0.8) as Weapon;
		...
		//Spellcasting weapon
		public const S_SWORD:Weapon = new Weapon("S.Sword", "S.Sword", "Sorcerer sword", "a sorcerer sword", "slash", 15, 2000, "This weapon was forged by elves mage knights. It is both an excelent catalyst and a deadly blade.", "Sword, StaffPart").withBuff('spellpower', +1) as Weapon;
		...
		public const H_WAND:Weapon = new Weapon("H.Wand", "H.Wand", "hellcat wand", "an hellcat wand", "smack", 1, 500, "A hellcat witch wand. Commonly used by hellcats to deliver their iconic fire magic.", "Wand", WT_WAND).withBuff('spellpower', +1.5) as Weapon;
		public const WDAGGER:Weapon = new Weapon("WDagger","W.Dagger","witch dagger","a witch dagger","stab",3,500,"A dagger engraved with magical runes. While not the best catalyst around it is light, discreet and excelent for delivering spells.", "Small,Sword,StaffPart", "Dagger").withBuff('spellpower', +0.80) as Weapon;

		public const OCCULUS:Weapon = new Weapon("Occulus", "Occulus", "Occulus", "an Occulus", "smack", 1, 500, "A wand rumored to have been the favorite catalyst of the now missing in action god of magic. The Occulus", "Sword, StaffPart").withBuff('spellpower', +1.5) as Weapon;
		public const ECLIPSE:Weapon = new Weapon("Eclipse", "Eclipse", "Eclipse", "an Eclipse", "smack", 1, 500, "A wand rumored to have been the favorite catalyst of the now missing in action god of magic. The Occulus", "Sword, StaffPart").withBuff('spellpower', +1.5) as Weapon;
		public const OCCULUS:UnicornStaff = new UnicornStaff();
		public const ECLIPSE:NocturnusStaff = new NocturnusStaff();
		...
		public const S_STAFF:Weapon = new Weapon("S.Staff", "S. Staff", "sapphire staff", "a sapphire staff", "smack", 2, 240, "This staff is made of very old wood and seems to tingle to the touch.  The top has sapphire embedded in it, and the wood is worn smooth from lots of use.  It probably belonged to a wizard at some point and would aid magic use.", "Staff", WT_STAFF).withBuff('spellpower', +0.2) as Weapon;
