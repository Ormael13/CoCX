		...
		override public function canUnequip(doOutput:Boolean):Boolean {
			if (!CoC.instance.player.hasPerk(PerkLib.Soulless)) {
				if (doOutput) {
					...
					outputText(""+(CoC.instance.player.hasStatusEffect(StatusEffects.MeetXuviel)?"Obviously, Xuviel wouldn’t want his target to escape his clutches so easily.":"Just what is going on!?")+" You need to find a way to remove this necklace!</b>");
				}
				return false;
			}
			else return true;
