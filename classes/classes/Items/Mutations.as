...
// Fuck yo dog shit we full-on wolf bitches now -Foxwells
		public function wolfPepper(type:Number,player:Player):void {
			...
			//normal legs
			if (rand(4) == 0) restoreLegs(tfSource);
			//normal arms
			if (rand(4) == 0) restoreArms(tfSource);
			//remove feather hair
			if (rand(4) == 0) removeFeatheryHair(tfSource);
			//remove basilisk hair
			if (rand(4) == 0) removeBassyHair(tfSource);
			...
			if (rand(5) == 0 && changes < changeLimit && !player.hasFur()) {
				...
				player.skinType = SKIN_TYPE_FUR;
				player.skinDesc = "fur";
				player.setFurColor(wolfFurColors, {type: UNDER_BODY_TYPE_FUR}, true);
