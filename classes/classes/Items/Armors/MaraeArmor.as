package classes.Items.Armors 
{
	...
	public class MaraeArmor extends Armor
	{
		...
		override public function useText():void
		{
			...
			if (!getGame().player.isTaur() && !getGame().player.isDrider()) outputText("Last but not least, you put your silken loincloth on to cover your groin. You thank Rathazul for that and you know that you easily have access to your ");
			...
			if (getGame().player.hasCock())
			{
				...
				{
					outputText("Your manhood is too big to be concealed by your silken loincloth. Part of your " + getGame().player.biggestCockIndex() + " is visible. ");
					if (getGame().player.cor < 33) outputText("You let out a sigh. ")
					if (getGame().player.cor >= 33 && getGame().player.cor < 66) outputText("You blush a bit, not sure how you feel. ")
					if (getGame().player.cor >= 66 || game.flags[kFLAGS.PC_FETISH] > 0) outputText("You admire how your manhood is visible. ")
				}
			}
