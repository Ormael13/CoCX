	...
	public class Appearance
	{
		...
		public static function cockAdjective(i_creature:Creature, i_cockIndex:Number = -1) 
		{
			...
			//Length 1/3 chance
				...
				if(i_creature.cocks[i_cockIndex].cockLength < 3) {
					...
					options = ["huge",
					"foot-long"];
					if(i_creature.cocks[i_cockIndex].cockType == CockTypesEnum.DOG) options.push("mastiff-like");
					else options.push("cucumber-length");					
					description += randomChoice(options);
				}
					...
					if(i_creature.cocks[i_cockIndex].cockType == CockTypesEnum.TENTACLE && rand(2) == 0) description += "coiled ";
					else {
						options = ["towering",
						"freakish",
						"monstrous",
		...
		public static function cockDescriptionShort(i_cockIndex:int, i_creature:Creature):String 
		{
			...
			if(int(Math.random()*3 == 0)) {
				if(i_creature.cocks[i_cockIndex].cockLength < 5) description = "short ";
				if(i_creature.cocks[i_cockIndex].cockLength >= 5 && i_creature.cocks[i_cockIndex].cockLength < 7) description = "average ";
				if(i_creature.cocks[i_cockIndex].cockLength >= 7 && i_creature.cocks[i_cockIndex].cockLength < 9) description = "long ";
				if(i_creature.cocks[i_cockIndex].cockLength >= 10 && i_creature.cocks[i_cockIndex].cockLength < 13) description = "huge ";
				if(i_creature.cocks[i_cockIndex].cockLength >= 13 && i_creature.cocks[i_cockIndex].cockLength < 18) description = "massive ";
				if(i_creature.cocks[i_cockIndex].cockLength >= 18 && i_creature.cocks[i_cockIndex].cockLength < 30) description = "enormous ";
				if(i_creature.cocks[i_cockIndex].cockLength >= 30) description = "towering ";	
				descripted = true;
			}
		...
		public static function ballsDescription(i_forcedSize:Boolean, i_plural:Boolean, i_creature:Creature, i_withArticle:Boolean = false):String 
		{
			...
			if (i_creature.ballSize > 1 && (rand(3) <= 1 || i_forcedSize))
			{
				...
				if (i_creature.ballSize >= 4 && i_creature.ballSize < 5) description += "grapefruit-sized";
				if (i_creature.ballSize >= 5 && i_creature.ballSize < 7) description += "cantaloupe-sized";
				if (i_creature.ballSize >= 7 && i_creature.ballSize < 9) description += "soccerball-sized";
				if (i_creature.ballSize >= 9 && i_creature.ballSize < 12) description += "basketball-sized";
			...
			if(i_creature.hasStatusAffect("Uniball") >= 0 && rand(2) == 0) {
				if(rand(3) == 0) description += " merged into a cute, spherical package";
				else if(rand(2) == 0) description += " combined into a round, girlish shape";
		...
		public static function buttDescription(i_character:Character):String
		{
			...
			if (i_character.buttRating >= 10) options.push("booty", "booty", "booty");
			description += randomChoice(options);
