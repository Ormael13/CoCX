...
//First encounter
//Boat
function kaijuMeeting():void {
	clearOutput();
	if(flags[KAIJU_MEETINGS] == 0) {
		...
		if(player.hasStatusAffect("FactoryOverload") >= 0 && !kaijuCock()) kaijuGrowsWangus();
		else if(flags[KAIJU_BAD_END_COUNTER] >= 5) kaijuBadEndToyBOOSH()
		else repeatKaijuEncounter();
	}
}

function kaijuCock():Boolean {
	return (flags[KAIJU_COCK] == 1);
}
...
function insultTheKaijuFirstMeeting():void {
	...
	if(kaijuCock()) {
		...
		if(player.hasStatusAffect("Knows Whitefire") >= 0 && (player.hasPerk("Blood Mage") < 0 && player.fatigue + spellCost(30) > 100)) {
...
function repeatKaijuEncounter():void {
	...
	if(itemCount("IncubiD") + itemCount("P.Draft") >= 10 && !kaijuCock()) {
...
function talkToKaiju():void {
	...
	//Dialog two: 
	else if(flags[KAIJU_TALK_CYCLE] == 2) {
	...
	else if(flags[KAIJU_TALK_CYCLE] == 3) {
		//Dialog three: 
...
function yesTurnKaijuFuta():void {
	clearOutput();
	flags[KAIJU_BAD_END_COUNTER]+=2;
	var counter:int = 10;
	while(counter > 0) {
		counter--;
		if(hasItem("P.Draft",1)) consumeItem("P.Draft",1);
		else if(hasItem("IncubiD",1)) consumeItem("IncubiD",1);
	}
...
function badEndPartTwo():void {
	...
	if(kaijuCock()) {
		...
		buttChange(500,true,true,false);
